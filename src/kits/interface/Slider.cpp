...
BSlider::BSlider(BRect frame, const char *name, const char *label,
    ...
    : BControl(frame, name, label, message, resizingMode, flags),
      fModificationMessage(NULL), fSnoozeAmount(20000),

      fMinLimitLabel(NULL), fMaxLimitLabel(NULL),

      fMinValue(minValue), fMaxValue(maxValue), fKeyIncrementValue(1),

      fHashMarkCount(0), fHashMarks(B_HASH_MARKS_NONE),

      fStyle(thumbType),

      fOrientation(B_HORIZONTAL), fBarThickness(6.0) {
  _InitBarColor();
...
BSlider::BSlider(BRect frame, const char *name, const char *label,
                 BMessage *message, int32 minValue, int32 maxValue,
                 orientation posture, thumb_style thumbType,
    ...
    : BControl(frame, name, label, message, resizingMode, flags),
      ...
      fOrientation(posture), fBarThickness(6.0) {
  _InitBarColor();
...
BSlider::BSlider(const char *name, const char *label, BMessage *message,
    ...
    : BControl(name, label, message, flags), fModificationMessage(NULL),
      fSnoozeAmount(20000),
...
BSlider::BSlider(BMessage *archive) : BControl(archive) {
...
void BSlider::_InitBarColor() {
  SetBarColor(
      be_control_look->SliderBarColor(ui_color(B_PANEL_BACKGROUND_COLOR)));
...
void BSlider::WindowActivated(bool state) { BControl::WindowActivated(state); }
...
void BSlider::AllDetached() { BControl::AllDetached(); }
...
void BSlider::MessageReceived(BMessage *message) {
...
void BSlider::MouseMoved(BPoint point, uint32 transit,
...
void BSlider::Pulse() { BControl::Pulse(); }

void BSlider::SetLabel(const char *label) { BControl::SetLabel(label); }
...
const char *BSlider::MinLimitLabel() const { return fMinLimitLabel; }

const char *BSlider::MaxLimitLabel() const { return fMaxLimitLabel; }
...
int32 BSlider::ValueForPoint(BPoint location) const {
  ...
  return (int32)roundf(
      ((position - min) * (fMaxValue - fMinValue) / (max - min)) + fMinValue);
}
...
void BSlider::SetEnabled(bool on) { BControl::SetEnabled(on); }
...
void BSlider::DrawHashMarks() {
  ...
  rgb_color base = ui_color(B_PANEL_BACKGROUND_COLOR);
  uint32 flags = be_control_look->Flags(this);
  be_control_look->DrawSliderHashMarks(view, frame, frame, base, fHashMarkCount,
                                       fHashMarks, flags, fOrientation);
}
...
void BSlider::DrawText() {
  ...
  if (Orientation() == B_HORIZONTAL) {
    ...
    if (fUpdateText != NULL) {
      be_control_look->DrawLabel(
          view, fUpdateText, base, flags,
          BPoint((bounds.Width() - StringWidth(fUpdateText)) / 2.0, baseLine));
    }
  }
}
...
const char *BSlider::UpdateText() const { return NULL; }
...
BRect BSlider::ThumbFrame() const {
  ...
  if (fStyle == B_BLOCK_THUMB) {
    if (Orientation() == B_HORIZONTAL) {
      frame.left = floorf(Position() * (_MaxPosition() - _MinPosition()) +
                          _MinPosition()) -
                   8;
      ...
      frame.top = floorf(Position() * (_MaxPosition() - _MinPosition()) +
                         _MinPosition()) -
                  8;
...
void BSlider::SetFlags(uint32 flags) { BControl::SetFlags(flags); }

void BSlider::SetResizingMode(uint32 mode) { BControl::SetResizingMode(mode); }
...
void BSlider::ResizeToPreferred() { BControl::ResizeToPreferred(); }
...
BMessage *BSlider::ModificationMessage() const { return fModificationMessage; }
...
int32 BSlider::SnoozeAmount() const { return fSnoozeAmount; }

void BSlider::SetKeyIncrementValue(int32 incrementValue) {
  fKeyIncrementValue = incrementValue;
}

int32 BSlider::KeyIncrementValue() const { return fKeyIncrementValue; }
...
int32 BSlider::HashMarkCount() const { return fHashMarkCount; }
...
hash_mark_location BSlider::HashMarks() const { return fHashMarks; }
...
thumb_style BSlider::Style() const { return fStyle; }
...
rgb_color BSlider::BarColor() const { return fBarColor; }
...
orientation BSlider::Orientation() const { return fOrientation; }
...
float BSlider::BarThickness() const { return fBarThickness; }
...
BPoint BSlider::_Location() const { return fLocation; }
...
void BSlider::_SetLocationForValue(int32 value) {
  ...
  float pos =
      (float)(value - fMinValue) / range * (_MaxPosition() - _MinPosition());
...
BSize BSlider::_ValidateMinSize() {
  if (fMinSize.width >= 0) {
  ...
  if (Orientation() == B_HORIZONTAL) {
    ...
    height +=
        rows * (ceilf(fontHeight.ascent) + ceilf(fontHeight.descent) + 4.0);
  } else {
    // B_VERTICAL
    width = 12.0f + fBarThickness;
    height = 32.0f;

    float lineHeightNoLeading =
...
extern "C" void B_IF_GCC_2(InvalidateLayout__7BSliderb,
                           _ZN7BSlider16InvalidateLayoutEb)(BView *view,
                                                            bool descendants) {
  perform_data_layout_invalidated data;
