...
SupportingApps::SupportingApps(DatabaseLocation *databaseLocation)
    : fDatabaseLocation(databaseLocation), fHaveDoneFullBuild(false) {}
...
/*! \brief Sets the list of supported types for the given application and
        ...
        \param fullSync If \c true, \c app is removed as a supporting
   application for any types for which it is no longer a supporting application
                        (including types which were removed as supporting types
   with previous callsto SetSupportedTypes(..., false)). If \c false, said
   mappings are not updated until the next SetSupportedTypes(..., true) call.
*/
status_t SupportingApps::SetSupportedTypes(const char *app,
                                           const BMessage *types,
                                           bool fullSync) {
...
/*! \brief Adds the given application signature to the set of supporting
        ...
        \param app The full application signature (i.e.
   "application/app-subtype") \return - B_OK: success, even if the app was
   already in the supporting apps list - "error code": failure
*/
...
/*! \brief Removes the given application signature from the set of supporting
        ...
        \param type The full mime type
        \param app The full application signature (i.e.
   "application/app-subtype") \return - B_OK: success, even if the app was not
   found in the supporting apps list - "error code": failure
*/
status_t SupportingApps::RemoveSupportingApp(const char *type,
                                             const char *app) {
  status_t err = type && app ? B_OK : B_BAD_VALUE;
  if (!err)
  ...
  if (status == B_OK) {
    ...
    while (true) {
      ...
      if (node.InitCheck() == B_OK &&
          node.ReadAttrString(kTypeAttr, &appSignature) >= B_OK) {
        // Read in the list of supported types
        BMessage msg;
        if (fDatabaseLocation->ReadMessageAttribute(
                appSignature, kSupportedTypesAttr, msg) == B_OK) {
          ...
          std::set<std::string> &supportedTypes =
              fSupportedTypes[appSignature.String()];
