...
// constructor
ResourcesContainer::ResourcesContainer() : fResources(), fIsModified(false) {}

// destructor
ResourcesContainer::~ResourcesContainer() { MakeEmpty(); }
...
// RemoveResource
ResourceItem *ResourcesContainer::RemoveResource(int32 index) {
  ...
  for (int32 i = 0; index == -1 && i < count; i++) {
    ...
    if (item->Type() == type &&
        ((name == NULL && itemName == NULL) ||
         (name != NULL && itemName != NULL && strcmp(name, itemName) == 0))) {
      index = i;
    }
  }
  return index;
...
// ResourceAt
ResourceItem *ResourcesContainer::ResourceAt(int32 index) const {
