...
#define CALLED(x...)                                                           \
  FunctionTracer _ft("BTextControl", __FUNCTION__, sFunctionDepth)
#define TRACE(x...)                                                            \
  {                                                                            \
    BString _to;                                                               \
...
static property_info sPropertyList[] = {{"Value",
...
struct BTextControl::LayoutData {
  LayoutData(float width, float height)
      : label_layout_item(NULL), text_view_layout_item(NULL),
        previous_width(width), previous_height(height), valid(false) {}
...
status_t BTextControl::AllArchived(BMessage *into) const {
  ...
  if (err == B_OK && archiver.IsArchived(fLayoutData->label_layout_item)) {
    err =
        archiver.AddArchivable(kLabelItemField, fLayoutData->label_layout_item);
  }

  return err;
}

status_t BTextControl::AllUnarchived(const BMessage *from) {
  status_t err;
...
void BTextControl::AllAttached() { BControl::AllAttached(); }

void BTextControl::AllDetached() { BControl::AllDetached(); }
...
void BTextControl::DetachedFromWindow() { BControl::DetachedFromWindow(); }
...
void BTextControl::Draw(BRect updateRect) {
  ...
  if (Label() != NULL) {
    ...
    be_control_look->DrawLabel(this, Label(), rect, updateRect, base, flags,
                               BAlignment(fLabelAlign, B_ALIGN_MIDDLE));
  }
}
...
void BTextControl::FrameResized(float width, float height) {
  CALLED();
...
status_t BTextControl::Invoke(BMessage *message) {
  return BControl::Invoke(message);
}

void BTextControl::LayoutInvalidated(bool descendants) {
  CALLED();

  fLayoutData->valid = false;
}

void BTextControl::MessageReceived(BMessage *message) {
...
void BTextControl::MouseMoved(BPoint where, uint32 transit,
                              const BMessage *dragMessage) {
  BControl::MouseMoved(where, transit, dragMessage);
}

void BTextControl::MouseUp(BPoint where) { BControl::MouseUp(where); }
...
const char *BTextControl::Text() const { return fText->Text(); }

int32 BTextControl::TextLength() const { return fText->TextLength(); }
...
void BTextControl::SetValue(int32 value) { BControl::SetValue(value); }

BTextView *BTextControl::TextView() const { return fText; }
...
void BTextControl::SetAlignment(alignment labelAlignment,
                                alignment textAlignment) {
...
float BTextControl::Divider() const { return fDivider; }
...
void BTextControl::SetEnabled(bool enable) {
  if (IsEnabled() == enable)
    return;
...
void BTextControl::GetPreferredSize(float *_width, float *_height) {
  CALLED();
...
BHandler *BTextControl::ResolveSpecifier(BMessage *message, int32 index,
...
BSize BTextControl::PreferredSize() {
  CALLED();
...
BAlignment BTextControl::LayoutAlignment() {
  CALLED();

  _ValidateLayoutData();
  return BLayoutUtils::ComposeAlignment(
      ExplicitAlignment(), BAlignment(B_ALIGN_LEFT, B_ALIGN_VERTICAL_CENTER));
}
...
status_t BTextControl::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BTextControl::MinSize();
    return B_OK;

  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BTextControl::MaxSize();
    return B_OK;
...
void BTextControl::_UpdateTextViewColors(bool enable) {
...
void BTextControl::_CommitValue() {}

void BTextControl::_InitData(const char *label, const BMessage *archive) {
  BRect bounds(Bounds());
...
void BTextControl::_InitText(const char *initialText, const BMessage *archive) {
  if (archive)
  ...
  if (fText == NULL) {
    ...
    fText = new BPrivate::_BTextInput_(frame, textRect, B_FOLLOW_ALL,
...
void BTextControl::_ValidateLayout() {
  CALLED();
...
void BTextControl::_LayoutTextView() {
  CALLED();
...
void BTextControl::_UpdateFrame() {
  CALLED();
...
void BTextControl::_ValidateLayoutData() {
  CALLED();
...
bool BTextControl::LabelLayoutItem::IsVisible() {
  return !fParent->IsHidden(fParent);
}

void BTextControl::LabelLayoutItem::SetVisible(bool visible) {
  // not allowed
}

BRect BTextControl::LabelLayoutItem::Frame() { return fFrame; }

void BTextControl::LabelLayoutItem::SetFrame(BRect frame) {
  fFrame = frame;
  fParent->_UpdateFrame();
}

void BTextControl::LabelLayoutItem::SetParent(BTextControl *parent) {
  fParent = parent;
}

BView *BTextControl::LabelLayoutItem::View() { return fParent; }

BSize BTextControl::LabelLayoutItem::BaseMinSize() {
...
BSize BTextControl::LabelLayoutItem::BaseMaxSize() { return BaseMinSize(); }

BSize BTextControl::LabelLayoutItem::BasePreferredSize() {
  return BaseMinSize();
}

BAlignment BTextControl::LabelLayoutItem::BaseAlignment() {
  return BAlignment(B_ALIGN_USE_FULL_WIDTH, B_ALIGN_USE_FULL_HEIGHT);
}

BRect BTextControl::LabelLayoutItem::FrameInParent() const {
  return fFrame.OffsetByCopy(-fParent->Frame().left, -fParent->Frame().top);
}

status_t BTextControl::LabelLayoutItem::Archive(BMessage *into,
                                                bool deep) const {
  BArchiver archiver(into);
...
BArchivable *BTextControl::LabelLayoutItem::Instantiate(BMessage *from) {
...
bool BTextControl::TextViewLayoutItem::IsVisible() {
  return !fParent->IsHidden(fParent);
}

void BTextControl::TextViewLayoutItem::SetVisible(bool visible) {
  // not allowed
}

BRect BTextControl::TextViewLayoutItem::Frame() { return fFrame; }

void BTextControl::TextViewLayoutItem::SetFrame(BRect frame) {
  fFrame = frame;
  fParent->_UpdateFrame();
}

void BTextControl::TextViewLayoutItem::SetParent(BTextControl *parent) {
  fParent = parent;
}

BView *BTextControl::TextViewLayoutItem::View() { return fParent; }

BSize BTextControl::TextViewLayoutItem::BaseMinSize() {
  fParent->_ValidateLayoutData();
...
BSize BTextControl::TextViewLayoutItem::BaseMaxSize() {
...
BSize BTextControl::TextViewLayoutItem::BasePreferredSize() {
  BSize size(BaseMinSize());
  // puh, no idea...
  size.width = 100;
...
BAlignment BTextControl::TextViewLayoutItem::BaseAlignment() {
  return BAlignment(B_ALIGN_USE_FULL_WIDTH, B_ALIGN_USE_FULL_HEIGHT);
}

BRect BTextControl::TextViewLayoutItem::FrameInParent() const {
  return fFrame.OffsetByCopy(-fParent->Frame().left, -fParent->Frame().top);
}

status_t BTextControl::TextViewLayoutItem::Archive(BMessage *into,
                                                   bool deep) const {
  BArchiver archiver(into);
  status_t err = BAbstractLayoutItem::Archive(into, deep);
  if (err == B_OK)
...
BArchivable *BTextControl::TextViewLayoutItem::Instantiate(BMessage *from) {
...
B_IF_GCC_2(InvalidateLayout__12BTextControlb,
           _ZN12BTextControl16InvalidateLayoutEb)(BView *view,
                                                  bool descendants) {
  perform_data_layout_invalidated data;
