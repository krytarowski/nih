...
int8 BUnicodeChar::Type(uint32 c) {
  BUnicodeChar();
  return u_charType(c);
}
...
// "Zp"),
//		but is not also a non-breaking space (U+00A0 NBSP or U+2007
// Figure  Space 		or U+202F Narrow NBSP).
...
unicode_east_asian_width BUnicodeChar::EastAsianWidth(uint32 c) {
  return (unicode_east_asian_width)u_getIntPropertyValue(
      c, UCHAR_EAST_ASIAN_WIDTH);
}
...
size_t BUnicodeChar::UTF8StringLength(const char *string, size_t maxLength) {
  size_t len = 0;
