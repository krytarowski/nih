...
static property_info sPropertyInfo[] = {{"ChannelCount",
...
BChannelControl::BChannelControl(BRect frame, const char *name,
                                 ...
                                 int32 channel_count, uint32 resizingMode,
                                 uint32 flags)
    : BControl(frame, name, label, model, resizingMode, flags),
      fChannelCount(channel_count), fCurrentChannel(0), fChannelMin(NULL),
      fChannelMax(NULL), fChannelValues(NULL), fMultiLabels(NULL),
...
BChannelControl::BChannelControl(const char *name, const char *label,
                                 BMessage *model, int32 channelCount,
                                 uint32 flags)
    : BControl(name, label, model, flags), fChannelCount(channelCount),
      fCurrentChannel(0), fChannelMin(NULL), fChannelMax(NULL),
      fChannelValues(NULL), fMultiLabels(NULL), fModificationMsg(NULL) {
...
BChannelControl::BChannelControl(BMessage *archive)
    : BControl(archive), fChannelCount(0), fCurrentChannel(0),
      fChannelMin(NULL), fChannelMax(NULL), fChannelValues(NULL),
      fMultiLabels(NULL), fModificationMsg(NULL) {
...
status_t BChannelControl::Archive(BMessage *data, bool deep) const {
  ...
  if (status == B_OK && fChannelValues != NULL && fChannelMax != NULL &&
      fChannelMin != NULL) {
    for (int32 i = 0; i < fChannelCount; i++) {
...
void BChannelControl::AttachedToWindow() { BControl::AttachedToWindow(); }

void BChannelControl::DetachedFromWindow() { BControl::DetachedFromWindow(); }

void BChannelControl::ResizeToPreferred() { BControl::ResizeToPreferred(); }

void BChannelControl::MessageReceived(BMessage *message) {
  BControl::MessageReceived(message);
}

BHandler *BChannelControl::ResolveSpecifier(BMessage *message, int32 index,
  ...
  if (propertyInfo.FindMatch(message, index, specifier, what, property) <
      B_OK) {
    target =
        BControl::ResolveSpecifier(message, index, specifier, what, property);
  }
...
int32 BChannelControl::CurrentChannel() const { return fCurrentChannel; }

int32 BChannelControl::CountChannels() const { return fChannelCount; }
...
status_t BChannelControl::SetLimitsFor(int32 channel, int32 minimum,
                                       int32 maximum) {
...
status_t BChannelControl::GetLimits(int32 *outMinimum,
...
status_t BChannelControl::SetLimitLabels(const char *minLabel,
                                         const char *maxLabel) {
...
status_t BChannelControl::SetLimitLabelsFor(int32 fromChannel,
                                            int32 channelCount,
                                            const char *minLabel,
                                            const char *maxLabel) {
