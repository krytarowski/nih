  ...
  if (ioctl(fd, B_GET_ICON, &iconData) != 0) {
    ...
    if (status == B_OK) {
      BBitmap *icon32 = new (std::nothrow) BBitmap(
          BRect(0, 0, size - 1, size - 1), B_BITMAP_NO_SERVER_LINK, B_RGBA32);
      BBitmap *icon8 = new (std::nothrow) BBitmap(
          BRect(0, 0, size - 1, size - 1), B_BITMAP_NO_SERVER_LINK, B_CMAP8);
...
status_t get_device_icon(const char *device, BBitmap *icon, icon_size which) {
  ...
  if (icon->ColorSpace() != B_CMAP8 ||
    ...
    bitmap = new (std::nothrow) BBitmap(BRect(0, 0, iconSize - 1, iconSize - 1),
...
status_t get_device_icon(const char *device, uint8 **_data, size_t *_size,
  ...
  status_t status =
      ioctl(fd, B_GET_VECTOR_ICON, &iconData, sizeof(device_icon));
  if (status != 0)
    status = errno;
