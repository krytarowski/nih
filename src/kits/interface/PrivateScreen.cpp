...
struct Screens {
  ...
  Screens() : list(2, true), fLock("screen list") {}
...
private:
  static void _InitSingleton() { sDefaultInstance = new Screens; }
...
BPrivateScreen *BPrivateScreen::Get(BWindow *window) {
...
BPrivateScreen *BPrivateScreen::Get(int32 id) { return _Get(id, true); }

BPrivateScreen *BPrivateScreen::_Get(int32 id, bool check) {
...
BPrivateScreen *BPrivateScreen::GetNext(BPrivateScreen *screen) {
  Screens *screens = Screens::Default();
  AutoLocker<Screens> locker(screens);
...
bool BPrivateScreen::IsValid() const { return BPrivateScreen::_IsValid(ID()); }
...
uint8 BPrivateScreen::IndexForColor(uint8 red, uint8 green, uint8 blue,
                                    uint8 alpha) {
...
status_t BPrivateScreen::GetBitmap(BBitmap **_bitmap, bool drawCursor,
                                   BRect *bounds) {
  if (_bitmap == NULL)
    return B_BAD_VALUE;
...
status_t BPrivateScreen::ReadBitmap(BBitmap *bitmap, bool drawCursor,
                                    BRect *bounds) {
  if (bitmap == NULL)
    return B_BAD_VALUE;
...
rgb_color BPrivateScreen::DesktopColor(uint32 workspace) {
...
status_t BPrivateScreen::SetMode(uint32 workspace, display_mode *mode,
                                 bool makeDefault) {
  if (mode == NULL)
    return B_BAD_VALUE;
...
status_t BPrivateScreen::GetPixelClockLimits(display_mode *mode, uint32 *low,
                                             uint32 *high) {
...
bool BPrivateScreen::_Release() { return --fReferenceCount == 0; }

sem_id BPrivateScreen::_RetraceSemaphore() {
  BPrivate::AppServerLink link;
...
BPrivateScreen::BPrivateScreen(int32 id)
    : fID(id), fReferenceCount(0), fColorMap(NULL), fRetraceSem(-1),
      fRetraceSemValid(false), fOwnsColorMap(false), fFrame(0, 0, 0, 0),
      fLastUpdate(0) {}
