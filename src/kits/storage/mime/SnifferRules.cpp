...
/*!
        ...
        \brief A parsed sniffer rule and its corresponding mime type and rule
   string

        The parse sniffer rule is stored in the \c rule member, which is a
   pointer to a \c Sniffer::Rule object. This design was chosen to allow \c
   sniffer_rule objects	(as opposed to \c sniffer_rule pointers) to be
   used with STL objects without unnecessary copying. As a consequence of this
   decision, the \c SnifferRules object managing the rule list is responsible
   for actually deleting each \c sniffer_rule's \c Sniffer::Rule object.
*/

// sniffer_rule Constructor
//! Creates a new \c sniffer_rule object
SnifferRules::sniffer_rule::sniffer_rule(Sniffer::Rule *rule) : rule(rule) {}
...
*/
bool operator<(const SnifferRules::sniffer_rule &left,
               const SnifferRules::sniffer_rule &right) {
...
SnifferRules::SnifferRules(DatabaseLocation *databaseLocation,
    ...
    : fDatabaseLocation(databaseLocation), fMimeSniffer(mimeSniffer),
      fMaxBytesNeeded(0), fHaveDoneFullBuild(false) {}
...
/*!	\brief Guesses a MIME type for the supplied entry_ref.
        ...
        \return
        - \c B_OK: success
        - \c Mime::kMimeGuessFailure: no match found (\a type is left
   unmodified) - error code: failure
*/
...
/*!	\brief Guesses a MIME type for the given chunk of data.
   ...
   unmodified) - error code: failure
*/
status_t SnifferRules::GuessMimeType(const void *buffer, int32 length,
                                     BString *type) {
...
status_t SnifferRules::SetSnifferRule(const char *type, const char *rule) {
  ...
  if (!err) {
    ...
    if (err)
      DBG(OUT(
          "ERROR: SnifferRules::SetSnifferRule(): rule parsing error:\n%s\n",
          parseError.String()));
  }
...
/*! \brief Crawls through the database, parses each sniffer rule it finds, adds
        each parsed rule to the rule list, and sorts the list by priority,
   largest first.
...
status_t SnifferRules::BuildRuleList() {
  ...
  if (!err) {
    ...
    while (true) {
      ...
      if (err) {
        ...
        if (entry.IsDirectory() && entry.GetName(supertype) == B_OK &&
  ...
  if (!err) {
    ...
    DBG(OUT(
        "Mime::SnifferRules::BuildRuleList() failed, error code == 0x%" B_PRIx32
        "\n",
        err));
  }
  return err;
...
/*!	\brief Guesses a MIME type for the supplied chunk of data.
        ...
        \param file The file to sniff. May be \c NULL. \a buffer is always
   given. \param buffer Pointer to a data buffer to sniff \param length The
   length of the data buffer pointed to by \a buffer \param type Pointer to a
   pre-allocated BString which is set to the resulting MIME type. \return - \c
   B_OK: success - \c Mime::kMimeGuessFailure: no match found (\a type is left
   unmodified) - error code: failure
*/
status_t SnifferRules::GuessMimeType(BFile *file, const void *buffer,
                                     int32 length, BString *type) {
  ...
  if (!err && fMimeSniffer != NULL) {
    addonPriority =
        fMimeSniffer->GuessMimeType(file, buffer, length, &mimeType);
  }
  ...
  if (!err) {
    ...
    for (std::list<sniffer_rule>::const_iterator i = fRuleList.begin();
      ...
      if (i->rule) {
        ...
        DBG(OUT("WARNING: "
                "Mime::SnifferRules::GuessMimeType(BPositionIO*,BString*): "
                "NULL sniffer_rule::rule member found in rule list for type == "
                "'%s', "
                "rule_string == '%s'\n",
                i->type.c_str(), i->rule_string.c_str()));
      }
...
/*! \brief Returns the maxmimum number of bytes needed in a data buffer for
        all the currently installed rules to be able to perform a complete
   sniff, or an error code if something goes wrong.

        If the internal rule list has not yet been built (this includes parsing
        all the installed rules), it will be.

        \return: If the return value is non-negative, it represents	the
   max number of bytes needed to do a complete sniff. Otherwise, the number
   returned is an error code.
*/
ssize_t SnifferRules::MaxBytesNeeded() {
  ...
  if (!err) {
    ...
    if (fMimeSniffer != NULL) {
      fMaxBytesNeeded =
          max_c(fMaxBytesNeeded, (ssize_t)fMimeSniffer->MinimalBufferSize());
    }
  }
  return err;
}
...
/*! \brief Handles a portion of the initial rule list construction for
        ...
        \param type The mime type of interest. The mime string is expected to be
   valid and lowercase. Both "supertype" and "supertype/subtype" mime types are
   allowed. \param bytesNeeded Returns the minimum number of bytes needed for
   this rule to perform a complete sniff. May not be NULL because I'm lazy and
   this function is for internal use only anyway. \return The return value is
   essentially ignored (as this function prints out the debug warning if a parse
   fails), but that being said: - \c B_OK: success - \c other error code:
   failure
*/
status_t SnifferRules::ProcessType(const char *type, ssize_t *bytesNeeded) {
  ...
  // Read the attr
  if (!err) {
    err = fDatabaseLocation->ReadStringAttribute(type, kSnifferRuleAttr, str);
  }
  // Parse the rule
  if (!err) {
