...
static void CoerceFormatTo24HourClock(BString &format) {
  ...
  for (char *previousPos = currentPos;
       (ch = BUnicodeChar::FromUTF8((const char **)&currentPos)) != 0;
...
enum ClockHoursState { CLOCK_HOURS_UNSET = 0, CLOCK_HOURS_24, CLOCK_HOURS_12 };
...
BFormattingConventions::BFormattingConventions(const char *id)
    : fCachedUse24HourClock(CLOCK_HOURS_UNSET),
      ...
      fUseStringsFromPreferredLanguage(false), fICULocale(new icu::Locale(id)) {
}
...
BFormattingConventions &BFormattingConventions::
operator=(const BFormattingConventions &other) {
  if (this == &other)
    return *this;
  ...
  for (int s = 0; s < B_DATE_FORMAT_STYLE_COUNT; ++s) {
    ...
    for (int t = 0; t < B_TIME_FORMAT_STYLE_COUNT; ++t) {
      ...
      fExplicitDateTimeFormats[s][t] = other.fExplicitDateTimeFormats[s][t];
    }
  }
  for (int s = 0; s < B_TIME_FORMAT_STYLE_COUNT; ++s) {
    fCachedTimeFormats[s] = other.fCachedTimeFormats[s];
...
BFormattingConventions::~BFormattingConventions() { delete fICULocale; }

bool BFormattingConventions::
operator==(const BFormattingConventions &other) const {
  if (this == &other)
    return true;
...
const char *BFormattingConventions::ID() const { return fICULocale->getName(); }
...
BMeasurementKind BFormattingConventions::MeasurementKind() const {
...
status_t BFormattingConventions::GetDateFormat(BDateFormatStyle style,
  ...
  outFormat = fExplicitDateFormats[style].Length() ? fExplicitDateFormats[style]
...
status_t BFormattingConventions::GetTimeFormat(BTimeFormatStyle style,
  ...
  outFormat = fExplicitTimeFormats[style].Length() ? fExplicitTimeFormats[style]
...
status_t BFormattingConventions::GetDateTimeFormat(BDateFormatStyle dateStyle,
  ...
  ObjectDeleter<DateFormat> dateFormatter(DateFormat::createDateTimeInstance(
      (DateFormat::EStyle)dateStyle, (DateFormat::EStyle)timeStyle,
      *fICULocale));
  if (dateFormatter.Get() == NULL)
    return B_NO_MEMORY;
...
void BFormattingConventions::SetExplicitDateTimeFormat(
...
void BFormattingConventions::CoerceFormatForClock(BString &outFormat) const {
  int8 use24HourClock = fExplicitUse24HourClock != CLOCK_HOURS_UNSET
                            ? fExplicitUse24HourClock
                            : fCachedUse24HourClock;
  if (use24HourClock != CLOCK_HOURS_UNSET) {
