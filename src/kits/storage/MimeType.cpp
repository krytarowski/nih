...
BMimeType::BMimeType() : fType(NULL), fCStatus(B_NO_INIT) {}

// Creates a BMimeType object and initializes it to the supplied
// MIME type.
BMimeType::BMimeType(const char *mimeType) : fType(NULL), fCStatus(B_NO_INIT) {
  SetTo(mimeType);
}

// Frees all resources associated with this object.
BMimeType::~BMimeType() { Unset(); }
...
status_t BMimeType::InitCheck() const { return fCStatus; }

// Returns the MIME string represented by this object
const char *BMimeType::Type() const { return fType; }
...
status_t BMimeType::GetPreferredApp(char *signature, app_verb verb) const {
  status_t err = InitCheck();
  if (err == B_OK) {
    err = default_database_location()->GetPreferredApp(Type(), signature, verb);
  }
...
status_t BMimeType::GetFileExtensions(BMessage *extensions) const {
  ...
  status_t err = InitCheck();
  if (err == B_OK) {
    err = default_database_location()->GetFileExtensions(Type(), *extensions);
  }
...
status_t BMimeType::GetShortDescription(char *description) const {
  status_t err = InitCheck();
  if (err == B_OK) {
    err = default_database_location()->GetShortDescription(Type(), description);
  }
...
status_t BMimeType::GetLongDescription(char *description) const {
  status_t err = InitCheck();
  if (err == B_OK) {
    err = default_database_location()->GetLongDescription(Type(), description);
  }
...
status_t BMimeType::GetSupportingApps(BMessage *signatures) const {
  ...
  if (err == B_OK) {
    err = (status_t)(signatures->what == B_REG_RESULT ? B_OK : B_BAD_REPLY);
  }
  if (err == B_OK)
...
status_t BMimeType::SetPreferredApp(const char *signature, app_verb verb) {
  ...
  BMessage message(signature && signature[0] ? B_REG_MIME_SET_PARAM
...
status_t BMimeType::SetShortDescription(const char *description) {
  ...
  BMessage message(description && description[0] ? B_REG_MIME_SET_PARAM
                                                 : B_REG_MIME_DELETE_PARAM);
  BMessage reply;
  status_t result;
...
/*static*/ status_t BMimeType::GetInstalledSupertypes(BMessage *supertypes) {
  ...
  if (err == B_OK) {
    err = (status_t)(supertypes->what == B_REG_RESULT ? B_OK : B_BAD_REPLY);
  }
  if (err == B_OK)
...
/*static*/ status_t BMimeType::GetInstalledTypes(const char *supertype,
                                                 BMessage *types) {
  if (types == NULL)
    return B_BAD_VALUE;
...
status_t BMimeType::GetIconForType(const char *type, BBitmap *icon,
  ...
  if (type) {
    ...
    if (err == B_OK) {
      err = default_database_location()->GetIconForType(Type(), type, *icon,
                                                        which);
    }
  } else
...
// the given type.
status_t BMimeType::GetIconForType(const char *type, uint8 **_data,
                                   size_t *_size) const {
...
// files of the given type.
status_t BMimeType::SetIconForType(const char *type, const BBitmap *icon,
                                   icon_size which) {
  status_t err = InitCheck();
...
// Sets the large or mini icon used by an application of this type for
// files of the given type.
status_t BMimeType::SetIconForType(const char *type, const uint8 *data,
                                   size_t dataSize) {
  status_t err = InitCheck();
...
status_t BMimeType::GuessMimeType(const void *buffer, int32 length,
                                  BMimeType *type) {
...
// Initializes this object to the supplied MIME type
status_t BMimeType::SetType(const char *mimeType) { return SetTo(mimeType); }
...
// assignment operator.
// Unimplemented
BMimeType &BMimeType::operator=(const BMimeType &) {
  return *this;
  // not implemented
}
...
/*!	Sets the list of MIME types supported by the MIME type (which is
   ...
   will afterwards include the signature of this application.
   ...
   be updated as well, i.e. whether this application shall be removed from their
        lists of supporting applications.

        If \a fullSync is \c false, this application will not be removed from
   the previously supported types' supporting apps lists until the next call to
   BMimeType::SetSupportedTypes() or BMimeType::DeleteSupportedTypes() with a \c
   true \a fullSync parameter, the next call to BMimeType::Delete(), or the next
   reboot.
...
/*!	Returns a list of mime types associated with the given file extension
   ...
   to by \a types. The types are stored in the message's "types" field, which is
   an array of \c B_STRING_TYPE values.
