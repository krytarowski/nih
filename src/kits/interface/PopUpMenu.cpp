...
BPopUpMenu::BPopUpMenu(const char *name, bool radioMode, bool labelFromMarked,
                       menu_layout layout)
    : BMenu(name, layout), fUseWhere(false), fAutoDestruct(false),
      fTrackThread(-1) {
  if (radioMode)
    SetRadioMode(true);
...
BPopUpMenu::BPopUpMenu(BMessage *archive)
    : BMenu(archive), fUseWhere(false), fAutoDestruct(false), fTrackThread(-1) {
}
...
BMenuItem *BPopUpMenu::Go(BPoint where, bool deliversMessage, bool openAnyway,
                          bool async) {
...
void BPopUpMenu::MessageReceived(BMessage *message) {
  BMenu::MessageReceived(message);
}

void BPopUpMenu::MouseDown(BPoint point) { BView::MouseDown(point); }

void BPopUpMenu::MouseUp(BPoint point) { BView::MouseUp(point); }

void BPopUpMenu::MouseMoved(BPoint point, uint32 code,
                            const BMessage *message) {
  BView::MouseMoved(point, code, message);
}

void BPopUpMenu::AttachedToWindow() { BMenu::AttachedToWindow(); }

void BPopUpMenu::DetachedFromWindow() { BMenu::DetachedFromWindow(); }

void BPopUpMenu::FrameMoved(BPoint newPosition) {
  BMenu::FrameMoved(newPosition);
}

void BPopUpMenu::FrameResized(float newWidth, float newHeight) {
  BMenu::FrameResized(newWidth, newHeight);
}

BHandler *BPopUpMenu::ResolveSpecifier(BMessage *message, int32 index,
...
status_t BPopUpMenu::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BPopUpMenu::MinSize();
    return B_OK;
  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BPopUpMenu::MaxSize();
    return B_OK;
...
void BPopUpMenu::ResizeToPreferred() { BMenu::ResizeToPreferred(); }

void BPopUpMenu::GetPreferredSize(float *_width, float *_height) {
  BMenu::GetPreferredSize(_width, _height);
}

void BPopUpMenu::MakeFocus(bool state) { BMenu::MakeFocus(state); }

void BPopUpMenu::AllAttached() { BMenu::AllAttached(); }

void BPopUpMenu::AllDetached() { BMenu::AllDetached(); }

void BPopUpMenu::SetAsyncAutoDestruct(bool on) { fAutoDestruct = on; }

bool BPopUpMenu::AsyncAutoDestruct() const { return fAutoDestruct; }
...
BPopUpMenu &BPopUpMenu::operator=(const BPopUpMenu &other) { return *this; }

BMenuItem *BPopUpMenu::_Go(BPoint where, bool autoInvoke, bool startOpened,
...
/* static */
int32 BPopUpMenu::_thread_entry(void *menuData) {
  ...
  data->selected =
      menu->_StartTrack(data->where, data->autoInvoke, data->startOpened, rect);
...
BMenuItem *BPopUpMenu::_StartTrack(BPoint where, bool autoInvoke,
...
BMenuItem *BPopUpMenu::_WaitMenu(void *_data) {
