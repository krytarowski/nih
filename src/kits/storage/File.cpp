...
BFile::BFile(const BFile &file) : fMode(0) { *this = file; }
...
BFile::BFile(const entry_ref *ref, uint32 openMode) : fMode(0) {
  SetTo(ref, openMode);
}

// Creates a BFile and initializes it to the file referred to by
// the supplied BEntry and according to the specified open mode.
BFile::BFile(const BEntry *entry, uint32 openMode) : fMode(0) {
  SetTo(entry, openMode);
}

// Creates a BFile and initializes it to the file referred to by
// the supplied path name and according to the specified open mode.
BFile::BFile(const char *path, uint32 openMode) : fMode(0) {
  SetTo(path, openMode);
}
...
status_t BFile::SetTo(const entry_ref *ref, uint32 openMode) {
  Unset();
...
status_t BFile::SetTo(const BEntry *entry, uint32 openMode) {
  Unset();
...
status_t BFile::SetTo(const char *path, uint32 openMode) {
  Unset();
...
// into a buffer.
ssize_t BFile::ReadAt(off_t location, void *buffer, size_t size) {
  if (InitCheck() != B_OK)
    return InitCheck();
  if (location < 0)
    return B_BAD_VALUE;
...
// into the file.
ssize_t BFile::WriteAt(off_t location, const void *buffer, size_t size) {
  if (InitCheck() != B_OK)
    return InitCheck();
  if (location < 0)
    return B_BAD_VALUE;
...
off_t BFile::Seek(off_t offset, uint32 seekMode) {
  if (InitCheck() != B_OK)
    return B_FILE_ERROR;
...
status_t BFile::GetSize(off_t *size) const { return BStatable::GetSize(size); }
...
*/
int BFile::get_fd() const { return fFd; }

//! Overrides BNode::close_fd() for binary compatibility with BeOS R5.
void BFile::close_fd() { BNode::close_fd(); }
