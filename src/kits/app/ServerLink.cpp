...
status_t ServerLink::AttachRegion(const BRegion &region) {
  ...
  if (region.fCount > 0) {
    ...
    return fSender->Attach(region.fData, region.fCount * sizeof(clipping_rect));
  }
...
status_t ServerLink::AttachGradient(const BGradient &gradient) {
  ...
  GTRACE(
      ("ServerLink::AttachGradient> color stop count == %d\n", (int)stopCount));
  ...
  case BGradient::TYPE_RADIAL_FOCUS: {
    ...
    const BGradientRadialFocus *radialFocus = (BGradientRadialFocus *)&gradient;
