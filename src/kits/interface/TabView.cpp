...
static property_info sPropertyList[] = {{"Selection",
...
BTab::BTab(BView *contentsView)
    : fEnabled(true), fSelected(false), fFocus(false), fView(contentsView),
      fTabView(NULL) {}

BTab::BTab(BMessage *archive)
    : BArchivable(archive), fSelected(false), fFocus(false), fView(NULL),
      fTabView(NULL) {
  bool disable;
...
bool BTab::IsSelected() const { return fSelected; }
...
void BTab::SetEnabled(bool enable) { fEnabled = enable; }

bool BTab::IsEnabled() const { return fEnabled; }

void BTab::MakeFocus(bool focus) { fFocus = focus; }

bool BTab::IsFocus() const { return fFocus; }
...
BView *BTab::View() const { return fView; }
...
void BTab::DrawFocusMark(BView *owner, BRect frame) {
  ...
  owner->StrokeLine(
      BPoint((frame.left + frame.right - width) / 2.0, frame.bottom - offset),
      BPoint((frame.left + frame.right + width) / 2.0, frame.bottom - offset));
}

void BTab::DrawLabel(BView *owner, BRect frame) {
  be_control_look->DrawLabel(
      owner, Label(), frame, frame, ui_color(B_PANEL_BACKGROUND_COLOR),
      IsEnabled() ? 0 : BPrivate::BControlLook::B_DISABLED,
      BAlignment(B_ALIGN_HORIZONTAL_CENTER, B_ALIGN_VERTICAL_CENTER));
}

void BTab::DrawTab(BView *owner, BRect frame, tab_position position,
                   bool full) {
...
BTabView::BTabView(BMessage *archive)
    : BView(BUnarchiver::PrepareArchive(archive)), fTabList(new BList),
      fContainerView(NULL), fFocus(-1) {
...
status_t BTabView::AllUnarchived(const BMessage *archive) {
...
status_t BTabView::Perform(perform_code code, void *_data) {
  switch (code) {
...
void BTabView::DetachedFromWindow() { BView::DetachedFromWindow(); }

void BTabView::AllAttached() { BView::AllAttached(); }

void BTabView::AllDetached() { BView::AllDetached(); }
...
void BTabView::MouseUp(BPoint where) { BView::MouseUp(where); }

void BTabView::MouseMoved(BPoint where, uint32 transit,
                          const BMessage *dragMessage) {
  BView::MouseMoved(where, transit, dragMessage);
}

void BTabView::Pulse() { BView::Pulse(); }
...
int32 BTabView::Selection() const { return fSelection; }
...
void BTabView::MakeFocus(bool focus) {
  BView::MakeFocus(focus);

  SetFocusTab(Selection(), focus);
}

void BTabView::SetFocusTab(int32 tab, bool focus) {
  if (tab >= CountTabs())
    tab = 0;
...
int32 BTabView::FocusTab() const { return fFocus; }
...
BRect BTabView::DrawTabs() {
  ...
  if (fBorderStyle != B_FANCY_BORDER) {
    ...
    be_control_look->DrawBorder(this, bounds, bounds, base, B_PLAIN_BORDER, 0,
                                BControlLook::B_TOP_BORDER |
                                    BControlLook::B_BOTTOM_BORDER);
...
void BTabView::DrawBox(BRect selectedTabRect) {
  ...
  if (fBorderStyle == B_FANCY_BORDER)
    ...
    be_control_look->DrawBorder(this, rect, rect, base, B_PLAIN_BORDER, 0,
...
BRect BTabView::TabFrame(int32 index) const {
  ...
  default:
    return BRect(offset + index * width, 0.0f, offset + index * width + width,
                 height);
  }
...
void BTabView::SetFlags(uint32 flags) { BView::SetFlags(flags); }

void BTabView::SetResizingMode(uint32 mode) { BView::SetResizingMode(mode); }

// #pragma mark -

void BTabView::ResizeToPreferred() { BView::ResizeToPreferred(); }

void BTabView::GetPreferredSize(float *_width, float *_height) {
...
void BTabView::FrameResized(float newWidth, float newHeight) {
...
button_width BTabView::TabWidth() const { return fTabWidthSetting; }
...
float BTabView::TabHeight() const { return fTabHeight; }
...
border_style BTabView::Border() const { return fBorderStyle; }

BView *BTabView::ContainerView() const { return fContainerView; }

int32 BTabView::CountTabs() const { return fTabList->CountItems(); }
...
void BTabView::_InitObject(bool layouted, button_width width) {
  fTabList = new BList;
...
void BTabView::_InitContainerView(bool layouted) {
...
BSize BTabView::_TabsMinSize() const {
...
float BTabView::_BorderWidth() const {
  switch (fBorderStyle) {
  default:
  case B_FANCY_BORDER:
    return 3.0f;
...
void BTabView::_LayoutContainerView(bool layouted) {
  ...
  if (layouted) {
    ...
    if (layout != NULL) {
      layout->SetInsets(borderWidth,
                        borderWidth + TabHeight() - topBorderOffset,
...
BTabView::BTabView(const BTabView &tabView) : BView(tabView) {
  // this is private and not functional, but exported
}
...
B_IF_GCC_2(_ReservedTabView1__8BTabView,
           _ZN8BTabView17_ReservedTabView1Ev)(BTabView *tabView,
