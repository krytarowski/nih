...
BPrintJob::BPrintJob(const char *jobName)
    : fPrintJobName(NULL), fSpoolFile(NULL), fError(B_NO_INIT),
      fSetupMessage(NULL), fDefaultSetupMessage(NULL), fAbort(0),
...
void BPrintJob::CommitJob() {
  ...
  fSpoolFile->WriteAttr(PSRV_SPOOL_ATTR_STATUS, B_STRING_TYPE, 0,
                        ...
                        strlen(PSRV_JOB_STATUS_WAITING) + 1);
...
int32 BPrintJob::FirstPage() { return fFirstPage; }

int32 BPrintJob::LastPage() { return fLastPage; }
...
void BPrintJob::_AddSetupSpec() { fSetupMessage->Flatten(fSpoolFile); }
...
PrintServerMessenger::PrintServerMessenger(uint32 what, BMessage *input)
    : fWhat(what), fInput(input), fRequest(NULL), fResult(NULL),
      fThreadCompleted(-1), fHiddenApplicationModalWindow(NULL) {
  RejectUserInput();
}
...
status_t PrintServerMessenger::SendRequest() {
  ...
  thread_id id =
      spawn_thread(MessengerThread, "async_request", B_NORMAL_PRIORITY, this);
  ...
  if (window != NULL) {
    ...
    while (true) {
      do {
        err = acquire_sem_etc(fThreadCompleted, 1, B_RELATIVE_TIMEOUT, 50000);
