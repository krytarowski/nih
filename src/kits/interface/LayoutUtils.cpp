...
// AddDistances
float BLayoutUtils::AddDistances(float a, float b) {
  float sum = a + b + 1;
  if (sum >= B_SIZE_UNLIMITED)
    return B_SIZE_UNLIMITED;
...
// AddDistances
float BLayoutUtils::AddDistances(float a, float b, float c) {
...
// SubtractDistances
float BLayoutUtils::SubtractDistances(float a, float b) {
  if (a < b)
    return -1;
  return a - b - 1;
}
...
// FixSizeConstraints
void BLayoutUtils::FixSizeConstraints(float &min, float &max,
                                      float &preferred) {
  if (max < min)
    max = min;
  if (preferred < min)
...
// FixSizeConstraints
void BLayoutUtils::FixSizeConstraints(BSize &min, BSize &max,
                                      BSize &preferred) {
...
// ComposeSize
BSize BLayoutUtils::ComposeSize(BSize size, BSize layoutSize) {
  if (!size.IsWidthSet())
...
// ComposeAlignment
BAlignment BLayoutUtils::ComposeAlignment(BAlignment alignment,
...
BRect BLayoutUtils::AlignOnRect(BRect rect, BSize size, BAlignment alignment) {
  rect.left +=
      (int)((rect.Width() - size.width) * alignment.RelativeHorizontal());
  rect.top +=
      (int)(((rect.Height() - size.height)) * alignment.RelativeVertical());
...
/*!	Offsets a rectangle's location so that it lies fully in a given
   rectangular frame.
        ...
        If the rectangle is too wide/high to fully fit in the frame, its
   left/top edge is offset to 0. The rect's size always remains unchanged.
...
*/
/*static*/ BRect BLayoutUtils::MoveIntoFrame(BRect rect, BSize frameSize) {
...
/*static*/ BString BLayoutUtils::GetLayoutTreeDump(BView *view) {
  BString result;
  _GetLayoutTreeDump(view, 0, result);
  return result;
}

/*static*/ BString BLayoutUtils::GetLayoutTreeDump(BLayoutItem *item) {
  BString result;
  _GetLayoutTreeDump(item, 0, false, result);
  return result;
}

/*static*/ void BLayoutUtils::_GetLayoutTreeDump(BView *view, int level,
                                                 BString &_output) {
  BString indent;
  ...
  _output << BString().SetToFormat(
      ...
      "%s  max:   (%f, %f)\n"
      "%s  pref:  (%f, %f)\n",
      indent.String(), view, typeid(*view).name(), indent.String(), frame.left,
      frame.top, frame.right, frame.bottom, indent.String(), min.width,
      min.height, indent.String(), max.width, max.height, indent.String(),
...
/*static*/ void BLayoutUtils::_GetLayoutTreeDump(BLayoutItem *item, int level,
  ...
  _output << BString().SetToFormat(
      ...
      indent.String(), min.width, min.height, indent.String(), max.width,
      max.height, indent.String(), preferred.width, preferred.height);
