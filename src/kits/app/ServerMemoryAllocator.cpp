...
ServerMemoryAllocator::ServerMemoryAllocator() : fAreas(4) {}
...
status_t ServerMemoryAllocator::InitCheck() { return B_OK; }
...
status_t ServerMemoryAllocator::AddArea(area_id serverArea, area_id &_area,
  ...
  mapping->local_area = clone_area(
      readOnly ? "server read-only memory" : "server_memory", &base,
      addressSpec, B_READ_AREA | (readOnly ? 0 : B_WRITE_AREA), serverArea);
...
status_t ServerMemoryAllocator::AreaAndBaseFor(area_id serverArea,
                                               area_id &_area, uint8 *&_base) {
