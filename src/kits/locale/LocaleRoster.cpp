...
BLocaleRoster::BLocaleRoster()
    : fData(new (std::nothrow) BPrivate::LocaleRosterData(
          BLanguage("en_US"), BFormattingConventions("en_US"))) {}

BLocaleRoster::~BLocaleRoster() { delete fData; }

/*static*/ BLocaleRoster *BLocaleRoster::Default() {
  return MutableLocaleRoster::Default();
}

status_t BLocaleRoster::Refresh() { return fData->Refresh(); }
...
const BLocale *BLocaleRoster::GetDefaultLocale() const {
...
status_t BLocaleRoster::GetFlagIconForCountry(BBitmap *flagIcon,
  ...
  size_t size;
  const void *buffer =
      resources->LoadResource(B_VECTOR_ICON_TYPE, normalizedCode, &size);
...
status_t BLocaleRoster::GetAvailableCatalogs(BMessage *languageList,
  ...
  for (int32 i = 0; i < count; ++i) {
    ...
    info->fLanguagesFunc(languageList, sigPattern, langPattern, fingerprint);
  }
...
/*!	\brief Looks up a localized filename from a catalog.
   ...
   lookup. \param traverse A boolean to decide if symlinks are to be traversed.
        \return
        - \c B_OK: success
...
status_t BLocaleRoster::GetLocalizedFileName(BString &localizedFileName,
  ...
  status_t status =
      _PrepareCatalogEntry(ref, signature, context, string, traverse);
...
static status_t _InitializeCatalog(void *param) {
  ...
  while (get_next_image_info(0, &cookie, &info) == B_OK) {
    if ((char *)info.data < (char *)catalog &&
        (char *)info.data + info.data_size > (char *)catalog) {
      found = true;
      break;
    }
  }
...
BCatalog *BLocaleRoster::_GetCatalog(BCatalog *catalog,
...
status_t BLocaleRoster::_PrepareCatalogEntry(const entry_ref &ref,
                                             BString &signature,
                                             BString &context, BString &string,
                                             bool traverse) {
  ...
  if (signature.Length() == 0 || context.Length() == 0 || string.Length() == 0)
    return B_ENTRY_NOT_FOUND;
