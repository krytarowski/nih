...
struct BGroupLayout::ItemLayoutData {
  ...
  ItemLayoutData() : weight(1) {}
};

BGroupLayout::BGroupLayout(orientation orientation, float spacing)
    : BTwoDimensionalLayout(), fOrientation(orientation) {
  SetSpacing(spacing);
}

BGroupLayout::BGroupLayout(BMessage *from) : BTwoDimensionalLayout(from) {
  bool isVertical;
...
float BGroupLayout::Spacing() const { return fHSpacing; }
...
orientation BGroupLayout::Orientation() const { return fOrientation; }
...
BLayoutItem *BGroupLayout::AddView(int32 index, BView *child) {
...
BArchivable *BGroupLayout::Instantiate(BMessage *from) {
  if (validate_instantiation(from, "BGroupLayout"))
    return new (nothrow) BGroupLayout(from);
  return NULL;
}

status_t BGroupLayout::ItemArchived(BMessage *into, BLayoutItem *item,
                                    int32 index) const {
  return into->AddFloat(kItemWeightField, _LayoutDataForItem(item)->weight);
}

status_t BGroupLayout::ItemUnarchived(const BMessage *from, BLayoutItem *item,
                                      int32 index) {
  float weight;
...
void BGroupLayout::GetItemDimensions(BLayoutItem *item,
