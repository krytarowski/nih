...
static const directory_which kBaseDirectoryConstants[] = {
    B_USER_SETTINGS_DIRECTORY, B_USER_NONPACKAGED_DATA_DIRECTORY,
    B_USER_DATA_DIRECTORY, B_SYSTEM_NONPACKAGED_DATA_DIRECTORY,
...
static void init_default_database_location() {
  ...
  for (size_t i = 0;
       i < sizeof(kBaseDirectoryConstants) / sizeof(kBaseDirectoryConstants[0]);
       i++) {
...
/*! \brief Returns properly formatted raw bitmap data, ready to be shipped off
        ...
        implementation is available. It takes the given bitmap, converts it to
   the B_CMAP8 color space if necessary and able, and returns said bitmap data
   in a newly allocated array pointed to by the pointer that's pointed to by \c
   data. The length of the array is stored in the integer pointed to by \c
   dataSize. The array is allocated with \c new[], and it's your responsibility
...
status_t get_icon_data(const BBitmap *icon, icon_size which, void **data,
  ...
  if (!err) {
    ...
    if (otherColorSpace) {
      icon8 =
          new (std::nothrow) BBitmap(bounds, B_BITMAP_NO_SERVER_LINK, B_CMAP8);
      if (!icon8)
        err = B_NO_MEMORY;
      if (!err)
