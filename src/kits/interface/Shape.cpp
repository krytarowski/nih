...
status_t BShapeIterator::IterateMoveTo(BPoint *point) { return B_OK; }
...
status_t BShapeIterator::IterateClose() { return B_OK; }

status_t BShapeIterator::IterateArcTo(float &rx, float &ry, float &angle,
                                      bool largeArc, bool counterClockWise,
                                      BPoint &point) {
  return B_OK;
}
...
BShape::BShape(BMessage *archive) : BArchivable(archive) {
  InitData();
  ...
  int32 i = 0;
  const uint32 *opPtr;
  while (archive->FindData("ops", B_INT32_TYPE, i++, (const void **)&opPtr,
                           &size) == B_OK) {
  ...
  i = 0;
  const BPoint *ptPtr;
  while (archive->FindData("pts", B_POINT_TYPE, i++, (const void **)&ptPtr,
                           &size) == B_OK) {
...
status_t BShape::Archive(BMessage *archive, bool deep) const {
  ...
  result = archive->AddData("pts", B_POINT_TYPE, data->ptList, sizeof(BPoint),
                            true, data->ptCount);
  if (result != B_OK)
    return result;
  ...
  if (result == B_OK) {
    result = archive->AddData("ops", B_INT32_TYPE, data->opList, sizeof(int32),
                              true, data->opCount);
  }
...
bool BShape::operator!=(const BShape &other) const { return !(*this == other); }
...
inline bool BShape::AllocateOps(int32 count) {
  ...
  uint32 *resizedArray =
      (uint32 *)realloc(data->opList, newSize * sizeof(uint32));
  if (resizedArray) {
...
inline bool BShape::AllocatePts(int32 count) {
  ...
  BPoint *resizedArray =
      (BPoint *)realloc(data->ptList, newSize * sizeof(BPoint));
  if (resizedArray) {
