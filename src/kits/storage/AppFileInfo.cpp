...
BAppFileInfo::BAppFileInfo() : fResources(NULL), fWhere(B_USE_BOTH_LOCATIONS) {}

BAppFileInfo::BAppFileInfo(BFile *file)
    : fResources(NULL), fWhere(B_USE_BOTH_LOCATIONS) {
  SetTo(file);
}

BAppFileInfo::~BAppFileInfo() { delete fResources; }
...
status_t BAppFileInfo::GetType(char *type) const {
  ...
  size_t read = 0;
  if (error == B_OK) {
    error = _ReadData(kTypeAttribute, kTypeResourceID, B_MIME_STRING_TYPE, type,
...
status_t BAppFileInfo::SetType(const char *type) {
  ...
  if (error == B_OK) {
    if (type != NULL) {
      ...
      // write the data
      if (error == B_OK) {
        error = _WriteData(kTypeAttribute, kTypeResourceID, B_MIME_STRING_TYPE,
                           type, typeLen + 1);
      }
    } else
...
status_t BAppFileInfo::GetCatalogEntry(char *catalogEntry) const {
  ...
  size_t read = 0;
  status_t error =
      _ReadData(kCatalogEntryAttribute, kCatalogEntryResourceID, B_STRING_TYPE,
                catalogEntry, B_MIME_TYPE_LENGTH * 3, read);
...
status_t BAppFileInfo::GetAppFlags(uint32 *flags) const {
  ...
  size_t read = 0;
  if (error == B_OK) {
    error = _ReadData(kAppFlagsAttribute, kAppFlagsResourceID, B_APP_FLAGS_TYPE,
                      flags, sizeof(uint32), read);
  }
  // check the read data
...
status_t BAppFileInfo::SetSupportedTypes(const BMessage *types,
  ...
  if (error == B_OK || error == B_ENTRY_NOT_FOUND) {
    ...
    if (types) {
      ...
      const char *type;
      for (int32 i = 0;
           error == B_OK && types->FindString("types", i, &type) == B_OK; i++) {
      ...
      // write the data
      if (error == B_OK) {
        error = _WriteData(kSupportedTypesAttribute, kSupportedTypesResourceID,
                           B_MESSAGE_TYPE, buffer, size);
      }
...
bool BAppFileInfo::IsSupportedType(const char *type) const {
  ...
  if (error == B_OK) {
    ...
    for (int32 i = 0;
         !found && types.FindString("types", i, &supportedType) == B_OK; i++) {
...
status_t BAppFileInfo::SetVersionInfo(const version_info *info,
  ...
  if (error == B_OK) {
    if (info != NULL) {
      ...
      if (error == B_OK) {
        error =
            _WriteData(kVersionInfoAttribute, kVersionInfoResourceID,
                       B_VERSION_INFO_TYPE, infos, 2 * sizeof(version_info));
      }
    } else
...
status_t BAppFileInfo::GetIconForType(const char *type, BBitmap *icon,
  ...
  if (error == B_OK) {
    error =
        BIconUtils::GetVectorIcon((uint8 *)allocatedBuffer, bytesRead, icon);
  ...
  // read the data
  if (error == B_OK) {
    bool tempBuffer = icon->ColorSpace() != B_CMAP8 || icon->Bounds() != bounds;
    uint8 *buffer = NULL;
    size_t read;
    if (tempBuffer) {
      ...
      if (!buffer) {
        ...
        error = _ReadData(attribute, -1, attrType, buffer, attrSize, read);
      }
    } else {
      error = _ReadData(attribute, -1, attrType, icon->Bits(), attrSize, read);
    }
    ...
    if (tempBuffer) {
      ...
      if (error == B_OK) {
        error = BIconUtils::ConvertFromCMAP8(buffer, (uint32)size, (uint32)size,
                                             (uint32)size, icon);
      }
      delete[] buffer;
    }
...
status_t BAppFileInfo::GetIconForType(const char *type, uint8 **data,
                                      size_t *size) const {
  if (InitCheck() != B_OK)
    return B_NO_INIT;
  ...
  status_t ret = _ReadData(attributeName.String(), -1, B_VECTOR_ICON_TYPE, NULL,
                           0, *size, &allocatedBuffer);
...
status_t BAppFileInfo::SetIconForType(const char *type, const BBitmap *icon,
  ...
  case B_MINI_ICON:
    ...
    attrSize = 16 * 16;
    resourceID =
        type != NULL ? kMiniIconForTypeResourceID : kMiniIconResourceID;
    break;
  case B_LARGE_ICON:
    attributeString = kLargeIconAttribute;
    bounds.Set(0, 0, 31, 31);
    ...
    attrSize = 32 * 32;
    resourceID =
        type != NULL ? kLargeIconForTypeResourceID : kLargeIconResourceID;
    break;
  default:
    error = B_BAD_VALUE;
    break;
  ...
  if (error == B_OK) {
    if (icon != NULL) {
      ...
      if (otherColorSpace) {
        ...
        if (error == B_OK) {
          error = _WriteData(attribute, resourceID, attrType, bitmap.Bits(),
                             attrSize, true);
        }
      } else {
        error = _WriteData(attribute, resourceID, attrType, icon->Bits(),
                           attrSize, true);
      }
...
status_t BAppFileInfo::SetIconForType(const char *type, const uint8 *data,
                                      size_t size, bool updateMimeDB) {
  if (InitCheck() != B_OK)
    return B_NO_INIT;
...
/*!	Reads data from an attribute or resource.
        ...
        \param allocatedBuffer If not \c NULL, the method allocates a buffer
                   large enough too store the whole data and writes a pointer to
   it into this variable. If \c NULL, the supplied buffer is used.
...
/*!	Writes data to an attribute or resource.
   ...
   the \a name / \a type pair or take the first unused ID >= \a id. If \c false,
   \a id is used.
