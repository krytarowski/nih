...
const static unsigned char kVerticalKnobData[] = {
...
const static unsigned char kHorizontalKnobData[] = {
...
static property_info sPropertyInfo[] = {{"Orientation",
...
BChannelSlider::BChannelSlider(BRect area, const char *name, const char *label,
                               BMessage *model, orientation orientation,
                               int32 channels, uint32 resizeMode, uint32 flags)
    : BChannelControl(area, name, label, model, channels, resizeMode, flags)
...
BChannelSlider::BChannelSlider(BMessage *archive) : BChannelControl(archive) {
  _InitData();
...
BArchivable *BChannelSlider::Instantiate(BMessage *archive) {
...
void BChannelSlider::AllAttached() { BChannelControl::AllAttached(); }

void BChannelSlider::DetachedFromWindow() {
  BChannelControl::DetachedFromWindow();
}

void BChannelSlider::AllDetached() { BChannelControl::AllDetached(); }

void BChannelSlider::MessageReceived(BMessage *message) {
  if (message->what == B_COLORS_UPDATED && fBacking != NULL &&
      fBackingView != NULL) {
    rgb_color color;
  ...
  case B_GET_PROPERTY:
    ...
    bool handled = false;
    status_t status =
        message->GetCurrentSpecifier(&index, &specifier, &what, &property);
...
void BChannelSlider::Draw(BRect updateRect) {
  ...
  if (Label()) {
    ...
    DrawString(Label(), BPoint((bounds.Width() - labelWidth) / 2.0, fBaseLine));
  }
...
void BChannelSlider::MouseMoved(BPoint where, uint32 code,
...
BHandler *BChannelSlider::ResolveSpecifier(BMessage *message, int32 index,
                                           BMessage *specifier, int32 form,
                                           const char *property) {
  BHandler *target = this;
  BPropertyInfo propertyInfo(sPropertyInfo);
  if (propertyInfo.FindMatch(message, index, specifier, form, property) !=
      B_OK) {
    target = BChannelControl::ResolveSpecifier(message, index, specifier, form,
                                               property);
  }
  return target;
}
...
void BChannelSlider::SetEnabled(bool on) { BChannelControl::SetEnabled(on); }

orientation BChannelSlider::Orientation() const {
...
int32 BChannelSlider::MaxChannelCount() const { return 32; }

bool BChannelSlider::SupportsIndividualLimits() const { return false; }
...
void BChannelSlider::DrawGroove(BView *into, int32 channel, BPoint leftTop,
  ...
  uint32 flags = 0;
  be_control_look->DrawSliderBar(into, rect, rect, base, barColor, flags,
                                 Orientation());
}
...
void BChannelSlider::DrawThumb(BView *into, int32 channel, BPoint where,
  ...
  rgb_color base = ui_color(B_PANEL_BACKGROUND_COLOR);
  uint32 flags = 0;
  be_control_look->DrawSliderThumb(into, rect, rect, base, flags,
                                   Orientation());
}

const BBitmap *BChannelSlider::ThumbFor(int32 channel, bool pressed) {
  if (fLeftKnob != NULL)
    return fLeftKnob;
  ...
  if (fIsVertical) {
    ...
    if (fLeftKnob != NULL) {
      fLeftKnob->SetBits(kHorizontalKnobData, sizeof(kHorizontalKnobData), 0,
                         B_CMAP8);
    }
  }
...
BRect BChannelSlider::ThumbFrameFor(int32 channel) {
  _UpdateFontDimens();
  ...
  if (thumb != NULL) {
    frame = thumb->Bounds();
    if (fIsVertical) {
      frame.OffsetBy(channel * frame.Width(),
                     (frame.Height() / 2.0) - (kPadding * 2.0) - 1.0);
    } else {
...
float BChannelSlider::ThumbDeltaFor(int32 channel) {
  float delta = 0.0;
  if (channel >= 0 && channel < MaxChannelCount()) {
    float range = ThumbRangeFor(channel);
    int32 limitRange = MaxLimitList()[channel] - MinLimitList()[channel];
    delta =
        (ValueList()[channel] - MinLimitList()[channel]) * range / limitRange;
...
float BChannelSlider::ThumbRangeFor(int32 channel) {
  _UpdateFontDimens();
  ...
  if (fIsVertical) {
    ...
    range =
        bounds.Height() - frame.Height() - (fLineFeed * 3.0) - (kPadding * 2.0);
  } else {
  ...
  if (fBacking && fBackingView) {
    ...
    if (fBacking->Lock()) {
      ...
      for (int32 channel = 0; channel < CountChannels(); channel++) {
        ...
        bool pressed =
            IsTracking() && (channel == fCurrentChannel || fAllChannels);
      ...
      if (fCurrentChannel != -1 && fMinPoint != 0) {
        char valueString[32];
        snprintf(valueString, 32, "%" B_PRId32, ValueFor(fCurrentChannel));
        ...
        fBackingView->DrawString(
            valueString,
            BPoint(valueRect.left + (valueRect.Width() - stringWidth) / 2.0,
                   valueRect.bottom - 1.0));
...
void BChannelSlider::_MouseMovedCommon(BPoint point, BPoint point2) {
  float floatValue = 0;
  int32 limitRange =
      MaxLimitList()[fCurrentChannel] - MinLimitList()[fCurrentChannel];
