...
status_t BPathFinder::SetTo(const void *codePointer, const char *dependency) {
  return _SetTo(codePointer, NULL, dependency);
}

status_t BPathFinder::SetTo(const char *path, const char *dependency) {
  return _SetTo(NULL, path, dependency);
}

status_t BPathFinder::SetTo(const entry_ref &ref, const char *dependency) {
  BPath path;
...
status_t BPathFinder::FindPath(const char *architecture,
  ...
  const char *dependency = fDependency.IsEmpty() ? NULL : fDependency.String();
  ...
  if (!fPath.IsEmpty()) {
    error =
        find_path_for_path_etc(fPath, dependency, architecture, baseDirectory,
                               subPath, flags, pathBuffer, sizeof(pathBuffer));
  } else {
    error = find_path_etc(fCodePointer, dependency, architecture, baseDirectory,
                          subPath, flags, pathBuffer, sizeof(pathBuffer));
  }
...
/*static*/ status_t BPathFinder::FindPaths(path_base_directory baseDirectory,
                                           const char *subPath, uint32 flags,
                                           BStringList &_paths) {
...
status_t BPathFinder::_SetTo(const void *codePointer, const char *path,
  ...
  if ((path != NULL && path[0] != '\0' && fPath.IsEmpty()) ||
      (dependency != NULL && dependency[0] != '\0' && fDependency.IsEmpty())) {
