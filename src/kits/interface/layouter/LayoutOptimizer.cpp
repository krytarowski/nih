...
/*!	\class BPrivate::Layout::LayoutOptimizer
        ...
        It does so by implementing an active set method algorithm. The basic
   idea is to start with the subset of the constraints that are barely satisfied
   by the feasible solution, i.e. including all equality constraints and those
        inequality constraints that are still satisfied, if restricted to
   equality constraints. This set is called active set, the contained
   constraints active constraints.

        Considering all of the active constraints equality constraints a new
        solution is computed, which still satisfies all those equality
   constraints and is optimal with respect to the optimization criterion.
        ...
        If the new solution does not equal the previous one, it might violate
   one or more of the inactive constraints. If that is the case, we add the
        most-violated constraint to the active set and adjust the new solution
   such that barely satisfies that constraint. Otherwise, we don't adjust the
        computed solution. With the adjusted respectively unadjusted solution
        we enter the next iteration, i.e. by computing a new optimal solution
...
// add_vectors_scaled
static inline void add_vectors_scaled(double *x, const double *y, double scalar,
                                      int n) {
  for (int i = 0; i < n; i++)
    x[i] += y[i] * scalar;
}
...
// allocate_matrix
static double **allocate_matrix(int m, int n) {
...
/*!	y = Ax
        A: m x n matrix
*/
static inline void multiply_matrix_vector(const double *const *A,
...
/*!	c = a*b
*/
static void multiply_matrices(const double *const *a, const double *const *b,
                              double **c, int m, int n, int l) {
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < l; j++) {
      double sum = 0;
      for (int k = 0; k < n; k++)
...
// transpose_matrix
static inline void transpose_matrix(const double *const *A, double **Atrans,
                                    int m, int n) {
  for (int i = 0; i < m; i++) {
    for (int k = 0; k < n; k++)
      Atrans[k][i] = A[i][k];
...
// copy_matrix
static inline void copy_matrix(const double *const *A, double **B, int m,
                               int n) {
  for (int i = 0; i < m; i++) {
    for (int k = 0; k < n; k++)
      B[i][k] = A[i][k];
...
static inline void multiply_optimization_matrix_vector(const double *x, int n,
                                                       double *y) {
...
static inline void multiply_optimization_matrix_matrix(const double *const *A,
                                                       int m, int n,
                                                       double **B) {
  if (m == 1) {
...
static int remove_linearly_dependent_rows(double **A, double **temp,
...
// MatrixDeleter
struct MatrixDelete {
  inline void operator()(double **matrix) { free_matrix(matrix); }
};
typedef BPrivate::AutoDeleter<double *, MatrixDelete> MatrixDeleter;

// Constraint
struct LayoutOptimizer::Constraint {
  Constraint(int32 left, int32 right, double value, bool equality, int32 index)
      : left(left), right(right), value(value), index(index),
        equality(equality) {}
...
LayoutOptimizer::LayoutOptimizer(int32 variableCount)
    : fVariableCount(variableCount), fConstraints(),
...
LayoutOptimizer::~LayoutOptimizer() {
  ...
  for (int32 i = 0;
       Constraint *constraint = (Constraint *)fConstraints.ItemAt(i); i++) {
    delete constraint;
  }
}
...
// Clone
LayoutOptimizer *LayoutOptimizer::Clone() const {
...
bool LayoutOptimizer::AddConstraint(int32 left, int32 right, double value,
                                    bool equality) {
  Constraint *constraint = new (nothrow)
      Constraint(left, right, value, equality, fConstraints.CountItems());
  if (constraint == NULL)
    return false;
...
bool LayoutOptimizer::AddConstraintsFrom(const LayoutOptimizer *other) {
  ...
  for (int32 i = 0; i < count; i++) {
    ...
    if (!AddConstraint(constraint->left, constraint->right, constraint->value,
                       constraint->equality)) {
      return false;
    }
  }
...
/*!	Solves the quadratic program (QP) given by the constraints added via
        ...
        The \a values array must contain a feasible solution when called and
   will be overwritten with the optimial solution the method computes.
*/
bool LayoutOptimizer::Solve(const double *desired, double size,
                            double *values) {
  ...
  Constraint *constraint =
      (Constraint *)fConstraints.RemoveItem(constraintCount - 1);
  delete constraint;
...
bool LayoutOptimizer::_Solve(const double *desired, double *values) {
  ...
  while (true) {
    ...
    // TODO: The fActiveMatrix is sparse (max 2 entries per row). There should
    // be some room for optimization.
    ...
    if (is_zero(p, fVariableCount)) {
      ...
      const int aam = remove_linearly_dependent_rows(
          aa, fTemp2, independentColumns, an, am);
      const int aan = am;
      if (aam != aan) {
      ...
      for (int32 i = 0; i < constraintCount; i++) {
        ...
        double alphaI = constraint->value - constraint->ActualValue(x);
        alphaI /= divider;
        if (alphaI < alpha) {
          alpha = alphaI;
          barrier = i;
