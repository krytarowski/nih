...
BPolygon::BPolygon(const BPoint *points, int32 count)
    : fBounds(0.0f, 0.0f, -1.0f, -1.0f), fCount(0), fPoints(NULL) {
...
BPolygon::~BPolygon() { free(fPoints); }
...
BRect BPolygon::Frame() const { return fBounds; }

void BPolygon::AddPoints(const BPoint *points, int32 count) {
  _AddPoints(points, count, true);
}

int32 BPolygon::CountPoints() const { return fCount; }

void BPolygon::MapTo(BRect source, BRect destination) {
...
bool BPolygon::_AddPoints(const BPoint *points, int32 count,
                          bool computeBounds) {
  if (points == NULL || count <= 0)
    return false;
  if (count > MAX_POINT_COUNT || (fCount + count) > MAX_POINT_COUNT) {
    fprintf(stderr,
            "BPolygon::_AddPoints(%" B_PRId32 ") - too many points"
            "\n",
            count);
    return false;
  }
  ...
  BPoint *newPoints =
      (BPoint *)realloc(fPoints, (fCount + count) * sizeof(BPoint));
  if (newPoints == NULL) {
...
void BPolygon::_MapPoint(BPoint *point, const BRect &source,
...
void BPolygon::_MapRectangle(BRect *rect, const BRect &source,
                             const BRect &destination) {
