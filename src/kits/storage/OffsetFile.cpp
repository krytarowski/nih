...
// constructor
OffsetFile::OffsetFile() : fFile(NULL), fOffset(0), fCurrentPosition(0) {}

// constructor
OffsetFile::OffsetFile(BFile *file, off_t offset)
    : fFile(NULL), fOffset(0), fCurrentPosition(0) {
  SetTo(file, offset);
}
...
// File
BFile *OffsetFile::File() const { return fFile; }

// ReadAt
ssize_t OffsetFile::ReadAt(off_t pos, void *buffer, size_t size) {
...
// WriteAt
ssize_t OffsetFile::WriteAt(off_t pos, const void *buffer, size_t size) {
  status_t error = InitCheck();
  ssize_t result = 0;
  if (error == B_OK)
...
// Seek
off_t OffsetFile::Seek(off_t position, uint32 seekMode) {
...
// Position
off_t OffsetFile::Position() const { return fCurrentPosition; }
...
// Offset
off_t OffsetFile::Offset() const { return fOffset; }
