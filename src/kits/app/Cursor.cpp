...
/**
        @note:	As BeOS only supports 16x16 monochrome cursors, and I would like
                        to see a nice shadowes one, we will need to extend this
   one.
 */
...
BCursor::BCursor(const void *cursorData)
    : fServerToken(-1), fNeedToFree(false) {
  ...
  if (data == B_HAND_CURSOR || data == B_I_BEAM_CURSOR) {
    ...
    fServerToken =
        data == B_HAND_CURSOR ? B_CURSOR_ID_SYSTEM_DEFAULT : B_CURSOR_ID_I_BEAM;
    return;
  }

  // Create a new cursor in the app_server

  if (data == NULL || data[0] != 16      // size
      || data[1] != 1                    // depth
...
BCursor::BCursor(BCursorID id) : fServerToken(id), fNeedToFree(false) {}

BCursor::BCursor(const BCursor &other) : fServerToken(-1), fNeedToFree(false) {
  *this = other;
}
...
BCursor::~BCursor() { _FreeCursorData(); }
...
status_t BCursor::Perform(perform_code d, void *arg) { return B_OK; }
