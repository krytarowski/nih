...
BUuid::BUuid() { memset(fValue, 0, sizeof(fValue)); }
...
BString BUuid::ToString() const {
  ...
  return BString().SetToFormat("%.8s-%.4s-%.4s-%.4s-%.12s", buffer, buffer + 8,
                               buffer + 12, buffer + 16, buffer + 20);
}

int BUuid::Compare(const BUuid &other) const {
