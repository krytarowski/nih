...
/*static*/ MutableLocaleRoster *MutableLocaleRoster::Default() {
 ...
 * language-name.
 */
BCatalogData *MutableLocaleRoster::CreateCatalog(const char *type,
 ...
 */
BCatalogData *MutableLocaleRoster::LoadCatalog(const entry_ref &catalogOwner,
                                               const char *language,
  ...
  for (int32 i = 0; i < count; ++i) {
    ...
    for (int32 l = 0; languages.FindString("language", l, &lang) == B_OK; ++l) {
      ...
      while ((pos = langName.FindLast('_')) >= 0) {
        ...
        nextCatalog = info->fInstantiateFunc(catalogOwner, langName.String(),
                                             fingerprint);
        if (nextCatalog) {
...
status_t MutableLocaleRoster::UnloadCatalog(BCatalogData *catalog) {
  ...
  while (catalog != NULL) {
    ...
    for (int32 i = 0; i < count; ++i) {
      CatalogAddOnInfo *info =
          static_cast<CatalogAddOnInfo *>(fData->fCatalogAddOnInfos.ItemAt(i));
