...
const char *kFileTypeNames[] = {
    "unknown",         "x86 resource file", "PPC resource file",
    "ELF object file", "PEF object file",   "empty file",
};
...
static void read_exactly(BPositionIO &file, off_t position, void *buffer,
  ...
  if (read < 0)
    ...
    if (errorMessage) {
      throw Exception("%s Read too few bytes (%ld/%lu).", errorMessage, read,
                      size);
    } else
...
static inline TV align_value(const TV &value, const TA &alignment) {
...
static void fill_pattern(const void *dataBegin, void *buffer,
...
static bool check_pattern(uint32 byteOffset, void *_buffer, uint32 count,
  ...
  for (uint32 i = 0; result && i < count; i++) {
    ...
    result = (value == kUnusedResourceDataPattern[(byteOffset / 4 + i) % 3]);
  }
  return result;
}
...
ResourceFile::ResourceFile()
    : fFile(), fFileType(FILE_TYPE_UNKNOWN), fHostEndianess(true),
      fEmptyResources(true) {}

ResourceFile::~ResourceFile() { Unset(); }
...
status_t ResourceFile::InitCheck() const { return fFile.InitCheck(); }
    ...
    if (error == B_OK) {
      ...
      if (!fHostEndianess) {
        if (resource.Type() == B_VERSION_INFO_TYPE) {
          ...
          swap_data(B_UINT32_TYPE, data, kVersionInfoIntCount * sizeof(uint32),
                    B_SWAP_ALWAYS);
        } else
...
status_t ResourceFile::ReadResources(ResourcesContainer &container,
                                     bool force) {
  status_t error = InitCheck();
...
void ResourceFile::_InitFile(BFile &file, bool clobber) {
  ...
  if (fileSize == 0) {
    ...
    //		Warnings::AddCurrentWarning("File magic is 0x%08lx. Should be
    //0x%08lx
    //"
    //									"for x86 resource file.
    //Try  anyway.",
    //									ntohl(*(uint32*)magic),
...
void ResourceFile::_InitELFXFile(BFile &file, uint64 fileSize) {
  ...
  if (hasProgramHeaderTable) {
    ...
    resourceOffset = std::max(resourceOffset, programHeaderTableOffset +
                                                  programHeaderTableSize);

    // load the program headers into memory
    uint8 *programHeaders =
        (uint8 *)malloc(programHeaderCount * programHeaderSize);
  ...
  if (hasSectionHeaderTable) {
    ...
    resourceOffset = std::max(resourceOffset, sectionHeaderTableOffset +
                                                  sectionHeaderTableSize);

    // load the section headers into memory
    uint8 *sectionHeaders =
        (uint8 *)malloc(sectionHeaderCount * sectionHeaderSize);
...
void ResourceFile::_InitPEFFile(BFile &file,
...
void ResourceFile::_ReadHeader(resource_parse_info &parseInfo) {
  ...
  // admin section size
  uint32 indexSectionSize =
      kResourceIndexSectionHeaderSize + kResourceIndexEntrySize * resourceCount;
  indexSectionSize =
      align_value(indexSectionSize, kResourceIndexSectionAlignment);
  ...
  if (adminSectionSize != indexSectionOffset + indexSectionSize) {
    throw Exception(B_IO_ERROR,
                    ...
                    "Is: %lu, should be: %lu.",
                    adminSectionSize, indexSectionOffset + indexSectionSize);
  }
...
void ResourceFile::_ReadIndex(resource_parse_info &parseInfo) {
  ...
  // index section size
  uint32 expectedIndexSectionSize =
      kResourceIndexSectionHeaderSize + kResourceIndexEntrySize * resourceCount;
  expectedIndexSectionSize =
      align_value(expectedIndexSectionSize, kResourceIndexSectionAlignment);
  ...
  if (indexSectionSize != expectedIndexSectionSize) {
    throw Exception(B_IO_ERROR,
                    ...
                    "Is: %lu, should be: %lu.",
                    indexSectionSize, expectedIndexSectionSize);
  }
  // unknown section offset
  uint32 unknownSectionOffset = _GetInt(header.rish_unknown_section_offset);
  ...
  uint32 indexTableOffset =
      indexSectionOffset + kResourceIndexSectionHeaderSize;
  int32 maxResourceCount =
      (unknownSectionOffset - indexTableOffset) / kResourceIndexEntrySize;
  int32 actualResourceCount = 0;
  bool tableEndReached = false;
  for (int32 i = 0; !tableEndReached && i < maxResourceCount; i++) {
    // read one entry
    tableEndReached =
        !_ReadIndexEntry(parseInfo, i, indexTableOffset, (i >= resourceCount));
    if (!tableEndReached)
      actualResourceCount++;
  }
  ...
  if (actualResourceCount != resourceCount) {
    if (actualResourceCount > resourceCount) {
      //			Warnings::AddCurrentWarning("Resource index
      // table contains "
      //										"%ld
      // entries, although it should be "
      //										"%ld
      // only.", actualResourceCount,
      //										resourceCount);
    }
...
bool ResourceFile::_ReadIndexEntry(resource_parse_info &parseInfo, int32 index,
  ...
  if (result && check_pattern(entryOffset, &entry, kResourceIndexEntrySize / 4,
                              fHostEndianess)) {
    if (!peekAhead) {
      //			Warnings::AddCurrentWarning("Unexpected end of
      // resource index "
      //										"table
      // at index: %ld (/%ld).",
      //										index
...
void ResourceFile::_ReadInfoTable(resource_parse_info &parseInfo) {
  ...
  while (area.check(data, minRemainderSize)) {
    ...
    const resource_info_block *infoBlock = (const resource_info_block *)data;
  ...
  if (bytesLeft != 0) {
    throw Exception(B_IO_ERROR,
                    ...
                    "table: %lu bytes are remaining.",
                    bytesLeft);
  }
  // check, if all items have been initialized
  for (int32 i = resourceCount - 1; i >= 0; i--) {
    if (!readIndices[i]) {
      //			Warnings::AddCurrentWarning("Resource item at
      // index %ld "
      //										"has
      // no info. Item removed.", i + 1);
...
bool ResourceFile::_ReadInfoTableEnd(const void *data, int32 dataSize) {
  ...
  if (hasTableEnd) {
    const resource_info_table_end *tableEnd =
        (const resource_info_table_end *)skip_bytes(
            data, dataSize - kResourceInfoTableEndSize);
  ...
  //	if (!hasTableEnd)
  //		Warnings::AddCurrentWarning("resource info table has no check
  // sum.");
  return hasTableEnd;
}
...
status_t ResourceFile::_WriteResources(ResourcesContainer &container) {
  ...
  try {
    ...
    resourcesHeader->rh_admin_section_size =
        indexSectionOffset + indexSectionSize;
    ...
    fill_pattern(buffer - indexSectionOffset, &indexHeader->rish_unused_data1,
                 1);
    fill_pattern(buffer - indexSectionOffset, indexHeader->rish_unused_data2,
                 25);
    fill_pattern(buffer - indexSectionOffset, &indexHeader->rish_unused_data3,
                 1);
    // index table
    ...
    fill_pattern(buffer - indexSectionOffset, entry, buffer + indexSectionSize);
        ...
        if (!fHostEndianess) {
          ...
          if (item->Type() == B_VERSION_INFO_TYPE) {
            ...
            swap_data(B_UINT32_TYPE, data,
                      kVersionInfoIntCount * sizeof(uint32), B_SWAP_ALWAYS);
          } else
    ...
    tableEnd->rite_check_sum =
        calculate_checksum(buffer, infoTableSize - kResourceInfoTableEndSize);
