...
*/
static inline int compare_string(const char *str1, const char *str2) {
  if (str1 == NULL) {
    if (str2 == NULL)
      return 0;
    return 1;
...
BPartition::BPartition()
    : fDevice(NULL), fParent(NULL), fPartitionData(NULL), fDelegate(NULL) {}

/*!	\brief Frees all resources associated with this object.
*/
BPartition::~BPartition() { _Unset(); }
...
/*!	\brief Returns the partition's offset relative to the beginning of the
        ...
        \return The partition's offset in bytes relative to the beginning of the
                        device it resides on.
*/
off_t BPartition::Offset() const { return _PartitionData()->offset; }

/*!	\brief Returns the size of the partition.
        \return The size of the partition in bytes.
*/
off_t BPartition::Size() const { return _PartitionData()->size; }

off_t BPartition::ContentSize() const { return _PartitionData()->content_size; }

/*!	\brief Returns the block size of the device.
        \return The block size of the device in bytes.
*/
uint32 BPartition::BlockSize() const { return _PartitionData()->block_size; }
...
*/
int32 BPartition::Index() const { return _PartitionData()->index; }

uint32 BPartition::Status() const { return _PartitionData()->status; }
...
/*!	\brief Returns the flags for this partitions.
        ...
        - \c B_VIRTUAL_PARTITION: There exists no on-disk partition this object
          represents. E.g. for floppies there will be a BPartition object
   spanning the whole floppy disk. - \c B_EMPTY_PARTITION: The partition
   represents no physical partition, but merely an empty slot. This mainly used
   to keep the indexing of partitions more persistent. This flag implies also \c
   B_HIDDEN_PARTITION.

        \return The flags for this partition.
*/
uint32 BPartition::Flags() const { return _PartitionData()->flags; }
...
/*!	\brief Returns the name of the partition.
        ...
        \return The name of the partition, or \c NULL, if the partitioning
   system does not support names.
*/
const char *BPartition::Name() const { return _PartitionData()->name; }
...
*/
const char *BPartition::Type() const { return _PartitionData()->type; }
...
*/
int32 BPartition::ID() const { return _PartitionData()->id; }
...
status_t BPartition::GetPath(BPath *path) const {
  ...
  if (Parent()->IsDevice()) {
    ...
    snprintf(indexBuffer, sizeof(indexBuffer), "%s_%" B_PRId32, path->Leaf(),
             Index());
  }
...
/*!	\brief Returns an icon for this partition.
        ...
        returns the same icon for each partition of a device. But this may
...
status_t BPartition::GetIcon(uint8 **_data, size_t *_size,
...
/*!	\brief Mounts the volume.
        ...
        \param mountPoint The directory where to mount the file system. May be
                   \c NULL, in which case a mount point in the root directory
   will be created automatically. \param mountFlags Currently only \c
   B_MOUNT_READ_ONLY is defined, which forces the volume to be mounted
   read-only. \param parameters File system specific mount parameters. \return
...
/*!	\brief Unmounts the volume.
        ...
        \param unmountFlags Currently only \c B_FORCE_UNMOUNT is defined, which
                   ...
                   open FDs. Be careful using this flag -- you risk the user's
   data.
...
*/
BDiskDevice *BPartition::Device() const { return fDevice; }

BPartition *BPartition::Parent() const { return fParent; }

BPartition *BPartition::ChildAt(int32 index) const {
  if (fDelegate != NULL) {
...
BPartition *BPartition::FindDescendant(partition_id id) const {
...
BPartition *BPartition::VisitEachChild(BDiskDeviceVisitor *visitor) const {
  if (visitor != NULL) {
    int32 level = _Level();
    for (int32 i = 0; BPartition *child = ChildAt(i); i++) {
...
BPartition *BPartition::VisitEachDescendant(BDiskDeviceVisitor *visitor) const {
  if (visitor != NULL)
...
bool BPartition::CanMove(
    BObjectList<BPartition> *unmovableDescendants,
  ...
  // have the same offset (relative to their parent) after moving. The only
  // thing we really have to ask is whether they need to be unmounted.
...
bool BPartition::CanSetContentName(bool *whileMounted) const {
  return _SupportsOperation(
      ...
      B_DISK_SYSTEM_SUPPORTS_SETTING_CONTENT_NAME_WHILE_MOUNTED, whileMounted);
}
...
bool BPartition::CanEditParameters() const {
  ...
  return parent->_SupportsChildOperation(
      this, B_DISK_SYSTEM_SUPPORTS_SETTING_PARAMETERS);
}
...
status_t BPartition::GetNextSupportedType(int32 *cookie, BString *type) const {
  ...
  return parent->fDelegate->GetNextSupportedChildType(fDelegate, cookie, type);
}

status_t BPartition::GetNextSupportedChildType(int32 *cookie,
                                               BString *type) const {
...
status_t BPartition::Uninitialize() { return fDelegate->Uninitialize(); }

bool BPartition::CanCreateChild() const {
  return _SupportsChildOperation(NULL, B_DISK_SYSTEM_SUPPORTS_CREATING_CHILD);
}

status_t BPartition::ValidateCreateChild(off_t *offset, off_t *size,
...
bool BPartition::CanDeleteChild(int32 index) const {
  ...
  return _SupportsChildOperation(child, B_DISK_SYSTEM_SUPPORTS_DELETING_CHILD);
}
...
*/
BPartition &BPartition::operator=(const BPartition &) { return *this; }
...
status_t BPartition::_RemoveObsoleteDescendants(user_partition_data *data,
                                                bool *updated) {
  ...
  for (int32 i = count - 1; i >= 0; i--) {
    ...
    for (int32 k = data->child_count - 1; k >= 0; k--) {
      if (data->children[k]->id == child->ID()) {
        ...
        found = true;
        status_t error =
            child->_RemoveObsoleteDescendants(data->children[k], updated);
        if (error != B_OK)
          return error;
...
status_t BPartition::_Update(user_partition_data *data, bool *updated) {
  ...
  fPartitionData = data;
  // check for changes
  if (data->offset != oldData->offset || data->size != oldData->size ||
      data->block_size != oldData->block_size ||
      data->status != oldData->status || data->flags != oldData->flags ||
      ...
      compare_string(data->content_parameters, oldData->content_parameters)) {
    *updated = true;
  }
...
BPartition *BPartition::_ChildAt(int32 index) const {
  if (index < 0 || index >= fPartitionData->child_count)
    return NULL;
  return (BPartition *)fPartitionData->children[index]->user_data;
}

int32 BPartition::_CountChildren() const { return fPartitionData->child_count; }
...
BPartition *BPartition::_VisitEachDescendant(BDiskDeviceVisitor *visitor,
                                             int32 level) {
  if (level < 0)
    level = _Level();
  if (_AcceptVisitor(visitor, level))
    return this;
  for (int32 i = 0; BPartition *child = ChildAt(i); i++) {
    if (BPartition *result = child->_VisitEachDescendant(visitor, level + 1)) {
      return result;
    }
  }
  return NULL;
...
const user_partition_data *BPartition::_PartitionData() const {
  return fDelegate ? fDelegate->PartitionData() : fPartitionData;
}

bool BPartition::_HasContent() const { return ContentType() != NULL; }
...
bool BPartition::_SupportsChildOperation(const BPartition *child,
                                         uint32 flag) const {
...
status_t BPartition::_CreateDelegates() {
  ...
  status_t error = fDelegate->InitHierarchy(
      fPartitionData, fParent != NULL ? fParent->fDelegate : NULL);
  if (error != B_OK)
    return error;
