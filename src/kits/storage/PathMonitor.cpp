...
/*! Returns empty path, if either \a parent or \a subPath is empty or an
        allocation fails.
 */
static BString make_path(const BString &parent, const char *subPath) {
  BString path = parent;
...
public:
  Ancestor(Ancestor *parent, const BString &path, size_t pathComponentOffset)
      : fParent(parent), fChild(NULL), fPath(path),
        fEntryRef(-1, -1, fPath.String() + pathComponentOffset), fNodeRef(),
        fWatchingFlags(0), fIsDirectory(false) {
  ...
  Ancestor *Parent() const { return fParent; }

  Ancestor *Child() const { return fChild; }

  const BString &Path() const { return fPath; }

  const char *Name() const { return fEntryRef.name; }

  bool Exists() const { return fNodeRef.device >= 0; }

  const NotOwningEntryRef &EntryRef() const { return fEntryRef; }
  ...
  status_t StartWatching(uint32 pathFlags, BHandler *target) {
    ...
    if (flags != 0) {
      ...
      TRACE("  started to watch ancestor %p (\"%s\", %#" B_PRIx32 ") -> %s\n",
  ...
  Ancestor *&HashNext() { return fHashNext; }
...
struct AncestorHashDefinition {
  ...
  size_t Hash(Ancestor *value) const { return HashKey(value->NodeRef()); }

  bool Compare(const node_ref &key, Ancestor *value) const {
    return key == value->NodeRef();
  }

  Ancestor *&GetLink(Ancestor *value) const { return value->HashNext(); }
};
...
struct EntryHashDefinition {
  ...
  size_t HashKey(const char *key) const { return BString::HashValue(key); }

  size_t Hash(Entry *value) const { return value->Name().HashValue(); }

  bool Compare(const char *key, Entry *value) const {
    return value->Name() == key;
  }

  Entry *&GetLink(Entry *value) const { return value->HashNext(); }
};
...
class Node {
public:
  Node(const node_ref &nodeRef) : fNodeRef(nodeRef) {}
  ...
  const EntryList &Entries() const { return fEntries; }

  bool HasEntries() const { return !fEntries.IsEmpty(); }

  Entry *FirstNodeEntry() const { return fEntries.Head(); }

  bool IsOnlyNodeEntry(Entry *entry) const {
    return entry == fEntries.Head() && fEntries.GetNext(entry) == NULL;
  }

  void AddNodeEntry(Entry *entry) { fEntries.Add(entry); }

  void RemoveNodeEntry(Entry *entry) { fEntries.Remove(entry); }

  Node *&HashNext() { return fHashNext; }
...
struct NodeHashDefinition {
  ...
  size_t Hash(Node *value) const { return HashKey(value->NodeRef()); }

  bool Compare(const node_ref &key, Node *value) const {
    return key == value->NodeRef();
  }

  Node *&GetLink(Node *value) const { return value->HashNext(); }
};
...
public:
  ...
  Entry *FindEntry(const char *name) const { return fEntries.Lookup(name); }
  ...
  void AddEntry(Entry *entry) { fEntries.Insert(entry); }

  void RemoveEntry(Entry *entry) { fEntries.Remove(entry); }

  EntryMap::Iterator GetEntryIterator() const { return fEntries.GetIterator(); }

  Entry *RemoveAllEntries() { return fEntries.Clear(true); }

private:
  Directory(const node_ref &nodeRef) : Node(nodeRef) {}
...
inline NotOwningEntryRef Entry::EntryRef() const {
...
public:
  PathHandler(const char *path, uint32 flags, const BMessenger &target,
              BLooper *looper);
  virtual ~PathHandler();
  ...
  const BString &OriginalPath() const { return fOriginalPath; }
...
private:
  ...
  status_t _StartWatchingAncestors(Ancestor *ancestor, bool notify);
  void _StopWatchingAncestors(Ancestor *ancestor, bool notify);
  ...
  bool _EntryCreated(const NotOwningEntryRef &entryRef, const node_ref &nodeRef,
                     bool isDirectory, bool dryRun, bool notify,
                     Entry **_entry);
  bool _EntryRemoved(const NotOwningEntryRef &entryRef, const node_ref &nodeRef,
                     bool dryRun, bool notify, Entry **_keepEntry);
  ...
  status_t _AddNode(const node_ref &nodeRef, bool isDirectory, bool notify,
                    Entry *entry = NULL, Node **_node = NULL);
  void _DeleteNode(Node *node, bool notify);
  Node *_GetNode(const node_ref &nodeRef) const;

  status_t _AddEntryIfNeeded(Directory *directory, const char *name,
                             const node_ref &nodeRef, bool isDirectory,
                             bool notify, Entry **_entry = NULL);
  void _DeleteEntry(Entry *entry, bool notify);
  void _DeleteEntryAlreadyRemovedFromParent(Entry *entry, bool notify);

  void _NotifyFilesCreatedOrRemoved(Entry *entry, int32 opcode) const;
  void _NotifyEntryCreatedOrRemoved(Entry *entry, int32 opcode) const;
  void _NotifyEntryCreatedOrRemoved(const entry_ref &entryRef,
  ...
  void _NotifyTarget(BMessage &message, const char *path) const;
...
struct PathHandlerHashDefinition {
  ...
  PathHandler *&GetLink(PathHandler *value) const { return value->HashNext(); }
};
...
struct Watcher : public PathHandlerMap {
  ...
  Watcher *&HashNext() { return fHashNext; }

private:
  Watcher(const BMessenger &target) : fTarget(target) {}
...
struct WatcherHashDefinition {
  ...
  size_t HashKey(const BMessenger &key) const { return key.HashValue(); }

  size_t Hash(Watcher *value) const { return HashKey(value->Target()); }

  bool Compare(const BMessenger &key, Watcher *value) const {
    return key == value->Target();
  }

  Watcher *&GetLink(Watcher *value) const { return value->HashNext(); }
};
...
PathHandler::PathHandler(const char *path, uint32 flags,
    ...
    : BHandler(path), fTarget(target), fFlags(flags), fStatus(B_OK),
      fOriginalPath(path), fPath(), fRoot(NULL), fBaseAncestor(NULL),
      fBaseNode(NULL), fAncestors(), fNodes() {
  ...
  if ((fFlags & B_WATCH_RECURSIVELY) != 0) {
    ...
    fFlags &=
        ~uint32(B_WATCH_FILES_ONLY | B_WATCH_DIRECTORIES_ONLY | B_WATCH_NAME);
  }
...
status_t PathHandler::InitCheck() const { return fStatus; }
...
status_t PathHandler::_CreateAncestors() {
  ...
  while (component < pathEnd) {
    const char *componentEnd =
        component == path ? component + 1 : strchr(component, '/');
    if (componentEnd == NULL)
      componentEnd = pathEnd;
    ...
    ancestor =
        new (std::nothrow) Ancestor(ancestor, ancestorPath, component - path);
...
status_t PathHandler::_StartWatchingAncestors(Ancestor *startAncestor,
                                              bool notify) {
  ...
  if (notify) {
    _NotifyEntryCreatedOrRemoved(fBaseAncestor->EntryRef(),
                                 fBaseAncestor->NodeRef(), fPath,
                                 fBaseAncestor->IsDirectory(), B_ENTRY_CREATED);
  }
  ...
  status_t error =
      _AddNode(fBaseAncestor->NodeRef(), fBaseAncestor->IsDirectory(),
               notify && _WatchFilesOnly(), NULL, &fBaseNode);
  if (error != B_OK)
    return error;
...
void PathHandler::_StopWatchingAncestors(Ancestor *ancestor, bool notify) {
  ...
  if (notify && fBaseAncestor->Exists() &&
      (fBaseAncestor->IsDirectory() ? !_WatchFilesOnly()
                                    : !_WatchDirectoriesOnly())) {
    _NotifyEntryCreatedOrRemoved(fBaseAncestor->EntryRef(),
                                 fBaseAncestor->NodeRef(), fPath,
                                 fBaseAncestor->IsDirectory(), B_ENTRY_REMOVED);
  }
...
void PathHandler::_EntryCreated(BMessage *message) {
  ...
  TRACE("%p->PathHandler::_EntryCreated(): entry: %" B_PRIdDEV ":%" B_PRIdINO
        ...
        this, entryRef.device, entryRef.directory, entryRef.name,
...
void PathHandler::_EntryMoved(BMessage *message) {
  ...
  if (message->FindInt32("node device", &nodeRef.device) != B_OK ||
      ...
      message->FindString("from name", (const char **)&fromEntryRef.name) !=
          B_OK ||
      message->FindString("name", (const char **)&toEntryRef.name) != B_OK) {
    return;
  }
  ...
  TRACE("%p->PathHandler::_EntryMoved(): entry: %" B_PRIdDEV ":%" B_PRIdINO
        ...
        ":%" B_PRIdINO "\n",
        this, fromEntryRef.device, fromEntryRef.directory, fromEntryRef.name,
        toEntryRef.device, toEntryRef.directory, toEntryRef.name,
        nodeRef.device, nodeRef.node);
    ...
    if (fromDirectoryNode != NULL || toDirectoryNode != NULL) {
      ...
      if ((fromDirectoryNode == NULL ||
          ...
          (toDirectoryNode == NULL ||
           _EntryCreated(toEntryRef, nodeRef, isDirectory, true, false,
                         NULL))) {
        ...
        if (fromDirectoryNode != NULL) {
          _EntryRemoved(fromEntryRef, nodeRef, false, false, &removedEntry);
        }

        // handle created
        Entry *createdEntry = NULL;
        if (toDirectoryNode != NULL) {
          _EntryCreated(toEntryRef, nodeRef, isDirectory, false, false,
                        &createdEntry);
        }
        ...
        if (_WatchFilesOnly() && isDirectory) {
          ...
          if (createdEntry != NULL) {
            _NotifyFilesCreatedOrRemoved(createdEntry, B_ENTRY_CREATED);
          }
        } else {
          BString fromPath;
          if (fromDirectoryNode != NULL) {
            fromPath =
                make_path(_NodePath(fromDirectoryNode), fromEntryRef.name);
          }

          BString path;
          if (toDirectoryNode != NULL) {
            path = make_path(_NodePath(toDirectoryNode), toEntryRef.name);
          }

          _NotifyEntryMoved(fromEntryRef, toEntryRef, nodeRef, fromPath, path,
                            isDirectory, fromDirectoryNode == NULL,
                            toDirectoryNode == NULL);
        }
...
// to create the directory earlier.
  ...
  if (fromIsBase || toIsBase) {
    ...
    if (!notifyFilesRecursively) {
      _NotifyEntryMoved(
          fromEntryRef, toEntryRef, nodeRef, fromIsBase ? fPath.String() : NULL,
          toIsBase ? fPath.String() : NULL, isDirectory, toIsBase, fromIsBase);
    }
    return;
  }
    ...
    if (!_WatchRecursively()) {
      if ((fFlags & B_WATCH_DIRECTORY) != 0) {
        _NotifyEntryCreatedOrRemoved(entryRef, nodeRef,
                                     make_path(fPath, entryRef.name),
                                     isDirectory, B_ENTRY_CREATED);
      }
      return true;
    }
  }
  ...
  if (directory == NULL) {
    ...
    if (!dryRun) {
      if (Entry *nodeEntry = directory->FirstNodeEntry()) {
        ...
        _EntryCreated(directoryEntryRef, entryRef.DirectoryNodeRef(), true,
                      false, notify, NULL);
      } else {
  ...
  if (ancestor != NULL) {
    ...
    if (!_WatchRecursively()) {
      if (notify && (fFlags & B_WATCH_DIRECTORY) != 0) {
        _NotifyEntryCreatedOrRemoved(entryRef, nodeRef,
                                     make_path(fPath, entryRef.name), false,
                                     B_ENTRY_REMOVED);
...
bool PathHandler::_CheckDuplicateEntryNotification(
    int32 opcode, const entry_ref &toEntryRef, const node_ref &nodeRef,
  ...
  fDuplicateEntryNotificationFromEntryRef =
      fromEntryRef != NULL ? *fromEntryRef : entry_ref();
  return false;
}
...
Ancestor *PathHandler::_GetAncestor(const node_ref &nodeRef) const {
  return fAncestors.Lookup(nodeRef);
}

status_t PathHandler::_AddNode(const node_ref &nodeRef, bool isDirectory,
                               bool notify, Entry *entry, Node **_node) {
  TRACE("%p->PathHandler::_AddNode(%" B_PRIdDEV ":%" B_PRIdINO
        ", isDirectory: %d, notify: %d)\n",
        this, nodeRef.device, nodeRef.node, isDirectory, notify);
  ...
  while (directory.GetNextRef(&entryRef) == B_OK) {
    ...
    bool isDirectory = S_ISDIR(st.st_mode);
    status_t error =
        _AddEntryIfNeeded(directoryNode, entryRef.name,
...
Node *PathHandler::_GetNode(const node_ref &nodeRef) const {
...
status_t PathHandler::_AddEntryIfNeeded(Directory *directory, const char *name,
  ...
  TRACE("%p->PathHandler::_AddEntryIfNeeded(%" B_PRIdDEV ":%" B_PRIdINO
        ...
        ", isDirectory: %d, notify: %d)\n",
        this, directory->NodeRef().device, directory->NodeRef().node, name,
        nodeRef.device, nodeRef.node, isDirectory, notify);
  ...
  status_t error =
      _AddNode(nodeRef, isDirectory, notify && _WatchFilesOnly(), entry);
  if (error != B_OK) {
...
void PathHandler::_DeleteEntryAlreadyRemovedFromParent(Entry *entry,
                                                       bool notify) {
  if (notify)
...
void PathHandler::_NotifyFilesCreatedOrRemoved(Entry *entry,
                                               int32 opcode) const {
...
void PathHandler::_NotifyEntryCreatedOrRemoved(Entry *entry,
                                               int32 opcode) const {
  Node *node = entry->Node();
...
void PathHandler::_NotifyEntryCreatedOrRemoved(const entry_ref &entryRef,
  ...
  TRACE("%p->PathHandler::_NotifyEntryCreatedOrRemoved(): entry %s: %" B_PRIdDEV
        ":%" B_PRIdINO ":\"%s\", node: %" B_PRIdDEV ":%" B_PRIdINO "\n",
...
void PathHandler::_NotifyEntryMoved(const entry_ref &fromEntryRef,
  ...
  TRACE("%p->PathHandler::_NotifyEntryMoved(): entry: %" B_PRIdDEV
        ":%" B_PRIdINO ":\"%s\" -> %" B_PRIdDEV ":%" B_PRIdINO
        ":\"%s\", node: %" B_PRIdDEV ":%" B_PRIdINO "\n",
        this, fromEntryRef.device, fromEntryRef.directory, fromEntryRef.name,
        toEntryRef.device, toEntryRef.directory, toEntryRef.name,
        nodeRef.device, nodeRef.node);
...
BString PathHandler::_NodePath(const Node *node) const {
  if (Entry *entry = node->FirstNodeEntry())
    return _EntryPath(entry);
  return node == fBaseNode ? fPath : BString();
}

BString PathHandler::_EntryPath(const Entry *entry) const {
...
/*static*/ status_t BPathMonitor::StartWatching(const char *path, uint32 flags,
  ...
  PathHandler *handler =
      new (std::nothrow) PathHandler(path, flags, target, sLooper);
...
/*static*/ status_t BPathMonitor::StopWatching(const char *path,
                                               const BMessenger &target) {
  if (sLooper == NULL)
    return B_BAD_VALUE;
...
BPathMonitor::SetWatchingInterface(BWatchingInterface *watchingInterface) {
  sWatchingInterface =
      watchingInterface != NULL ? watchingInterface : sDefaultWatchingInterface;
}
...
status_t BPathMonitor::BWatchingInterface::WatchNode(const node_ref *node,
                                                     uint32 flags,
                                                     const BMessenger &target) {
