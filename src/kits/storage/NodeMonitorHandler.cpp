#include "private/storage/NodeMonitorHandler.h"

/*
 * static util functions for the super lazy
 */

/* static */ status_t NodeMonitorHandler::make_entry_ref(dev_t device,
                                                         ino_t directory,
                                                         const char *name,
                                                         entry_ref *ref) {
  ref->device = device;
...
/* static */ void NodeMonitorHandler::make_node_ref(dev_t device, ino_t node,
                                                    node_ref *ref) {
  ref->device = device;
  ref->node = node;
}
...
NodeMonitorHandler::NodeMonitorHandler(const char *name) : BHandler(name) {
  // nothing to do
}
...
/* virtual */ void NodeMonitorHandler::EntryCreated(const char *name,
                                                    ino_t directory,
                                                    dev_t device, ino_t node) {
  // ignore
}

/* virtual */ void NodeMonitorHandler::EntryRemoved(const char *name,
                                                    ino_t directory,
                                                    dev_t device, ino_t node) {
  // ignore
}
...
NodeMonitorHandler::EntryMoved(const char *name, const char *fromName,
                               ...
                               dev_t device, ino_t node, dev_t nodeDevice) {
  // ignore
}

/* virtual */ void NodeMonitorHandler::StatChanged(ino_t node, dev_t device,
                                                   int32 statFields) {
  // ignore
}
...
/* virtual */ void NodeMonitorHandler::DeviceMounted(dev_t new_device,
                                                     dev_t device,
                                                     ino_t directory) {
  // ignore
}
