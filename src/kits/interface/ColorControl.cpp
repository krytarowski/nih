...
BColorControl::~BColorControl() { delete fOffscreenBitmap; }

void BColorControl::_InitData(color_control_layout layout, float size,
  ...
  if (data != NULL) {
    ...
    float labelWidth = std::max(StringWidth(red), std::max(StringWidth(green),
                                                           StringWidth(blue))) +
                       kTextFieldsHSpacing;
    ...
    fRedText = new BTextControl(
        textRect, "_red", red, "0", new BMessage(kMsgColorEntered),
        B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW | B_NAVIGABLE);
    ...
    fGreenText = new BTextControl(
        textRect, "_green", green, "0", new BMessage(kMsgColorEntered),
        B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW | B_NAVIGABLE);
    ...
    textRect.OffsetBy(0, _TextRectOffset());
    fBlueText = new BTextControl(
        textRect, "_blue", blue, "0", new BMessage(kMsgColorEntered),
        B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW | B_NAVIGABLE);
...
void BColorControl::_LayoutView() {
...
void BColorControl::SetLayout(BLayout *layout) {
...
void BColorControl::SetValue(int32 value) {
...
void BColorControl::MessageReceived(BMessage *message) {
  ...
  case B_SCREEN_CHANGED: {
    ...
    if (message->FindRect("frame", &frame) == B_OK &&
      ...
      _InitData((color_control_layout)fColumns, fCellSize, useOffscreen, data);
      if (useOffscreen)
        _InitOffscreen();
...
void BColorControl::_DrawColorArea(BView *target, BRect updateRect) {
  BRect rect = _PaletteFrame();
  ...
  if (be_control_look != NULL) {
    ...
    be_control_look->DrawTextControlBorder(target, rect, updateRect, base,
                                           flags);
  } else {
    // first bevel
  ...
  if (fPaletteMode) {
    ...
    int colEnd = min_c(fColumns, 2 + int(updateRect.right) / int(fCellSize));
    int rowBegin = max_c(0, -1 + int(updateRect.top) / int(fCellSize));
    int rowEnd = min_c(fRows, 2 + int(updateRect.bottom) / int(fCellSize));
    ...
    for (int col = colBegin; col < colEnd; col++) {
      for (int row = rowBegin; row < rowEnd; row++) {
        ...
        target->FillRect(
            BRect(x + 1, y + 1, x + fCellSize - 1, y + fCellSize - 1));
      }
    }
  } else {
    ...
    _DrawColorRamp(_RampFrame(0), target, white, compColor, 0, false,
                   updateRect);
    _DrawColorRamp(_RampFrame(1), target, red, compColor, 0, false, updateRect);
...
void BColorControl::_DrawSelectors(BView *target) {
  rgb_color base = ui_color(B_PANEL_BACKGROUND_COLOR);
  rgb_color lightenmax = tint_color(base, B_LIGHTEN_MAX_TINT);
  ...
  if (fPaletteMode) {
    ...
    for (int i = 1; i < 4; i++) {
      ...
      target->StrokeEllipse(center, kSelectorSize, kSelectorSize, B_SOLID_LOW);
      if (i == fFocusedRamp) {
        target->StrokeEllipse(center, kSelectorSize / 2, kSelectorSize / 2,
                              B_SOLID_LOW);
      }
...
void BColorControl::_DrawColorRamp(BRect rect, BView *target,
                                   rgb_color baseColor, rgb_color compColor,
                                   int16 flag, bool focused, BRect updateRect) {
...
BPoint BColorControl::_SelectorPosition(const BRect &rampRect,
                                        uint8 shade) const {
  float radius = kSelectorSize / 2 + kSelectorPenSize / 2;

  return BPoint(
      rampRect.left + kSelectorHSpacing + radius +
          shade * (rampRect.Width() - 2 * (kSelectorHSpacing + radius)) / 255,
      rampRect.top + rampRect.Height() / 2);
}

BRect BColorControl::_PaletteFrame() const {
  return fPaletteFrame.InsetByCopy(-kBevelSpacing, -kBevelSpacing);
}

BRect BColorControl::_RampFrame(uint8 rampIndex) const {
  float rampHeight = (float)(fRows * fCellSize / kRampCount);
...
void BColorControl::_SetCellSize(float size) {
  BFont font;
  GetFont(&font);
  fCellSize =
      std::max(kMinCellSize, ceilf(size * font.Size() / kDefaultFontSize));
}

float BColorControl::_TextRectOffset() {
  return std::max(fRedText->Bounds().Height(),
                  ceilf(_PaletteFrame().Height() / 3));
}

BRect BColorControl::_PaletteSelectorFrame(uint8 colorIndex) const {
...
void BColorControl::_InitOffscreen() {
...
void BColorControl::_InvalidateSelector(int16 ramp, rgb_color color,
                                        bool focused) {
  if (fPaletteMode)
    return;
  ...
  float invalidateRadius = focused ? kSelectorSize + kSelectorPenSize / 2
                                   : kSelectorSize / 2 + kSelectorPenSize;

  uint8 colorValue =
      ramp == 1 ? color.red : ramp == 2 ? color.green : color.blue;
...
float BColorControl::CellSize() const { return fCellSize; }
...
void BColorControl::MouseDown(BPoint point) {
  ...
  if (fPaletteMode) {
    ...
    uint8 shade = (unsigned char)max_c(
        0, min_c((point.x - _RampFrame(0).left) * 255 / _RampFrame(0).Width(),
                 255));
...
void BColorControl::DetachedFromWindow() { BControl::DetachedFromWindow(); }

void BColorControl::GetPreferredSize(float *_width, float *_height) {
  BRect rect = _PaletteFrame();
...
status_t BColorControl::Invoke(BMessage *message) {
  return BControl::Invoke(message);
}

void BColorControl::FrameMoved(BPoint newPosition) {
  BControl::FrameMoved(newPosition);
}

void BColorControl::FrameResized(float newWidth, float newHeight) {
  BControl::FrameResized(newWidth, newHeight);
}

BHandler *BColorControl::ResolveSpecifier(BMessage *message, int32 index,
...
void BColorControl::MakeFocus(bool focused) {
  fFocusedRamp = !fPaletteMode && focused ? 1 : -1;
  BControl::MakeFocus(focused);
}

void BColorControl::AllAttached() { BControl::AllAttached(); }

void BColorControl::AllDetached() { BControl::AllDetached(); }

status_t BColorControl::SetIcon(const BBitmap *icon, uint32 flags) {
  return BControl::SetIcon(icon, flags);
}

status_t BColorControl::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BColorControl::MinSize();
    return B_OK;

  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BColorControl::MaxSize();
    return B_OK;
