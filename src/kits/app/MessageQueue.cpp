...
BMessageQueue::BMessageQueue()
    : fHead(NULL), fTail(NULL), fMessageCount(0), fLock("BMessageQueue Lock") {}
...
int32 BMessageQueue::CountMessages() const { return fMessageCount; }

bool BMessageQueue::IsEmpty() const { return fMessageCount == 0; }
...
BMessage *BMessageQueue::FindMessage(int32 index) const {
  ...
  for (BMessage *message = fHead; message != NULL;
       message = message->fQueueLink) {
...
bool BMessageQueue::Lock() { return fLock.Lock(); }

void BMessageQueue::Unlock() { fLock.Unlock(); }

bool BMessageQueue::IsLocked() const { return fLock.IsLocked(); }
...
bool BMessageQueue::IsLocked() { return fLock.IsLocked(); }
