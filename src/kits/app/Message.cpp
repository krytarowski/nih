...
#define DEBUG_FUNCTION_ENTER                                                   \
  debug_printf("msg thread: %ld; this: %p; header: %p; fields: %p;"            \
               ...
               find_thread(NULL), this, fHeader, fFields, fData, what,         \
               (char *)&what, __LINE__, __PRETTY_FUNCTION__);
...
template <typename Type> static void print_to_stream_type(uint8 *pointer) {
  Type *item = (Type *)pointer;
  item->PrintToStream();
}

template <typename Type>
static void print_type(const char *format, uint8 *pointer) {
  Type *item = (Type *)pointer;
  printf(format, *item, *item);
}

template <typename Type>
static void print_type3(const char *format, uint8 *pointer) {
  Type *item = (Type *)pointer;
  printf(format, *item, *item, *item);
}

static status_t handle_reply(port_id replyPort, int32 *_code, bigtime_t timeout,
                             BMessage *reply) {
...
void *BMessage::operator new(size_t size, const std::nothrow_t &noThrow) {
  DEBUG_FUNCTION_ENTER2;
  return sMsgCache->Get(size);
}

void *BMessage::operator new(size_t, void *pointer) {
  DEBUG_FUNCTION_ENTER2;
  return pointer;
}

void BMessage::operator delete(void *pointer, size_t size) {
  DEBUG_FUNCTION_ENTER2;
  if (pointer == NULL)
    return;
...
bool BMessage::HasSameData(const BMessage &other, bool ignoreFieldOrder,
                           bool deep) const {
  if (this == &other)
    return true;
  ...
  for (uint32 i = 0; i < fHeader->field_count; i++) {
    ...
    if (ignoreFieldOrder) {
      ...
      const char *otherName = (const char *)other.fData + otherField->offset;
    ...
    uint8 *otherData =
        other.fData + otherField->offset + otherField->name_length;
...
status_t BMessage::_InitCommon(bool initHeader) {
  DEBUG_FUNCTION_ENTER;
  what = 0;
...
status_t BMessage::GetInfo(type_code typeRequested, int32 index,
...
bool BMessage::IsSystem() const {
  ...
  // The BeBook says:
  //		... we've adopted a strict convention for assigning values to
  // all 		Be-defined constants.  The value assigned will always be
  // formed by
  //		combining four characters into a multicharacter constant, with
  // the 		characters limited to uppercase letters and the underbar
...
void BMessage::_PrintToStream(const char *indent) const {
  DEBUG_FUNCTION_ENTER;
  ...
  for (uint32 i = 0; i < fHeader->field_count; i++, field++) {
    ...
    for (uint32 j = 0; j < field->count; j++) {
      if (field->count == 1) {
        printf("%s        %s = ", indent, (char *)(fData + field->offset));
      } else {
      ...
      case B_REF_TYPE: {
        ...
        printf("entry_ref(device=%d, directory=%" B_PRIdINO ", name=\"%s\", ",
               (int)ref.device, ref.directory, ref.name);
  ...
  while (*nextField >= 0) {
    ...
    if (strncmp((const char *)(fData + field->offset), oldEntry,
      ...
      *nextField = index;
      ...
      result = _ResizeData(field->offset + 1, newLength - field->name_length);
      if (result != B_OK)
        return result;
...
bool BMessage::WasDelivered() const {
  DEBUG_FUNCTION_ENTER;
  return fHeader != NULL && (fHeader->flags & MESSAGE_FLAG_WAS_DELIVERED) != 0;
}
...
BMessenger BMessage::ReturnAddress() const {
  ...
  BMessenger::Private(messenger).SetTo(fHeader->reply_team, fHeader->reply_port,
...
bool BMessage::WasDropped() const {
  DEBUG_FUNCTION_ENTER;
  return fHeader != NULL && (fHeader->flags & MESSAGE_FLAG_WAS_DROPPED) != 0;
}
...
status_t BMessage::SendReply(BMessage *reply, BHandler *replyTo,
                             bigtime_t timeout) {
  DEBUG_FUNCTION_ENTER;
...
status_t BMessage::SendReply(BMessage *reply, BMessenger replyTo,
                             bigtime_t timeout) {
  DEBUG_FUNCTION_ENTER;
  if (fHeader == NULL)
    return B_NO_INIT;
  ...
  if ((fHeader->flags & MESSAGE_FLAG_REPLY_REQUIRED) != 0) {
    ...
    if (result != B_OK &&
        set_port_owner(messengerPrivate.Port(), messengerPrivate.Team()) ==
            B_BAD_TEAM_ID) {
...
status_t BMessage::SendReply(BMessage *reply, BMessage *replyToReply,
  ...
  if ((fHeader->flags & MESSAGE_FLAG_REPLY_REQUIRED) != 0) {
    ...
    reply->fHeader->flags |= MESSAGE_FLAG_IS_REPLY;
    status_t result =
        messenger.SendMessage(reply, replyToReply, sendTimeout, replyTimeout);
    reply->fHeader->flags &= ~MESSAGE_FLAG_IS_REPLY;

    if (result != B_OK) {
      if (set_port_owner(messengerPrivate.Port(), messengerPrivate.Team()) ==
          B_BAD_TEAM_ID) {
        delete_port(messengerPrivate.Port());
      }
    }
  ...
  reply->AddMessage("_previous_", this);
  reply->fHeader->flags |=
      MESSAGE_FLAG_IS_REPLY | (fHeader->flags & MESSAGE_FLAG_REPLY_AS_KMESSAGE);
  status_t result =
      messenger.SendMessage(reply, replyToReply, sendTimeout, replyTimeout);
  reply->fHeader->flags &= ~MESSAGE_FLAG_IS_REPLY;
  reply->RemoveName("_previous_");
...
/*	The concept of message sending by area:
        ...
        The traditional way of sending a message is to send it by flattening it
   to a buffer, pushing it through a port, reading it into the outputbuffer and
        unflattening it from there (copying the data again). While this works ok
        for small messages it does not make any sense for larger ones and may
   even hit some port capacity limit. Often in the life of a BMessage, it will
   be sent to someone. Almost as often the one receiving the message will not
   need to change the message in any way, but uses it "read only" to get
   information from it. This means that all that copying is pretty pointless in
   the first place since we could simply pass the original buffers on. It's
   obviously not exactly as simple as this, since we cannot just use the memory
   of one application in another - but we can share areas with eachother.
        ...
        Only if write access is necessary the message will be copyed from the
   area to its own buffers (like in the unflatten step before). The double
   copying is reduced to a single copy in most cases and we safe the slower
   route of moving the data through a port. Additionally we save us the
   reference counting with the use of areas that are reference counted
   internally. So we don't have to worry about leaving an area behind or
   deleting one that is still in use.
*/

status_t BMessage::_FlattenToArea(message_header **_header) const {
  DEBUG_FUNCTION_ENTER;
  if (fHeader == NULL)
    return B_NO_INIT;
  ...
  *_header = header;
  ...
  area_id area = create_area("BMessage data", (void **)&address, B_ANY_ADDRESS,
...
status_t BMessage::_Dereference() {
  DEBUG_FUNCTION_ENTER;
  if (fHeader == NULL)
    return B_NO_INIT;
...
status_t BMessage::_CopyForWrite() {
  DEBUG_FUNCTION_ENTER;
  if (fHeader == NULL)
    return B_NO_INIT;
...
status_t BMessage::_ValidateMessage() {
  DEBUG_FUNCTION_ENTER;
  if (fHeader == NULL)
    return B_NO_INIT;
...
status_t BMessage::GetCurrentSpecifier(int32 *index, BMessage *specifier,
  ...
  if (specifier) {
    if (FindMessage(B_SPECIFIER_ENTRY, fHeader->current_specifier, specifier) !=
        B_OK)
      return B_BAD_SCRIPT_SYNTAX;
...
bool BMessage::HasSpecifiers() const {
  DEBUG_FUNCTION_ENTER;
  return fHeader != NULL && (fHeader->flags & MESSAGE_FLAG_HAS_SPECIFIERS) != 0;
}
...
void BMessage::_UpdateOffsets(uint32 offset, int32 change) {
...
status_t BMessage::_ResizeData(uint32 offset, int32 change) {
  if (change == 0)
    return B_OK;
...
uint32 BMessage::_HashName(const char *name) const {
  char ch;
  uint32 result = 0;
...
status_t BMessage::_FindField(const char *name, type_code type,
...
status_t BMessage::_AddField(const char *name, type_code type, bool isFixedSize,
                             field_header **result) {
  if (fHeader == NULL)
    return B_NO_INIT;
  ...
  if (fFieldsAvailable <= 0) {
    ...
    field_header *newFields =
        (field_header *)realloc(fFields, count * sizeof(field_header));
...
status_t BMessage::_RemoveField(field_header *field) {
  status_t result =
...
void BMessage::_StaticReInitForkedChild() {
  DEBUG_FUNCTION_ENTER2;
...
void BMessage::_StaticCacheCleanup() {
  DEBUG_FUNCTION_ENTER2;
  delete sMsgCache;
  sMsgCache = NULL;
}

int32 BMessage::_StaticGetCachedReplyPort() {
  DEBUG_FUNCTION_ENTER2;
  int index = -1;
...
status_t BMessage::_SendMessage(port_id port, team_id portOwner, int32 token,
  ...
  if (direct != NULL) {
    ...
    if (header->message_area >= 0) {
      ...
      void *address = NULL;
      area_id transfered = _kern_transfer_area(header->message_area, &address,
  ...
  if (!replyTo.IsValid()) {
    BMessenger::Private(replyTo).SetTo(fHeader->reply_team, fHeader->reply_port,
                                       fHeader->reply_target);
  ...
  if (direct == NULL) {
    KTRACE("BMessage send remote: team: %ld, port: %ld, token: %ld, "
           "message: '%c%c%c%c'",
           portOwner, port, token, char(what >> 24), char(what >> 16),
           char(what >> 8), (char)what);
  ...
  if (direct != NULL) {
    KTRACE("BMessage send direct: port: %ld, token: %ld, "
           "message: '%c%c%c%c'",
           port, token, char(what >> 24), char(what >> 16), char(what >> 8),
           (char)what);
...
// tests if the queue of the reply port is really empty
...

  {
    ...
    BMessenger::Private(replyTarget).SetTo(team, replyPort, B_PREFERRED_TOKEN);
    // TODO: replying could also use a BDirectMessageTarget like mechanism
    // for local targets
    result = _SendMessage(port, -1, token, sendTimeout, true, replyTarget);
  }
...
status_t BMessage::_SendFlattenedMessage(void *data, int32 size, port_id port,
...
#define DEFINE_FUNCTIONS(type, typeName, typeCode)                             \
  status_t BMessage::Add##typeName(const char *name, type val) {               \
    return AddData(name, typeCode, &val, sizeof(type), true);                  \
  }                                                                            \
                                                                               \
  status_t BMessage::Find##typeName(const char *name, type *p) const {         \
    void *ptr = NULL;                                                          \
    ssize_t bytes = 0;                                                         \
    status_t error = B_OK;                                                     \
                                                                               \
    *p = type();                                                               \
    ...
    return error;                                                              \
                                                                               ...
                                                                               \
  status_t BMessage::Find##typeName(const char *name, int32 index, type *p)    \
      const {                                                                  \
    void *ptr = NULL;                                                          \
    ssize_t bytes = 0;                                                         \
    status_t error = B_OK;                                                     \
    ...
    *p = type();                                                               \
    ...
    return error;                                                              \
                                                                               ...
                                                                               \
  status_t BMessage::Replace##typeName(const char *name, type value) {         \
    return ReplaceData(name, typeCode, 0, &value, sizeof(type));               \
  }                                                                            \
                                                                               \
  status_t BMessage::Replace##typeName(const char *name, int32 index,          \
                                       type value) {                           \
    return ReplaceData(name, typeCode, index, &value, sizeof(type));           \
  }                                                                            \
                                                                               \
  bool BMessage::Has##typeName(const char *name, int32 index) const {          \
...
#define DEFINE_LAZY_FIND_FUNCTION(type, typeName, initialize)                  \
  type BMessage::Find##typeName(const char *name, int32 index) const {         \
    type val = initialize;                                                     \
...
#define DEFINE_SET_GET_FUNCTIONS(type, typeName, typeCode)                     \
  type BMessage::Get##typeName(const char *name, type defaultValue) const {    \
    return Get##typeName(name, 0, defaultValue);                               \
  }                                                                            \
                                                                               \
  type BMessage::Get##typeName(const char *name, int32 index,                  \
    ...
    return defaultValue;                                                       \
                                                                               ...
                                                                               \
  status_t BMessage::Set##typeName(const char *name, type value) {             \
...
#define DEFINE_SET_GET_BY_REFERENCE_FUNCTIONS(type, typeName, typeCode)        \
  type BMessage::Get##typeName(const char *name, const type &defaultValue)     \
      const {                                                                  \
    return Get##typeName(name, 0, defaultValue);                               \
  }                                                                            \
                                                                               \
  type BMessage::Get##typeName(const char *name, int32 index,                  \
    ...
    return defaultValue;                                                       \
                                                                               ...
                                                                               \
  status_t BMessage::Set##typeName(const char *name, const type &value) {      \
    return SetData(name, typeCode, &value, sizeof(type));                      \
  }
...
status_t BMessage::AddStrings(const char *name, const BStringList &list) {
...
status_t BMessage::AddFlat(const char *name, const BFlattenable *object,
                           int32 count) {
  if (object == NULL)
    return B_BAD_VALUE;
...
status_t BMessage::Append(const BMessage &other) {
  ...
  for (uint32 i = 0; i < other.fHeader->field_count; i++, field++) {
    ...
    const void *data =
        (const void *)(other.fData + field->offset + field->name_length);
    ...
    for (uint32 j = 0; j < field->count; j++) {
      ...
      status_t status = AddData(name, field->type, data, size, isFixed, 1);
      if (status != B_OK)
        return status;
...
status_t BMessage::FindAlignment(const char *name, int32 index,
                                 BAlignment *alignment) const {
  if (!alignment)
    return B_BAD_VALUE;
  ...
  status_t err =
      FindData(name, B_ALIGNMENT_TYPE, index, (const void **)&data, &bytes);
...
status_t BMessage::FindStrings(const char *name, BStringList *list) const {
  if (list == NULL)
    return B_BAD_VALUE;
...
status_t BMessage::FindPointer(const char *name, int32 index,
  ...
  ssize_t size = 0;
  status_t error =
      FindData(name, B_POINTER_TYPE, index, (const void **)&data, &size);
...
status_t BMessage::FindMessenger(const char *name, int32 index,
  ...
  ssize_t size = 0;
  status_t error =
      FindData(name, B_MESSENGER_TYPE, index, (const void **)&data, &size);
...
status_t BMessage::FindRef(const char *name, int32 index,
                           entry_ref *ref) const {
  if (ref == NULL)
    return B_BAD_VALUE;
  ...
  ssize_t size = 0;
  status_t error =
      FindData(name, B_REF_TYPE, index, (const void **)&data, &size);
...
status_t BMessage::FindMessage(const char *name, int32 index,
  ...
  ssize_t size = 0;
  status_t error =
      FindData(name, B_MESSAGE_TYPE, index, (const void **)&data, &size);
...
status_t BMessage::ReplaceAlignment(const char *name, int32 index,
                                    const BAlignment &alignment) {
  int32 data[2] = {alignment.horizontal, alignment.vertical};
...
status_t BMessage::ReplaceRef(const char *name, int32 index,
                              const entry_ref *ref) {
  size_t size = sizeof(entry_ref) + B_PATH_NAME_LENGTH;
  char buffer[size];
...
bool BMessage::HasFlat(const char *name, const BFlattenable *object) const {
  return HasFlat(name, 0, object);
}

bool BMessage::HasFlat(const char *name, int32 index,
...
const char *BMessage::GetString(const char *name, int32 index,
                                const char *defaultValue) const {
  const char *value;
...
status_t BMessage::SetString(const char *name, const BString &value) {
  return SetData(name, B_STRING_TYPE, value.String(), value.Length() + 1,
                 false);
}

status_t BMessage::SetString(const char *name, const char *value) {
  return SetData(name, B_STRING_TYPE, value, strlen(value) + 1, false);
}

status_t BMessage::SetData(const char *name, type_code type, const void *data,
