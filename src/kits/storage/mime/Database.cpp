...
/*!
   ...
   performed by this class on the mime type strings passed to it. It's assumed
...
Database::Database(DatabaseLocation *databaseLocation, MimeSniffer *mimeSniffer,
    ...
    : fStatus(B_NO_INIT), fLocation(databaseLocation),
...
/*! \brief Returns the initialization status of the object.
        ...
        - "error code": failure
*/
status_t Database::InitCheck() const { return fStatus; }
...
/*!	\brief Installs the given type in the database
        ...
        \param type Pointer to a NULL-terminated string containing the MIME type
   of interest \param decsription Pointer to a NULL-terminated string containing
   the new long description \return - B_OK: success - B_FILE_EXISTS: the type is
   already installed - "error code": failure
*/
...
/*!	\brief Removes the given type from the database
        \param type Pointer to a NULL-terminated string containing the MIME type
   of interest \return - B_OK: success - "error code": failure
*/
...
status_t Database::_SetStringValue(const char *type, int32 what,
                                   const char *attribute,
                                   type_code attributeType, size_t maxLength,
                                   const char *value) {
...
/*!	\brief Sets the application hint for the given MIME type
        \param type Pointer to a NULL-terminated string containing the MIME type
   of interest \param decsription Pointer to an entry_ref containing the
   location of an application that should be used when launching an application
   with this signature.
*/
status_t Database::SetAppHint(const char *type, const entry_ref *ref) {
  ...
  return _SetStringValue(type, B_APP_HINT_CHANGED, kAppHintAttr, kAppHintType,
                         B_PATH_NAME_LENGTH, path.Path());
}
...
/*! \brief Stores a BMessage describing the format of attributes typically
   ...
   containing information about the file attributes typically associated with
   the MIME type. \return - \c B_OK: Success - "error code": Failure
*/
status_t Database::SetAttrInfo(const char *type, const BMessage *info) {
  ...
  bool didCreate = false;
  status_t status =
      fLocation->WriteMessageAttribute(type, kAttrInfoAttr, *info, &didCreate);
  if (status == B_OK) {
    if (didCreate)
      _SendInstallNotification(type);
    else
...
*/
status_t Database::SetShortDescription(const char *type,
                                       const char *description) {
  DBG(OUT("Database::SetShortDescription()\n"));

  return _SetStringValue(type, B_SHORT_DESCRIPTION_CHANGED,
                         kShortDescriptionAttr, kShortDescriptionType,
                         B_MIME_TYPE_LENGTH, description);
}
...
*/
status_t Database::SetLongDescription(const char *type,
                                      const char *description) {
...
/*!
        \brief Sets the list of filename extensions associated with the MIME
   type

        The list of extensions is given in a pre-allocated BMessage pointed to
   by the \c extensions parameter. Please see BMimeType::SetFileExtensions() for
   a description of the expected message format.

        \param extensions Pointer to a pre-allocated, properly formatted
   BMessage containing the new list of file extensions to associate with this
   MIME type. \return - \c B_OK: Success - "error code": Failure
*/
status_t Database::SetFileExtensions(const char *type,
                                     const BMessage *extensions) {
  ...
  bool didCreate = false;
  status_t status = fLocation->WriteMessageAttribute(type, kFileExtensionsAttr,
...
*/
status_t Database::SetIcon(const char *type, const BBitmap *icon,
                           icon_size which) {
  if (icon != NULL)
...
*/
status_t Database::SetIcon(const char *type, const void *data,
                           size_t dataSize) {
...
/*! \brief Sets the large or mini icon used by an application of this type for
   ...
   subtype of \c "application/"; that is the intended use however, and
   application-specific icons are not expected to be present for non-application
   types.

        The bitmap data pointed to by \c data must be of the proper size (\c
   32x32 for \c B_LARGE_ICON, \c 16x16 for \c B_MINI_ICON) and the proper color
        space (B_CMAP8).
   ...
   color depth \param dataSize The length of the array pointed to by \c data
        \param size The size icon you're expecting (\c B_LARGE_ICON or \c
   B_MINI_ICON) \return - \c B_OK: Success - "error code": Failure
...
status_t Database::SetIconForType(const char *type, const char *fileType,
  ...
  std::string attr;
  if (fileType) {
    attr = (which == B_MINI_ICON ? kMiniIconAttrPrefix : kLargeIconAttrPrefix) +
...
status_t Database::SetIconForType(const char *type, const char *fileType,
  ...
  if (didCreate) {
    ...
    if (fileType) {
      _SendMonitorUpdate(B_ICON_FOR_TYPE_CHANGED, type, fileType, true,
                         B_META_MIME_MODIFIED);
    } else {
      _SendMonitorUpdate(B_ICON_CHANGED, type, true, B_META_MIME_MODIFIED);
    }
  }
  return err;
}
...
/*!	\brief Sets the signature of the preferred application for the given
   app verb
        ...
        \param type Pointer to a NULL-terminated string containing the MIME type
   of interest \param signature Pointer to a NULL-terminated string containing
   the MIME signature of the new preferred application \param verb \c app_verb
   action for which the new preferred application is applicable
*/
status_t Database::SetPreferredApp(const char *type, const char *signature,
                                   app_verb verb) {
...
status_t Database::SetSnifferRule(const char *type, const char *rule) {
  ...
  bool didCreate = false;
  status_t status =
      fLocation->WriteAttribute(type, kSnifferRuleAttr, rule, strlen(rule) + 1,
...
*/
status_t Database::SetSupportedTypes(const char *type, const BMessage *types,
                                     bool fullSync) {
  ...
  for (int32 i = 0; types->FindString("types", i, &supportedType) == B_OK;
       i++) {
  ...
  // Write the attr
  bool didCreate = false;
  status_t status = fLocation->WriteMessageAttribute(type, kSupportedTypesAttr,
                                                     *types, &didCreate);
...
/*! \brief Fetches a BMessage listing all the MIME supertypes currently
        ...
        The types are copied into the \c "super_types" field of the passed-in \c
   BMessage. The \c BMessage must be pre-allocated.
...
/*! \brief Fetches a BMessage listing all the MIME subtypes of the given
   ...
   whose subtypes you wish to retrieve. \param subtypes Pointer to a
   pre-allocated \c BMessage into which the appropriate MIME subtypes will be
   copied. \return - \c B_OK: Success - "error code": Failure
*/
status_t Database::GetInstalledTypes(const char *supertype,
                                     BMessage *subtypes) {
...
/*!	\brief Guesses a MIME type for the entry referred to by the given
   ...
   attribute), the type returned is \c "application/x-vnd.be-meta-mime". - If
   the entry is a directory, the type returned is \c
   "application/x-vnd.be-directory". - If the entry is a symlink, the type
   returned is \c "application/x-vnd.be-symlink". - If the entry is a regular
   file, the file data is sniffed and, the type returned is the mime type with
   the matching rule of highest priority. - If sniffing fails, the filename is
   checked for known extensions. - If the extension check fails, the type
   returned is \c "application/octet-stream".
        ...
        \param type Pointer to a pre-allocated BString which is set to the
                   resulting MIME type.
        \return
        - \c B_OK: success (even if the guess returned is
   "application/octet-stream") - other error code: failure
*/
...
/*!	\brief Guesses a MIME type for the supplied chunk of data.
        ...
        - error code: failure
*/
status_t Database::GuessMimeType(const void *buffer, int32 length,
                                 BString *result) {
...
/*!	\brief Guesses a MIME type for the given filename.
        ...
        name extension), not the entry or corresponding data it refers to (in
   fact, an entry with that name need not exist at all.
...
/*!	\brief Subscribes the given BMessenger to the MIME monitor service
        ...
        <table>
                ...
                <tr>
                        ...
                        <td> Bitmask describing which attributes were changed
   (see below) </td>
                </tr>
                <tr>
                        <td> \c be:extra_type </td>
                        <td> \c B_STRING_TYPE </td>
                        <td> Additional MIME type string (applicable to
   B_ICON_FOR_TYPE_CHANGED notifications only)</td>
                </tr>
                <tr>
                        <td> \c be:large_icon </td>
                        <td> \c B_BOOL_TYPE </td>
                        <td> \c true if the large icon was changed, \c false if
   the small icon was changed (applicable to B_ICON_[FOR_TYPE_]CHANGED updates
   only) </td>
                </tr>
        </table>

        The \c be:which field of the message describes which attributes were
   updated, and may be the bitwise \c OR of any of the following values:
...
/*!	\brief Deletes the app hint attribute for the given type
        ...
        A \c B_APP_HINT_CHANGED notification is sent to the mime monitor
   service. \param type The mime type of interest \return - B_OK: success -
   B_ENTRY_NOT_FOUND: no such attribute existed - "error code": failure
*/
...
/*!	\brief Deletes the attribute info attribute for the given type
        ...
        A \c B_ATTR_INFO_CHANGED notification is sent to the mime monitor
   service. \param type The mime type of interest \return - B_OK: success -
   B_ENTRY_NOT_FOUND: no such attribute existed - "error code": failure
*/
...
/*!	\brief Deletes the short description attribute for the given type
        ...
        A \c B_SHORT_DESCRIPTION_CHANGED notification is sent to the mime
   monitor service. \param type The mime type of interest \return - B_OK:
   success - B_ENTRY_NOT_FOUND: no such attribute existed - "error code":
   failure
*/
...
/*!	\brief Deletes the long description attribute for the given type
        ...
        A \c B_LONG_DESCRIPTION_CHANGED notification is sent to the mime monitor
   service. \param type The mime type of interest \return - B_OK: success -
   B_ENTRY_NOT_FOUND: no such attribute existed - "error code": failure
*/
...
/*!	\brief Deletes the associated file extensions attribute for the given
   type

        A \c B_FILE_EXTENSIONS_CHANGED notification is sent to the mime monitor
   service. \param type The mime type of interest \return - B_OK: success -
   B_ENTRY_NOT_FOUND: no such attribute existed - "error code": failure
*/
...
status_t Database::DeleteIcon(const char *type) {
  ...
  if (status == B_OK) {
    _SendMonitorUpdate(B_ICON_CHANGED, type, true, B_META_MIME_DELETED);
  } else if (status == B_ENTRY_NOT_FOUND)
    status = B_OK;
...
/*!	\brief Deletes the icon of the given size associated with the given file
   ...
   for a better description of what the *IconForType() functions are used for.)

        A \c B_ICON_FOR_TYPE_CHANGED notification is sent to the mime monitor
   service. \param type The mime type of the application whose custom icon you
   are deleting. \param fileType The mime type for which you no longer wish \c
   type to have a custom icon. \param which The icon size of interest \return -
   B_OK: success - B_ENTRY_NOT_FOUND: no such attribute existed - "error code":
   failure
*/
status_t Database::DeleteIconForType(const char *type, const char *fileType,
  ...
  std::string attr =
      (which == B_MINI_ICON ? kMiniIconAttrPrefix : kLargeIconAttrPrefix) +
      BPrivate::Storage::to_lower(fileType);
...
/*!	\brief Deletes the vector icon associated with the given file
   ...
   service. \param type The mime type of the application whose custom icon you
   are deleting. \param fileType The mime type for which you no longer wish \c
   type to have a custom icon. \return - B_OK: success - B_ENTRY_NOT_FOUND: no
   such attribute existed - "error code": failure
*/
status_t Database::DeleteIconForType(const char *type, const char *fileType) {
  if (fileType == NULL)
    return B_BAD_VALUE;
...
/*! A \c B_PREFERRED_APP_CHANGED notification is sent to the mime monitor
   service. \param type The mime type of interest \param which The app verb of
   interest \return - B_OK: success - B_ENTRY_NOT_FOUND: no such attribute
   existed - "error code": failure
*/
status_t Database::DeletePreferredApp(const char *type, app_verb verb) {
  ...
  /*! \todo The R5 monitor makes no note of which app_verb value was updated. If
          additional app_verb values besides \c B_OPEN are someday added, the
     format of the MIME monitor messages will need to be augmented.
  */
  if (status == B_OK)
...
/*! A \c B_SNIFFER_RULE_CHANGED notification is sent to the mime monitor
   service, and the corresponding rule is removed from the internal database of
   sniffer rules. \param type The mime type of interest \return - B_OK: success
        - B_ENTRY_NOT_FOUND: no such attribute existed
        - "error code": failure
*/
...
/*! A \c B_SUPPORTED_TYPES_CHANGED notification is sent to the mime monitor
   service. If \c fullSync is \c true, the given type is removed from the
   internal list of supporting applictions for each previously supported type.
   If \c fullSync is \c false, the said removal will occur the next time
   SetSupportedTypes() or DeleteSupportedTypes() is called with a \c true \c
   fullSync paramter, or \c Delete() is called for the given type. \param type
   The mime type of interest \param fullSync Whether or not to remove the type
   as a supporting app for all previously supported types \return - B_OK:
   success - B_ENTRY_NOT_FOUND: no such attribute existed - "error code":
   failure
*/
...
//! \brief Sends a \c B_MIME_TYPE_CREATED notification to the mime monitor
//! service
...
//! \brief Sends a \c B_MIME_TYPE_DELETED notification to the mime monitor
//! service
...
*/
status_t Database::_SendMonitorUpdate(int32 which, const char *type,
                                      const char *extraType, bool largeIcon,
                                      int32 action) {
...
// _SendMonitorUpdate
/*! \brief Sends an update notification to all BMessengers that have subscribed
   to the MIME Monitor service \param BMessage A preformatted MIME monitor
   message to be sent to all subscribers
*/
status_t Database::_SendMonitorUpdate(BMessage &msg) {
  ...
  for (i = fMonitorMessengers.begin(); i != fMonitorMessengers.end(); i++) {
    ...
    if (err) {
      DBG(OUT("Database::_SendMonitorUpdate(BMessage&): DeliverMessage failed, "
              "0x%lx\n",
              err));
    }
  }
...
Database::_FindDeferredInstallNotification(const char *type, bool remove) {
  for (int32 i = 0;
       DeferredInstallNotification *notification =
           (DeferredInstallNotification *)fDeferredInstallNotifications.ItemAt(
               i);
       i++) {
...
bool Database::_CheckDeferredInstallNotification(int32 which,
                                                 const char *type) {
  AutoLocker<BLocker> locker(fDeferredInstallNotificationsLocker);
