...
BClipboard::BClipboard(const char *name, bool transient) : fLock("clipboard") {
  if (name != NULL)
    fName = strdup(name);
  else
    fName = strdup("system");
...
const char *BClipboard::Name() const { return (const char *)fName; }
...
/*!	\brief Returns the (locally cached) number of commits to the clipboard.
        ...
        or another (even from another application). This method returns a
   locally cached value, which might already be obsolete. For an up-to-date
   value SystemCount() can be invoked.

        \return The number of commits to the clipboard.
*/
uint32 BClipboard::LocalCount() const { return fCount; }
...
void BClipboard::Unlock() { fLock.Unlock(); }

bool BClipboard::IsLocked() const { return fLock.IsLocked(); }
...
status_t BClipboard::Commit() { return Commit(false); }
...
BMessenger BClipboard::DataSource() const { return fDataSource; }
...
bool BClipboard::_AssertLocked() const {
...
status_t BClipboard::_DownloadFromSystem(bool force) {
