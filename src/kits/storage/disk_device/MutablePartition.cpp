...
void BMutablePartition::UninitializeContents() {
  DeleteAllChildren();
  SetVolumeID(-1);
  SetContentName(NULL);
...
// Offset
off_t BMutablePartition::Offset() const { return fData->offset; }

// SetOffset
void BMutablePartition::SetOffset(off_t offset) {
...
// Size
off_t BMutablePartition::Size() const { return fData->size; }

// SetSize
void BMutablePartition::SetSize(off_t size) {
  if (fData->size != size) {
    fData->size = size;
...
// ContentSize
off_t BMutablePartition::ContentSize() const { return fData->content_size; }

// SetContentSize
void BMutablePartition::SetContentSize(off_t size) {
...
// BlockSize
off_t BMutablePartition::BlockSize() const { return fData->block_size; }

// SetBlockSize
void BMutablePartition::SetBlockSize(off_t blockSize) {
...
// Status
uint32 BMutablePartition::Status() const { return fData->status; }
...
// Flags
uint32 BMutablePartition::Flags() const { return fData->flags; }
...
// VolumeID
dev_t BMutablePartition::VolumeID() const { return fData->volume; }

// SetVolumeID
void BMutablePartition::SetVolumeID(dev_t volumeID) {
...
// Index
int32 BMutablePartition::Index() const { return fData->index; }

// Name
const char *BMutablePartition::Name() const { return fData->name; }

// SetName
status_t BMutablePartition::SetName(const char *name) {
...
// ContentName
const char *BMutablePartition::ContentName() const {
  return fData->content_name;
}

// SetContentName
status_t BMutablePartition::SetContentName(const char *name) {
...
// Type
const char *BMutablePartition::Type() const { return fData->type; }

// SetType
status_t BMutablePartition::SetType(const char *type) {
...
// ContentType
const char *BMutablePartition::ContentType() const {
  return fData->content_type;
}

// SetContentType
status_t BMutablePartition::SetContentType(const char *type) {
...
// Parameters
const char *BMutablePartition::Parameters() const { return fData->parameters; }

// SetParameters
status_t BMutablePartition::SetParameters(const char *parameters) {
...
// ContentParameters
const char *BMutablePartition::ContentParameters() const {
  return fData->content_parameters;
}

// SetContentParameters
status_t BMutablePartition::SetContentParameters(const char *parameters) {
...
// CreateChild
status_t BMutablePartition::CreateChild(int32 index,
...
// CreateChild
status_t BMutablePartition::CreateChild(int32 index, const char *type,
                                        const char *name,
...
// DeleteChild
status_t BMutablePartition::DeleteChild(BMutablePartition *child) {
...
// Parent
BMutablePartition *BMutablePartition::Parent() const { return fParent; }

// ChildAt
BMutablePartition *BMutablePartition::ChildAt(int32 index) const {
...
// SetChangeFlags
void BMutablePartition::SetChangeFlags(uint32 flags) { fChangeFlags = flags; }

// ChangeFlags
uint32 BMutablePartition::ChangeFlags() const { return fChangeFlags; }
...
// ChildCookie
void *BMutablePartition::ChildCookie() const { return fChildCookie; }

// SetChildCookie
void BMutablePartition::SetChildCookie(void *cookie) { fChildCookie = cookie; }

// constructor
BMutablePartition::BMutablePartition(BPartition::Delegate *delegate)
    : fDelegate(delegate), fData(NULL), fParent(NULL), fChangeFlags(0),
      fChildCookie(NULL) {}

// Init
status_t BMutablePartition::Init(const user_partition_data *partitionData,
  ...
  SET_STRING_RETURN_ON_ERROR(fData->content_name, partitionData->content_name);
  SET_STRING_RETURN_ON_ERROR(fData->type, partitionData->type);
  SET_STRING_RETURN_ON_ERROR(fData->content_type, partitionData->content_type);
...
// PartitionData
const user_partition_data *BMutablePartition::PartitionData() const {
  return fData;
}

// GetDelegate
BPartition::Delegate *BMutablePartition::GetDelegate() const {
  return fDelegate;
}
