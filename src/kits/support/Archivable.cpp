...
static status_t demangle_class_name(const char *name, BString &out) {
...
static void mangle_class_name(const char *name, BString &out) {
  // TODO: add support for template classes
  //	_find__t12basic_string3ZcZt18string_char_traits1ZcZt24__default_alloc_template2b0i0PCccUlUl
  ...
  //			(for __GNUC__ > 2)
  //			this isn't always the proper mangled class name, it
  //should
  //			actually have an 'N' prefix and 'E' suffix if the name
  //is 			in > 0 namespaces, but these would have to be removed in
  //			build_function_name() (the only place this function is
  //called) 			so we don't add them. 	or this:
...
static void build_function_name(const BString &className, BString &funcName) {
  funcName = "";
...
static bool add_private_namespace(BString &name) {
...
static instantiation_func find_function_in_image(BString &funcName, image_id id,
                                                 status_t &err) {
...
static status_t check_signature(const char *signature, image_info &info) {
  if (signature == NULL) {
...
instantiation_func find_instantiation_func(const char *className,
  ...
  for (int32 pass = 0; pass < 2; pass++) {
    ...
    while (instantiationFunc == NULL &&
           get_next_image_info(threadInfo.team, &cookie, &imageInfo) == B_OK) {
      instantiationFunc = find_function_in_image(funcName, imageInfo.id, err);
    }
...
BArchivable::BArchivable() : fArchivingToken(NULL_TOKEN) {}

BArchivable::BArchivable(BMessage *from) : fArchivingToken(NULL_TOKEN) {
...
status_t BArchivable::Perform(perform_code d, void *arg) {
  ...
  case PERFORM_CODE_ALL_UNARCHIVED: {
    perform_data_all_unarchived *data = (perform_data_all_unarchived *)arg;
  ...
  case PERFORM_CODE_ALL_ARCHIVED: {
    perform_data_all_archived *data = (perform_data_all_archived *)arg;
...
status_t BArchivable::AllUnarchived(const BMessage *archive) { return B_OK; }

status_t BArchivable::AllArchived(BMessage *archive) const { return B_OK; }

// #pragma mark - BArchiver

BArchiver::BArchiver(BMessage *archive)
    : fManager(BManagerBase::ArchiveManager(archive)), fArchive(archive),
      fFinished(false) {
  if (fManager == NULL)
...
status_t BArchiver::AddArchivable(const char *name, BArchivable *archivable,
                                  bool deep) {
  int32 token;
...
status_t BArchiver::GetTokenForArchivable(BArchivable *archivable, bool deep,
                                          int32 &_token) {
...
status_t BArchiver::Finish(status_t err) {
  if (fFinished)
...
BMessage *BArchiver::ArchiveMessage() const { return fArchive; }

void BArchiver::RegisterArchivable(const BArchivable *archivable) {
...
BUnarchiver::BUnarchiver(const BMessage *archive)
    : fManager(BManagerBase::UnarchiveManager(archive)), fArchive(archive),
      fFinished(false) {}
...
template <>
status_t BUnarchiver::GetObject<BArchivable>(int32 token,
...
bool BUnarchiver::IsInstantiated(const char *field, int32 index) {
  int32 token;
...
status_t BUnarchiver::Finish(status_t err) {
  if (fFinished)
    debugger("Finish() called multiple times on same BArchiver.");
...
const BMessage *BUnarchiver::ArchiveMessage() const { return fArchive; }
...
bool BUnarchiver::IsArchiveManaged(const BMessage *archive) {
...
template <>
status_t BUnarchiver::InstantiateObject<BArchivable>(BMessage *from,
                                                     BArchivable *&object) {
...
BMessage *BUnarchiver::PrepareArchive(BMessage *&archive) {
...
void BUnarchiver::RegisterArchivable(BArchivable *archivable) {
  _CallDebuggerIfManagerNull();
  fManager->RegisterArchivable(archivable);
}

void BUnarchiver::_CallDebuggerIfManagerNull() {
  if (!fManager)
...
BArchivable *instantiate_object(BMessage *archive, image_id *_id) {
  ...
  instantiation_func instantiationFunc =
      BPrivate::find_instantiation_func(className, signature, _id);
  ...
  if (!instantiationFunc && hasSignature) {
    ...
    if (instantiationFunc == NULL) {
      syslog(LOG_ERR,
             ...
             "Instantiate static function for class %s.",
             className);
    ...
    syslog(LOG_ERR,
           ...
           "in archive, looking for class \"%s\".",
           className);
    *status = B_NAME_NOT_FOUND;
    return NULL;
  }
...
bool validate_instantiation(BMessage *from, const char *className) {
  ...
  for (int32 pass = 0; pass < 2; pass++) {
    ...
    for (int32 index = 0;
         from->FindString(B_CLASS_FIELD, index, &archiveClassName) == B_OK;
         ++index) {
...
extern "C" status_t _ReservedArchivable2__11BArchivable(BArchivable *archivable,
                                                        BMessage *archive) {
  // AllArchived
