...
static inline int32 icon_layout_scale() {
...
alert_type BAlert::Type() const { return (alert_type)fType; }

void BAlert::SetType(alert_type type) { fType = type; }

void BAlert::SetText(const char *text) { TextView()->SetText(text); }

void BAlert::SetIcon(BBitmap *bitmap) { fIconView->SetBitmap(bitmap); }

void BAlert::SetButtonSpacing(button_spacing spacing) {
  fButtonSpacing = spacing;
}

void BAlert::SetButtonWidth(button_width width) { fButtonWidth = width; }
...
void BAlert::FrameResized(float newWidth, float newHeight) {
...
int32 BAlert::CountButtons() const { return (int32)fButtons.size(); }
...
BTextView *BAlert::TextView() const { return fTextView; }
...
void BAlert::Quit() { BWindow::Quit(); }

bool BAlert::QuitRequested() { return BWindow::QuitRequested(); }
...
BPoint BAlert::AlertPosition(float width, float height) {
  ...
  result.y =
      screenFrame.top + (screenFrame.Height() / 4.0) - ceil(height / 3.0);

  return result;
}

status_t BAlert::Perform(perform_code code, void *_data) {
  switch (code) {
...
void BAlert::_Init(const char *text, const char *button0, const char *button1,
                   const char *button2, button_width buttonWidth,
                   button_spacing spacing, alert_type type) {
  fInvoker = NULL;
  fAlertSem = -1;
  fAlertValue = -1;
...
BBitmap *BAlert::_CreateTypeIcon() {
  ...
  icon = new (std::nothrow)
      BBitmap(BRect(0, 0, iconSize - 1, iconSize - 1), 0, B_RGBA32);
  ...
  rawIcon = (const uint8 *)resources.LoadResource(B_VECTOR_ICON_TYPE, iconName,
                                                  &size);
  if (rawIcon != NULL &&
  ...
  rawIcon =
      (const uint8 *)resources.LoadResource(B_LARGE_ICON_TYPE, iconName, &size);
  if (rawIcon == NULL) {
...
BButton *BAlert::_CreateButton(int32 which, const char *label) {
...
void BAlert::_Prepare() {
  ...
  float minWindowWidth =
      (fButtonSpacing == B_OFFSET_SPACING ? kWindowOffsetMinWidth
                                          : kWindowMinWidth) *
      fontFactor;
...
TAlertView::TAlertView() : BView("TAlertView", B_WILL_DRAW), fIconBitmap(NULL) {
...
void TAlertView::GetPreferredSize(float *_width, float *_height) {
...
void TAlertView::Draw(BRect updateRect) {
  ...
  DrawBitmapAsync(fIconBitmap,
                  BPoint(18 * iconLayoutScale, 6 * iconLayoutScale));
}

//	#pragma mark - _BAlertFilter_

_BAlertFilter_::_BAlertFilter_(BAlert *alert)
    : BMessageFilter(B_KEY_DOWN), fAlert(alert) {}
