...
struct DiskSystemAddOnManager::AddOnImage {
  AddOnImage(image_id image) : image(image), refCount(0) {}

  ~AddOnImage() { unload_add_on(image); }
...
struct DiskSystemAddOnManager::AddOn {
  AddOn(AddOnImage *image, BDiskSystemAddOn *addOn)
      : image(image), addOn(addOn), refCount(1) {}
...
// Default
DiskSystemAddOnManager *DiskSystemAddOnManager::Default() {
  if (sManager == NULL)
...
// Lock
bool DiskSystemAddOnManager::Lock() { return fLock.Lock(); }

// Unlock
void DiskSystemAddOnManager::Unlock() { fLock.Unlock(); }
...
status_t DiskSystemAddOnManager::LoadDiskSystems() {
  ...
  if (error == B_OK) {
    error = _LoadAddOns(alreadyLoaded, B_SYSTEM_NONPACKAGED_ADDONS_DIRECTORY);
  }
...
// AddOnAt
BDiskSystemAddOn *DiskSystemAddOnManager::AddOnAt(int32 index) const {
  AddOn *addOn = _AddOnAt(index);
  return addOn ? addOn->addOn : NULL;
}

// GetAddOn
BDiskSystemAddOn *DiskSystemAddOnManager::GetAddOn(const char *name) {
  if (!name)
    return NULL;
...
void DiskSystemAddOnManager::PutAddOn(BDiskSystemAddOn *_addOn) {
  ...
  for (int32 i = 0; AddOn *addOn = (AddOn *)fAddOnsToBeUnloaded.ItemAt(i);
       i++) {
    if (_addOn == addOn->addOn) {
      _PutAddOn(i);
      return;
...
DiskSystemAddOnManager::DiskSystemAddOnManager()
    : fLock("disk system add-ons manager"), fAddOns(), fAddOnsToBeUnloaded(),
      fLoadCount(0) {}
...
status_t DiskSystemAddOnManager::_LoadAddOns(StringSet &alreadyLoaded,
  ...
  while (directory.GetNextRef(&ref) == B_OK) {
    ...
    for (int32 i = 0; i < count; i++) {
      BDiskSystemAddOn *diskSystemAddOn = (BDiskSystemAddOn *)addOns.ItemAt(i);
