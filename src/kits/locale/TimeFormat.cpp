...
BTimeFormat::BTimeFormat() : BFormat() {}

BTimeFormat::BTimeFormat(const BLanguage &language,
                         const BFormattingConventions &conventions)
    : BFormat(language, conventions) {}

BTimeFormat::BTimeFormat(const BTimeFormat &other) : BFormat(other) {}

BTimeFormat::~BTimeFormat() {}

void BTimeFormat::SetTimeFormat(BTimeFormatStyle style, const BString &format) {
  fConventions.SetExplicitTimeFormat(style, format);
}

// #pragma mark - Formatting

ssize_t BTimeFormat::Format(char *string, size_t maxSize, time_t time,
...
status_t BTimeFormat::Format(BString &string, int *&fieldPositions,
...
status_t BTimeFormat::GetTimeFields(BDateElement *&fields, int &fieldCount,
  ...
  UnicodeString icuString;
  time_t now;
  timeFormatter->format((UDate)time(&now) * 1000, icuString, &positionIterator,
                        error);
...
status_t BTimeFormat::Parse(BString source, BTimeFormatStyle style,
                            BTime &output) {
  ObjectDeleter<DateFormat> timeFormatter(_CreateTimeFormatter(style));
  if (timeFormatter.Get() == NULL)
    return B_NO_MEMORY;
  ...
  ParsePosition p(0);
  UDate date =
      timeFormatter->parse(UnicodeString::fromUTF8(source.String()), p);
