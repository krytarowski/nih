...
public:
  ...
  int32 Count() const { return fInfos.CountItems(); }
...
private:
  ...
  Info *_InfoAt(int32 index) const { return (Info *)fInfos.ItemAt(index); }
...
BGridLayout::BGridLayout(float horizontal, float vertical)
    : fGrid(NULL), fColumnCount(0), fRowCount(0), fRowInfos(new RowInfoArray),
      fColumnInfos(new RowInfoArray), fMultiColumnItems(0), fMultiRowItems(0) {
  SetSpacing(horizontal, vertical);
}

BGridLayout::BGridLayout(BMessage *from)
    : BTwoDimensionalLayout(BUnarchiver::PrepareArchive(from)), fGrid(NULL),
      fColumnCount(0), fRowCount(0), fRowInfos(new RowInfoArray),
      fColumnInfos(new RowInfoArray), fMultiColumnItems(0), fMultiRowItems(0) {
...
int32 BGridLayout::CountColumns() const { return fColumnCount; }

int32 BGridLayout::CountRows() const { return fRowCount; }

float BGridLayout::HorizontalSpacing() const { return fHSpacing; }

float BGridLayout::VerticalSpacing() const { return fVSpacing; }
...
float BGridLayout::RowWeight(int32 row) const { return fRowInfos->Weight(row); }
...
BLayoutItem *BGridLayout::AddView(int32 index, BView *child) {
  return BTwoDimensionalLayout::AddView(index, child);
}

BLayoutItem *BGridLayout::AddView(BView *child, int32 column, int32 row,
...
BArchivable *BGridLayout::Instantiate(BMessage *from) {
  if (validate_instantiation(from, "BGridLayout"))
    return new BGridLayout(from);
  return NULL;
}

status_t BGridLayout::ItemArchived(BMessage *into, BLayoutItem *item,
                                   int32 index) const {
...
status_t BGridLayout::ItemUnarchived(const BMessage *from, BLayoutItem *item,
                                     int32 index) {
  ItemLayoutData *data = _LayoutDataForItem(item);
  ...
  if (result == B_OK) {
    result = from->FindInt32(kItemDimensionsField, ++index, &dimensions.height);
  }

  if (result != B_OK)
    return result;

  if (!_AreGridCellsEmpty(dimensions.x, dimensions.y, dimensions.width,
                          dimensions.height))
    return B_BAD_DATA;
...
bool BGridLayout::HasMultiColumnItems() { return fMultiColumnItems > 0; }

bool BGridLayout::HasMultiRowItems() { return fMultiRowItems > 0; }

int32 BGridLayout::InternalCountColumns() { return fColumnCount; }

int32 BGridLayout::InternalCountRows() { return fRowCount; }
