...
InstalledTypes::InstalledTypes(DatabaseLocation *databaseLocation)
    : fDatabaseLocation(databaseLocation), fCachedMessage(NULL),
      fCachedSupertypesMessage(NULL), fHaveDoneFullBuild(false) {}
...
/*! \brief Returns a list of all currently installed types of the given
        ...
        information.
*/
status_t InstalledTypes::GetInstalledTypes(const char *supertype,
                                           BMessage *types) {
...
/*! \brief Adds the given subtype to the given supertype's lists of installed
   ...
   please) \return - B_OK: success - B_NAME_IN_USE: The subtype already exists
   in the subtype list - "error code": failure
*/
...
/*! \brief Reads through the database and builds a complete set of installed
   types lists.
            ...
            while (true) {
              ...
              if (err) {
                ...
                if (node.InitCheck() == B_OK &&
                  ...
                  // Add the subtype
                  if (_AddSubtype(supertypeRef, type.String() + subStart + 1) !=
                      B_OK) {
                    DBG(OUT("Mime::InstalledTypes::BuildInstalledTypesList()"
...
/*! \brief Allocates a new BMessage into the BMessage pointer pointed to by \c
   result and fills it with a complete list of installed types.
*/
...
/*! \brief Allocates a new BMessage into the BMessage pointer pointed to by \c
   result and fills it with a complete list of installed supertypes.
*/
status_t
