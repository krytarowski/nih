...
#include "os/locale/TextEncoding.h"
...
BTextEncoding::BTextEncoding(BString name)
    : fName(name), fUtf8Converter(NULL), fConverter(NULL) {}

BTextEncoding::BTextEncoding(const char *data, size_t length)
    : fUtf8Converter(NULL), fConverter(NULL) {
...
status_t BTextEncoding::Decode(const char *input, size_t &inputLength,
  ...
  ucnv_convertEx(fUtf8Converter, fConverter, &target, output + outputLength,
                 &base, input + inputLength, NULL, NULL, NULL, NULL, FALSE,
                 TRUE, &error);
...
status_t BTextEncoding::Encode(const char *input, size_t &inputLength,
                               char *output, size_t &outputLength) {
  const char *base = input;
  char *target = output;
...
status_t BTextEncoding::Flush(char *output, size_t &outputLength) {
  ...
  ucnv_convertEx(fConverter, fUtf8Converter, &target, output + outputLength,
                 NULL, NULL, NULL, NULL, NULL, NULL, FALSE, TRUE, &error);
...
BString BTextEncoding::GetName() { return fName; }
};
