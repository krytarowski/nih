...
static const char *const kTrueValueStrings[] = {"1",  "true",   "yes",
                                                "on", "enable", "enabled"};
...
BDriverParameterIterator::BDriverParameterIterator() : fDelegate(NULL) {}
...
BDriverParameterIterator::~BDriverParameterIterator() { _SetTo(NULL, false); }

bool BDriverParameterIterator::HasNext() const {
  return fDelegate != NULL ? fDelegate->HasNext() : false;
}

BDriverParameter BDriverParameterIterator::Next() {
  if (fDelegate == NULL)
...
BDriverParameterIterator &BDriverParameterIterator::
operator=(const BDriverParameterIterator &other) {
  _SetTo(other.fDelegate, true);
  return *this;
}

void BDriverParameterIterator::_SetTo(Delegate *delegate, bool addReference) {
  if (fDelegate != NULL)
...
public:
  Iterator(const driver_parameter *parameters, int32 count)
      : Delegate(), fParameters(parameters), fCount(count) {}
  ...
  virtual bool HasNext() const { return fParameters != NULL && fCount > 0; }
...
public:
  NameIterator(const driver_parameter *parameters, int32 count,
               const BString &name)
      : Delegate(), fParameters(parameters), fCount(count), fName(name) {
    _FindNext(false);
  }
...
const driver_parameter *BDriverParameterContainer::Parameters() const {
  int32 count;
  return GetParametersAndCount(count);
}

BDriverParameter BDriverParameterContainer::ParameterAt(int32 index) const {
  int32 count;
...
bool BDriverParameterContainer::FindParameter(
    const char *name, BDriverParameter *_parameter) const {
  if (name == NULL)
    return false;
...
BDriverParameterIterator BDriverParameterContainer::ParameterIterator() const {
  int32 count;
  if (const driver_parameter *parameters = GetParametersAndCount(count)) {
...
const char *BDriverParameterContainer::GetParameterValue(
    const char *name, const char *unknownValue, const char *noValue) const {
  BDriverParameter parameter;
...
bool BDriverParameterContainer::GetBoolParameterValue(const char *name,
                                                      bool unknownValue,
                                                      bool noValue) const {
  BDriverParameter parameter;
  if (!FindParameter(name, &parameter))
...
int64 BDriverParameterContainer::GetInt64ParameterValue(const char *name,
                                                        int64 unknownValue,
                                                        int64 noValue) const {
  BDriverParameter parameter;
  if (!FindParameter(name, &parameter))
...
BDriverSettings::BDriverSettings()
    : BDriverParameterContainer(), fSettingsHandle(NULL), fSettings(NULL) {}

BDriverSettings::~BDriverSettings() { Unset(); }

status_t BDriverSettings::Load(const char *driverNameOrAbsolutePath) {
  Unset();
...
status_t BDriverSettings::Load(const entry_ref &ref) {
  Unset();
...
status_t BDriverSettings::SetToString(const char *string) {
  Unset();
...
BDriverParameter::BDriverParameter()
    : BDriverParameterContainer(), fParameter(NULL) {}

BDriverParameter::BDriverParameter(const driver_parameter *parameter)
    : BDriverParameterContainer(), fParameter(parameter) {}

BDriverParameter::BDriverParameter(const BDriverParameter &other)
    : BDriverParameterContainer(), fParameter(other.fParameter) {}

BDriverParameter::~BDriverParameter() {}

void BDriverParameter::SetTo(const driver_parameter *parameter) {
  fParameter = parameter;
}

bool BDriverParameter::IsValid() const { return fParameter != NULL; }

const char *BDriverParameter::Name() const {
...
const char *const *BDriverParameter::Values() const {
...
BDriverParameter &BDriverParameter::operator=(const BDriverParameter &other) {
