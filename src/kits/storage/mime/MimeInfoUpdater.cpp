...
static status_t update_icon(BAppFileInfo &appFileInfoRead,
                            BAppFileInfo &appFileInfoWrite, const char *type,
...
status_t MimeInfoUpdater::Do(const entry_ref &entry, bool *_entryIsDir) {
  ...
  if (!err) {
    ...
    updateAppInfo =
        (updateType || fForce == B_UPDATE_MIME_INFO_FORCE_KEEP_TYPE);
  }
  ...
  if (!err && updateType) {
    ...
    ssize_t bytes = node.WriteAttr(kFileTypeAttr, kFileTypeType, 0, type, len);
    if (bytes < B_OK)
      err = bytes;
    else
  ...
  if (!err && updateAppInfo && node.IsFile() &&
    ...
    if (err == B_OK) {
      err = appFileInfoWrite.SetSupportedTypes(&supportedTypes, false, false);
    ...
    if (hasSupportedTypes) {
      ...
      for (int32 i = 0;
        ...
        // vector icon
        err = update_icon(appFileInfoRead, appFileInfoWrite, supportedType);
        if (err != B_OK)
          return err;
