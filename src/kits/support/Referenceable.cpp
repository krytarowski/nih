...
BReferenceable::BReferenceable() : fReferenceCount(1) {}
...
BReferenceable::~BReferenceable() {
  ...
  if (fReferenceCount == 1) {
    ...
    if (testOffset > 1024 || -testOffset > 1024) {
      ...
      if (result == B_OK && (this < info.stack_base || this > info.stack_end)) {
        snprintf(message, sizeof(message),
                 "Deleted referenceable "
                 "object that's not on the stack (this: %p, stack_base: %p,"
                 " stack_end: %p)\n",
                 this, info.stack_base, info.stack_end);
        enterDebugger = true;
      }
    }
  } else if (fReferenceCount != 0) {
    snprintf(message, sizeof(message),
             "Deleted referenceable object %p with "
...
void BReferenceable::LastReferenceReleased() { delete this; }
