...
BCollator::BCollator()
    : fDefaultStrength(B_COLLATE_PRIMARY), fIgnorePunctuation(true) {
...
BCollator::BCollator(const char *locale, int8 strength, bool ignorePunctuation)
    : fDefaultStrength(strength), fIgnorePunctuation(ignorePunctuation) {
  UErrorCode error = U_ZERO_ERROR;
  fICUCollator = Collator::createInstance(locale, error);
}

BCollator::BCollator(BMessage *archive)
    : BArchivable(archive), fICUCollator(NULL),
      fDefaultStrength(B_COLLATE_PRIMARY), fIgnorePunctuation(true) {
  int32 data;
...
BCollator::BCollator(const BCollator &other) : fICUCollator(NULL) {
  *this = other;
}

BCollator::~BCollator() { delete fICUCollator; }
...
BCollator &BCollator::operator=(const BCollator &source) {
  if (&source != this) {
    ...
    fICUCollator =
        source.fICUCollator != NULL ? source.fICUCollator->clone() : NULL;
...
int8 BCollator::DefaultStrength() const { return fDefaultStrength; }

void BCollator::SetIgnorePunctuation(bool ignore) {
  fIgnorePunctuation = ignore;
}

bool BCollator::IgnorePunctuation() const { return fIgnorePunctuation; }

status_t BCollator::GetSortKey(const char *string, BString *key,
                               int8 strength) const {
  ...
  UErrorCode error = U_ZERO_ERROR;
  int requiredSize = fICUCollator->getSortKey(
      UnicodeString(string, length, NULL, error), buffer, length * 2);
  if (requiredSize > length * 2) {
    ...
    error = U_ZERO_ERROR;
    fICUCollator->getSortKey(UnicodeString(string, length, NULL, error), buffer,
                             requiredSize);
  }
...
int BCollator::Compare(const char *s1, const char *s2, int8 strength) const {
  _SetStrength(strength);
...
status_t BCollator::Archive(BMessage *archive, bool deep) const {
  ...
  UErrorCode error = U_ZERO_ERROR;
  int size = static_cast<RuleBasedCollator *>(fICUCollator)
                 ->cloneBinary(NULL, 0, error);
  // This WILL fail with U_BUFFER_OVERFLOW_ERROR. But we get the needed
  // size.
  error = U_ZERO_ERROR;
  uint8_t *buffer = (uint8_t *)malloc(size);
  static_cast<RuleBasedCollator *>(fICUCollator)
...
BArchivable *BCollator::Instantiate(BMessage *archive) {
