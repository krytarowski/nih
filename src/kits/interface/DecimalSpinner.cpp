...
static double roundTo(double value, uint32 n) {
...
BDecimalSpinner::BDecimalSpinner(BMessage *data) : BAbstractSpinner(data) {
  _InitObject();
...
BArchivable *BDecimalSpinner::Instantiate(BMessage *data) {
...
void BDecimalSpinner::Decrement() { SetValue(Value() - Step()); }

void BDecimalSpinner::Increment() { SetValue(Value() + Step()); }
...
void BDecimalSpinner::SetValue(int32 value) { SetValue((double)value); }
