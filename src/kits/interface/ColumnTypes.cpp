...
void BTitledColumn::GetColumnName(BString *into) const { *into = fTitle; }
...
void BTitledColumn::DrawString(const char *string, BView *parent, BRect rect) {
  ...
  case B_ALIGN_CENTER:
    parent->MovePenTo(
        rect.left + kTEXT_MARGIN + ((width - font.StringWidth(string)) / 2), y);
    break;
...
void BTitledColumn::SetTitle(const char *title) { fTitle.SetTo(title); }
...
float BTitledColumn::FontHeight() const { return fFontHeight; }

float BTitledColumn::GetPreferredWidth(BField *_field, BView *parent) const {
...
const char *BStringField::String() const { return fString.String(); }

void BStringField::SetWidth(float width) { fWidth = width; }

float BStringField::Width() { return fWidth; }

void BStringField::SetClippedString(const char *val) { fClippedString = val; }

bool BStringField::HasClippedString() const {
  return !fClippedString.IsEmpty();
}

const char *BStringField::ClippedString() { return fClippedString.String(); }
...
float BStringColumn::GetPreferredWidth(BField *_field, BView *parent) const {
  BStringField *field = static_cast<BStringField *>(_field);
...
BDateField::BDateField(time_t *time)
    : fTime(*localtime(time)), fUnixTime(*time), fSeconds(0),
      fClippedString(""), fWidth(0) {
  fSeconds = mktime(&fTime);
}

void BDateField::SetWidth(float width) { fWidth = width; }

float BDateField::Width() { return fWidth; }

void BDateField::SetClippedString(const char *string) {
  fClippedString = string;
}

const char *BDateField::ClippedString() { return fClippedString.String(); }

time_t BDateField::Seconds() { return fSeconds; }

time_t BDateField::UnixTime() { return fUnixTime; }
...
void BSizeField::SetSize(off_t size) { fSize = size; }

off_t BSizeField::Size() { return fSize; }
...
void BIntegerField::SetValue(int32 value) { fInteger = value; }

int32 BIntegerField::Value() { return fInteger; }
...
int BIntegerColumn::CompareFields(BField *field1, BField *field2) {
  return (((BIntegerField *)field1)->Value() -
          ((BIntegerField *)field2)->Value());
}
...
void GraphColumn::DrawField(BField *field, BRect rect, BView *parent) {
  ...
  parent->MovePenTo(rect.left + rect.Width() / 2 - width / 2,
...
const BBitmap *BBitmapField::Bitmap() { return fBitmap; }

void BBitmapField::SetBitmap(BBitmap *bitmap) { fBitmap = bitmap; }
