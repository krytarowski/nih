...
BUrl::BUrl(const char *url)
    : fUrlString(), fProtocol(), fUser(), fPassword(), fHost(), fPort(0),
      fPath(), fRequest(), fHasHost(false), fHasFragment(false) {
  SetUrlString(url);
}
...
BUrl::BUrl(const BUrl &other)
    : BArchivable(), fUrlString(), fProtocol(other.fProtocol),
      fUser(other.fUser), fPassword(other.fPassword), fHost(other.fHost),
      fPort(other.fPort), fPath(other.fPath), fRequest(other.fRequest),
      fFragment(other.fFragment), fUrlStringValid(other.fUrlStringValid),
      fAuthorityValid(other.fAuthorityValid),
      fUserInfoValid(other.fUserInfoValid), fHasProtocol(other.fHasProtocol),
      fHasUserName(other.fHasUserName), fHasPassword(other.fHasPassword),
      fHasHost(other.fHasHost), fHasPort(other.fHasPort),
      fHasPath(other.fHasPath), fHasRequest(other.fHasRequest),
...
BUrl::BUrl(const BUrl &base, const BString &location)
    : fUrlString(), fProtocol(), fUser(), fPassword(), fHost(), fPort(0),
      fPath(), fRequest(), fAuthorityValid(false), fUserInfoValid(false),
      fHasUserName(false), fHasPassword(false), fHasHost(false),
      fHasPort(false), fHasFragment(false) {
...
BUrl &BUrl::SetProtocol(const BString &protocol) {
  fProtocol = protocol;
...
const BString &BUrl::Protocol() const { return fProtocol; }

const BString &BUrl::UserName() const { return fUser; }

const BString &BUrl::Password() const { return fPassword; }
...
const BString &BUrl::Host() const { return fHost; }

int BUrl::Port() const { return fPort; }
...
const BString &BUrl::Path() const { return fPath; }

const BString &BUrl::Request() const { return fRequest; }

const BString &BUrl::Fragment() const { return fFragment; }
...
bool BUrl::HasProtocol() const { return fHasProtocol; }

bool BUrl::HasAuthority() const { return fHasHost || fHasUserName; }

bool BUrl::HasUserName() const { return fHasUserName; }

bool BUrl::HasPassword() const { return fHasPassword; }

bool BUrl::HasUserInfo() const { return fHasUserName || fHasPassword; }

bool BUrl::HasHost() const { return fHasHost; }

bool BUrl::HasPort() const { return fHasPort; }

bool BUrl::HasPath() const { return fHasPath; }

bool BUrl::HasRequest() const { return fHasRequest; }

bool BUrl::HasFragment() const { return fHasFragment; }
...
bool BUrl::HasPreferredApplication() const {
  ...
  if (appSignature.IFindFirst("application/") == 0 && mime.IsValid())
    return true;
...
//		BAlert ... Too long URL!
  ...
  char *argv[] = {const_cast<char *>("BUrlInvokedApplication"),
                  const_cast<char *>(urlString.String()), NULL};
...
/*static*/ BString BUrl::UrlEncode(const BString &url, bool strict,
                                   bool directory) {
  return _DoUrlEncodeChunk(url, strict, directory);
}

/*static*/ BString BUrl::UrlDecode(const BString &url, bool strict) {
...
/*static*/ BArchivable *BUrl::Instantiate(BMessage *archive) {
...
bool BUrl::operator!=(BUrl &other) const { return !(*this == other); }
...
BUrl::operator const char *() const { return UrlString(); }
...
bool BUrl::_ContainsDelimiter(const BString &url) {
...
static bool explode_is_protocol_char(char c) {
  return isalnum(c) || c == '+' || c == '.' || c == '-';
}

static bool explode_is_authority_char(char c) {
  return !(c == '/' || c == '?' || c == '#');
}

static bool explode_is_path_char(char c) { return !(c == '#' || c == '?'); }

static bool explode_is_request_char(char c) { return c != '#'; }

static int32 char_offset_until_fn_false(const char *url, int32 len,
                                        int32 offset,
...
/*
 * This function takes a URL in string-form and parses the components of the URL
 * out.
 */
status_t BUrl::_ExplodeUrlString(const BString &url) {
  _ResetFields();
  ...
  // ^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\\?([^#]*))?(#(.*))?
  // The ensuing logic attempts to simulate the behaviour of extracting the
  // groups from the string without requiring a group-capable regex engine.

  while (offset < length) {
    switch (state) {

    case EXPLODE_PROTOCOL: {
      int32 end_protocol = char_offset_until_fn_false(url_c, length, offset,
    ...
    case EXPLODE_AUTHORITY: {
      int end_authority = char_offset_until_fn_false(url_c, length, offset,
    ...
    case EXPLODE_REQUEST: // query
    {
      if (url_c[offset] == '?') {
        offset++;
        int end_request = char_offset_until_fn_false(url_c, length, offset,
...
BString BUrl::_MergePath(const BString &relative) const {
...
static bool authority_is_username_char(char c) {
  return !(c == ':' || c == '@');
}

static bool authority_is_password_char(char c) { return !(c == '@'); }

static bool authority_is_ipv6_host_char(char c) {
  return (c >= 'A' && c <= 'F') || (c >= 'a' && c <= 'f') ||
         (c >= '0' && c <= '9') || c == ':';
}

static bool authority_is_host_char(char c) { return !(c == ':' || c == '/'); }

static bool authority_is_port_char(char c) { return c >= '0' && c <= '9'; }
...
void BUrl::SetAuthority(const BString &authority) {
  ...
  while (AUTHORITY_COMPLETE != state && offset < length) {
    ...
    case AUTHORITY_USERNAME: {
      if (hasUsernamePassword) {
        int32 end_username = char_offset_until_fn_false(
            authority_c, length, offset, authority_is_username_char);

        SetUserName(BString(&authority_c[offset], end_username - offset));
    ...
    case AUTHORITY_PASSWORD: {
      if (hasUsernamePassword && ':' == authority[offset]) {
        ...
        int32 end_password = char_offset_until_fn_false(
            authority_c, length, offset, authority_is_password_char);

        SetPassword(BString(&authority_c[offset], end_password - offset));
    ...
    case AUTHORITY_HOST: {
      ...
      if (authority_c[offset] == '[') {
        int32 end_ipv6_host = char_offset_until_fn_false(
            authority_c, length, offset + 1, authority_is_ipv6_host_char);

        if (authority_c[end_ipv6_host] == ']') {
          SetHost(BString(&authority_c[offset], (end_ipv6_host - offset) + 1));
      ...
      if (AUTHORITY_HOST == state) {
        int32 end_host = char_offset_until_fn_false(authority_c, length, offset,
    ...
    case AUTHORITY_PORT: {
      if (authority_c[offset] == ':') {
        offset++;
        int32 end_port = char_offset_until_fn_false(authority_c, length, offset,
...
/*static*/ BString BUrl::_DoUrlEncodeChunk(const BString &chunk, bool strict,
                                           bool directory) {
  BString result;
...
/*static*/ BString BUrl::_DoUrlDecodeChunk(const BString &chunk, bool strict) {
  BString result;
  ...
  for (int32 i = 0; i < chunk.Length(); i++) {
    if (chunk[i] == '+' && !strict)
      ...
      if (chunk[i] == '%' && i < chunk.Length() - 2 && isxdigit(chunk[i + 1]) &&
          isxdigit(chunk[i + 2])) {
...
BString BUrl::_UrlMimeType() const {
  BString mime;
