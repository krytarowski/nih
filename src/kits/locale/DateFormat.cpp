...
BDateFormat::BDateFormat(const BLocale *locale) : BFormat(locale) {}

BDateFormat::BDateFormat(const BLanguage &language,
                         const BFormattingConventions &conventions)
    : BFormat(language, conventions) {}

BDateFormat::BDateFormat(const BDateFormat &other) : BFormat(other) {}
...
void BDateFormat::SetDateFormat(BDateFormatStyle style, const BString &format) {
...
status_t BDateFormat::Format(BString &string, int *&fieldPositions,
                             int &fieldCount, const time_t time,
                             const BDateFormatStyle style) const {
  ObjectDeleter<DateFormat> dateFormatter(_CreateDateFormatter(style));
  if (dateFormatter.Get() == NULL)
    return B_NO_MEMORY;
...
status_t BDateFormat::GetFields(BDateElement *&fields, int &fieldCount,
  ...
  UnicodeString icuString;
  time_t now;
  dateFormatter->format((UDate)time(&now) * 1000, icuString, &positionIterator,
                        error);
...
status_t BDateFormat::Parse(BString source, BDateFormatStyle style,
                            BDate &output) {
  ...
  ParsePosition p(0);
  UDate date =
      dateFormatter->parse(UnicodeString::fromUTF8(source.String()), p);
