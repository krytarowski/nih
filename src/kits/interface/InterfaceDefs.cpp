...
static const char *kColorNames[kColorWhichCount] = {
    ...
    "B_KEYBOARD_NAVIGATION_COLOR", "B_DESKTOP_COLOR",
    "B_MENU_SELECTED_BACKGROUND_COLOR", "B_MENU_ITEM_TEXT_COLOR",
    "B_MENU_SELECTED_ITEM_TEXT_COLOR", "B_MENU_SELECTED_BORDER_COLOR",
    "B_PANEL_TEXT_COLOR", "B_DOCUMENT_BACKGROUND_COLOR",
    "B_DOCUMENT_TEXT_COLOR", "B_CONTROL_BACKGROUND_COLOR",
    "B_CONTROL_TEXT_COLOR", "B_CONTROL_BORDER_COLOR",
    "B_CONTROL_HIGHLIGHT_COLOR", "B_NAVIGATION_PULSE_COLOR", "B_SHINE_COLOR",
    "B_SHADOW_COLOR", "B_TOOLTIP_BACKGROUND_COLOR", "B_TOOLTIP_TEXT_COLOR",
    "B_WINDOW_TEXT_COLOR", "B_WINDOW_INACTIVE_TAB_COLOR",
    "B_WINDOW_INACTIVE_TEXT_COLOR", "B_WINDOW_BORDER_COLOR",
    ...
    "B_SCROLL_BAR_THUMB_COLOR", "B_LINK_TEXT_COLOR", "B_LINK_HOVER_COLOR",
    "B_LINK_VISITED_COLOR", "B_LINK_ACTIVE_COLOR", "B_STATUS_BAR_COLOR",
    // 100...
...
void set_subpixel_antialiasing(bool subpix) {
  BPrivate::AppServerLink link;
...
status_t get_subpixel_antialiasing(bool *subpix) {
  BPrivate::AppServerLink link;
...
void set_hinting_mode(uint8 hinting) {
  BPrivate::AppServerLink link;
...
status_t get_hinting_mode(uint8 *hinting) {
  BPrivate::AppServerLink link;
...
void set_average_weight(uint8 averageWeight) {
  BPrivate::AppServerLink link;
...
status_t get_average_weight(uint8 *averageWeight) {
  BPrivate::AppServerLink link;
...
void set_is_subpixel_ordering_regular(bool subpixelOrdering) {
  BPrivate::AppServerLink link;
...
status_t get_is_subpixel_ordering_regular(bool *subpixelOrdering) {
  BPrivate::AppServerLink link;
...
bool focus_follows_mouse() { return mouse_mode() == B_FOCUS_FOLLOWS_MOUSE; }
...
status_t get_mouse_bitmap(BBitmap **bitmap, BPoint *hotspot) {
  ...
  BBitmap *cursorBitmap = new (std::nothrow)
      BBitmap(BRect(0, 0, cursorWidth - 1, cursorHeight - 1), B_RGBA32);
...
*/
bool get_decorator(BString &path) {
  BPrivate::AppServerLink link;
...
*/
status_t set_decorator(const BString &path) {
  BPrivate::AppServerLink link;
...
*/
status_t preview_decorator(const BString &path, BWindow *window) {
  if (window == NULL)
    return B_ERROR;
...
status_t get_application_order(int32 workspace, team_id **_applications,
                               int32 *_count) {
  BPrivate::AppServerLink link;
...
void do_window_action(int32 windowToken, int32 action, BRect zoomRect,
                      bool zoom) {
  BPrivate::AppServerLink link;
...
void truncate_string(BString &string, uint32 mode, float width,
                     ...
                     float ellipsisWidth, int32 charCount) {
    ...
    for (int32 i = 0; i < charCount; i++) {
      ...
      if (leftWidth + rightWidth + charWidth > width) {
        ...
        while (leftWidth + rightWidth + ellipsisWidth > width) {
          ...
          if (leftIndex > 0 &&
              (rightIndex == charCount - 1 || leftWidth > rightWidth)) {
