...
BRegion::BRegion()
    : fCount(0), fDataSize(0), fBounds((clipping_rect){0, 0, 0, 0}),
      fData(NULL) {
...
BRegion::BRegion(const BRect rect)
    : fCount(0), fDataSize(1), fBounds((clipping_rect){0, 0, 0, 0}),
      fData(&fBounds) {
  if (!rect.IsValid())
    return;
...
BRegion::BRegion(const clipping_rect &clipping)
    : fCount(1), fDataSize(1), fBounds(clipping), fData(&fBounds) {}
...
void BRegion::Set(BRect rect) { Set(_Convert(rect)); }
...
BRect BRegion::Frame() const {
  return BRect(fBounds.left, fBounds.top, fBounds.right - 1,
               fBounds.bottom - 1);
}
...
clipping_rect BRegion::FrameInt() const {
  return (clipping_rect){fBounds.left, fBounds.top, fBounds.right - 1,
                         fBounds.bottom - 1};
}
...
int32 BRegion::CountRects() { return fCount; }

// Returns the number of rects contained in the region.
int32 BRegion::CountRects() const { return fCount; }
...
void BRegion::OffsetBy(const BPoint &point) { OffsetBy(point.x, point.y); }
...
void BRegion::ScaleBy(BSize scale) { ScaleBy(scale.Width(), scale.Height()); }
...
void BRegion::Include(BRect rect) { Include(_Convert(rect)); }
...
/*!	\brief Modifies the region, excluding the area represented by the given
   BRect. \param rect The BRect to be excluded.
*/
void BRegion::Exclude(BRect rect) { Exclude(_Convert(rect)); }

// Modifies the region, excluding the area represented by the given
// clipping_rect.
...
*/
void BRegion::_AdoptRegionData(BRegion &region) {
...
/*!
        ...
        \param newSize The amount of rectangles that the region should be
                able to hold.
*/
bool BRegion::_SetSize(int32 newSize) {
  ...
  if (newSize > 0) {
    if (fData == &fBounds) {
      ...
      clipping_rect *resizedData =
          (clipping_rect *)realloc(fData, newSize * sizeof(clipping_rect));
      if (!resizedData) {
...
clipping_rect BRegion::_Convert(const BRect &rect) const {
  return (clipping_rect){(int)floorf(rect.left), (int)floorf(rect.top),
                         (int)ceilf(rect.right), (int)ceilf(rect.bottom)};
}

clipping_rect BRegion::_ConvertToInternal(const BRect &rect) const {
  return (clipping_rect){(int)floorf(rect.left), (int)floorf(rect.top),
                         (int)ceilf(rect.right) + 1,
                         (int)ceilf(rect.bottom) + 1};
}

clipping_rect BRegion::_ConvertToInternal(const clipping_rect &rect) const {
  return (clipping_rect){rect.left, rect.top, rect.right + 1, rect.bottom + 1};
}
