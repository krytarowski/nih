...
LinkSender::LinkSender(port_id port)
    : fPort(port), fTargetTeam(-1), fBuffer(NULL), fBufferSize(0),

      fCurrentEnd(0), fCurrentStart(0), fCurrentStatus(B_OK) {}

LinkSender::~LinkSender() { free(fBuffer); }

void LinkSender::SetPort(port_id port) { fPort = port; }
...
status_t LinkSender::StartMessage(int32 code, size_t minSize) {
  ...
  if (fBufferSize > 0 &&
      (minSize > SpaceLeft() || fCurrentStart >= kWatermark)) {
  ...
  STRACE(("info: LinkSender buffered header %ld (%lx) [%lu %lu %lu].\n", code,
          code, header->size, header->code, header->flags));
...
status_t LinkSender::Attach(const void *passedData, size_t passedSize) {
  ...
  if (useArea) {
    ...
    senderArea =
        create_area("LinkSenderArea", &address, B_ANY_ADDRESS, alignedSize,
    ...
    senderArea =
        _kern_transfer_area(senderArea, &address, B_ANY_ADDRESS, fTargetTeam);
...
status_t LinkSender::Flush(bigtime_t timeout, bool needsReply) {
  ...
  STRACE(("info: LinkSender Flush() waiting to send messages of %ld bytes on "
          "port %ld.\n",
          fCurrentEnd, fPort));
  ...
  if (timeout != B_INFINITE_TIMEOUT) {
    do {
      err = write_port_etc(fPort, kLinkCode, fBuffer, fCurrentEnd,
  ...
  if (err < B_OK) {
    STRACE(("error info: LinkSender Flush() failed for %ld bytes (%s) on port "
            "%ld.\n",
