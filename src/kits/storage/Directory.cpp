...
BDirectory::BDirectory() : fDirFd(-1) {}

BDirectory::BDirectory(const BDirectory &dir) : fDirFd(-1) { *this = dir; }

BDirectory::BDirectory(const entry_ref *ref) : fDirFd(-1) { SetTo(ref); }

BDirectory::BDirectory(const node_ref *nref) : fDirFd(-1) { SetTo(nref); }

BDirectory::BDirectory(const BEntry *entry) : fDirFd(-1) { SetTo(entry); }

BDirectory::BDirectory(const char *path) : fDirFd(-1) { SetTo(path); }

BDirectory::BDirectory(const BDirectory *dir, const char *path) : fDirFd(-1) {
  SetTo(dir, path);
}
...
bool BDirectory::Contains(const char *path, int32 nodeFlags) const {
...
bool BDirectory::Contains(const BEntry *entry, int32 nodeFlags) const {
  ...
  if (error == B_OK) {
    ...
    while (error == B_OK && next) {
      if (GetNextDirents(&entry, sizeof(entry), 1) != 1) {
        ...
        next = (!strcmp(entry.d_name, ".") || !strcmp(entry.d_name, ".."));
      }
    }
    if (error == B_OK) {
...
int32 BDirectory::GetNextDirents(dirent *buf, size_t bufSize, int32 count) {
  if (!buf)
    return B_BAD_VALUE;
  if (InitCheck() != B_OK)
    return B_FILE_ERROR;
...
status_t BDirectory::CreateDirectory(const char *path, BDirectory *dir) {
  if (!path)
    return B_BAD_VALUE;

  // create the dir
  status_t error =
      _kern_create_dir(fDirFd, path, (S_IRWXU | S_IRWXG | S_IRWXO) & ~__gUmask);
  if (error != B_OK)
    return error;
...
status_t BDirectory::CreateFile(const char *path, BFile *file,
                                bool failIfExists) {
  if (!path)
    return B_BAD_VALUE;
...
status_t BDirectory::CreateSymLink(const char *path, const char *linkToPath,
                                   BSymLink *link) {
  if (!path || !linkToPath)
    return B_BAD_VALUE;

  // create the symlink
  status_t error = _kern_create_symlink(
      fDirFd, path, linkToPath, (S_IRWXU | S_IRWXG | S_IRWXO) & ~__gUmask);
  if (error != B_OK)
    return error;
...
status_t BDirectory::_GetStatFor(const char *path, struct stat *st) const {
  if (!st)
    return B_BAD_VALUE;
  if (InitCheck() != B_OK)
    return B_NO_INIT;
...
int BDirectory::get_fd() const { return fDirFd; }
...
status_t create_directory(const char *path, mode_t mode) {
  ...
  do {
    ...
    status_t error = BPrivate::Storage::parse_first_path_component(
