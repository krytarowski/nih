...
static int compare_families(const family *a, const family *b) {
...
FontList::FontList()
    : BLocker("font list"), fLastFamily(NULL), fLastUpdate(0), fRevision(0) {}

FontList::~FontList() {}

/*static*/ FontList *FontList::Default() {
...
bool FontList::UpdatedOnServer() { return _RevisionOnServer() != fRevision; }

status_t FontList::FamilyAt(int32 index, font_family *_family, uint32 *_flags) {
...
status_t FontList::StyleAt(font_family familyName, int32 index,
                           font_style *_style, uint16 *_face, uint32 *_flags) {
  BAutolock locker(this);
...
int32 FontList::CountStyles(font_family familyName) {
  BAutolock locker(this);
...
status_t FontList::_UpdateIfNecessary() {
...
int32 FontList::_RevisionOnServer() {
  BPrivate::AppServerLink link;
...
family *FontList::_FindFamily(font_family name) {
  ...
  family.name = name;
  fLastFamily =
      const_cast< ::family *>(fFamilies.BinarySearch(family, compare_families));
  return fLastFamily;
}

/*static*/ void FontList::_InitSingleton() { sDefaultInstance = new FontList; }
...
void _set_system_font_(const char *which, font_family family, font_style style,
                       float size) {
...
status_t _get_system_default_font_(const char *which, font_family family,
...
int32 count_font_families() { return FontList::Default()->CountFamilies(); }
...
status_t get_font_family(int32 index, font_family *_name, uint32 *_flags) {
  if (_name == NULL)
    return B_BAD_VALUE;
...
// Retrieves the family name at the specified index
status_t get_font_style(font_family family, int32 index, font_style *_name,
                        uint32 *_flags) {
...
BFont::BFont()
    : // initialise for be_plain_font (avoid circular definition)
      fFamilyID(0),
      fStyleID(0), fSize(10.0), fShear(90.0), fRotation(0.0),
      fFalseBoldWidth(0.0), fSpacing(B_BITMAP_SPACING),
      fEncoding(B_UNICODE_UTF8), fFace(0), fFlags(0),
...
BFont::BFont(const BFont &font) { *this = font; }
...
void BFont::SetFalseBoldWidth(float width) { fFalseBoldWidth = width; }

void BFont::SetSpacing(uint8 spacing) { fSpacing = spacing; }

void BFont::SetEncoding(uint8 encoding) { fEncoding = encoding; }
...
void BFont::SetFlags(uint32 flags) { fFlags = flags; }
...
uint32 BFont::FamilyAndStyle() const { return (fFamilyID << 16UL) | fStyleID; }

float BFont::Size() const { return fSize; }

float BFont::Shear() const { return fShear; }

float BFont::Rotation() const { return fRotation; }

float BFont::FalseBoldWidth() const { return fFalseBoldWidth; }

uint8 BFont::Spacing() const { return fSpacing; }

uint8 BFont::Encoding() const { return fEncoding; }

uint16 BFont::Face() const { return fFace; }

uint32 BFont::Flags() const { return fFlags; }
...
void BFont::GetTruncatedStrings(const char *stringArray[], int32 numStrings,
  ...
  if (stringArray != NULL && numStrings > 0) {
    ...
    for (int32 i = 0; i < numStrings; i++) {
      ...
      truncate_string(resultArray[i], mode, width, escapementArray, fSize,
...
void BFont::GetEscapements(const char charArray[], int32 numChars,
...
void BFont::GetEdges(const char charArray[], int32 numChars,
...
void BFont::GetBoundingBoxesAsGlyphs(const char charArray[], int32 numChars,
...
void BFont::GetBoundingBoxesAsString(const char charArray[], int32 numChars,
                                     font_metric_mode mode,
                                     escapement_delta *delta,
                                     BRect boundingBoxArray[]) const {
...
void BFont::_GetBoundingBoxes(const char charArray[], int32 numChars,
                              font_metric_mode mode, bool string_escapement,
                              escapement_delta *delta, BRect boundingBoxArray[],
                              bool asString) const {
  ...
  link.StartMessage(asString ? AS_GET_BOUNDINGBOXES_STRING
...
bool BFont::operator==(const BFont &font) const {
  return fFamilyID == font.fFamilyID && fStyleID == font.fStyleID &&
         fSize == font.fSize && fShear == font.fShear &&
         fRotation == font.fRotation &&
         fFalseBoldWidth == font.fFalseBoldWidth && fSpacing == font.fSpacing &&
         fEncoding == font.fEncoding && fFace == font.fFace;
}
