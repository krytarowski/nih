...
entry_ref::entry_ref() : device((dev_t)-1), directory((ino_t)-1), name(NULL) {}
...
entry_ref::entry_ref(const entry_ref &ref)
    : device(ref.device), directory(ref.directory), name(NULL) {
  set_name(ref.name);
}

entry_ref::~entry_ref() { free(name); }
...
bool entry_ref::operator==(const entry_ref &ref) const {
  return (device == ref.device && directory == ref.directory &&
          (name == ref.name ||
...
BEntry::BEntry() : fDirFd(-1), fName(NULL), fCStatus(B_NO_INIT) {}

BEntry::BEntry(const BDirectory *dir, const char *path, bool traverse)
    : fDirFd(-1), fName(NULL), fCStatus(B_NO_INIT) {
...
status_t BEntry::InitCheck() const { return fCStatus; }
...
status_t BEntry::GetRef(entry_ref *ref) const {
  ...
  struct stat st;
  status_t error =
      _kern_read_stat(fDirFd, NULL, false, &st, sizeof(struct stat));
  if (error == B_OK) {
...
bool BEntry::operator!=(const BEntry &item) const { return !(*this == item); }
...
status_t BEntry::set_stat(struct stat &st, uint32 what) {
  ...
  return _kern_write_stat(fDirFd, fName, false, &st, sizeof(struct stat), what);
}
...
/*!	Sets the entry to point to the entry specified by the path \a path
   ...
   considered. \param path Pointer to a path relative to the given directory.
...
status_t BEntry::_SetTo(int dirFD, const char *path, bool traverse) {
  ...
  while (true) {
    if (!path || strcmp(path, ".") == 0) {
      ...
      status_t error = BPrivate::Storage::parse_path(path, dirPath, leafName);
      if (error != B_OK)
        return error;
      ...
      } else if (strcmp(leafName, ".") == 0 || strcmp(leafName, "..") == 0) {
...
*/
void BEntry::_Dump(const char *name) {
  if (name != NULL) {
    printf("------------------------------------------------------------\n");
    printf("%s\n", name);
    printf("------------------------------------------------------------\n");
  ...
  struct stat st;
  if (fDirFd != -1 &&
      _kern_read_stat(fDirFd, NULL, false, &st, sizeof(struct stat)) == B_OK) {
...
status_t BEntry::_GetStat(struct stat *st) const {
  if (fCStatus != B_OK)
    return B_NO_INIT;
