...
BIcon::BIcon() : fEnabledBitmaps(8, true), fDisabledBitmaps(8, true) {}
...
status_t BIcon::SetTo(const BBitmap *bitmap, uint32 flags) {
  ...
  if ((flags & (B_TRIM_ICON_BITMAP | B_TRIM_ICON_BITMAP_KEEP_ASPECT)) != 0 &&
    ...
    if (bitmap->ColorSpace() == B_RGBA32) {
      error = _TrimBitmap(bitmap, (flags & B_TRIM_ICON_BITMAP_KEEP_ASPECT) != 0,
                          trimmedBitmap);
    } else {
...
bool BIcon::SetBitmap(BBitmap *bitmap, uint32 which) {
  BitmapList &list = (which & B_DISABLED_ICON_BITMAP) == 0 ? fEnabledBitmaps
                                                           : fDisabledBitmaps;
...
BBitmap *BIcon::Bitmap(uint32 which) const {
...
BBitmap *BIcon::CreateBitmap(const BRect &bounds, color_space colorSpace,
                             uint32 which) {
...
status_t BIcon::SetExternalBitmap(const BBitmap *bitmap, uint32 which,
                                  uint32 flags) {
...
BBitmap *BIcon::CopyBitmap(const BBitmap &bitmapToClone, uint32 which) {
...
/*static*/ status_t BIcon::UpdateIcon(const BBitmap *bitmap, uint32 flags,
                                      BIcon *&_icon) {
  if (bitmap == NULL) {
    delete _icon;
    _icon = NULL;
...
/*static*/ status_t BIcon::SetIconBitmap(const BBitmap *bitmap, uint32 which,
                                         uint32 flags, BIcon *&_icon) {
  bool newIcon = false;
  if (_icon == NULL) {
    if (bitmap == NULL)
      return B_OK;
...
/*static*/ BBitmap *BIcon::_ConvertToRGB32(const BBitmap *bitmap,
                                           bool noAppServerLink) {
  BBitmap *rgb32Bitmap = new (std::nothrow)
      BBitmap(bitmap->Bounds(), noAppServerLink ? B_BITMAP_NO_SERVER_LINK : 0,
              B_RGBA32);
  if (rgb32Bitmap == NULL)
    return NULL;
...
status_t BIcon::_MakeBitmaps(const BBitmap *bitmap, uint32 flags) {
  ...
  if ((flags & B_CREATE_DISABLED_ICON_BITMAPS) != 0) {
    disabledBitmap = CreateBitmap(
        b, format, B_INACTIVE_ICON_BITMAP | B_DISABLED_ICON_BITMAP);
  ...
  if (disabledBitmap != NULL && clickedBitmap != NULL) {
    disabledClickedBitmap =
        CreateBitmap(b, format, B_ACTIVE_ICON_BITMAP | B_DISABLED_ICON_BITMAP);
      ...
      for (int32 x = 0; x < pixels; x++) {
        ...
        if (dBits != NULL) {
          uint8 grey = ((uint16)nBits[nOffset + 0] * 10 +
                        nBits[nOffset + 1] * 60 + nBits[nOffset + 2] * 30) /
                       100;
  ...
  if ((flags & B_CREATE_PARTIALLY_ACTIVE_ICON_BITMAP) != 0) {
    if (CopyBitmap(clickedBitmap, B_PARTIALLY_ACTIVATE_ICON_BITMAP) == NULL)
      return B_NO_MEMORY;
    if ((flags & B_CREATE_DISABLED_ICON_BITMAPS) != 0) {
      if (CopyBitmap(disabledClickedBitmap, B_PARTIALLY_ACTIVATE_ICON_BITMAP |
