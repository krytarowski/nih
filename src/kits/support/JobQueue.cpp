...
/*!	Sort jobs by:
                1. descending count of dependencies (only jobs without
   dependencies are runnable) 2. job ticket number (order in which jobs were
   added to the queue)
*/
bool JobQueue::JobPriorityLess::operator()(const BJob *left,
                                           const BJob *right) const {
...
class JobQueue::JobPriorityQueue : public std::set<BJob *, JobPriorityLess> {};

// #pragma mark -

JobQueue::JobQueue() : fLock("job queue"), fNextTicketNumber(1) {
  fInitStatus = _Init();
}
...
status_t JobQueue::InitCheck() const { return fInitStatus; }
...
status_t JobQueue::Pop(bigtime_t timeout, bool returnWhenEmpty, BJob **_job) {
  ...
  if (lock.IsLocked()) {
    while (true) {
      ...
      do {
        lock.Unlock();
        result = acquire_sem_etc(fHaveRunnableJobSem, 1, B_RELATIVE_TIMEOUT,
                                 timeout);
        if (!lock.Lock())
          return B_ERROR;
...
size_t JobQueue::CountJobs() const {
