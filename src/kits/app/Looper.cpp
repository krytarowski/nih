...
enum { BLOOPER_PROCESS_INTERNALLY = 0, BLOOPER_HANDLER_BY_INDEX };
...
BLooper::BLooper(BMessage *data) : BHandler(data) {
  int32 portCapacity;
...
BMessage *BLooper::CurrentMessage() const { return fLastMessage; }
...
void BLooper::DispatchExternalMessage(BMessage *message, BHandler *handler,
                                      bool &_detached) {
  AssertLocked();
...
BMessageQueue *BLooper::MessageQueue() const { return fDirectTarget->Queue(); }
...
BHandler *BLooper::PreferredHandler() const { return fPreferred; }
...
void BLooper::Quit() {
  ...
  if (!fRunCalled) {
    ...
    PRINT(
        ("  Run() has already been called and we are not the looper thread\n"));
...
bool BLooper::QuitRequested() { return true; }
...
thread_id BLooper::Thread() const { return fThread; }

team_id BLooper::Team() const { return BPrivate::current_team(); }

BLooper *BLooper::LooperForThread(thread_id thread) {
  return gLooperList.LooperForThread(thread);
}

thread_id BLooper::LockingThread() const { return fOwner; }

int32 BLooper::CountLocks() const { return fOwnerCount; }

int32 BLooper::CountLockRequests() const { return fAtomicCount; }

sem_id BLooper::Sem() const { return fLockSem; }

BHandler *BLooper::ResolveSpecifier(BMessage *message, int32 index,
                                    BMessage *specifier, int32 what,
                                    const char *property) {
  /**
          @note	When I was first dumping the results of
     GetSupportedSuites() from various classes, the use of the extra_data field
     was quite mysterious to me.  Then I dumped BApplication and compared the
                          result against the BeBook's docs for scripting
     BApplication.  A bunch of it isn't documented, but what is tipped me to the
     idea that the extra_data is being used as a quick and dirty way to tell
                          what scripting "command" has been sent, e.g., for easy
     use in a switch statement.  Would certainly be a lot faster than a bunch of
                          string comparisons -- which wouldn't tell the whole
     story anyway, because of the same name being used for multiple properties.
   */
...
BList *BLooper::CommonFilterList() const { return fCommonFilters; }
...
BLooper &BLooper::operator=(const BLooper &other) {
...
*/
status_t BLooper::_Lock(BLooper *looper, port_id port, bigtime_t timeout) {
...
status_t BLooper::_LockComplete(BLooper *looper, int32 oldCount,
                                thread_id thread, sem_id sem,
                                bigtime_t timeout) {
  status_t err = B_OK;
...
void BLooper::_InitData(const char *name, int32 priority, port_id port,
                        int32 portCapacity) {
  fOwner = B_ERROR;
  fCachedStack = 0;
  fRunCalled = false;
...
void BLooper::AddMessage(BMessage *message) {
...
void BLooper::_AddMessagePriv(BMessage *message) {
...
BMessage *BLooper::ConvertToMessage(void *buffer, int32 code) {
...
void BLooper::_QuitRequested(BMessage *message) {
...
BHandler *BLooper::_TopLevelFilter(BMessage *message, BHandler *target) {
  if (message == NULL)
    return target;
...
BHandler *BLooper::_HandlerFilter(BMessage *message, BHandler *target) {
...
BHandler *BLooper::_ApplyFilters(BList *list, BMessage *message,
                                 BHandler *target) {
  ...
  for (int32 i = 0; i < list->CountItems(); ++i) {
    ...
    if (filter->FiltersAnyCommand() || filter->Command() == message->what) {
      ...
      if (delivery == B_ANY_DELIVERY ||
        ...
        if (source == B_ANY_SOURCE || (source == B_REMOTE_SOURCE && remote) ||
...
BHandler *BLooper::resolve_specifier(BHandler *target, BMessage *message) {
  // check params
  if (!target || !message)
    return NULL;
  ...
  do {
    ...
    target = newTarget;
    newTarget =
        target->ResolveSpecifier(message, index, &specifier, form, property);
...
port_id _get_looper_port_(const BLooper *looper) { return looper->fMsgPort; }
