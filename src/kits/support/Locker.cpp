...
/*!	This constructor is not documented.  The final argument is ignored for
        ...
        CONSTRUCTOR!
*/
BLocker::BLocker(const char *name, bool benaphoreStyle, bool) {
  InitLocker(name, benaphoreStyle);
}

BLocker::~BLocker() { delete_sem(fSemaphoreID); }
...
thread_id BLocker::LockingThread() const { return fLockOwner; }
...
int32 BLocker::CountLocks() const { return fRecursiveCount; }

int32 BLocker::CountLockRequests() const { return fBenaphoreCount; }

sem_id BLocker::Sem() const { return fSemaphoreID; }

void BLocker::InitLocker(const char *name, bool benaphore) {
  if (name == NULL)
    name = "some BLocker";
...
bool BLocker::AcquireLock(bigtime_t timeout, status_t *error) {
  ...
  if (!IsLocked()) {
    ...
    if (oldBenaphoreCount > 0) {
      do {
        status = acquire_sem_etc(fSemaphoreID, 1, B_RELATIVE_TIMEOUT, timeout);
