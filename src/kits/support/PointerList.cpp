/*
** Copyright 2003-2004, Stefano Ceccherini (burton666@libero.it). All rights
*reserved. *           2004, Michael Pfeiffer (laplace@users.sourceforge.net).
...
int32 AbstractPointerListHelper::BinarySearchIndex(const void *key,
                                                   const BList *list) {
  int32 index;
...
void *AbstractPointerListHelper::BinarySearch(const void *key,
                                              const BList *list) {
  int32 index;
  const void **items = static_cast<const void **>(list->Items());
...
public:
  ...
  int Compare(const void *a, const void *b) { return fCompareFunc(a, b); }
...
public:
  PointerListHelperWithState(
      ...
      : fCompareFunc(compareFunc), fState(state) {
    // nothing to do
  }
...
public:
  PointerListHelperUsePredicate(_PointerList_::UnaryPredicateGlue predicate)
...
_PointerList_::_PointerList_(int32 itemsPerBlock, bool own)
    : BList(itemsPerBlock), owning(own) {}

_PointerList_::_PointerList_(const _PointerList_ &list)
    : BList(list), owning(list.owning) {}
...
void *_PointerList_::EachElement(GenericEachFunction function, void *arg) {
...
void _PointerList_::SortItems(GenericCompareFunction compareFunc) {
  PointerListHelper helper(compareFunc);
  helper.SortItems(this);
}

void _PointerList_::SortItems(GenericCompareFunctionWithState compareFunc,
                              void *state) {
  PointerListHelperWithState helper(compareFunc, state);
  helper.SortItems(this);
}

void _PointerList_::HSortItems(GenericCompareFunction compareFunc) {
  PointerListHelper helper(compareFunc);
  helper.HSortItems(this);
}

void _PointerList_::HSortItems(GenericCompareFunctionWithState compareFunc,
                               void *state) {
  PointerListHelperWithState helper(compareFunc, state);
  helper.HSortItems(this);
}

void *_PointerList_::BinarySearch(const void *key,
...
void *_PointerList_::BinarySearch(const void *key,
                                  GenericCompareFunctionWithState compareFunc,
                                  void *state) const {
  PointerListHelperWithState helper(compareFunc, state);
  return helper.BinarySearch(key, this);
}

int32 _PointerList_::BinarySearchIndex(
    const void *key, GenericCompareFunction compareFunc) const {
  PointerListHelper helper(compareFunc);
  return helper.BinarySearchIndex(key, this);
}

int32 _PointerList_::BinarySearchIndex(
    const void *key, GenericCompareFunctionWithState compareFunc,
    void *state) const {
  PointerListHelperWithState helper(compareFunc, state);
  return helper.BinarySearchIndex(key, this);
}

int32 _PointerList_::BinarySearchIndexByPredicate(
    const void *key, UnaryPredicateGlue predicate) const {
  PointerListHelperUsePredicate helper(predicate);
  return helper.BinarySearchIndex(key, this);
}

bool _PointerList_::ReplaceItem(int32 index, void *newItem) {
...
bool _PointerList_::MoveItem(int32 from, int32 to) {
  if (from == to)
    return true;
