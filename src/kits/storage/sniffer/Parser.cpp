...
#include <new>
#include <os/support/String.h> // BString
#include <stdio.h>
...
/*! The resulting parsed Rule structure is stored in \c rule, which
        must be pre-allocated. If parsing fails, a descriptive error message
   (meant to be viewed in a monospaced font) is placed in the pre-allocated \c
   BString pointed to by \c parseError (which may be \c NULL if you don't care
   about the error message).

        \param rule Pointer to a NULL-terminated string containing the sniffer
                    rule to be parsed
        \param result Pointer to a pre-allocated \c Rule object into which the
   result of parsing is placed upon success. \param parseError Point to
   pre-allocated \c BString object into which a descriptive error message is
   stored upon failure.
...
*/
status_t BPrivate::Storage::Sniffer::parse(const char *rule, Rule *result,
                                           BString *parseError) {
  Parser parser;
...
Token::Token(TokenType type, const ssize_t pos) : fType(type), fPos(pos) {
  //	if (type != EmptyToken)
  //		cout << "New Token, fType == " << tokenTypeToString(fType) <<
  //endl;
}

Token::~Token() {}

TokenType Token::Type() const { return fType; }

const std::string &Token::String() const {
  throw new Err(
      "Sniffer scanner error: Token::String() called on non-string token",
      fPos);
}

int32 Token::Int() const {
  throw new Err(
      "Sniffer scanner error: Token::Int() called on non-integer token", fPos);
}

double Token::Float() const {
  throw new Err(
      "Sniffer scanner error: Token::Float() called on non-float token", fPos);
}

ssize_t Token::Pos() const { return fPos; }
  ...
  if (Type() == ref.Type()) {
    ...
    case CharacterString:
      ...
      //strcmp(String(), ref.String()));
      {
        ...
        /*
                                        // strcmp() seems to choke on certain,
        non-normal ASCII chars
                                        // (i.e. chars outside the usual
        alphabets, but still valid
                                        // as far as ASCII is concerned), so
        we'll just compare the
...
StringToken::StringToken(const std::string &str, const ssize_t pos)
    : Token(CharacterString, pos), fString(str) {}

StringToken::~StringToken() {}

const std::string &StringToken::String() const { return fString; }
...
IntToken::IntToken(const int32 value, const ssize_t pos)
    : Token(Integer, pos), fValue(value) {}

IntToken::~IntToken() {}

int32 IntToken::Int() const { return fValue; }

double IntToken::Float() const { return (double)fValue; }
...
FloatToken::FloatToken(const double value, const ssize_t pos)
    : Token(FloatingPoint, pos), fValue(value) {}

FloatToken::~FloatToken() {}

double FloatToken::Float() const { return fValue; }
...
TokenStream::TokenStream(const std::string &string)
    : fCStatus(B_NO_INIT), fPos(-1), fStrLen(-1) {
  SetTo(string);
}

TokenStream::TokenStream() : fCStatus(B_NO_INIT), fPos(-1), fStrLen(-1) {}

TokenStream::~TokenStream() { Unset(); }
...
status_t TokenStream::SetTo(const std::string &string) {
  ...
  char lastLastChar =
      0; // For three char lookahead (have I mentioned I hate octal?)
  ...
  while (keepLooping) {
    ...
    case tsssStart:
      ...
      case 0x3: // End-Of-Text
        if (stream.IsEmpty())
          ...
          throw new Err(
              std::string("Sniffer pattern error: invalid character '") + ch +
                  "'",
              pos);
        break;
      ...
      case '&':
        AddToken(Ampersand, pos);
        break;
      case '(':
        AddToken(LeftParen, pos);
        break;
      case ')':
        AddToken(RightParen, pos);
        break;
      case ':':
        AddToken(Colon, pos);
        break;
      case '[':
        AddToken(LeftBracket, pos);
        break;
      ...
      case '\\':
        ...
        state = tsssEscape;
        escapedState =
            tsssUnquoted; // Unquoted strings begin with an escaped character
        break;

      case ']':
        AddToken(RightBracket, pos);
        break;
      case '|':
        AddToken(Divider, pos);
        break;
    ...
    case tsssOneDouble:
      ...
      case 0x3:
        if (stream.IsEmpty())
          throw new Err(
              std::string(
                  "Sniffer pattern error: unterminated double-quoted string"),
              pos);
        else
          charStr += ch;
        break;
    ...
    case tsssZeroX:
      if (isHexChar(ch)) {
        ...
        throw new Err(std::string("Sniffer pattern error: incomplete hex code"),
                      pos);
      break;
    ...
    case tsssLonelyDecimalPoint:
      if (isDecimalChar(ch)) {
        ...
        throw new Err(
            std::string(
                "Sniffer pattern error: incomplete floating point number"),
            pos);
      break;
    ...
    case tsssLonelyMinusOrPlus:
      if (isDecimalChar(ch)) {
        ...
        throw new Err(std::string("Sniffer pattern error: incomplete signed "
                                  "number or invalid flag"),
                      pos);
      break;
    ...
    case tsssLonelyFloatExtension:
      if (ch == '+' || ch == '-') {
        ...
        throw new Err(std::string("Sniffer pattern error: incomplete "
                                  "extended-notation floating point number"),
                      pos);
      break;
    ...
    case tsssEscape:
      if (isOctalChar(ch)) {
        ...
        if (ch == 0x3 && stream.IsEmpty())
          throw new Err(
              std::string("Sniffer pattern error: incomplete escape sequence"),
              pos);
        else {
          charStr += escapeChar(ch);
          state =
              escapedState; // Return to the state we were in before the escape
        }
      }
      break;
    ...
    case tsssEscapeX:
      if (isHexChar(ch)) {
        ...
        throw new Err(
            std::string("Sniffer pattern error: incomplete escaped hex code"),
            pos);
      break;
...
status_t TokenStream::InitCheck() const { return fCStatus; }

//! Returns a pointer to the next token in the stream.
/*! The TokenStream object retains owner ship of the Token object returned by
   Get(). If Get() is called at the end of the stream, a pointer to a Err object
   is thrown.
*/
const Token *TokenStream::Get() {
  if (fCStatus != B_OK)
    throw new Err("Sniffer parser error: TokenStream::Get() called on "
                  "uninitialized TokenStream object",
                  -1);
  if (fPos < (ssize_t)fTokenList.size())
    return fTokenList[fPos++];
  else {
    throw new Err("Sniffer pattern error: unterminated rule", EndPos());
    //		fPos++;			// Increment fPos to keep Unget()s
    //consistent 		return NULL;	// Return NULL to signal end
    //of list
  }
}

//! Places token returned by the most recent call to Get() back on the head of
//! the stream.
/*! If Unget() is called at the beginning of the stream, a pointer to a Err
 * object is thrown.
*/
void TokenStream::Unget() {
  if (fCStatus != B_OK)
    throw new Err("Sniffer parser error: TokenStream::Unget() called on "
                  "uninitialized TokenStream object",
                  -1);
  if (fPos > 0)
    fPos--;
  else
    throw new Err("Sniffer parser error: TokenStream::Unget() called at "
                  "beginning of token stream",
                  -1);
}

/*! \brief Reads the next token in the stream and verifies it is of the given
   type, throwing a pointer to a Err object if it is not.
*/
void TokenStream::Read(TokenType type) {
  const Token *t = Get();
  if (t->Type() != type) {
    throw new Err((std::string("Sniffer pattern error: expected ") +
                   tokenTypeToString(type) + ", found " +
...
/*! CondRead() peeks at the next token in the stream. If it is of the given
   type, the token is removed from the stream and \c true is returned. If it is
   not of the given type, false is returned and the token remains at the head of
   the stream.
*/
...
ssize_t TokenStream::Pos() const {
  return fPos < (ssize_t)fTokenList.size() ? fTokenList[fPos]->Pos() : fStrLen;
}

ssize_t TokenStream::EndPos() const { return fStrLen; }
...
char hexToChar(char hex) {
  if ('0' <= hex && hex <= '9')
    ...
    throw new Err(std::string("Sniffer parser error: invalid hex digit '") +
                      hex + "' passed to hexToChar()",
                  -1);
}

char octalToChar(char octal) { return octalToChar('0', '0', octal); }

char octalToChar(char hi, char low) { return octalToChar('0', hi, low); }
...
char octalToChar(char hi, char mid, char low) {
  if (isOctalChar(hi) && isOctalChar(mid) && isOctalChar(low)) {
    ...
    if ((hi - '0') <= 3)
      ...
      throw new Err("Sniffer pattern error: invalid octal literal (octals must "
                    "be between octal 0 and octal 377 inclusive)",
                    -1);
  } else
    throw new Err(
        std::string(
            "Sniffer parser error: invalid octal digit passed to hexToChar()"),
        -1);
}
...
bool isOctalChar(char ch) { return ('0' <= ch && ch <= '7'); }

bool isDecimalChar(char ch) { return ('0' <= ch && ch <= '9'); }
...
Parser::Parser()
    : fOutOfMemErr(new (std::nothrow)
                       Err("Sniffer parser error: out of memory", -1)) {}

Parser::~Parser() { delete fOutOfMemErr; }
...
std::string Parser::ErrorMessage(Err *err, const char *rule) {
  const char *msg =
      (err && err->Msg()) ? err->Msg() : "Sniffer parser error: Unexpected "
                                         "error with no supplied error message";
...
void Parser::ParseRule(Rule *result) {
  if (!result)
    throw new Err(
        "Sniffer parser error: NULL Rule object passed to Parser::ParseRule()",
        -1);
...
double Parser::ParsePriority() {
  ...
  if (t->Type() == FloatingPoint || t->Type() == Integer) {
    ...
    throw new Err("Sniffer pattern error: match level expected",
                  t->Pos()); // Same as R5
}

std::vector<DisjList *> *Parser::ParseConjList() {
...
DisjList *Parser::ParseDisjList() {
  ...
  if (t1->Type() == LeftParen) {
    ...
    const Token *tokenOfInterest =
        (t2->Type() == CaseInsensitiveFlag) ? stream.Get() : t2;
    ...
    throw new Err("Sniffer pattern error: missing pattern",
                  t1->Pos()); // Same as R5
  }
...
DisjList *Parser::ParsePatternList(Range range) {
...
DisjList *Parser::ParseRPatternList() {
...
inline void Parser::ThrowOutOfMemError(ssize_t pos) {
  if (fOutOfMemErr)
...
void Parser::ThrowUnexpectedTokenError(TokenType expected, const Token *found) {
  throw new Err((std::string("Sniffer pattern error: expected ") +
                 tokenTypeToString(expected) + ", found " +
                 (found ? tokenTypeToString(found->Type()) : "NULL token"))
                    .c_str(),
                (found ? found->Pos() : stream.EndPos()));
}

void Parser::ThrowUnexpectedTokenError(TokenType expected1, TokenType expected2,
                                       const Token *found) {
  throw new Err((std::string("Sniffer pattern error: expected ") +
                 tokenTypeToString(expected1) + " or " +
                 tokenTypeToString(expected2) + ", found " +
                 (found ? tokenTypeToString(found->Type()) : "NULL token"))
                    .c_str(),
                (found ? found->Pos() : stream.EndPos()));
}
