...
BBufferedDataIO::BBufferedDataIO(BDataIO &stream, size_t bufferSize,
    ...
    : fStream(stream), fPosition(0), fSize(0), fDirty(false),
      fOwnsStream(ownsStream), fPartialReads(partialReads) {
...
BDataIO *BBufferedDataIO::Stream() const { return &fStream; }

size_t BBufferedDataIO::BufferSize() const { return fBufferSize; }

bool BBufferedDataIO::OwnsStream() const { return fOwnsStream; }
