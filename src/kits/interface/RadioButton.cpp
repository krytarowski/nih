...
BRadioButton::BRadioButton(const char *name, const char *label,
    ...
    : BControl(name, label, message, flags | B_FRAME_EVENTS), fOutlined(false) {
}
...
BRadioButton::BRadioButton(BMessage *data) : BControl(data), fOutlined(false) {}
...
void BRadioButton::AttachedToWindow() { BControl::AttachedToWindow(); }
...
void BRadioButton::GetPreferredSize(float *_width, float *_height) {
  font_height fontHeight;
  GetFontHeight(&fontHeight);
  ...
  if (const char *label = Label()) {
    width += be_control_look->DefaultLabelSpacing() + ceilf(StringWidth(label));
    height =
        std::max(height, ceilf(6.0f + fontHeight.ascent + fontHeight.descent));
  }
...
void BRadioButton::ResizeToPreferred() { BControl::ResizeToPreferred(); }
...
void BRadioButton::MouseMoved(BPoint where, uint32 code,
...
void BRadioButton::DetachedFromWindow() { BControl::DetachedFromWindow(); }
...
void BRadioButton::MakeFocus(bool focus) { BControl::MakeFocus(focus); }

void BRadioButton::AllAttached() { BControl::AllAttached(); }

void BRadioButton::AllDetached() { BControl::AllDetached(); }

status_t BRadioButton::GetSupportedSuites(BMessage *message) {
  return BControl::GetSupportedSuites(message);
}

status_t BRadioButton::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BRadioButton::MinSize();
    return B_OK;

  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BRadioButton::MaxSize();
    return B_OK;
...
BAlignment BRadioButton::LayoutAlignment() {
  return BLayoutUtils::ComposeAlignment(
      ExplicitAlignment(), BAlignment(B_ALIGN_LEFT, B_ALIGN_VERTICAL_UNSET));
}

status_t BRadioButton::SetIcon(const BBitmap *icon, uint32 flags) {
...
BRect BRadioButton::_KnobFrame() const {
  font_height fontHeight;
  GetFontHeight(&fontHeight);
  return _KnobFrame(fontHeight);
}

BRect BRadioButton::_KnobFrame(const font_height &fontHeight) const {
  // Same as BCheckBox...
  return BRect(0.0f, 2.0f, ceilf(3.0f + fontHeight.ascent),
               ceilf(5.0f + fontHeight.ascent));
}

void BRadioButton::_Redraw() {
  BRect bounds(Bounds());
