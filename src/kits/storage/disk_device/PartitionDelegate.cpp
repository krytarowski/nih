...
BPartition::Delegate::Delegate(BPartition *partition)
    : fPartition(partition), fMutablePartition(this), fDiskSystem(NULL),
...
// MutablePartition
BMutablePartition *BPartition::Delegate::MutablePartition() {
  return &fMutablePartition;
}

// MutablePartition
const BMutablePartition *BPartition::Delegate::MutablePartition() const {
  return &fMutablePartition;
}

// InitHierarchy
status_t
BPartition::Delegate::InitHierarchy(const user_partition_data *partitionData,
                                    Delegate *parent) {
...
// InitAfterHierarchy
status_t BPartition::Delegate::InitAfterHierarchy() {
  ...
  BDiskSystemAddOn *addOn = manager->GetAddOn(fMutablePartition.ContentType());
  if (!addOn) {
...
// PartitionData
const user_partition_data *BPartition::Delegate::PartitionData() const {
  return fMutablePartition.PartitionData();
}

// ChildAt
BPartition::Delegate *BPartition::Delegate::ChildAt(int32 index) const {
...
// IsModified
bool BPartition::Delegate::IsModified() const {
...
// Defragment
status_t BPartition::Delegate::Defragment() {
  // TODO: Implement!
  return B_BAD_VALUE;
}

// Repair
status_t BPartition::Delegate::Repair(bool checkOnly) {
...
// ValidateResize
status_t BPartition::Delegate::ValidateResize(off_t *size) const {
  if (!fPartitionHandle)
    return B_NO_INIT;
...
// ValidateResizeChild
status_t BPartition::Delegate::ValidateResizeChild(Delegate *child,
                                                   off_t *size) const {
  if (!fPartitionHandle || !child)
    return B_NO_INIT;

  return fPartitionHandle->ValidateResizeChild(&child->fMutablePartition, size);
}

// Resize
status_t BPartition::Delegate::Resize(off_t size) {
  if (!fPartitionHandle)
    return B_NO_INIT;
...
// ResizeChild
status_t BPartition::Delegate::ResizeChild(Delegate *child, off_t size) {
  if (!fPartitionHandle || !child)
    return B_NO_INIT;
...
// ValidateMove
status_t BPartition::Delegate::ValidateMove(off_t *offset) const {
  if (!fPartitionHandle)
    return B_NO_INIT;
...
// ValidateMoveChild
status_t BPartition::Delegate::ValidateMoveChild(Delegate *child,
                                                 off_t *offset) const {
  if (!fPartitionHandle || !child)
    return B_NO_INIT;

  return fPartitionHandle->ValidateMoveChild(&child->fMutablePartition, offset);
}

// Move
status_t BPartition::Delegate::Move(off_t offset) {
  if (!fPartitionHandle)
    return B_NO_INIT;
...
// MoveChild
status_t BPartition::Delegate::MoveChild(Delegate *child, off_t offset) {
  if (!fPartitionHandle || !child)
    return B_NO_INIT;
...
status_t BPartition::Delegate::ValidateSetContentName(BString *name) const {
  if (!fPartitionHandle)
    return B_NO_INIT;
...
// ValidateSetName
status_t BPartition::Delegate::ValidateSetName(Delegate *child,
                                               BString *name) const {
  if (!fPartitionHandle || !child)
    return B_NO_INIT;
...
// SetContentName
status_t BPartition::Delegate::SetContentName(const char *name) {
  if (!fPartitionHandle)
    return B_NO_INIT;
...
// SetName
status_t BPartition::Delegate::SetName(Delegate *child, const char *name) {
  if (!fPartitionHandle || !child)
    return B_NO_INIT;
...
// ValidateSetType
status_t BPartition::Delegate::ValidateSetType(Delegate *child,
...
// SetType
status_t BPartition::Delegate::SetType(Delegate *child, const char *type) {
  if (!fPartitionHandle || !child)
    return B_NO_INIT;
...
status_t BPartition::Delegate::SetContentParameters(const char *parameters) {
  if (!fPartitionHandle)
    return B_NO_INIT;
...
// SetParameters
status_t BPartition::Delegate::SetParameters(Delegate *child,
...
// IsSubSystem
bool BPartition::Delegate::IsSubSystem(Delegate *child,
...
// CanInitialize
bool BPartition::Delegate::CanInitialize(const char *diskSystem) const {
  // get the disk system add-on
  DiskSystemAddOnManager *manager = DiskSystemAddOnManager::Default();
  BDiskSystemAddOn *addOn = manager->GetAddOn(diskSystem);
  if (!addOn)
...
// ValidateInitialize
status_t BPartition::Delegate::ValidateInitialize(const char *diskSystem,
                                                  BString *name,
                                                  const char *parameters) {
  // get the disk system add-on
  DiskSystemAddOnManager *manager = DiskSystemAddOnManager::Default();
  ...
  status_t result =
      addOn->ValidateInitialize(&fMutablePartition, name, parameters);
...
// Initialize
status_t BPartition::Delegate::Initialize(const char *diskSystem,
                                          const char *name,
                                          const char *parameters) {
  // get the disk system add-on
  DiskSystemAddOnManager *manager = DiskSystemAddOnManager::Default();
  ...
  BPartitionHandle *handle;
  status_t result =
      addOn->Initialize(&fMutablePartition, name, parameters, &handle);
...
// Uninitialize
status_t BPartition::Delegate::Uninitialize() {
  if (fPartitionHandle) {
    _FreeHandle();
...
// GetPartitioningInfo
status_t BPartition::Delegate::GetPartitioningInfo(BPartitioningInfo *info) {
  if (!fPartitionHandle)
    return B_NO_INIT;
...
// GetParameterEditor
status_t BPartition::Delegate::GetParameterEditor(
    B_PARAMETER_EDITOR_TYPE type, BPartitionParameterEditor **editor) const {
  if (!fPartitionHandle)
    return B_NO_INIT;
...
// CreateChild
status_t BPartition::Delegate::CreateChild(off_t start, off_t size,
...
// DeleteChild
status_t BPartition::Delegate::DeleteChild(Delegate *child) {
  if (!fPartitionHandle || !child)
    return B_NO_INIT;
...
// _FreeHandle
void BPartition::Delegate::_FreeHandle() {
  if (fPartitionHandle) {
