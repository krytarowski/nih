...
static property_info sWindowPropInfo[] = {
    ...
    {"MenuBar", {}, {B_DIRECT_SPECIFIER}, NULL, 0, {}},
...
BWindow::unpack_cookie::unpack_cookie()
    : message((BMessage *)~0UL),
      ...
      index(0), focus_token(B_NULL_TOKEN), last_view_token(B_NULL_TOKEN),
      found_focus(false), tokens_scanned(false) {}

//	#pragma mark - BWindow::Shortcut

BWindow::Shortcut::Shortcut(uint32 key, uint32 modifiers, BMenuItem *item)
    : fKey(PrepareKey(key)), fModifiers(PrepareModifiers(modifiers)),
      fMenuItem(item), fMessage(NULL), fTarget(NULL) {}

BWindow::Shortcut::Shortcut(uint32 key, uint32 modifiers, BMessage *message,
                            BHandler *target)
    : fKey(PrepareKey(key)), fModifiers(PrepareModifiers(modifiers)),
      fMenuItem(NULL), fMessage(message), fTarget(target) {}
...
bool BWindow::Shortcut::Matches(uint32 key, uint32 modifiers) const {
...
/*static*/
uint32 BWindow::Shortcut::PrepareKey(uint32 key) {
...
BWindow::BWindow(BMessage *data) : BLooper(data) {
  ...
  if (data->FindFloat("_sizel", 0, &fMinWidth) == B_OK &&
    ...
    SetSizeLimits(fMinWidth, fMaxWidth, fMinHeight, fMaxHeight);
...
BWindow::BWindow(BRect frame, int32 bitmapToken) : BLooper("offscreen bitmap") {
...
status_t BWindow::Archive(BMessage *data, bool deep) const {
  ...
  if (fMinWidth != 0.0 || fMinHeight != 0.0 || fMaxWidth != 32768.0 ||
      fMaxHeight != 32768.0) {
    if (ret == B_OK)
...
void BWindow::DispatchMessage(BMessage *message, BHandler *target) {
  if (message == NULL)
    return;
  ...
  case B_HIDE_APPLICATION: {
    ...
    for (int32 i = 0; i < list.CountItems(); i++) {
      do_minimize_team(BRect(), (team_id)(addr_t)list.ItemAt(i), false);
    }
    break;
  }
  ...
  case B_WINDOW_RESIZED: {
    ...
    if (message->FindInt32("width", &width) == B_OK &&
      ...
      while (
          (pendingMessage = MessageQueue()->FindMessage(B_WINDOW_RESIZED, 0))) {
        int32 nextWidth;
  ...
  case B_WINDOW_ACTIVATED:
    ...
    while (true) {
      BMessage *pendingMessage =
          MessageQueue()->FindMessage(B_WINDOW_ACTIVATED, 0);
    ...
    if (active != fActive) {
      ...
      BMessage message(inputMethodAware ? IS_FOCUS_IM_AWARE_VIEW
  ...
  case B_KEY_DOWN: {
    if (!_HandleKeyDown(message)) {
      if (BView *view = dynamic_cast<BView *>(target)) {
        ...
        const char *string;
        ssize_t bytes;
        if (message->FindData("bytes", B_STRING_TYPE, (const void **)&string,
                              &bytes) == B_OK) {
  ...
  case B_MOUSE_MOVED: {
    if (BView *view = dynamic_cast<BView *>(target)) {
      uint32 eventOptions = view->fEventOptions | view->fMouseEventOptions;
      ...
      if (noHistory || (dropIfLate && (system_time() - eventTime > 20000))) {
    ...
    if (fLink->FlushWithReply(code) == B_OK && code == B_OK) {
      ...
      for (int32 i = 0; i < count; i++) {
        ...
        if (BView *view = _FindView(info->token))
          ...
          printf("_UPDATE_ - didn't find view by token: %" B_PRId32 "\n",
                 info->token);
        }
...
void BWindow::SetSizeLimits(float minWidth, float maxWidth, float minHeight,
                            float maxHeight) {
...
void BWindow::GetSizeLimits(float *_minWidth, float *_maxWidth,
...
void BWindow::UpdateSizeLimits() {
  ...
  if ((fFlags & B_AUTO_UPDATE_SIZE_LIMITS) != 0) {
    ...
    SetSizeLimits(minSize.width, maxSize.width, minSize.height, maxSize.height);
  }
}
...
void BWindow::Zoom() {
  ...
  BPoint zoomedLeftTop =
      screen.Frame().LeftTop() + BPoint(borderWidth, tabHeight + borderWidth);
...
void BWindow::SetPulseRate(bigtime_t rate) {
  ...
  if (rate > 0) {
    if (fPulseRunner == NULL) {
      ...
      fPulseRunner =
          new (std::nothrow) BMessageRunner(BMessenger(this), &message, rate);
    } else {
...
bigtime_t BWindow::PulseRate() const { return fPulseRate; }
...
void BWindow::AddShortcut(uint32 key, uint32 modifiers, BMessage *message,
  ...
  Shortcut *shortcut =
      new (std::nothrow) Shortcut(key, modifiers, message, target);
  if (shortcut == NULL)
    return;
...
BView *BWindow::CurrentFocus() const { return fFocus; }
...
BRect BWindow::Frame() const { return fFrame; }
...
BSize BWindow::Size() const { return BSize(fFrame.Width(), fFrame.Height()); }

const char *BWindow::Title() const { return fTitle; }
...
bool BWindow::IsActive() const { return fActive; }

void BWindow::SetKeyMenuBar(BMenuBar *bar) { fKeyMenuBar = bar; }

BMenuBar *BWindow::KeyMenuBar() const { return fKeyMenuBar; }

bool BWindow::IsModal() const {
  return fFeel == B_MODAL_SUBSET_WINDOW_FEEL ||
         fFeel == B_MODAL_APP_WINDOW_FEEL || fFeel == B_MODAL_ALL_WINDOW_FEEL ||
...
status_t BWindow::Perform(perform_code code, void *_data) {
  switch (code) {
...
window_type BWindow::Type() const { return _ComposeType(fLook, fFeel); }
...
window_look BWindow::Look() const { return fLook; }
...
window_feel BWindow::Feel() const { return fFeel; }
...
uint32 BWindow::Flags() const { return fFlags; }

status_t BWindow::SetWindowAlignment(window_alignment mode, int32 h,
                                     int32 hOffset, int32 width,
                                     int32 widthOffset, int32 v, int32 vOffset,
                                     int32 height, int32 heightOffset) {
  if ((mode & (B_BYTE_ALIGNMENT | B_PIXEL_ALIGNMENT)) == 0 ||
      (hOffset >= 0 && hOffset <= h) || (vOffset >= 0 && vOffset <= v) ||
...
status_t BWindow::GetWindowAlignment(window_alignment *mode, int32 *h,
                                     int32 *hOffset, int32 *width,
                                     int32 *widthOffset, int32 *v,
                                     int32 *vOffset, int32 *height,
...
BView *BWindow::LastMouseMovedView() const { return fLastMouseMovedView; }
...
void BWindow::MoveTo(BPoint point) { MoveTo(point.x, point.y); }
...
void BWindow::CenterIn(const BRect &rect) {
  ...
  MoveTo(BLayoutUtils::AlignInFrame(
             rect, Size(),
             BAlignment(B_ALIGN_HORIZONTAL_CENTER, B_ALIGN_VERTICAL_CENTER))
             .LeftTop());
}

void BWindow::CenterOnScreen() { CenterIn(BScreen(this).Frame()); }

// Centers the window on the screen with the passed in id.
void BWindow::CenterOnScreen(screen_id id) { CenterIn(BScreen(id).Frame()); }
...
bool BWindow::IsHidden() const { return fShowLevel > 0; }

bool BWindow::QuitRequested() { return BLooper::QuitRequested(); }

thread_id BWindow::Run() { return BLooper::Run(); }
...
BLayout *BWindow::GetLayout() const { return fTopView->GetLayout(); }
...
bool BWindow::IsOffscreenWindow() const { return fOffscreen; }
...
BHandler *BWindow::ResolveSpecifier(BMessage *message, int32 index,
...
void BWindow::_InitData(BRect frame, const char *title, window_look look,
                        window_feel feel, uint32 flags, uint32 workspace,
                        int32 bitmapToken) {
  ...
  AddShortcut('M', B_COMMAND_KEY | B_CONTROL_KEY, new BMessage(_MINIMIZE_),
              NULL);
  ...
  AddShortcut('F', B_COMMAND_KEY | B_CONTROL_KEY, new BMessage(_SEND_TO_FRONT_),
              NULL);
  AddShortcut('B', B_COMMAND_KEY | B_CONTROL_KEY, new BMessage(_SEND_BEHIND_),
              NULL);
  ...
  port_id receivePort =
      create_port(B_LOOPER_PORT_DEFAULT_CAPACITY, "w<app_server");
  if (receivePort < B_OK) {
    // TODO: huh?
...
void BWindow::_SetName(const char *title) {
  if (title == NULL)
    title = "";
...
//!	Reads all pending messages from the window port and put them into the
//!queue.
void BWindow::_DequeueAll() {
...
/*!	This here is an almost complete code duplication to
   BLooper::task_looper() but with some important differences:
         a)	it uses the _DetermineTarget() method to tell what the later
   target of a message will be, if no explicit target is supplied. b)	it
   calls _UnpackMessage() and _SanitizeMessage() to duplicate the message to all
   of its intended targets, and to add all fields the target would expect in
   such a message.
    ...
    while (!fTerminating && dispatchNextMessage) {
      ...
      if (fLastMessage == NULL) {
        ...
        if (usePreferred) {
          ...
          gDefaultTokens.GetToken(messagePrivate.GetTarget(), B_HANDLER_TOKEN,
                                  (void **)&handler);
...
window_type BWindow::_ComposeType(window_look look, window_feel feel) const {
  switch (feel) {
...
void BWindow::_DecomposeType(window_type type, window_look *_look,
...
void BWindow::_SetFocus(BView *focusView, bool notifyInputServer) {
  if (fFocus == focusView)
    return;
  ...
  if (notifyInputServer && fActive) {
    ...
    BMessage msg(inputMethodAware ? IS_FOCUS_IM_AWARE_VIEW
                                  : IS_UNFOCUS_IM_AWARE_VIEW);
...
/*!
        \brief Determines the target of a message received for the
                focus view.
*/
BHandler *BWindow::_DetermineTarget(BMessage *message, BHandler *target) {
  if (target == NULL)
    target = this;
  ...
  case B_KEY_UP: {
    ...
    if (DefaultButton() != NULL &&
        message->FindInt32("raw_char", &rawChar) == B_OK && rawChar == B_ENTER)
...
/*!	\brief Determines whether or not this message has targeted the focus
   view.

        This will return \c false only if the message did not go to the
   preferred handler, or if the packed message does not contain address the
   focus view at all.
*/
bool BWindow::_IsFocusMessage(BMessage *message) {
...
*/
bool BWindow::_UnpackMessage(unpack_cookie &cookie, BMessage **_message,
  ...
  // check if the focus token is still valid (could have been removed in the
  // mean time)
  BHandler *handler;
  if (gDefaultTokens.GetToken(cookie.focus_token, B_HANDLER_TOKEN,
                              (void **)&handler) != B_OK ||
      handler->Looper() != this)
...
/*!	Some messages don't get to the window in a shape an application should
   see. This method is supposed to give a message the last grinding before it's
   acceptable for the receiving application.
*/
void BWindow::_SanitizeMessage(BMessage *message, BHandler *target,
                               bool usePreferred) {
  if (target == NULL)
    return;
...
*/
bool BWindow::_StealMouseMessage(BMessage *message, bool &deleteMessage) {
  BMessage::Private messagePrivate(message);
  if (!messagePrivate.UsePreferredTarget()) {
  ...
  if (message->FindInt32("_token", 0, &token) == B_OK) {
    ...
    if (message->what == B_MOUSE_MOVED) {
      ...
      uint32 transit =
          _TransitForMouseMoved(fLastMouseMovedView, viewUnderMouse);
      if (transit == B_ENTERED_VIEW || transit == B_EXITED_VIEW)
        deleteMessage = false;
    }
...
uint32 BWindow::_TransitForMouseMoved(BView *view,
...
*/
void BWindow::_Switcher(int32 rawKey, uint32 modifiers, bool repeat) {
...
/*!	Handles keyboard input before it gets forwarded to the target handler.
        ...
        TODO: must also convert the incoming key to the font encoding of the
   target
*/
bool BWindow::_HandleKeyDown(BMessage *event) {
...
bool BWindow::_HandleUnmappedKeyDown(BMessage *event) {
  // Only handle special functions when the event targeted the active focus
  // view
  if (!_IsFocusMessage(event))
    return false;
...
void BWindow::_KeyboardNavigation() {
  ...
  BView *nextFocus;
  int32 jumpGroups =
      (modifiers & B_OPTION_KEY) != 0 ? B_NAVIGABLE_JUMP : B_NAVIGABLE;
...
/*!
        \brief Return the position of the window centered horizontally to the
   passed in \a frame and vertically 3/4 from the top of \a frame.
...
*/
BPoint BWindow::AlertPosition(const BRect &frame) {
...
BView *BWindow::_FindView(int32 token) {
  BHandler *handler;
  if (gDefaultTokens.GetToken(token, B_HANDLER_TOKEN, (void **)&handler) !=
      B_OK) {
    return NULL;
  }
...
BView *BWindow::_FindView(BView *view, BPoint point) const {
...
BView *BWindow::_FindNextNavigable(BView *focus, uint32 flags) {
  if (focus == NULL)
    focus = fTopView;
...
BView *BWindow::_FindPreviousNavigable(BView *focus, uint32 flags) {
  if (focus == NULL)
    focus = fTopView;
...
*/
BView *BWindow::_LastViewChild(BView *parent) {
  while (true) {
...
void BWindow::SetIsFilePanel(bool isFilePanel) { fIsFilePanel = isFilePanel; }

bool BWindow::IsFilePanel() const { return fIsFilePanel; }

void BWindow::_GetDecoratorSize(float *_borderWidth, float *_tabHeight) const {
...
void BWindow::_SendShowOrHideMessage() {
...
extern "C" void _ReservedWindow1__7BWindow(BWindow *window, BLayout *layout) {
  // SetLayout()
