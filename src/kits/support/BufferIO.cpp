...
BBufferIO::BBufferIO(BPositionIO *stream, size_t bufferSize, bool ownsStream)
    : fBufferStart(0), fStream(stream), fBuffer(NULL), fBufferUsed(0),
      fBufferIsDirty(false), fOwnsStream(ownsStream)
...
ssize_t BBufferIO::ReadAt(off_t pos, void *buffer, size_t size) {
  ...
  if (size > fBufferUsed || pos < fBufferStart ||
...
ssize_t BBufferIO::WriteAt(off_t pos, const void *buffer, size_t size) {
  ...
  if (pos > fBufferStart + (off_t)fBufferUsed || pos < fBufferStart) {
    ssize_t read;
    off_t where = pos;
...
off_t BBufferIO::Position() const { return fPosition; }
...
BPositionIO *BBufferIO::Stream() const { return fStream; }

size_t BBufferIO::BufferSize() const { return fBufferSize; }

bool BBufferIO::OwnsStream() const { return fOwnsStream; }

void BBufferIO::SetOwnsStream(bool ownsStream) { fOwnsStream = ownsStream; }
