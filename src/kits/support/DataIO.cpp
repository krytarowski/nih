...
ssize_t BDataIO::Read(void *buffer, size_t size) { return B_NOT_SUPPORTED; }

ssize_t BDataIO::Write(const void *buffer, size_t size) {
  return B_NOT_SUPPORTED;
}

status_t BDataIO::Flush() { return B_OK; }

status_t BDataIO::ReadExactly(void *buffer, size_t size, size_t *_bytesRead) {
...
status_t BDataIO::WriteExactly(const void *buffer, size_t size,
...
BDataIO &BDataIO::operator=(const BDataIO &) {
  // Copying not allowed
  return *this;
}
...
extern "C" status_t _ZN7BDataIO16_ReservedDataIO1Ev(BDataIO *self) {
...
status_t BPositionIO::ReadAtExactly(off_t position, void *buffer, size_t size,
                                    size_t *_bytesRead) {
  uint8 *out = (uint8 *)buffer;
  size_t bytesRemaining = size;
  status_t error = B_OK;
...
status_t BPositionIO::WriteAtExactly(off_t position, const void *buffer,
...
status_t BPositionIO::SetSize(off_t size) { return B_ERROR; }
...
BMemoryIO::BMemoryIO(void *buffer, size_t length)
    : fReadOnly(false), fBuffer(static_cast<char *>(buffer)), fLength(length),
      fBufferSize(length), fPosition(0) {}

BMemoryIO::BMemoryIO(const void *buffer, size_t length)
    : fReadOnly(true),
      fBuffer(const_cast<char *>(static_cast<const char *>(buffer))),
      fLength(length), fBufferSize(length), fPosition(0) {}
...
off_t BMemoryIO::Position() const { return fPosition; }
...
BMemoryIO &BMemoryIO::operator=(const BMemoryIO &) {
  // Copying not allowed
  return *this;
}
...
BMallocIO::BMallocIO()
    : fBlockSize(256), fMallocSize(0), fLength(0), fData(NULL), fPosition(0) {}

BMallocIO::~BMallocIO() { free(fData); }
...
off_t BMallocIO::Position() const { return fPosition; }
...
const void *BMallocIO::Buffer() const { return fData; }

size_t BMallocIO::BufferLength() const { return fLength; }
...
BMallocIO &BMallocIO::operator=(const BMallocIO &) {
  // copying not allowed...
  return *this;
}
