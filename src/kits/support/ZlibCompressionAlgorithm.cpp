...
static size_t sanitize_buffer_size(size_t size) {
...
BZlibCompressionParameters::BZlibCompressionParameters(int compressionLevel)
    : BCompressionParameters(), fCompressionLevel(compressionLevel),
      fBufferSize(kDefaultBufferSize), fGzipFormat(false) {}
...
size_t BZlibCompressionParameters::BufferSize() const { return fBufferSize; }

void BZlibCompressionParameters::SetBufferSize(size_t size) {
  fBufferSize = sanitize_buffer_size(size);
}

bool BZlibCompressionParameters::IsGzipFormat() const { return fGzipFormat; }
...
BZlibDecompressionParameters::BZlibDecompressionParameters()
    : BDecompressionParameters(), fBufferSize(kDefaultBufferSize) {}

BZlibDecompressionParameters::~BZlibDecompressionParameters() {}

size_t BZlibDecompressionParameters::BufferSize() const { return fBufferSize; }
...
struct BZlibCompressionAlgorithm::CompressionStrategy {
  ...
  static int Init(z_stream &stream,
    ...
    return deflateInit2(&stream, compressionLevel, Z_DEFLATED,
                        MAX_WBITS + (gzipFormat ? 16 : 0), MAX_MEM_LEVEL,
                        Z_DEFAULT_STRATEGY);
  }

  static void Uninit(z_stream &stream) { deflateEnd(&stream); }
...
struct BZlibCompressionAlgorithm::DecompressionStrategy {
  ...
  static void Uninit(z_stream &stream) { inflateEnd(&stream); }
...
struct BZlibCompressionAlgorithm::Stream : BaseClass {
  Stream(BDataIO *io) : BaseClass(io), fStreamInitialized(false) {}
  ...
  virtual status_t ProcessData(const void *input, size_t inputSize,
                               ...
                               size_t &bytesConsumed, size_t &bytesProduced) {
    return _ProcessData(input, inputSize, output, outputSize, bytesConsumed,
                        bytesProduced, false);
  }
  ...
  virtual status_t FlushPendingData(void *output, size_t outputSize,
    ...
    return _ProcessData(NULL, 0, output, outputSize, bytesConsumed,
                        bytesProduced, true);
  }
...
private:
  status_t _ProcessData(const void *input, size_t inputSize, void *output,
                        size_t outputSize, size_t &bytesConsumed,
...
status_t BZlibCompressionAlgorithm::CreateCompressingInputStream(
    BDataIO *input, const BCompressionParameters *parameters,
    BDataIO *&_stream) {
#ifdef B_ZLIB_COMPRESSION_SUPPORT
...
status_t BZlibCompressionAlgorithm::CreateCompressingOutputStream(
    BDataIO *output, const BCompressionParameters *parameters,
    BDataIO *&_stream) {
#ifdef B_ZLIB_COMPRESSION_SUPPORT
...
status_t BZlibCompressionAlgorithm::CreateDecompressingInputStream(
    BDataIO *input, const BDecompressionParameters *parameters,
    BDataIO *&_stream) {
  return Stream<BAbstractInputStream, DecompressionStrategy>::Create(
      input, parameters, _stream);
}

status_t BZlibCompressionAlgorithm::CreateDecompressingOutputStream(
    BDataIO *output, const BDecompressionParameters *parameters,
    BDataIO *&_stream) {
  return Stream<BAbstractOutputStream, DecompressionStrategy>::Create(
      output, parameters, _stream);
}

status_t BZlibCompressionAlgorithm::CompressBuffer(
    const void *input, size_t inputSize, void *output, size_t outputSize,
    size_t &_compressedSize, const BCompressionParameters *parameters) {
#ifdef B_ZLIB_COMPRESSION_SUPPORT
...
status_t BZlibCompressionAlgorithm::DecompressBuffer(
    const void *input, size_t inputSize, void *output, size_t outputSize,
