...
static property_info sProperties[] = {
    ...
    {"Selection",
     ...
     "\"data\" chooses selection or deselection.",
     0,
     {B_BOOL_TYPE}},
...
BListView::BListView(BMessage *archive) : BView(archive) {
  int32 listType;
...
void BListView::DetachedFromWindow() { BView::DetachedFromWindow(); }

void BListView::AllAttached() { BView::AllAttached(); }

void BListView::AllDetached() { BView::AllDetached(); }

void BListView::FrameResized(float newWidth, float newHeight) {
  _FixupScrollBar();
...
void BListView::WindowActivated(bool active) { BView::WindowActivated(active); }
...
void BListView::MessageReceived(BMessage *message) {
  ...
  case B_SET_PROPERTY: {
    ...
    switch (
        propInfo.FindMatch(message, 0, &specifier, message->what, property)) {
    case B_ERROR:
      BView::MessageReceived(message);
      break;
...
void BListView::KeyDown(const char *bytes, int32 numBytes) {
  ...
  case B_UP_ARROW: {
    if (fAnchorIndex > 0) {
      if (!extend || fAnchorIndex <= fFirstSelected) {
        ...
        do
          fAnchorIndex--;
        while (fAnchorIndex > 0 && !ItemAt(fAnchorIndex)->IsEnabled());
      }
    }
  ...
  case B_DOWN_ARROW: {
    ...
    if (fAnchorIndex < lastItem) {
      if (!extend || fAnchorIndex >= fLastSelected) {
        ...
        do
          fAnchorIndex++;
        while (fAnchorIndex < lastItem && !ItemAt(fAnchorIndex)->IsEnabled());
      }
    }
  ...
  case B_PAGE_DOWN: {
    ...
    if (BListItem *item = LastItem()) {
      ...
      scrollOffset.y =
          std::min(item->Bottom() - Bounds().Height(), scrollOffset.y);
    }
    ScrollTo(scrollOffset);
    break;
  }
...
void BListView::MouseDown(BPoint where) {
  ...
  if (timeDelta < doubleClickSpeed && fabs(delta.x) < kDoubleClickThreshold &&
      fabs(delta.y) < kDoubleClickThreshold && fTrack->item_index == index) {
    doubleClick = true;
  }
  ...
  if (!doubleClick) {
    ...
    MouseDownThread<BListView>::TrackMouse(this, &BListView::_DoneTracking,
                                           &BListView::_Track);
  }
...
void BListView::MouseUp(BPoint where) { BView::MouseUp(where); }

void BListView::MouseMoved(BPoint where, uint32 code,
...
void BListView::ResizeToPreferred() { BView::ResizeToPreferred(); }

void BListView::GetPreferredSize(float *_width, float *_height) {
  int32 count = CountItems();
...
BSize BListView::MaxSize() { return BView::MaxSize(); }
...
void BListView::SetFont(const BFont *font, uint32 mask) {
...
void BListView::ScrollTo(BPoint point) { BView::ScrollTo(point); }
...
bool BListView::AddList(BList *list) { return AddList(list, CountItems()); }
...
BMessage *BListView::InvocationMessage() const { return BInvoker::Message(); }

uint32 BListView::InvocationCommand() const { return BInvoker::Command(); }

BMessage *BListView::SelectionMessage() const { return fSelectMessage; }
...
list_view_type BListView::ListType() const { return fListType; }
...
BListItem *BListView::LastItem() const { return (BListItem *)fList.LastItem(); }

bool BListView::HasItem(BListItem *item) const { return IndexOf(item) != -1; }

int32 BListView::CountItems() const { return fList.CountItems(); }
...
bool BListView::IsEmpty() const { return fList.IsEmpty(); }
...
void BListView::InvalidateItem(int32 index) { Invalidate(ItemFrame(index)); }
...
void BListView::ScrollToSelection() {
  ...
  float scrollPos = itemFrame.top < Bounds().top
                        ? itemFrame.top
                        : itemFrame.bottom - Bounds().Height();
...
BHandler *BListView::ResolveSpecifier(BMessage *message, int32 index,
...
status_t BListView::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BListView::MinSize();
    return B_OK;
  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BListView::MaxSize();
    return B_OK;
...
BListView &BListView::operator=(const BListView & /*other*/) { return *this; }

// #pragma mark -

void BListView::_InitObject(list_view_type type) {
  fListType = type;
  fFirstSelected = -1;
  fLastSelected = -1;
  fAnchorIndex = -1;
...
void BListView::_FixupScrollBar() {
...
void BListView::_InvalidateFrom(int32 index) {
...
int32 BListView::_CalcFirstSelected(int32 after) {
  if (after >= CountItems())
    return -1;
...
bool BListView::_SwapItems(int32 a, int32 b) {
...
bool BListView::_MoveItem(int32 from, int32 to) {
...
bool BListView::_ReplaceItem(int32 index, BListItem *item) {
  if (item == NULL)
    return false;
...
void BListView::_RescanSelection(int32 from, int32 to) {
  if (from > to) {
    int32 tmp = from;
    from = to;
    to = tmp;
...
void BListView::_RecalcItemTops(int32 start, int32 end) {
  int32 count = CountItems();
...
void BListView::_DoneTracking(BPoint where) {
  fTrack->try_drag = false;
  fTrack->is_dragging = false;
}

void BListView::_Track(BPoint where, uint32) {
  if (fTrack->item_index >= 0 && fTrack->try_drag) {
    ...
    if (dragDistance >= 5.0f) {
      fTrack->try_drag = false;
      fTrack->is_dragging = InitiateDrag(fTrack->drag_start, fTrack->item_index,
                                         fTrack->was_selected);
    }
  }
