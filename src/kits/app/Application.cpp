...
static property_info sPropertyInfo[] = {
    {"Window",
     ...
     {},
     {}},
    {"Window", {}, {B_NAME_SPECIFIER}, NULL, kWindowByName, {}, {}, {}},
    {"Looper",
     {},
     {B_INDEX_SPECIFIER, B_REVERSE_INDEX_SPECIFIER},
     NULL,
     ...
     {},
     {}},
    {"Looper", {}, {B_ID_SPECIFIER}, NULL, kLooperByID, {}, {}, {}},
    {"Looper", {}, {B_NAME_SPECIFIER}, NULL, kLooperByName, {}, {}, {}},
    {"Name",
     {B_GET_PROPERTY},
     {B_DIRECT_SPECIFIER},
     NULL,
...
/*!
        \brief Checks whether the supplied string is a valid application
   signature.
...
BApplication &BApplication::operator=(const BApplication &rhs) { return *this; }

void BApplication::_InitData(const char *signature, bool initGUI,
                             status_t *_error) {
...
#ifndef RUN_WITHOUT_REGISTRAR
  bool registerApp =
      signature == NULL || (strcasecmp(signature, B_REGISTRAR_SIGNATURE) != 0 &&
  ...
  if (preRegistered) {
    ...
    fInitError =
        BRoster::Private().CompleteRegistration(team, thread, appInfo.port);
  } else if (fInitError == B_OK) {
    // not pre-registered -- try to register the application
    team_id otherTeam = -1;
    if (registerApp) {
      fInitError = BRoster::Private().AddApplication(signature, &ref, appFlags,
                                                     team, thread, fMsgPort,
                                                     true, NULL, &otherTeam);
      if (fInitError != B_OK) {
...
port_id BApplication::_GetPort(const char *signature) {
...
status_t BApplication::InitCheck() const { return fInitError; }
...
bool BApplication::QuitRequested() { return _QuitAllWindows(false); }
...
BHandler *BApplication::ResolveSpecifier(BMessage *message, int32 index,
  ...
  if (propInfo.FindMatch(message, 0, specifier, what, property, &data) >= 0) {
    ...
    case kLooperByName: {
      ...
      for (int32 i = 0;; i++) {
        ...
        if (looper->Name() != NULL && strcmp(looper->Name(), name) == 0) {
          message->PopSpecifier();
          BMessenger(looper).SendMessage(message);
          break;
        }
...
bool BApplication::IsLaunching() const { return !fReadyToRunCalled; }

const char *BApplication::Signature() const { return fAppName; }
...
bool BApplication::ScriptReceived(BMessage *message, int32 index,
                                  BMessage *specifier, int32 what,
                                  const char *property) {
...
void BApplication::BeginRectTracking(BRect rect, bool trackWhole) {
  BPrivate::AppServerLink link;
...
status_t BApplication::_SetupServerAllocator() {
...
status_t BApplication::_InitGUIContext() {
  // An app_server connection is necessary for a lot of stuff, so get that
  // first.
...
status_t BApplication::_ConnectToServer() {
  status_t status = create_desktop_connection(fServerLink, "a<app_server", 100);
  if (status != B_OK)
    return status;
...
void BApplication::_ReconnectToServer() {
  delete_port(fServerLink->SenderPort());
  delete_port(fServerLink->ReceiverPort());
...
bool BApplication::_WindowQuitLoop(bool quitFilePanels, bool force) {
  int32 index = 0;
  while (true) {
    BWindow *window = WindowAt(index);
    if (window == NULL)
...
bool BApplication::_QuitAllWindows(bool force) {
  AssertLocked();
...
uint32 BApplication::InitialWorkspace() { return fInitialWorkspace; }

int32 BApplication::_CountWindows(bool includeMenus) const {
  uint32 count = 0;
...
BWindow *BApplication::_WindowAt(uint32 index, bool includeMenus) const {
  AutoLocker<BLooperList> listLock(gLooperList);
  if (!listLock.IsLocked())
    return NULL;
...
/*static*/ void BApplication::_InitAppResources() {
  entry_ref ref;
  bool found = false;
