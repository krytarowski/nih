...
struct MimeSnifferAddonManager::AddonReference {
  AddonReference(BMimeSnifferAddon *addon)
      : fAddon(addon), fReferenceCount(1) {}

  ~AddonReference() { delete fAddon; }

  BMimeSnifferAddon *Addon() const { return fAddon; }

  void GetReference() { atomic_add(&fReferenceCount, 1); }
...
MimeSnifferAddonManager::MimeSnifferAddonManager()
    : fLock("mime sniffer manager"), fAddons(20), fMinimalBufferSize(0) {}

// destructor
MimeSnifferAddonManager::~MimeSnifferAddonManager() {}

// Default
MimeSnifferAddonManager *MimeSnifferAddonManager::Default() { return sManager; }
...
// MinimalBufferSize
size_t MimeSnifferAddonManager::MinimalBufferSize() {
  return fMinimalBufferSize;
}

// GuessMimeType
float MimeSnifferAddonManager::GuessMimeType(const char *fileName,
                                             BMimeType *type) {
  // get addons
  ...
  for (int32 i = 0; i < count; i++) {
    ...
    float priority = addons[i]->Addon()->GuessMimeType(fileName, &currentType);
...
// GuessMimeType
float MimeSnifferAddonManager::GuessMimeType(BFile *file, const void *buffer,
  ...
  for (int32 i = 0; i < count; i++) {
    BMimeType currentType;
    float priority =
        addons[i]->Addon()->GuessMimeType(file, buffer, length, &currentType);
    if (priority > bestPriority) {
      type->SetTo(currentType.Type());
      bestPriority = priority;
    }
...
// _GetAddons
status_t MimeSnifferAddonManager::_GetAddons(AddonReference **&references,
                                             int32 &count) {
  BAutolock locker(fLock);
  if (!locker.IsLocked())
    return B_ERROR;
...
// _PutAddons
void MimeSnifferAddonManager::_PutAddons(AddonReference **references,
                                         int32 count) {
  for (int32 i = 0; i < count; i++)
