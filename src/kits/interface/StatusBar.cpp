...
enum internalFlags { kCustomBarColor = 1 };
...
BStatusBar::BStatusBar(BRect frame, const char *name, const char *label,
    ...
    : BView(frame, name, B_FOLLOW_LEFT | B_FOLLOW_TOP, B_WILL_DRAW),
      fLabel(label), fTrailingLabel(trailingLabel) {
  _InitObject();
}
...
BStatusBar::BStatusBar(BMessage *archive) : BView(archive) {
  _InitObject();
...
void BStatusBar::DetachedFromWindow() { BView::DetachedFromWindow(); }

void BStatusBar::AllAttached() { BView::AllAttached(); }

void BStatusBar::AllDetached() { BView::AllDetached(); }

// #pragma mark -

void BStatusBar::WindowActivated(bool state) { BView::WindowActivated(state); }

void BStatusBar::MakeFocus(bool state) { BView::MakeFocus(state); }

// #pragma mark -

void BStatusBar::GetPreferredSize(float *_width, float *_height) {
  if (_width) {
    ...
    *_width = ceilf(StringWidth(fLabel.String())) +
              ...
              ceilf(StringWidth(fTrailingText.String())) + 5;
  }
...
void BStatusBar::ResizeToPreferred() { BView::ResizeToPreferred(); }

void BStatusBar::FrameMoved(BPoint newPosition) {
  BView::FrameMoved(newPosition);
}

void BStatusBar::FrameResized(float newWidth, float newHeight) {
...
void BStatusBar::Draw(BRect updateRect) {
  ...
  if (be_control_look != NULL) {
    be_control_look->DrawStatusBar(this, rect, updateRect, backgroundColor,
...
void BStatusBar::MessageReceived(BMessage *message) {
  ...
  case B_COLORS_UPDATED: {
    ...
    if ((fInternalFlags & kCustomBarColor) == 0) {
      message->FindColor(ui_color_name(B_STATUS_BAR_COLOR), &fBarColor);
    }
...
void BStatusBar::MouseDown(BPoint point) { BView::MouseDown(point); }

void BStatusBar::MouseUp(BPoint point) { BView::MouseUp(point); }
...
void BStatusBar::Update(float delta, const char *text,
...
void BStatusBar::SetTo(float value, const char *text,
                       const char *trailingText) {
  SetText(text);
...
float BStatusBar::CurrentValue() const { return fCurrent; }

float BStatusBar::MaxValue() const { return fMax; }

rgb_color BStatusBar::BarColor() const { return fBarColor; }
...
float BStatusBar::BarHeight() const {
  if (!fCustomBarHeight && fBarHeight == -1) {
    ...
    font_height fontHeight;
    GetFontHeight(&fontHeight);
    const_cast<BStatusBar *>(this)->fBarHeight =
        fontHeight.ascent + fontHeight.descent + 5;
  }

  return ceilf(fBarHeight);
}

const char *BStatusBar::Text() const { return fText.String(); }

const char *BStatusBar::TrailingText() const { return fTrailingText.String(); }

const char *BStatusBar::Label() const { return fLabel.String(); }
...
status_t BStatusBar::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BStatusBar::MinSize();
    return B_OK;
  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BStatusBar::MaxSize();
    return B_OK;
...
extern "C" void _ReservedStatusBar1__10BStatusBar(BStatusBar *self, float value,
                                                  const char *text,
                                                  const char *trailingText) {
...
BStatusBar &BStatusBar::operator=(const BStatusBar &other) { return *this; }

// #pragma mark -

void BStatusBar::_InitObject() {
  fMax = 100.0;
  fCurrent = 0.0;
...
void BStatusBar::_SetTextData(BString &text, const char *source,
...
*/
BRect BStatusBar::_BarFrame(const font_height *fontHeight) const {
  float top = 2;
  if (_HasText()) {
    if (fontHeight == NULL) {
      font_height height;
...
float BStatusBar::_BarPosition(const BRect &barFrame) const {
  if (fCurrent == 0)
    return barFrame.left - 1;

  return roundf(barFrame.left - 1 + (fCurrent * (barFrame.Width() + 3) / fMax));
}

bool BStatusBar::_HasText() const {
