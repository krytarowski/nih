...
BArchiveManager *BManagerBase::ArchiveManager(const BMessage *archive) {
...
BUnarchiveManager *BManagerBase::UnarchiveManager(const BMessage *archive) {
  BManagerBase *manager = ManagerPointer(archive);
  if (!manager)
    return NULL;
...
struct BArchiveManager::ArchiveInfo {
  ArchiveInfo() : token(-1), archive(NULL) {}

  ~ArchiveInfo() { delete archive; }
...
BArchiveManager::BArchiveManager(const BArchiver *creator)
    : BManagerBase(creator->ArchiveMessage(), BManagerBase::ARCHIVE_MANAGER),
      fTokenMap(), fCreator(creator), fError(B_OK) {}

BArchiveManager::~BArchiveManager() {
  fTopLevelArchive->AddBool(kManagedField, true);
}

status_t BArchiveManager::GetTokenForArchivable(BArchivable *archivable,
                                                int32 &_token) {
  if (!archivable) {
    _token = NULL_TOKEN;
    return B_OK;
...
status_t BArchiveManager::ArchiveObject(BArchivable *archivable, bool deep,
                                        int32 &_token) {
  if (!archivable) {
    _token = NULL_TOKEN;
    return B_OK;
...
status_t BArchiveManager::ArchiverLeaving(const BArchiver *archiver,
                                          status_t err) {
  if (fError == B_OK)
    fError = err;
  ...
  if (archiver == fCreator && fError == B_OK) {
    ...
    for (int32 i = 0; i < count; i++) {
      ...
      if (fError == B_OK && i > 0) {
        fError = fTopLevelArchive->AddMessage(kArchivableField, pair.first);
      }
...
struct BUnarchiveManager::ArchiveInfo {
  ArchiveInfo() : archivable(NULL), archive(), adopted(false) {}
...
BUnarchiveManager::BUnarchiveManager(BMessage *archive)
    : BManagerBase(archive, BManagerBase::UNARCHIVE_MANAGER), fObjects(NULL),
      fObjectCount(0), fTokenInProgress(0), fRefCount(0), fError(B_OK) {
...
BUnarchiveManager::~BUnarchiveManager() { delete[] fObjects; }
