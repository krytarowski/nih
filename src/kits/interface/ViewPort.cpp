...
public:
  ViewPortLayout(BViewPort *viewPort)
      : BAbstractLayout(), fViewPort(viewPort), fHasViewChild(false),
        fIsCacheValid(false), fMin(), fMax(), fPreferred() {}
  ...
  BLayoutItem *ChildItem() const { return ItemAt(0); }
  ...
  virtual void LayoutInvalidated(bool children) { fIsCacheValid = false; }
...
BViewPort::BViewPort(BView *child) : BView(NULL, 0), fChild(NULL) {
  _Init();
  SetChildView(child);
}

BViewPort::BViewPort(BLayoutItem *child) : BView(NULL, 0), fChild(NULL) {
  _Init();
  SetChildItem(child);
}

BViewPort::BViewPort(const char *name, BView *child)
    : BView(name, 0), fChild(NULL) {
  _Init();
  SetChildView(child);
}
...
BView *BViewPort::ChildView() const { return fLayout->ChildView(); }
...
BLayoutItem *BViewPort::ChildItem() const { return fLayout->ChildItem(); }
