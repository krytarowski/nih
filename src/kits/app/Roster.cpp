...
/*!	Extracts an app_info from a BMessage.
        ...
        \param info A pointer to a pre-allocated app_info to be filled in with
   the info found in the message.
...
/*!	Checks whether or not an application can be used.
        ...
        \return A status code, \c B_OK on success oir other error codes
   specifying why the application cannot be used. \retval B_OK The application
   can be used. \retval B_ENTRY_NOT_FOUND \a ref doesn't refer to and existing
   entry. \retval B_IS_A_DIRECTORY \a ref refers to a directory. \retval
   B_LAUNCH_FAILED_APP_IN_TRASH The application executable is in the trash.
*/
static status_t can_app_be_used(const entry_ref *ref) {
  ...
  BVolume volume;
  if (error == B_OK && volume.SetTo(ref->device) == B_OK &&
      find_directory(B_TRASH_DIRECTORY, &trashPath, false, &volume) == B_OK &&
      directory.SetTo(trashPath.Path()) == B_OK && directory.Contains(&entry)) {
...
*/
static int32 compare_queried_apps(const entry_ref *app1,
  ...
  if (searchPathes != NULL) {
    ...
    if (searchBuffer != NULL) {
      ...
      while (path != NULL) {
        ...
        bool found1 =
            !strncmp(path, path1.Path(), length) && path1.Path()[length] == '/';
        bool found2 =
            !strncmp(path, path2.Path(), length) && path2.Path()[length] == '/';
        ;
  ...
  bool hasVersionInfo1 =
      (appFileInfo1.GetVersionInfo(&versionInfo1, B_APP_VERSION_KIND) == B_OK);
  bool hasVersionInfo2 =
      (appFileInfo2.GetVersionInfo(&versionInfo2, B_APP_VERSION_KIND) == B_OK);
...
/*!	Finds an app by signature on any mounted volume.
        ...
        \retval B_LAUNCH_FAILED_APP_NOT_FOUND: An application with this
...
app_info::app_info()
    : thread(-1), team(-1), port(-1), flags(B_REG_DEFAULT_APP_FLAGS), ref() {
  signature[0] = '\0';
}
...
BRoster::ArgVector::ArgVector()
    : fArgc(0), fArgs(NULL), fAppPath(), fDocPath() {}

//!	Frees all resources associated with the ArgVector.
BRoster::ArgVector::~ArgVector() { Unset(); }
...
*/
status_t BRoster::ArgVector::Init(int argc, const char *const *args,
...
BRoster::BRoster()
    : fMessenger(), fMimeMessenger(),
      fMimeMessengerInitOnce(INIT_ONCE_UNINITIALIZED), fNoRegistrar(false) {
  _InitMessenger();
}
...
bool BRoster::IsRunning(entry_ref *ref) const { return (TeamFor(ref) >= 0); }
...
void BRoster::GetAppList(const char *signature, BList *teamIDList) const {
  status_t error = B_OK;
...
status_t BRoster::GetAppInfo(const char *signature, app_info *info) const {
  status_t error = B_OK;
...
status_t BRoster::FindApp(const char *mimeType, entry_ref *app) const {
...
status_t BRoster::Broadcast(BMessage *message, BMessenger replyTo) const {
...
status_t BRoster::Launch(const char *mimeType, BMessage *initialMessage,
...
status_t BRoster::Launch(const char *mimeType, BList *messageList,
                         team_id *_appTeam) const {
  if (mimeType == NULL)
    return B_BAD_VALUE;
...
status_t BRoster::Launch(const char *mimeType, int argc,
...
status_t BRoster::Launch(const entry_ref *ref, const BMessage *initialMessage,
                         team_id *_appTeam) const {
  if (ref == NULL)
    return B_BAD_VALUE;
...
status_t BRoster::Launch(const entry_ref *ref, const BList *messageList,
...
status_t BRoster::Launch(const entry_ref *ref, int argc,
...
void BRoster::AddToRecentDocuments(const entry_ref *document,
  ...
  if (error == B_OK) {
    error = message.AddString(
        "app sig", signature != NULL ? signature : callingApplicationSignature);
  }
  fMessenger.SendMessage(&message, &reply);
  if (error == B_OK) {
    error = reply.what == B_REG_RESULT ? (status_t)B_OK : (status_t)B_BAD_REPLY;
...
/*!	Shuts down the system.
        ...
        When \c synchronous is \c true and the method succeeds, it doesn't
   return.

        \param reboot If \c true, the system will be rebooted instead of being
               powered off.
        \param confirm If \c true, the user will be asked to confirm to shut
   down the system. \param synchronous If \c false, the method will return as
   soon as the shutdown process has been initiated successfully (or an error
        ...
        \retval B_SHUTTING_DOWN, when there's already a shutdown process in
                progress,
        \retval B_SHUTDOWN_CANCELLED, when the user cancelled the shutdown
   process,
*/
status_t BRoster::_ShutDown(bool reboot, bool confirm, bool synchronous) {
  status_t error = B_OK;
...
/*!	(Pre-)Registers an application with the registrar.
        ...
        unknown. If no team ID is supplied, \a pToken should be valid and, if
   the the pre-registration succeeds, will be filled with a unique token
   assigned by the roster.
...
status_t BRoster::_AddApplication(const char *signature, const entry_ref *ref,
  ...
  if (error == B_OK) {
    if (reply.what == B_REG_SUCCESS) {
      ...
      if (pToken != NULL && reply.FindInt32("token", (int32 *)pToken) != B_OK) {
        *pToken = 0;
      }
    }
  }
...
*/
status_t BRoster::_SetSignature(team_id team, const char *signature) const {
  status_t error = B_OK;
...
//!	\todo Really needed?
void BRoster::_SetThread(team_id team, thread_id thread) const {}

/*!	Sets the team and thread IDs of a pre-registered application.

        After an application has been pre-registered via AddApplication(),
   without supplying a team ID, the team and thread IDs have to be set using
   this method.
...
/*!	Completes the registration process for a pre-registered application.
        ...
        \return A status code.
        \retval B_OK Everything went fine.
        \retval B_REG_APP_NOT_PRE_REGISTERED \a team did not identify an
   existing application or the identified application was already fully
                registered.
*/
status_t BRoster::_CompleteRegistration(team_id team, thread_id thread,
                                        port_id port) const {
  status_t error = B_OK;
...
/*!	Returns whether an application is registered.
        ...
        \return \c B_OK, if the application is registered and all requested
                information could be retrieved, or another error code, if the
   app is not registered or an error occurred.
*/
status_t BRoster::_IsAppRegistered(const entry_ref *ref, team_id team,
                                   uint32 token, bool *preRegistered,
                                   app_info *info) const {
  status_t error = B_OK;
...
/*!	Unregisters a (pre-)registered application.
        ...
        \return A status code.
        \retval B_OK Everything went fine.
        \retval B_REG_APP_NOT_REGISTERED The supplied team ID does not identify
   a (pre-)registered application.
*/
status_t BRoster::_RemoveApp(team_id team) const {
  status_t error = B_OK;
...
void BRoster::_ApplicationCrashed(team_id team) {
  BPrivate::DesktopLink link;
  if (link.InitCheck() != B_OK)
    return;

  if (link.StartMessage(AS_APP_CRASHED) == B_OK && link.Attach(team) == B_OK) {
    link.Flush();
  }
}
...
/*!	Tells the registrar which application is currently active.
        ...
        wait for a reply.
*/
status_t BRoster::_UpdateActiveApp(team_id team) const {
  if (team < B_OK)
...
/*!	Launches the application associated with the supplied MIME type or
        ...
        The application to be started is searched the same way FindApp() does
   it.

        At least one of \a mimeType or \a ref must not be \c NULL. If \a
   mimeType is supplied, \a ref is ignored for finding the application.
        ...
        The supplied \a argc and \a args are (if containing at least one
   argument) put into a \c B_ARGV_RECEIVED message and sent to the launched
   application "on launch". The caller retains ownership of the supplied \a
   args. In case the method fails with \c B_ALREADY_RUNNING the message is
        ...
        \param mimeType MIME type for which the application shall be launched.
               May be \c NULL.
        \param ref entry_ref referring to the file for which an application
   shall be launched. May be \c NULL. \param messageList Optional list of
   messages to be sent to the application "on launch". May be \c NULL. \param
   argc Specifies the number of elements in \a args. \param args An array of
   C-strings to be sent as B_ARGV_RECEIVED messaged to the launched application.
        ...
        \retval B_LAUNCH_FAILED_NO_PREFERRED_APP Neither with the supplied type
                nor with its supertype (if the supplied isn't a supertype
   itself) a preferred application is associated. \retval
   B_LAUNCH_FAILED_APP_NOT_FOUND The supplied type is not installed or its
   preferred application could not be found. \retval
   B_LAUNCH_FAILED_APP_IN_TRASH The supplied type's preferred application was in
   the trash. \retval B_LAUNCH_FAILED_EXECUTABLE The found application was not
                executable.
*/
status_t BRoster::_LaunchApp(const char *mimeType, const entry_ref *ref,
  ...
  do {
    ...
    error = argVector.Init(argc, args, &appRef, wasDocument ? docRef : NULL);
        ...
        if (!isScript) {
          ...
          if (!wasDocument) {
            ...
            if (appType.InitCheck() == B_OK &&
                appType.GetAppHint(&hintRef) == B_OK && appRef == hintRef) {
  ...
  if (error == B_OK && !fNoRegistrar) {
    ...
    const entry_ref *_ref =
        argvOnly || !wasDocument || argVector.Count() > 1 ? NULL : docRef;
    if (!(argvOnly && alreadyRunning)) {
      _SendToRunning(team, argVector.Count(), argVector.Args(), _messageList,
                     _ref, alreadyRunning);
    }
  }
...
void BRoster::_DumpRoster() const {}
...
/*!	Finds an application associated with a MIME type or a file.
        ...
        \param ref The file for which an application shall be found.
               May be \c NULL.
        \param appRef A pointer to a pre-allocated entry_ref to be filled with
               a reference to the found application's executable. May be \c
   NULL. \param signature A pointer to a pre-allocated char buffer of at least
   size \c B_MIME_TYPE_LENGTH to be filled with the signature of the found
   application. May be \c NULL. \param appFlags A pointer to a pre-allocated
   uint32 variable to be filled with the app flags of the found application. May
   be \c NULL. \param wasDocument A pointer to a pre-allocated bool variable to
   be set to \c true, if the supplied file was not identifying an application,
               to \c false otherwise. Has no meaning, if a \a inType is
   supplied. May be \c NULL.
...
status_t BRoster::_ResolveApp(const char *inType, entry_ref *ref,
                              entry_ref *_appRef, char *_signature,
                              uint32 *_appFlags, bool *_wasDocument) const {
  ...
  if (inType != NULL) {
    ...
    error = _TranslateRef(ref, &appMeta, &appRef, &appFile, _wasDocument);
  }
  ...
  if (error == B_OK) {
    ...
    if (_signature != NULL) {
      ...
      if (appMeta.IsValid()) {
        strlcpy(_signature, appMeta.Type(), B_MIME_TYPE_LENGTH);
      } else
        _signature[0] = '\0';
    }
...
/*!	\brief Finds an application associated with a file.
        ...
        \param wasDocument A pointer to a pre-allocated bool variable to be set
   to \c true, if the supplied file was not identifying an application, to \c
   false otherwise. May be \c NULL.

        \return A status code.
        \retval B_OK: Everything went fine.
        \retval B_BAD_VALUE: \c NULL \a ref, \a appMeta, \a appRef or \a
   appFile.

        \see FindApp() for other error codes.
*/
status_t BRoster::_TranslateRef(entry_ref *ref, BMimeType *appMeta,
...
/*!	Finds an application associated with a MIME type.
        ...
        \see FindApp() for other error codes.
*/
status_t BRoster::_TranslateType(const char *mimeType, BMimeType *appMeta,
  ...
  if (error == B_OK && type.GetSupportingApps(&supportingSignatures) == B_OK) {
    int32 subCount;
    ...
    if (!addedSecondarySignature) {
      ...
      for (int32 i = 0; error == B_OK && i < subCount &&
                        supportingSignatures.FindString(
                            kSigField, i, &supportingType) == B_OK;
           i++) {
    ...
    for (int32 i = 0;
         error == B_OK && i < subCount &&
         supportingSignatures.FindString(kSigField, i, &supportingType) == B_OK;
         i++) {
      if (strcmp(primarySignature, supportingType) != 0 &&
          strcmp(secondarySignature, supportingType) != 0) {
        error = signatures.AddString(kSigField, supportingType);
  ...
  for (int32 i = 0; signatures.FindString(kSigField, i, &signature) == B_OK;
       i++) {
    if (signature[0] == '\0')
      continue;
...
/*!	Gets the type of a file either from the node info or by sniffing.
        ...
        The method first tries to get the file type from the supplied node info.
   If that didn't work, the given entry ref is sniffed.

        \param file An entry_ref referring to the file in question.
        \param nodeInfo A BNodeInfo initialized to the file.
        \param mimeType A pointer to a pre-allocated char buffer of at least
   size \c B_MIME_TYPE_LENGTH to be filled with the MIME type sniffed for the
   file.
...
*/
status_t BRoster::_GetFileType(const entry_ref *file, BNodeInfo *nodeInfo,
                               char *mimeType) const {
...
/*!	Sends messages to a running team.
        ...
        If \a messageList is not \c NULL or empty, those messages are sent
   first, then follow \c B_ARGV_RECEIVED, \c B_REFS_RECEIVED and finally \c
   B_READ_TO_RUN.
...
*/
status_t BRoster::_SendToRunning(team_id team, int argc,
  ...
  if (error == B_OK) {
    ...
    if (messageList != NULL) {
      for (int32 i = 0; BMessage *message = (BMessage *)messageList->ItemAt(i);
           i++) {
...
*/
void BRoster::_SetWithoutRegistrar(bool noRegistrar) {
  fNoRegistrar = noRegistrar;
}

void BRoster::_InitMessenger() {
...
// find the registrar port
  ...
  if (BLaunchRoster().GetData(B_REGISTRAR_SIGNATURE, data) == B_OK) {
    ...
    if (port >= 0 && team != current_team()) {
      ...
      BMessenger::Private(fMessenger).SetTo(team, port, B_PREFERRED_TOKEN);
    }
  }
#else
  ...
  if (rosterPort >= 0 && get_port_info(rosterPort, &info) == B_OK) {
    ...
    BMessenger::Private(fMessenger)
        .SetTo(info.team, rosterPort, B_PREFERRED_TOKEN);
  }
#endif
...
/*static*/ status_t BRoster::_InitMimeMessenger(void *data) {
  ...
  BMessage reply;
  status_t error =
      roster->fMessenger.SendMessage(&request, &reply, 1000000LL, 5000000LL);
  if (error == B_OK && reply.what == B_REG_SUCCESS) {
...
BMessenger &BRoster::_MimeMessenger() {
...
*/
void BRoster::_AddToRecentApps(const char *signature) const {
  status_t error = B_OK;
  // compose the request message
...
void BRoster::_ClearRecentDocuments() const {
...
//!	Sends a request to the roster to clear the recent documents list.
void BRoster::_ClearRecentFolders() const {
...
void BRoster::_ClearRecentApps() const {
...
*/
void BRoster::_LoadRecentLists(const char *filename) const {
  status_t error = B_OK;
...
*/
void BRoster::_SaveRecentLists(const char *filename) const {
  status_t error = B_OK;
