...
BRemoveEngine::BRemoveEngine() : fController(NULL) {}

BRemoveEngine::~BRemoveEngine() {}

BRemoveEngine::BController *BRemoveEngine::Controller() const {
  return fController;
}
...
status_t BRemoveEngine::_RemoveEntry(const char *path) {
  ...
  if (S_ISDIR(st.st_mode)) {
    ...
    while (directory.GetNextDirents(entry, sizeof(buffer), 1) == 1) {
      ...
      if (error != B_OK) {
        if (fController != NULL && fController->EntryFinished(path, error)) {
          return B_OK;
        }
        return error;
      }
  ...
  if (S_ISDIR(st.st_mode)) {
    if (rmdir(path) < 0) {
      return _HandleEntryError(path, errno, "Failed to remove \"%s\": %s\n",
                               path, strerror(errno));
    }
  } else {
    if (unlink(path) < 0) {
      return _HandleEntryError(path, errno, "Failed to unlink \"%s\": %s\n",
                               path, strerror(errno));
    }
  }
...
bool BRemoveEngine::BController::EntryStarted(const char *path) { return true; }

bool BRemoveEngine::BController::EntryFinished(const char *path,
                                               status_t error) {
  return error == B_OK;
}

void BRemoveEngine::BController::ErrorOccurred(const char *message,
                                               status_t error) {}
