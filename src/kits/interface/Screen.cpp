...
BScreen::BScreen(screen_id id) { fScreen = BPrivateScreen::Get(id.id); }

BScreen::BScreen(BWindow *window) { fScreen = BPrivateScreen::Get(window); }

BScreen::~BScreen() { BPrivateScreen::Put(fScreen); }

bool BScreen::IsValid() { return fScreen != NULL && fScreen->IsValid(); }
...
status_t BScreen::GetBitmap(BBitmap **_bitmap, bool drawCursor, BRect *bounds) {
  if (fScreen != NULL)
...
status_t BScreen::ReadBitmap(BBitmap *bitmap, bool drawCursor, BRect *bounds) {
  if (fScreen != NULL)
...
status_t BScreen::ProposeMode(display_mode *target, const display_mode *low,
...
status_t BScreen::GetModeList(display_mode **_modeList, uint32 *_count) {
  if (fScreen != NULL)
...
status_t BScreen::GetMode(uint32 workspace, display_mode *mode) {
  if (fScreen != NULL)
...
status_t BScreen::SetMode(display_mode *mode, bool makeDefault) {
  if (fScreen != NULL)
...
status_t BScreen::SetMode(uint32 workspace, display_mode *mode,
                          bool makeDefault) {
  if (fScreen != NULL)
...
status_t BScreen::GetPixelClockLimits(display_mode *mode, uint32 *_low,
                                      uint32 *_high) {
  if (fScreen != NULL)
...
BPrivate::BPrivateScreen *BScreen::private_screen() { return fScreen; }

status_t BScreen::ProposeDisplayMode(display_mode *target,
