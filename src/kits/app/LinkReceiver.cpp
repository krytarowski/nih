...
LinkReceiver::LinkReceiver(port_id port)
    : fReceivePort(port), fRecvBuffer(NULL), fRecvPosition(0), fRecvStart(0),
      fRecvBufferSize(0), fDataSize(0), fReplySize(0), fReadError(B_OK) {}

LinkReceiver::~LinkReceiver() { free(fRecvBuffer); }

void LinkReceiver::SetPort(port_id port) { fReceivePort = port; }

status_t LinkReceiver::GetNextMessage(int32 &code, bigtime_t timeout) {
  fReadError = B_OK;

  int32 remaining = fDataSize - (fRecvStart + fReplySize);
  STRACE(("info: LinkReceiver GetNextReply() reports %ld bytes remaining in "
          "buffer.\n",
          remaining));
  ...
  if (remaining < (int32)sizeof(message_header)) {
    ...
    STRACE(("error info: LinkReceiver remaining %ld bytes is less than header "
            "size.\n",
            remaining));
    ResetBuffer();
    return B_ERROR;
  ...
  if (fReplySize > remaining || fReplySize < (int32)sizeof(message_header)) {
    STRACE(("error info: LinkReceiver message size of %ld bytes smaller than "
            "header size.\n",
            fReplySize));
    ResetBuffer();
    return B_ERROR;
...
status_t LinkReceiver::AdjustReplyBuffer(bigtime_t timeout) {
  ...
  if (kInitialBufferSize == kMaxBufferSize) {
    ...
    ssize_t bufferSize;
    do {
      bufferSize = port_buffer_size_etc(
          fReceivePort, timeout == B_INFINITE_TIMEOUT ? 0 : B_RELATIVE_TIMEOUT,
          timeout);
...
status_t LinkReceiver::ReadFromPort(bigtime_t timeout) {
  ...
  while (true) {
    if (timeout != B_INFINITE_TIMEOUT) {
      ...
      do {
        bytesRead =
            read_port(fReceivePort, &code, fRecvBuffer, fRecvBufferSize);
      } while (bytesRead == B_INTERRUPTED);
    }
...
status_t LinkReceiver::ReadRegion(BRegion *region) {
  ...
  if (status >= B_OK) {
    if (!region->_SetSize(region->fCount))
      ...
      status = Read(region->fData, region->fCount * sizeof(clipping_rect));
    }
    if (status < B_OK)
      region->MakeEmpty();
  }
...
static BGradient *gradient_for_type(BGradient::Type type) {
  switch (type) {
