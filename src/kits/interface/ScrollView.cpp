...
BScrollView::BScrollView(const char *name, BView *target, uint32 resizingMode,
    ...
    : BView(_ComputeFrame(target, horizontal, vertical, border,
            ...
            name, resizingMode, _ModifyFlags(flags, border)),
      fTarget(target), fBorder(border) {
...
BScrollView::BScrollView(const char *name, BView *target, uint32 flags,
    ...
    : BView(name, _ModifyFlags(flags, border)), fTarget(target),
      fBorder(border) {
  _Init(horizontal, vertical);
}

BScrollView::BScrollView(BMessage *archive)
    : BView(archive), fHighlighted(false) {
  int32 border;
  fBorder = archive->FindInt32("_style", &border) == B_OK ? (border_style)border
                                                          : B_FANCY_BORDER;
...
BArchivable *BScrollView::Instantiate(BMessage *archive) {
...
status_t BScrollView::Archive(BMessage *archive, bool deep) const {
...
status_t BScrollView::AllUnarchived(const BMessage *archive) {
...
void BScrollView::DetachedFromWindow() { BView::DetachedFromWindow(); }

void BScrollView::AllAttached() { BView::AllAttached(); }

void BScrollView::AllDetached() { BView::AllDetached(); }
...
void BScrollView::Draw(BRect updateRect) {
  ...
  be_control_look->DrawScrollViewFrame(
      ...
      base, fBorder, flags, fBorders);
}

void BScrollView::FrameMoved(BPoint newPosition) {
  BView::FrameMoved(newPosition);
}

void BScrollView::FrameResized(float newWidth, float newHeight) {
  ...
  if (fHorizontalScrollBar != NULL && fVerticalScrollBar != NULL) {
    ...
    scrollCorner.left =
        min_c(fPreviousWidth - fVerticalScrollBar->Frame().Height(),
              fHorizontalScrollBar->Frame().right + 1);
    scrollCorner.top =
        min_c(fPreviousHeight - fHorizontalScrollBar->Frame().Width(),
...
void BScrollView::MessageReceived(BMessage *message) {
  BView::MessageReceived(message);
}

void BScrollView::MouseDown(BPoint where) { BView::MouseDown(where); }

void BScrollView::MouseMoved(BPoint where, uint32 code,
                             const BMessage *dragMessage) {
  BView::MouseMoved(where, code, dragMessage);
}

void BScrollView::MouseUp(BPoint where) { BView::MouseUp(where); }

void BScrollView::WindowActivated(bool active) {
  if (fHighlighted)
    Invalidate();
...
void BScrollView::GetPreferredSize(float *_width, float *_height) {
...
void BScrollView::MakeFocus(bool focus) { BView::MakeFocus(focus); }

BSize BScrollView::MinSize() {
  BSize size =
      _ComputeSize(fTarget != NULL ? fTarget->MinSize() : BSize(16, 16));

  return BLayoutUtils::ComposeSize(ExplicitMinSize(), size);
}

BSize BScrollView::MaxSize() {
  BSize size =
      _ComputeSize(fTarget != NULL ? fTarget->MaxSize()
                                   : BSize(B_SIZE_UNLIMITED, B_SIZE_UNLIMITED));

  return BLayoutUtils::ComposeSize(ExplicitMaxSize(), size);
}

BSize BScrollView::PreferredSize() {
  BSize size =
      _ComputeSize(fTarget != NULL ? fTarget->PreferredSize() : BSize(32, 32));
...
border_style BScrollView::Border() const { return fBorder; }
...
uint32 BScrollView::Borders() const { return fBorders; }
...
status_t BScrollView::SetBorderHighlighted(bool highlight) {
  ...
  Invalidate(
      BRect(bounds.left, bounds.top + 1, bounds.left, bounds.bottom - 1));
  Invalidate(
      BRect(bounds.right, bounds.top + 1, bounds.right, bounds.bottom - 1));
  Invalidate(BRect(bounds.left, bounds.bottom, bounds.right, bounds.bottom));

  return B_OK;
}

bool BScrollView::IsBorderHighlighted() const { return fHighlighted; }
...
void BScrollView::SetTarget(BView *target) {
  ...
  if (target != NULL) {
    ...
    target->MoveTo(
        (fBorders & BControlLook::B_LEFT_BORDER) != 0 ? borderSize : 0,
        (fBorders & BControlLook::B_TOP_BORDER) != 0 ? borderSize : 0);
...
BView *BScrollView::Target() const { return fTarget; }

// #pragma mark - Scripting methods

BHandler *BScrollView::ResolveSpecifier(BMessage *message, int32 index,
                                        BMessage *specifier, int32 what,
                                        const char *property) {
  return BView::ResolveSpecifier(message, index, specifier, what, property);
}

status_t BScrollView::GetSupportedSuites(BMessage *message) {
  return BView::GetSupportedSuites(message);
}

//	#pragma mark - Perform

status_t BScrollView::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BScrollView::MinSize();
    return B_OK;

  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BScrollView::MaxSize();
    return B_OK;
...
void BScrollView::LayoutInvalidated(bool descendants) {}

void BScrollView::DoLayout() {
...
void BScrollView::_Init(bool horizontal, bool vertical) {
  fHorizontalScrollBar = NULL;
  fVerticalScrollBar = NULL;
  fHighlighted = false;
  ...
  if (horizontal) {
    fHorizontalScrollBar = new BScrollBar(BRect(0, 0, 14, 14), "_HSB_", fTarget,
                                          0, 1000, B_HORIZONTAL);
    AddChild(fHorizontalScrollBar);
  }

  if (vertical) {
    fVerticalScrollBar = new BScrollBar(BRect(0, 0, 14, 14), "_VSB_", fTarget,
                                        0, 1000, B_VERTICAL);
...
float BScrollView::_BorderSize() const { return _BorderSize(fBorder); }

BRect BScrollView::_InnerFrame() const {
  BRect frame = Bounds();
...
BSize BScrollView::_ComputeSize(BSize targetSize) const {
  BRect frame = _ComputeFrame(BRect(0, 0, targetSize.width, targetSize.height));

  return BSize(frame.Width(), frame.Height());
}

BRect BScrollView::_ComputeFrame(BRect targetRect) const {
  return _ComputeFrame(targetRect, fHorizontalScrollBar != NULL,
                       fVerticalScrollBar != NULL, fBorder, fBorders);
}

void BScrollView::_AlignScrollBars(bool horizontal, bool vertical,
                                   BRect targetFrame) {
  if (horizontal) {
...
*/
/*static*/ BRect BScrollView::_ComputeFrame(BRect frame, bool horizontal,
...
/*static*/ BRect BScrollView::_ComputeFrame(BView *target, bool horizontal,
                                            bool vertical, border_style border,
                                            uint32 borders) {
  return _ComputeFrame(target != NULL ? target->Frame() : BRect(0, 0, 16, 16),
                       horizontal, vertical, border, borders);
}

/*! This method returns the size of the specified border.
*/
/*static*/ float BScrollView::_BorderSize(border_style border) {
...
*/
/*static*/ int32 BScrollView::_ModifyFlags(int32 flags, border_style border) {
  ...
  if (border != B_NO_BORDER)
    return flags | B_WILL_DRAW |
           (flags & B_FULL_UPDATE_ON_RESIZE ? 0 : B_FRAME_EVENTS);

  return flags & ~(B_WILL_DRAW | B_FRAME_EVENTS | B_FULL_UPDATE_ON_RESIZE);
}

/*static*/ void BScrollView::_InsetBorders(BRect &frame, border_style border,
                                           uint32 borders, bool expand) {
...
B_IF_GCC_2(InvalidateLayout__11BScrollViewb,
           _ZN11BScrollView16InvalidateLayoutEb)(BScrollView *view,
                                                 bool descendants) {
  perform_data_layout_invalidated data;
