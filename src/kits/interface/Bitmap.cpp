...
BBitmap::Private::Private(BBitmap *bitmap) : fBitmap(bitmap) {}

void BBitmap::Private::ReconnectToAppServer() {
  fBitmap->_ReconnectToAppServer();
}

/*!	\brief Returns the number of bytes per row needed to store the actual
                   bitmap data (not including any padding) given a color space
   and a row width. \param colorSpace The color space. \param width The width.
        \return The number of bytes per row needed to store data for a row, or
                        0, if the color space is not supported.
*/
static inline int32 get_raw_bytes_per_row(color_space colorSpace, int32 width) {
  int32 bpr = 0;
  switch (colorSpace) {
  // supported
  case B_RGB32:
...
/*!	\brief Returns the number of bytes per row needed to store the bitmap
                   data (including any padding) given a color space and a row
   width. \param colorSpace The color space. \param width The width. \return The
   number of bytes per row needed to store data for a row, or 0, if the color
...
/*!	\brief Creates and initializes a BBitmap.
        ...
        \param bytesPerRow The number of bytes per row the bitmap should use.
                   \c B_ANY_BYTES_PER_ROW to let the constructor choose an
   appropriate value. \param screenID ???
*/
BBitmap::BBitmap(BRect bounds, uint32 flags, color_space colorSpace,
                 int32 bytesPerRow, screen_id screenID)
    : fBasePointer(NULL), fSize(0), fColorSpace(B_NO_COLOR_SPACE),
      fBounds(0, 0, -1, -1), fBytesPerRow(0), fWindow(NULL), fServerToken(-1),
      fAreaOffset(-1), fArea(-1), fServerArea(-1), fFlags(0),
...
/*!	\brief Creates and initializes a BBitmap.
        ...
        \param acceptsViews \c true, if the bitmap shall accept BViews, i.e. if
                   it shall be possible to attach BView to the bitmap and draw
   into it. \param needsContiguous If \c true a physically contiguous chunk of
...
BBitmap::BBitmap(BRect bounds, color_space colorSpace, bool acceptsViews,
  ...
  _InitObject(bounds, colorSpace, flags, B_ANY_BYTES_PER_ROW, B_MAIN_SCREEN_ID);
}
...
BBitmap::BBitmap(const BBitmap &source, uint32 flags)
  ...
  _InitObject(source.Bounds(), source.ColorSpace(), flags, source.BytesPerRow(),
              B_MAIN_SCREEN_ID);
...
*/
BBitmap::~BBitmap() { _CleanUp(); }

/*!	\brief Unarchives a bitmap from a BMessage.
        \param data The archive.
*/
BBitmap::BBitmap(BMessage *data)
    : BArchivable(data), fBasePointer(NULL), fSize(0),
      fColorSpace(B_NO_COLOR_SPACE), fBounds(0, 0, -1, -1), fBytesPerRow(0),
      fWindow(NULL), fServerToken(-1), fAreaOffset(-1), fArea(-1),
      fServerArea(-1), fFlags(0), fInitError(B_NO_INIT) {
  int32 flags;
...
*/
int32 BBitmap::BitsLength() const { return fSize; }
...
*/
int32 BBitmap::BytesPerRow() const { return fBytesPerRow; }

/*!	\brief Returns the bitmap's color space.
        \return The bitmap's color space.
*/
color_space BBitmap::ColorSpace() const { return fColorSpace; }

/*!	\brief Returns the bitmap's dimensions.
        \return The bitmap's dimensions.
*/
BRect BBitmap::Bounds() const { return fBounds; }
...
*/
uint32 BBitmap::Flags() const { return fFlags; }
...
/*!	\brief Assigns data to the bitmap.
        ...
        \note As this methods is apparently a bit strange to use, Haiku
   introduces ImportBits() methods, which are recommended to be used instead.
        ...
        \param offset The offset (in bytes) relative to beginning of the bitmap
                   data specifying the position at which the source data shall
   be written. \param colorSpace Color space of the source data.
*/
...
/*!	\brief Assigns data to the bitmap.
        ...
        \param offset The offset (in bytes) relative to beginning of the bitmap
                   data specifying the position at which the source data shall
   be written. \param colorSpace Color space of the source data. \return - \c
   B_OK: Everything went fine. - \c B_BAD_VALUE: \c NULL \a data, invalid \a bpr
   or \a offset, or unsupported \a colorSpace.
*/
status_t BBitmap::ImportBits(const void *data, int32 length, int32 bpr,
  ...
  return BPrivate::ConvertBits(data, (uint8 *)fBasePointer + offset, length,
                               fSize - offset, bpr, fBytesPerRow, colorSpace,
                               fColorSpace, width, fBounds.IntegerHeight() + 1);
}
...
/*!	\briefly Assigns another bitmap's data to this bitmap.
        ...
        The supplied bitmap must have the exactly same dimensions as this
   bitmap. Its data is converted to the color space of this bitmap.
...
status_t BBitmap::ImportBits(const BBitmap *bitmap) {
  ...
  return ImportBits(bitmap->Bits(), bitmap->BitsLength(), bitmap->BytesPerRow(),
                    0, bitmap->ColorSpace());
}
...
/*!	\brief Assigns data to the bitmap.
        ...
        Allows for a BPoint offset in the source and in the bitmap. The region
        of the source at \a from extending \a width and \a height is assigned
        (and converted if necessary) to the bitmap at \a to. The source bitmap
   is clipped to the bitmap and they don't need to have the same dimensions.
...
*/
status_t BBitmap::ImportBits(const BBitmap *bitmap, BPoint from, BPoint to,
                             int32 width, int32 height) {
  if (InitCheck() != B_OK)
    return B_NO_INIT;
  ...
  return ImportBits(bitmap->Bits(), bitmap->BitsLength(), bitmap->BytesPerRow(),
                    bitmap->ColorSpace(), from, to, width, height);
}
...
/*!	\brief Returns the BView at a certain index in the bitmap's list of
   views. \param index The index of the BView to be returned. \return The BView
   at index \a index or \c NULL, if the index is out of range.
*/
...
/*!	\brief Returns a bitmap's BView with a certain name.
        ...
        \return The BView with the name \a name or \c NULL, if the bitmap
   doesn't know a view with that name.
*/
...
/*!	\brief Locks the off-screen window that belongs to the bitmap.
        ...
        \return \c true, if the lock was acquired successfully, \c false
                        otherwise.
*/
bool BBitmap::Lock() { return fWindow != NULL ? fWindow->Lock() : false; }
...
BBitmap &BBitmap::operator=(const BBitmap &source) {
  _CleanUp();
  fInitError = B_NO_INIT;
...
status_t BBitmap::Perform(perform_code d, void *arg) {
...
int32 BBitmap::_ServerToken() const { return fServerToken; }
...
/*!	\brief Initializes the bitmap.
   ...
   appropriate value. \param screenID ???
*/
void BBitmap::_InitObject(BRect bounds, color_space colorSpace, uint32 flags,
                          int32 bytesPerRow, screen_id screenID) {
  // printf("BBitmap::InitObject(bounds: BRect(%.1f, %.1f, %.1f, %.1f), format:
  // %ld, flags: %ld, bpr: %ld\n", 	   bounds.left, bounds.top,
  //bounds.right, bounds.bottom, colorSpace, flags, bytesPerRow);
    ...
    if ((flags & B_BITMAP_NO_SERVER_LINK) != 0) {
      ...
      if (link.FlushWithReply(error) == B_OK && error == B_OK) {
        ...
        if ((allocationFlags & kNewAllocatorArea) != 0) {
          error = allocator->AddArea(fServerArea, fArea, fBasePointer, size);
        } else {
...
void BBitmap::_ReconnectToAppServer() {
  BPrivate::AppServerLink link;
