...
static property_info sPropertyList[] = {{"Enabled",
...
void BControl::AttachedToWindow() {
  ...
  if (ViewColor() == B_TRANSPARENT_COLOR || Parent() == NULL) {
    AdoptSystemColors();
  }
...
void BControl::DetachedFromWindow() { BView::DetachedFromWindow(); }

void BControl::AllAttached() { BView::AllAttached(); }

void BControl::AllDetached() { BView::AllDetached(); }
...
void BControl::MakeFocus(bool focus) {
  if (focus == IsFocus())
    return;
...
void BControl::MouseDown(BPoint where) { BView::MouseDown(where); }

void BControl::MouseUp(BPoint where) { BView::MouseUp(where); }

void BControl::MouseMoved(BPoint where, uint32 code,
...
const char *BControl::Label() const { return fLabel; }
...
void BControl::SetValueNoUpdate(int32 value) { fValue = value; }

int32 BControl::Value() const { return fValue; }
...
bool BControl::IsEnabled() const { return fEnabled; }

void BControl::GetPreferredSize(float *_width, float *_height) {
  BView::GetPreferredSize(_width, _height);
}

void BControl::ResizeToPreferred() { BView::ResizeToPreferred(); }
...
status_t BControl::Invoke(BMessage *message) {
  ...
  // ToDo: is this correct? If message == NULL (even if IsWatched()), we always
  // return B_BAD_VALUE
  status_t err;
  if (message)
...
status_t BControl::Perform(perform_code code, void *_data) {
  ...
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BControl::MinSize();
    return B_OK;
  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BControl::MaxSize();
    return B_OK;
...
status_t BControl::SetIconBitmap(const BBitmap *bitmap, uint32 which,
                                 uint32 flags) {
...
bool BControl::IsFocusChanging() const { return fFocusChanging; }

bool BControl::IsTracking() const { return fTracking; }

void BControl::SetTracking(bool state) { fTracking = state; }

extern "C" status_t B_IF_GCC_2(_ReservedControl1__8BControl,
