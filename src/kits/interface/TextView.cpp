...
// 	 to refresh only changed parts of text (currently we often redraw the
// whole
//   text)
...
#define CALLED(x...)                                                           \
  FunctionTracer _ft("BTextView", __FUNCTION__, sFunctionDepth)
#define TRACE(x...)                                                            \
  {                                                                            \
    BString _to;                                                               \
...
struct BTextView::LayoutData {
  LayoutData()
      : leftInset(0), topInset(0), rightInset(0), bottomInset(0), valid(false) {
  }

  void UpdateInsets(const BRect &bounds, const BRect &textRect) {
    // we disallow negative insets, as they would cause parts of the
    // text to be hidden
    leftInset = textRect.left >= bounds.left ? textRect.left - bounds.left : 0;
    topInset = textRect.top >= bounds.top ? textRect.top - bounds.top : 0;
    rightInset = bounds.right >= textRect.right ? bounds.right - textRect.right
                                                : leftInset;
...
static property_info sPropertyList[] = {
    ...
    {"Text",
     ...
     "Returns the text in the specified range in the BTextView.",
     0,
     {B_STRING_TYPE, 0}},
    {"Text",
     {B_SET_PROPERTY, 0},
     {B_RANGE_SPECIFIER, B_REVERSE_RANGE_SPECIFIER, 0},
     "Removes or inserts text into the specified range in the BTextView.",
     0,
     {B_STRING_TYPE, 0}},
    {
        "text_run_array",
...
BTextView::BTextView(BMessage *archive) : BView(archive) {
  CALLED();
  BRect rect;
  ...
  if (archive->FindData("_dis_ch", B_RAW_TYPE, (const void **)&disallowedChars,
                        &disallowedCount) == B_OK) {
  ...
  if (archive->FindData("_runs", B_RAW_TYPE, &flattenedRun, &runSize) == B_OK) {
    text_run_array *runArray =
        UnflattenRunArray(flattenedRun, (int32 *)&runSize);
    if (runArray) {
...
void BTextView::DetachedFromWindow() { BView::DetachedFromWindow(); }
...
void BTextView::MouseDown(BPoint where) {
  ...
  bool multipleClick = clickTime - fClickTime < clickSpeed &&
  ...
  if (fClickTime == clickTime) {
    ...
    BMessenger messenger(this);
    fClickRunner =
        new (nothrow) BMessageRunner(messenger, &message, clickSpeed, 1);
  }
...
void BTextView::MouseMoved(BPoint where, uint32 code,
...
void BTextView::KeyDown(const char *bytes, int32 numBytes) {
  ...
  default:
    ...
    if (fDisallowedChars && fDisallowedChars->HasItem(
...
void BTextView::FrameResized(float newWidth, float newHeight) {
  BView::FrameResized(newWidth, newHeight);
  _UpdateScrollbars();
}

void BTextView::MakeFocus(bool focus) {
...
void BTextView::MessageReceived(BMessage *message) {
  ...
  case B_COUNT_PROPERTIES: {
    ...
    if (propInfo.FindMatch(message, 0, &specifier, specifier.what, property) <
        B_OK) {
      BView::MessageReceived(message);
      break;
    }
    ...
    switch (message->what) {
    case B_GET_PROPERTY:
      handled = _GetProperty(&specifier, specifier.what, property, &reply);
      break;

    case B_SET_PROPERTY:
      handled = _SetProperty(&specifier, specifier.what, property, &reply);
      break;

    case B_COUNT_PROPERTIES:
      handled = _CountProperties(&specifier, specifier.what, property, &reply);
      break;
...
status_t BTextView::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BTextView::MinSize();
    return B_OK;
  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BTextView::MaxSize();
    return B_OK;
...
void BTextView::SetText(const char *text, const text_run_array *runs) {
  SetText(text, text ? strlen(text) : 0, runs);
}

void BTextView::SetText(const char *text, int32 length,
...
void BTextView::SetText(BFile *file, int32 offset, int32 length,
                        const text_run_array *runs) {
  CALLED();
...
void BTextView::Insert(const char *text, const text_run_array *runs) {
  if (text != NULL)
    _DoInsertText(text, strlen(text), fSelStart, runs);
}

void BTextView::Insert(const char *text, int32 length,
                       const text_run_array *runs) {
  if (text != NULL && length > 0)
    _DoInsertText(text, strnlen(text, length), fSelStart, runs);
}

void BTextView::Insert(int32 offset, const char *text, int32 length,
                       const text_run_array *runs) {
...
void BTextView::Delete() { Delete(fSelStart, fSelEnd); }
...
const char *BTextView::Text() const { return fText->RealText(); }

int32 BTextView::TextLength() const { return fText->Length(); }
...
int32 BTextView::CountLines() const { return fLines->NumLines(); }

int32 BTextView::CurrentLine() const { return LineAt(fSelStart); }
...
void BTextView::Paste(BClipboard *clipboard) {
  ...
  if (clip != NULL) {
    ...
    if (clip->FindData("text/plain", B_MIME_TYPE, (const void **)&text,
                       &length) == B_OK) {
      ...
      if (fUndo) {
        delete fUndo;
        fUndo = new PasteUndoBuffer(this, text, length, runArray, runLength);
      }
...
bool BTextView::AcceptsDrop(const BMessage *message) {
  return fEditable && message && message->HasData("text/plain", B_MIME_TYPE);
}
...
void BTextView::SelectAll() { Select(0, fText->Length()); }

void BTextView::GetSelection(int32 *_start, int32 *_end) const {
  int32 start = 0;
  int32 end = 0;
...
void BTextView::GetFontAndColor(int32 offset, BFont *_font,
                                rgb_color *_color) const {
  fStyles->GetStyle(offset, _font, _color);
}

void BTextView::GetFontAndColor(BFont *_font, uint32 *_mode, rgb_color *_color,
                                bool *_sameColor) const {
  fStyles->ContinuousGetStyle(_font, _mode, _color, _sameColor, fSelStart,
                              fSelEnd);
}
...
BPoint BTextView::PointAt(int32 offset, float *_height) const {
  // pin offset at reasonable values
  if (offset < 0)
    offset = 0;
  else if (offset > fText->Length())
...
void BTextView::FindWord(int32 offset, int32 *_fromOffset, int32 *_toOffset) {
  if (offset < 0) {
    if (_fromOffset)
      *_fromOffset = 0;
...
float BTextView::LineWidth(int32 lineNumber) const {
...
float BTextView::LineHeight(int32 lineNumber) const {
...
void BTextView::ScrollToOffset(int32 offset) {
  ...
  // horizontal
  float extraSpace =
      fAlignment == B_ALIGN_LEFT ? ceilf(bounds.IntegerWidth() / 2) : 0.0;
...
void BTextView::ScrollToSelection() { ScrollToOffset(fSelStart); }
...
BRect BTextView::TextRect() const { return fTextRect; }

void BTextView::_ResetTextRect() {
...
void BTextView::SetInsets(float left, float top, float right, float bottom) {
  if (fLayoutData->leftInset == left && fLayoutData->topInset == top &&
      fLayoutData->rightInset == right && fLayoutData->bottomInset == bottom)
    return;
...
void BTextView::GetInsets(float *_left, float *_top, float *_right,
                          float *_bottom) const {
  if (_left)
...
void BTextView::SetStylable(bool stylable) { fStylable = stylable; }

bool BTextView::IsStylable() const { return fStylable; }
...
float BTextView::TabWidth() const { return fTabWidth; }
...
bool BTextView::IsSelectable() const { return fSelectable; }
...
bool BTextView::IsEditable() const { return fEditable; }
...
bool BTextView::DoesWordWrap() const { return fWrap; }
...
int32 BTextView::MaxBytes() const { return fMaxBytes; }
...
void BTextView::SetAlignment(alignment align) {
  // Do a reality check
  if (fAlignment != align && (align == B_ALIGN_LEFT || align == B_ALIGN_RIGHT ||
...
alignment BTextView::Alignment() const { return fAlignment; }

void BTextView::SetAutoindent(bool state) { fAutoindent = state; }

bool BTextView::DoesAutoindent() const { return fAutoindent; }
...
color_space BTextView::ColorSpace() const { return fColorSpace; }
...
bool BTextView::IsResizable() const { return fResizable; }
...
bool BTextView::DoesUndo() const { return fUndo != NULL; }
...
bool BTextView::IsTypingHidden() const { return fText->PasswordMode(); }

// #pragma mark - Size methods

void BTextView::ResizeToPreferred() { BView::ResizeToPreferred(); }

void BTextView::GetPreferredSize(float *_width, float *_height) {
  CALLED();
  ...
  if (_width) {
    ...
    if (width < fLayoutData->min.width || (Flags() & B_SUPPORTS_LAYOUT) != 0) {
...
bool BTextView::HasHeightForWidth() {
  if (IsEditable())
...
void BTextView::GetHeightForWidth(float width, float *min, float *max,
                                  float *preferred) {
  if (IsEditable()) {
...
void BTextView::LayoutInvalidated(bool descendants) {
  CALLED();

  fLayoutData->valid = false;
}

void BTextView::DoLayout() {
...
void BTextView::_ValidateLayoutData() {
  ...
  fLayoutData->preferred.height =
      fTextRect.Height() + fLayoutData->topInset + fLayoutData->bottomInset;
...
void BTextView::AllAttached() { BView::AllAttached(); }

void BTextView::AllDetached() { BView::AllDetached(); }

/* static */
text_run_array *BTextView::AllocRunArray(int32 entryCount, int32 *outSize) {
...
/* static */
text_run_array *BTextView::CopyRunArray(const text_run_array *orig,
                                        int32 countDelta) {
...
/* static */
void BTextView::FreeRunArray(text_run_array *array) {
  if (array == NULL)
    return;
...
/* static */
void *BTextView::FlattenRunArray(const text_run_array *runArray, int32 *_size) {
  CALLED();
  int32 size = sizeof(flattened_text_run_array) +
               (runArray->count - 1) * sizeof(flattened_text_run);
  ...
  for (int32 i = 0; i < runArray->count; i++) {
    ...
    array->styles[i].size =
        B_HOST_TO_BENDIAN_FLOAT(runArray->runs[i].font.Size());
    array->styles[i].shear =
        B_HOST_TO_BENDIAN_FLOAT(runArray->runs[i].font.Shear());
    array->styles[i].face =
        B_HOST_TO_BENDIAN_INT16(runArray->runs[i].font.Face());
...
/* static */
text_run_array *BTextView::UnflattenRunArray(const void *data, int32 *_size) {
  CALLED();
  ...
  for (int32 i = 0; i < count; i++) {
    ...
    runArray->runs[i].font.SetSize(
        B_BENDIAN_TO_HOST_FLOAT(array->styles[i].size));
    runArray->runs[i].font.SetShear(
        B_BENDIAN_TO_HOST_FLOAT(array->styles[i].shear));
...
void BTextView::InsertText(const char *text, int32 length, int32 offset,
                           const text_run_array *runs) {
  CALLED();
...
void BTextView::GetDragParameters(BMessage *drag, BBitmap **bitmap,
  ...
  if (styles != NULL) {
    drag->AddData("application/x-vnd.Be-text_run_array", B_MIME_TYPE, styles,
                  size);
...
*/
void BTextView::_InitObject(BRect textRect, const BFont *initialFont,
...
void BTextView::_HandleBackspace() {
  if (fUndo) {
...
void BTextView::_HandleArrowKey(uint32 arrowKey, int32 modifiers) {
...
void BTextView::_HandlePageKey(uint32 pageKey, int32 modifiers) {
  if (modifiers < 0) {
    BMessage *currentMessage = Window()->CurrentMessage();
    if (currentMessage == NULL ||
        currentMessage->FindInt32("modifiers", &modifiers) != B_OK) {
  ...
  case B_PAGE_UP: {
    ...
    BPoint nextPos(currentPos.x, currentPos.y + lineHeight - Bounds().Height());
...
*/
void BTextView::_HandleAlphaKey(const char *bytes, int32 numBytes) {
  // TODO: block input if not editable (Andrew)
  if (fUndo) {
    TypingUndoBuffer *undoBuffer = dynamic_cast<TypingUndoBuffer *>(fUndo);
    if (!undoBuffer) {
...
*/
void BTextView::_Refresh(int32 fromOffset, int32 toOffset, bool scroll) {
  // TODO: Cleanup
...
*/
void BTextView::_RecalculateLineBreaks(int32 *startLine, int32 *endLine) {
  CALLED();

  // are we insane?
  *startLine = (*startLine < 0) ? 0 : *startLine;
  *endLine =
      (*endLine > fLines->NumLines() - 1) ? fLines->NumLines() - 1 : *endLine;
...
int32 BTextView::_FindLineBreak(int32 fromOffset, float *_ascent,
                                float *_descent, float *inOutWidth) {
  *_ascent = 0.0;
  *_descent = 0.0;
  ...
  if (fromOffset >= limit) {
    ...
    if (fStyles->NumRuns() > 0) {
      fStyles->Iterate(fromOffset, 1, fInline, NULL, NULL, _ascent, _descent);
    } else {
  ...
  while (offset < limit && !done) {
    ...
    deltaWidth = _TabExpandedStyledWidth(offset, done ? delta - 1 : delta,
                                         &ascent, &descent);
    strWidth += deltaWidth;
  ...
  if (offset - fromOffset < 1) {
    ...
    *_descent = 0.0;
    ...
    for (offset = _NextInitialByte(current); current < limit;
      ...
      strWidth += _StyledWidth(current, offset - current, &ascent, &descent);
      if (strWidth >= *inOutWidth) {
...
int32 BTextView::_PreviousLineStart(int32 offset) {
  if (offset <= 0)
    return 0;
...
int32 BTextView::_NextLineEnd(int32 offset) {
...
int32 BTextView::_PreviousWordBoundary(int32 offset) {
...
int32 BTextView::_NextWordBoundary(int32 offset) {
  int32 textLen = fText->Length();
  uint32 charType = _CharClassification(offset);
  while (offset < textLen) {
    offset = _NextInitialByte(offset);
...
int32 BTextView::_PreviousWordStart(int32 offset) {
  if (offset <= 1)
    return 0;
...
int32 BTextView::_NextWordEnd(int32 offset) {
  int32 textLen = fText->Length();
  if (_CharClassification(offset) != CHAR_CLASS_DEFAULT) {
    // skip non-word characters
    while (offset < textLen) {
...
*/
float BTextView::_TabExpandedStyledWidth(int32 offset, int32 length,
                                         float *_ascent,
...
/*!	Calculate the width of the text within the given limits.
        ...
        \param _ascent A pointer to a float which will contain the maximum
   ascent. \param _descent A pointer to a float which will contain the maximum
   descent.

        \return The width for the text within the given limits.
*/
float BTextView::_StyledWidth(int32 fromOffset, int32 length, float *_ascent,
                              float *_descent) const {
  if (length == 0) {
  ...
  const BFont *font = NULL;
  int32 numBytes;
  while ((numBytes = fStyles->Iterate(fromOffset, length, fInline, &font, NULL,
                                      &ascent, &descent)) != 0) {
...
float BTextView::_ActualTabWidth(float location) const {
...
void BTextView::_DoInsertText(const char *text, int32 length, int32 offset,
                              const text_run_array *runs) {
  _CancelInputMethod();
...
void BTextView::_DoDeleteText(int32 fromOffset, int32 toOffset) { CALLED(); }

void BTextView::_DrawLine(BView *view, const int32 &lineNum,
  ...
  while ((numBytes =
              fStyles->Iterate(offset, length, fInline, &font, &color)) != 0) {
    view->SetFont(font);
...
void BTextView::_DrawLines(int32 startLine, int32 endLine, int32 startOffset,
                           bool erase) {
  if (!Window())
    return;
...
void BTextView::_RequestDrawLines(int32 startLine, int32 endLine) {
  if (!Window())
    return;
  ...
  BRect invalidRect(Bounds().left, from->origin + fTextRect.top, Bounds().right,
                    to != NULL ? to->origin + fTextRect.top : fTextRect.bottom);
  Invalidate(invalidRect);
  Window()->UpdateIfNeeded();
}

void BTextView::_DrawCaret(int32 offset, bool visible) {
  float lineHeight;
...
*/
void BTextView::_DragCaret(int32 offset) {
...
void BTextView::_StopMouseTracking() {
  delete fTrackingMouse;
  fTrackingMouse = NULL;
}

bool BTextView::_PerformMouseUp(BPoint where) {
  if (fTrackingMouse == NULL)
    return false;
...
bool BTextView::_PerformMouseMoved(BPoint where, uint32 code) {
  fWhere = where;
  ...
  case 3:
    ...
    if (currentOffset <= fTrackingMouse->anchor) {
      fTrackingMouse->selStart = (*fLines)[_LineAt(currentOffset)]->offset;
      ...
      fTrackingMouse->selEnd = (*fLines)[_LineAt(currentOffset) + 1]->offset;
    }
    break;
  ...
  default:
    ...
    if (currentOffset <= fTrackingMouse->anchor) {
      ...
      fTrackingMouse->selEnd =
          fTrackingMouse->shiftDown ? fSelEnd : fTrackingMouse->anchor;
    } else {
      fTrackingMouse->selStart =
          fTrackingMouse->shiftDown ? fSelStart : fTrackingMouse->anchor;
...
*/
void BTextView::_TrackMouse(BPoint where, const BMessage *message, bool force) {
  BRegion textRegion;
...
void BTextView::_TrackDrag(BPoint where) {
  CALLED();
  if (Bounds().Contains(where))
...
bool BTextView::_MessageDropped(BMessage *message, BPoint where,
                                BPoint offset) {
  ASSERT(message);

  void *from = NULL;
  bool internalDrop = false;
  if (message->FindPointer("be:originator", &from) == B_OK && from == this &&
      fSelEnd != fSelStart)
    internalDrop = true;
  ...
  if (message->FindData("text/plain", B_MIME_TYPE, (const void **)&text,
    ...
    ssize_t runLength = 0;
    if (fStylable) {
      message->FindData("application/x-vnd.Be-text_run_array", B_MIME_TYPE,
                        (const void **)&runArray, &runLength);
    }
    ...
    if (fUndo) {
      delete fUndo;
      fUndo = new DropUndoBuffer(this, text, dataLength, runArray, runLength,
                                 dropOffset, internalDrop);
    }
...
void BTextView::_PerformAutoScrolling() {
  ...
  if (CountLines() > 1) {
    ...
    // prevent from scrolling out of view
    if (scrollBy.y != 0.0) {
      float bottomMax = floorf(fTextRect.bottom + fLayoutData->bottomInset);
...
void BTextView::_UpdateScrollbars() {
  BRect bounds(Bounds());
  ...
  if (horizontalScrollBar != NULL) {
    ...
    long dataWidth =
        (long)ceilf(fTextRect.IntegerWidth() + fLayoutData->leftInset +
  ...
  if (verticalScrollBar != NULL) {
    ...
    long dataHeight =
        (long)ceilf(fTextRect.IntegerHeight() + fLayoutData->topInset +
...
void BTextView::_ScrollBy(float horizontal, float vertical) {
  BRect bounds = Bounds();
...
void BTextView::_ScrollTo(float x, float y) {
  BRect bounds = Bounds();
  long viewWidth = bounds.IntegerWidth();
  long viewHeight = bounds.IntegerHeight();
...
void BTextView::_AutoResize(bool redraw) {
  if (!fResizable)
    return;
...
void BTextView::_NewOffscreen(float padding) {
  if (fOffscreen != NULL)
    _DeleteOffscreen();
...
void BTextView::_DeleteOffscreen() {
  if (fOffscreen != NULL && fOffscreen->Lock()) {
    delete fOffscreen;
    fOffscreen = NULL;
  }
...
void BTextView::_Activate() {
  ...
  if (Window() != NULL) {
    ...
    if (!Window()->HasShortcut(B_LEFT_ARROW, B_COMMAND_KEY | B_SHIFT_KEY) &&
      ...
      Window()->AddShortcut(B_LEFT_ARROW, B_COMMAND_KEY | B_SHIFT_KEY, message,
                            this);

      message = new BMessage(kMsgNavigateArrow);
      message->AddInt32("key", B_RIGHT_ARROW);
      message->AddInt32("modifiers", B_COMMAND_KEY | B_SHIFT_KEY);
      Window()->AddShortcut(B_RIGHT_ARROW, B_COMMAND_KEY | B_SHIFT_KEY, message,
                            this);
    ...
    if (!Window()->HasShortcut(B_LEFT_ARROW, B_OPTION_KEY | B_SHIFT_KEY) &&
        !Window()->HasShortcut(B_RIGHT_ARROW, B_OPTION_KEY | B_SHIFT_KEY)) {
      message = new BMessage(kMsgNavigateArrow);
      message->AddInt32("key", B_LEFT_ARROW);
      message->AddInt32("modifiers", B_OPTION_KEY | B_SHIFT_KEY);
      Window()->AddShortcut(B_LEFT_ARROW, B_OPTION_KEY | B_SHIFT_KEY, message,
                            this);

      message = new BMessage(kMsgNavigateArrow);
      message->AddInt32("key", B_RIGHT_ARROW);
      message->AddInt32("modifiers", B_OPTION_KEY | B_SHIFT_KEY);
      Window()->AddShortcut(B_RIGHT_ARROW, B_OPTION_KEY | B_SHIFT_KEY, message,
                            this);
    ...
    if (!Window()->HasShortcut(B_UP_ARROW, B_OPTION_KEY | B_SHIFT_KEY) &&
        !Window()->HasShortcut(B_DOWN_ARROW, B_OPTION_KEY | B_SHIFT_KEY)) {
      message = new BMessage(kMsgNavigateArrow);
      message->AddInt32("key", B_UP_ARROW);
      message->AddInt32("modifiers", B_OPTION_KEY | B_SHIFT_KEY);
      Window()->AddShortcut(B_UP_ARROW, B_OPTION_KEY | B_SHIFT_KEY, message,
                            this);

      message = new BMessage(kMsgNavigateArrow);
      message->AddInt32("key", B_DOWN_ARROW);
      message->AddInt32("modifiers", B_OPTION_KEY | B_SHIFT_KEY);
      Window()->AddShortcut(B_DOWN_ARROW, B_OPTION_KEY | B_SHIFT_KEY, message,
                            this);
...
*/
void BTextView::_NormalizeFont(BFont *font) {
  if (font) {
...
void BTextView::_SetRunArray(int32 startOffset, int32 endOffset,
                             const text_run_array *runs) {
...
*/
uint32 BTextView::_CharClassification(int32 offset) const {
...
*/
int32 BTextView::_NextInitialByte(int32 offset) const {
  if (offset >= fText->Length())
    return offset;
...
*/
int32 BTextView::_PreviousInitialByte(int32 offset) const {
  if (offset <= 0)
    return 0;
...
bool BTextView::_GetProperty(BMessage *specifier, int32 form,
...
bool BTextView::_SetProperty(BMessage *specifier, int32 form,
                             const char *property, BMessage *reply) {
  CALLED();
  if (strcmp(property, "selection") == 0) {
    int32 index, range;
...
bool BTextView::_CountProperties(BMessage *specifier, int32 form,
                                 const char *property, BMessage *reply) {
  CALLED();
...
void BTextView::_HandleInputMethodChanged(BMessage *message) {
  // TODO: block input if not editable (Andrew)
  ASSERT(fInline != NULL);
...
/*!	Called when the object receives a \c B_INPUT_METHOD_LOCATION_REQUEST
        message.
*/
void BTextView::_HandleInputMethodLocationRequest() {
  ASSERT(fInline != NULL);
...
void BTextView::_CancelInputMethod() {
  if (!fInline)
    return;
...
/*!	Returns the line number of the character at the given \a offset.
        ...
        \return The line number of the character at the given \a offset as an
   int32.
*/
int32 BTextView::_LineAt(int32 offset) const {
...
*/
int32 BTextView::_LineAt(const BPoint &point) const {
  return fLines->PixelToLine(point.y - fTextRect.top);
}

/*!	Returns whether or not the given \a offset is on the empty line at the
   end of the buffer.
*/
bool BTextView::_IsOnEmptyLastLine(int32 offset) const {
  return (offset == TextLength() && offset > 0 &&
          fText->RealCharAt(offset - 1) == B_ENTER);
}

void BTextView::_ApplyStyleRange(int32 fromOffset, int32 toOffset, uint32 mode,
  ...
  fStyles->SetStyleRange(fromOffset, toOffset, fText->Length(), mode, font,
                         color);
}

float BTextView::_NullStyleHeight() const {
  const BFont *font = NULL;
  fStyles->GetNullStyle(&font, NULL);
...
void BTextView::_ShowContextMenu(BPoint where) {
  bool isRedo;
...
void BTextView::_FilterDisallowedChars(char *text, ssize_t &length,
                                       text_run_array *runArray) {
  if (!fDisallowedChars)
    return;
  ...
  if (runArray) {
    ...
    for (int i = 0; i < runArray->count; i++) {
      ...
      while (stringIndex < runArray->runs[i].offset && stringIndex < length) {
...
BTextView::TextTrackState::TextTrackState(BMessenger messenger)
    : clickOffset(0), shiftDown(false), anchor(0), selStart(0), selEnd(0),
      fRunner(NULL) {
  BMessage message(_PING_);
...
BTextView::TextTrackState::~TextTrackState() { delete fRunner; }

void BTextView::TextTrackState::SimulateMouseMovement(BTextView *textView) {
  BPoint where;
  uint32 buttons;
...
extern "C" void
B_IF_GCC_2(InvalidateLayout__9BTextViewb,
           _ZN9BTextView16InvalidateLayoutEb)(BTextView *view,
                                              bool descendants) {
  perform_data_layout_invalidated data;
