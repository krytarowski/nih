...
bool BMenuScroller::IsEnabled() const { return fEnabled; }

void BMenuScroller::SetEnabled(bool enabled) { fEnabled = enabled; }

//	#pragma mark -

UpperScroller::UpperScroller(BRect frame) : BMenuScroller(frame) {}
...
LowerScroller::LowerScroller(BRect frame) : BMenuScroller(frame) {}
...
void BMenuFrame::Draw(BRect updateRect) {
  if (fMenu != NULL && fMenu->CountItems() == 0) {
    ...
    SetHighColor(
        tint_color(ui_color(B_MENU_BACKGROUND_COLOR), B_DISABLED_LABEL_TINT));
    BPoint where((Bounds().Width() - fMenu->StringWidth(kEmptyMenuLabel)) / 2,
...
BMenuWindow::BMenuWindow(const char *name)
    ...
    : BWindow(BRect(0, 0, 0, 0), name, B_BORDERED_WINDOW_LOOK, kMenuWindowFeel,
      ...
      fMenu(NULL), fMenuFrame(NULL), fUpperScroller(NULL), fLowerScroller(NULL),
      fScrollStep(19) {
  SetSizeLimits(2, 10000, 2, 10000);
}

BMenuWindow::~BMenuWindow() { DetachMenu(); }
...
void BMenuWindow::AttachScrollers() {
  ...
  if (fUpperScroller == NULL) {
    fUpperScroller =
        new UpperScroller(BRect(0, 0, frame.right, kScrollerHeight - 1));
    AddChild(fUpperScroller);
  }

  if (fLowerScroller == NULL) {
    fLowerScroller = new LowerScroller(BRect(
        0, frame.bottom - kScrollerHeight + 1, frame.right, frame.bottom));
...
void BMenuWindow::SetSmallStep(float step) { fScrollStep = step; }
...
bool BMenuWindow::HasScrollers() const {
  return fMenuFrame != NULL && fUpperScroller != NULL && fLowerScroller != NULL;
}
