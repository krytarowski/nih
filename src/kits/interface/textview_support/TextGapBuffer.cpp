...
TextGapBuffer::TextGapBuffer()
    : fItemCount(0), fBuffer(NULL),
      fBufferCount(kTextGapBufferBlockSize + fItemCount), fGapIndex(fItemCount),
      fGapCount(fBufferCount - fGapIndex), fScratchBuffer(NULL),
      fScratchSize(0), fPasswordMode(false) {
...
void TextGapBuffer::InsertText(const char *inText, int32 inNumItems,
                               int32 inAtIndex) {
  if (inNumItems < 1)
    return;
...
bool TextGapBuffer::InsertText(BFile *file, int32 fileOffset, int32 inNumItems,
                               int32 inAtIndex) {
  off_t fileSize;

  if (file->GetSize(&fileSize) != B_OK || !file->IsReadable())
    return false;
...
bool TextGapBuffer::PasswordMode() const { return fPasswordMode; }

void TextGapBuffer::SetPasswordMode(bool state) { fPasswordMode = state; }
