...
struct BAbstractLayout::Proxy {
  ...
  Proxy(proxy_type type) : type(type) {}
  ...
  virtual status_t AddDataToArchive(BMessage *archive, bool ancestorHidden) = 0;
...
struct BAbstractLayout::DataProxy : Proxy {
  ...
  DataProxy()
      : Proxy(DATA_PROXY_TYPE), minSize(), maxSize(), preferredSize(),
        alignment(), frame(-1, -1, 0, 0), visible(true) {}

  BSize MinSize() const { return minSize; }

  void SetMinSize(const BSize &min) { minSize = min; }

  BSize MaxSize() const { return maxSize; }

  void SetMaxSize(const BSize &max) { maxSize = max; }

  BSize PreferredSize() const { return preferredSize; }

  void SetPreferredSize(const BSize &preferred) { preferredSize = preferred; }

  BAlignment Alignment() const { return this->alignment; }

  void SetAlignment(const BAlignment &align) { this->alignment = align; }
  ...
  bool IsVisible(bool) const { return visible; }
...
struct BAbstractLayout::ViewProxy : Proxy {
  ViewProxy(BView *target) : Proxy(VIEW_PROXY_TYPE), view(target) {}

  BSize MinSize() const { return view->ExplicitMinSize(); }

  void SetMinSize(const BSize &min) { view->SetExplicitMinSize(min); }

  BSize MaxSize() const { return view->ExplicitMaxSize(); }

  void SetMaxSize(const BSize &min) { view->SetExplicitMaxSize(min); }

  BSize PreferredSize() const { return view->ExplicitPreferredSize(); }

  void SetPreferredSize(const BSize &preferred) {
    view->SetExplicitPreferredSize(preferred);
  }

  BAlignment Alignment() const { return view->ExplicitAlignment(); }

  void SetAlignment(const BAlignment &alignment) {
    view->SetExplicitAlignment(alignment);
  }

  BRect Frame() const { return view->Frame(); }
  ...
  status_t RestoreDataFromArchive(const BMessage *archive) { return B_OK; }
...
BAbstractLayout::~BAbstractLayout() { delete fExplicitData; }
...
BSize BAbstractLayout::BaseMinSize() { return BSize(0, 0); }

BSize BAbstractLayout::BaseMaxSize() {
  return BSize(B_SIZE_UNLIMITED, B_SIZE_UNLIMITED);
}

BSize BAbstractLayout::BasePreferredSize() { return BSize(0, 0); }

BAlignment BAbstractLayout::BaseAlignment() {
  return BAlignment(B_ALIGN_USE_FULL_WIDTH, B_ALIGN_USE_FULL_HEIGHT);
}

BRect BAbstractLayout::Frame() { return fExplicitData->Frame(); }
...
status_t BAbstractLayout::ItemUnarchived(const BMessage *from,
...
void BAbstractLayout::AttachedToLayout() { BLayout::AttachedToLayout(); }
