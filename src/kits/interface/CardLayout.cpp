...
BCardLayout::BCardLayout()
    : BAbstractLayout(), fMin(0, 0), fMax(B_SIZE_UNLIMITED, B_SIZE_UNLIMITED),
      fPreferred(0, 0), fVisibleItem(NULL), fMinMaxValid(false) {}

BCardLayout::BCardLayout(BMessage *from)
    : BAbstractLayout(BUnarchiver::PrepareArchive(from)), fMin(0, 0),
      fMax(B_SIZE_UNLIMITED, B_SIZE_UNLIMITED), fPreferred(0, 0),
      fVisibleItem(NULL), fMinMaxValid(false) {
  BUnarchiver(from).Finish();
}

BCardLayout::~BCardLayout() {}

BLayoutItem *BCardLayout::VisibleItem() const { return fVisibleItem; }

int32 BCardLayout::VisibleIndex() const { return IndexOfItem(fVisibleItem); }

void BCardLayout::SetVisibleItem(int32 index) { SetVisibleItem(ItemAt(index)); }
...
void BCardLayout::LayoutInvalidated(bool children) { fMinMaxValid = false; }
...
status_t BCardLayout::ItemArchived(BMessage *into, BLayoutItem *item,
                                   int32 index) const {
...
BArchivable *BCardLayout::Instantiate(BMessage *from) {
