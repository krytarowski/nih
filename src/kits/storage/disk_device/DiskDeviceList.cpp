...
BDiskDeviceList::BDiskDeviceList(bool useOwnLocker)
    : fLocker(NULL), fDevices(20, true), fSubscribed(false) {
  if (useOwnLocker)
...
*/
BDiskDeviceList::~BDiskDeviceList() { delete fLocker; }
...
*/
int32 BDiskDeviceList::CountDevices() const { return fDevices.CountItems(); }
...
*/
BDiskDevice *BDiskDeviceList::DeviceAt(int32 index) const {
...
/*!	\brief Iterates through the all devices in the list.
        ...
        \return The respective device, if the iteration was terminated early,
                        \c NULL otherwise.
*/
BDiskDevice *BDiskDeviceList::VisitEachDevice(BDiskDeviceVisitor *visitor) {
  if (visitor) {
...
/*!	\brief Iterates through the all devices' partitions.
        ...
        \return The respective partition, if the iteration was terminated early,
                        \c NULL otherwise.
*/
BPartition *BDiskDeviceList::VisitEachPartition(BDiskDeviceVisitor *visitor) {
  if (visitor) {
    for (int32 i = 0; BDiskDevice *device = DeviceAt(i); i++) {
...
*/
BDiskDevice *BDiskDeviceList::DeviceWithID(int32 id) const {
...
*/
BPartition *BDiskDeviceList::PartitionWithID(int32 id) const {
  IDFinderVisitor visitor(id);
...
/*!	\brief Catch-all method invoked by the \c Partition*() hooks, save by
        ...
        If you're interested only in the fact, that something about the
   partition changed, you can just override this hook instead of the ones
   telling you exactly what happened.

        \param partition The concerned partition.
        \param event The event that occurred, if you are interested in it after
   all.
*/
...
// _FindDevice
/*!	\brief Returns the device for the ID contained in a motification
   message. \param message The notification message. \return The device with the
   ID, or \c NULL, if the ID or the device could not be found.
*/
BDiskDevice *BDiskDeviceList::_FindDevice(BMessage *message) {
...
/*!	\brief Returns the partition for the ID contained in a motification
        ...
        \return The partition with the ID, or \c NULL, if the ID or the
   partition could not be found.*/
BPartition *BDiskDeviceList::_FindPartition(BMessage *message) {
  BPartition *partition = NULL;
  int32 id;
...
/*!	\brief Finds the device for the ID contained in a motification message
                   and updates it.
        \param message The notification message.
        \return The device with the ID, or \c NULL, if the ID or the device
   could not be found.
*/
BDiskDevice *BDiskDeviceList::_UpdateDevice(BMessage *message) {
