...
/*!	\class BDiskDeviceRoster
        \brief An interface for iterating through the disk devices known to the
                   system and for a notification mechanism provided to listen to
   their changes.
*/

/*!	\brief find_directory constants of the add-on dirs to be searched. */
static const directory_which kAddOnDirs[] = {
    B_USER_NONPACKAGED_ADDONS_DIRECTORY, B_USER_ADDONS_DIRECTORY,
    B_SYSTEM_NONPACKAGED_ADDONS_DIRECTORY, B_SYSTEM_ADDONS_DIRECTORY,
};
...
BDiskDeviceRoster::BDiskDeviceRoster()
    : fDeviceCookie(0), fDiskSystemCookie(0), fJobCookie(0)
...
/*!	\brief Returns the next BDiskDevice.
   ...
   to represent the next device. \return - \c B_OK: Everything went fine. - \c
   B_ENTRY_NOT_FOUND: The end of the list of devices had already been reached.
        - another error code
*/
status_t BDiskDeviceRoster::GetNextDevice(BDiskDevice *device) {
  ...
  size_t neededSize = 0;
  partition_id id = _kern_get_next_disk_device_id(&fDeviceCookie, &neededSize);
  if (id < 0)
    return id;
...
status_t BDiskDeviceRoster::GetNextDiskSystem(BDiskSystem *system) {
  ...
  status_t error = _kern_get_next_disk_system_info(&fDiskSystemCookie, &info);
  if (error == B_OK)
...
status_t BDiskDeviceRoster::GetDiskSystem(BDiskSystem *system,
                                          const char *name) {
  if (!system)
    return B_BAD_VALUE;
  ...
  while (_kern_get_next_disk_system_info(&cookie, &info) == B_OK) {
    if (!strcmp(name, info.name) || !strcmp(name, info.short_name) ||
...
partition_id BDiskDeviceRoster::RegisterFileDevice(const char *filename) {
  if (!filename)
    return B_BAD_VALUE;
...
/*!	\brief Pre-order traverses the trees spanned by the BDiskDevices and
   their subobjects.
...
status_t BDiskDeviceRoster::FindPartitionByVolume(const BVolume &volume,
  ...
  public:
    FindPartitionVisitor(dev_t volume) : fVolume(volume) {}
...
/*!	\brief Returns a BDiskDevice for a given ID.
        ...
        - other error codes
*/
status_t BDiskDeviceRoster::GetDeviceWithID(int32 id,
...
status_t BDiskDeviceRoster::GetDeviceForPath(const char *filename,
                                             BDiskDevice *device) {
  if (!filename || !device)
    return B_BAD_VALUE;
...
/*!	\brief Adds a target to the list of targets to be notified on disk
   device events.

        \todo List the event mask flags, the events and describe the layout of
