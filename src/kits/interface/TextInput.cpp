...
_BTextInput_::_BTextInput_(BMessage *archive)
    : BTextView(archive), fPreviousText(NULL) {
  MakeResizable(true);
}

_BTextInput_::~_BTextInput_() { free(fPreviousText); }
...
void _BTextInput_::InsertText(const char *inText, int32 inLength,
  ...
  if (inLength == 1) {
    ...
    BTextView::InsertText(filteredText.String(), inLength, inOffset, inRuns);
  }
