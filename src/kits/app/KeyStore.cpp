...
status_t BKeyStore::GetKey(const char *keyring, BKeyType type,
...
status_t BKeyStore::AddKey(const BKey &key) { return AddKey(NULL, key); }
...
status_t BKeyStore::RemoveKey(const BKey &key) { return RemoveKey(NULL, key); }
...
status_t BKeyStore::GetNextKey(BKeyType type, BKeyPurpose purpose,
...
status_t BKeyStore::GetNextKey(const char *keyring, BKeyType type,
...
status_t BKeyStore::RemoveMasterUnlockKey() { return RemoveUnlockKey(NULL); }
...
status_t BKeyStore::LockMasterKeyring() { return LockKeyring(NULL); }
...
status_t BKeyStore::GeneratePassword(BPasswordKey &password, size_t length,
                                     uint32 flags) {
  return B_ERROR;
}

float BKeyStore::PasswordStrength(const char *password) { return 0; }
