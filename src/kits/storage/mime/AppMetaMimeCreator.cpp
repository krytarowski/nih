...
status_t AppMetaMimeCreator::Do(const entry_ref &entry, bool *_entryIsDir) {
  ...
  if (status == B_OK &&
      (fForce || typeNode.GetAttrInfo(kShortDescriptionAttr, &info) != B_OK)) {
    AutoLocker<DatabaseLocker> databaseLocker(fDatabaseLocker);
  ...
  if (status == B_OK &&
      (fForce || typeNode.GetAttrInfo(kSupportedTypesAttr, &info) != B_OK)) {
  ...
  for (int32 i = 0; supportedTypes.FindString("types", i, &type) == B_OK; i++) {
    ...
    if (status == B_OK &&
      ...
      AutoLocker<DatabaseLocker> databaseLocker(fDatabaseLocker);
      status =
          fDatabase->SetIconForType(signature, type, &miniIcon, B_MINI_ICON);
    }
    // large icon
    if (status == B_OK &&
      ...
      AutoLocker<DatabaseLocker> databaseLocker(fDatabaseLocker);
      status =
          fDatabase->SetIconForType(signature, type, &largeIcon, B_LARGE_ICON);
    }
  }
