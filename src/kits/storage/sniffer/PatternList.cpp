...
PatternList::PatternList(Range range) : DisjList(), fRange(range) {}
...
status_t PatternList::InitCheck() const { return fRange.InitCheck(); }

Err *PatternList::GetErr() const { return fRange.GetErr(); }
...
*/
ssize_t PatternList::BytesNeeded() const {
  ...
  if (result == B_OK) {
    result = 0; // I realize it already *is* zero if it == B_OK, but just in
                // case that changes...
    std::vector<Pattern *>::const_iterator i;
    for (i = fList.begin(); i != fList.end(); i++) {
      if (*i) {
