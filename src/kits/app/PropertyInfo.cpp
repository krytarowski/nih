...
BPropertyInfo::BPropertyInfo(property_info *propertyInfo, value_info *valueInfo,
                             bool freeOnDelete)
    : fPropInfo(propertyInfo), fValueInfo(valueInfo), fPropCount(0),
      fInHeap(freeOnDelete), fValueCount(0) {
  if (fPropInfo != NULL) {
...
BPropertyInfo::~BPropertyInfo() { FreeMem(); }

int32 BPropertyInfo::FindMatch(BMessage *message, int32 index,
                               BMessage *specifier, int32 form,
                               const char *property, void *data) const {
...
bool BPropertyInfo::IsFixedSize() const { return false; }

type_code BPropertyInfo::TypeCode() const { return B_PROPERTY_INFO_TYPE; }
...
ssize_t BPropertyInfo::FlattenedSize() const {
  ...
  if (fPropInfo) {
    ...
    for (int32 pi = 0; fPropInfo[pi].name != NULL; pi++) {
      ...
      for (int32 i = 0; i < 3 && fPropInfo[pi].ctypes[i].pairs[0].name != 0;
           i++) {
        for (int32 j = 0; j < 5 && fPropInfo[pi].ctypes[i].pairs[j].name != 0;
             j++) {
...
status_t BPropertyInfo::Unflatten(type_code code, const void *buffer,
  ...
  if (flags & 1) {
    fPropInfo = static_cast<property_info *>(
        malloc(sizeof(property_info) * (fPropCount + 1)));
      ...
      for (int32 i = 0; tmpInt != 0; i++) {
        for (int32 j = 0; tmpInt != 0; j++) {
          ...
          fPropInfo[pi].ctypes[i].pairs[j].name =
              strdup(static_cast<const char *>(buffer) + flatData.Position());
          flatData.Seek(strlen(fPropInfo[pi].ctypes[i].pairs[j].name) + 1,
                        SEEK_CUR);
  ...
  if (flags & 2) {
    ...
    fValueInfo = static_cast<value_info *>(
        malloc(sizeof(value_info) * (fValueCount + 1)));
    ...
    for (int32 vi = 0; vi < fValueCount; vi++) {
      ...
      if (swapRequired) {
        fValueInfo[vi].kind =
            static_cast<value_kind>(B_SWAP_INT32(fValueInfo[vi].kind));
...
const property_info *BPropertyInfo::Properties() const { return fPropInfo; }

const value_info *BPropertyInfo::Values() const { return fValueInfo; }

int32 BPropertyInfo::CountProperties() const { return fPropCount; }

int32 BPropertyInfo::CountValues() const { return fValueCount; }
...
BPropertyInfo &BPropertyInfo::operator=(const BPropertyInfo &) { return *this; }
