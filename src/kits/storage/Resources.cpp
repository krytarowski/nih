...
BResources::BResources()
    : fFile(), fContainer(NULL), fResourceFile(NULL), fReadOnly(false) {
...
const BFile &BResources::File() const { return fFile; }
...
const void *BResources::LoadResource(type_code type, const char *name,
                                     size_t *_size) {
  // find the resource
  status_t error = InitCheck();
  ResourceItem *resource = NULL;
...
bool BResources::GetResourceInfo(type_code byType, int32 andIndex,
                                 int32 *idFound, const char **nameFound,
                                 size_t *lengthFound) {
  ResourceItem *item = NULL;
  if (InitCheck() == B_OK) {
    item = fContainer->ResourceAt(fContainer->IndexOfType(byType, andIndex));
  }
  if (item) {
    if (idFound)
      *idFound = item->ID();
...
status_t BResources::ReadResource(type_code type, int32 id, void *data,
                                  off_t offset, size_t length) {
  status_t error = (data && offset >= 0 ? B_OK : B_BAD_VALUE);
  if (error == B_OK)
    error = InitCheck();
...
// its data (deprecated, use LoadResource() instead).
void *BResources::FindResource(type_code type, const char *name,
                               size_t *lengthFound) {
  void *result = NULL;
  size_t size = 0;
