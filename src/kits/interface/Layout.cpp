...
const uint32 B_RELAYOUT_NOT_OK = B_LAYOUT_INVALID | B_LAYOUT_IN_PROGRESS;
...
BLayout::BLayout()
    : fState(B_LAYOUT_ALL_CLEAR), fAncestorsVisible(true),
      fInvalidationDisabled(0), fContext(NULL), fOwner(NULL), fTarget(NULL),
      fItems(20) {}
...
BView *BLayout::Owner() const { return fOwner; }

BView *BLayout::TargetView() const { return fTarget; }

BView *BLayout::View() { return fOwner; }

BLayoutItem *BLayout::AddView(BView *child) { return AddView(-1, child); }
...
bool BLayout::AddItem(BLayoutItem *item) { return AddItem(-1, item); }
...
int32 BLayout::CountItems() const { return fItems.CountItems(); }
...
bool BLayout::AncestorsVisible() const { return fAncestorsVisible; }
...
void BLayout::RequireLayout() { fState |= B_LAYOUT_REQUIRED; }

bool BLayout::IsValid() { return (fState & B_LAYOUT_INVALID) == 0; }

void BLayout::DisableLayoutInvalidation() { fInvalidationDisabled++; }
...
void BLayout::_LayoutWithinContext(bool force, BLayoutContext *context) {
  // printf("BLayout(%p)::_LayoutWithinContext(%i, %p), state %x, fContext
...
status_t BLayout::AllUnarchived(const BMessage *from) {
  ...
  for (int32 i = 0; i < itemCount && err == B_OK; i++) {
    BLayoutItem *item;
    err = unarchiver.FindObject(kLayoutItemField, i,
                                BUnarchiver::B_DONT_ASSUME_OWNERSHIP, item);
    if (err != B_OK)
      return err;
...
status_t BLayout::ItemArchived(BMessage *into, BLayoutItem *item,
                               int32 index) const {
  return B_OK;
}

status_t BLayout::ItemUnarchived(const BMessage *from, BLayoutItem *item,
                                 int32 index) {
  return B_OK;
}

bool BLayout::ItemAdded(BLayoutItem *item, int32 atIndex) { return true; }
...
void BLayout::ResetLayoutInvalidation() { fState &= ~B_LAYOUT_CACHE_INVALID; }

BLayoutContext *BLayout::LayoutContext() const { return fContext; }
