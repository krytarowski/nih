...
#define CALLED(x...)                                                           \
  FunctionTracer _ft("BMenuField", __FUNCTION__, sFunctionDepth)
#define TRACE(x...)                                                            \
  {                                                                            \
    BString _to;                                                               \
...
struct BMenuField::LayoutData {
  LayoutData()
      : label_layout_item(NULL), menu_bar_layout_item(NULL),
        previous_height(-1), valid(false) {}
...
status_t BMenuField::AllArchived(BMessage *into) const {
  status_t err;
...
status_t BMenuField::AllUnarchived(const BMessage *from) {
  ...
  if (unarchiver.IsInstantiated(kLabelItemField)) {
    ...
    err = unarchiver.FindObject(
        kLabelItemField, BUnarchiver::B_DONT_ASSUME_OWNERSHIP, labelItem);
...
void BMenuField::MouseDown(BPoint where) {
  ...
  fMenuTaskID = spawn_thread((thread_func)_thread_entry, "_m_task_",
...
void BMenuField::MakeFocus(bool focused) {
  if (IsFocus() == focused)
    return;
...
void BMenuField::MessageReceived(BMessage *message) {
  BView::MessageReceived(message);
}

void BMenuField::WindowActivated(bool active) {
...
void BMenuField::MouseMoved(BPoint point, uint32 code,
                            const BMessage *message) {
  BView::MouseMoved(point, code, message);
}

void BMenuField::MouseUp(BPoint where) { BView::MouseUp(where); }

void BMenuField::DetachedFromWindow() { BView::DetachedFromWindow(); }

void BMenuField::AllDetached() { BView::AllDetached(); }

void BMenuField::FrameMoved(BPoint newPosition) {
  BView::FrameMoved(newPosition);
}

void BMenuField::FrameResized(float newWidth, float newHeight) {
...
BMenu *BMenuField::Menu() const { return fMenu; }

BMenuBar *BMenuField::MenuBar() const { return fMenuBar; }

BMenuItem *BMenuField::MenuItem() const { return fMenuBar->ItemAt(0); }
...
const char *BMenuField::Label() const { return fLabel; }
...
bool BMenuField::IsEnabled() const { return fEnabled; }

void BMenuField::SetAlignment(alignment label) { fAlign = label; }

alignment BMenuField::Alignment() const { return fAlign; }

void BMenuField::SetDivider(float position) {
...
float BMenuField::Divider() const { return fDivider; }
...
BHandler *BMenuField::ResolveSpecifier(BMessage *message, int32 index,
...
void BMenuField::GetPreferredSize(float *_width, float *_height) {
  CALLED();
...
BSize BMenuField::PreferredSize() {
  CALLED();
...
BLayoutItem *BMenuField::CreateLabelLayoutItem() {
...
BLayoutItem *BMenuField::CreateMenuBarLayoutItem() {
...
status_t BMenuField::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BMenuField::MinSize();
    return B_OK;

  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BMenuField::MaxSize();
    return B_OK;
...
void BMenuField::LayoutInvalidated(bool descendants) {
  CALLED();

  fLayoutData->valid = false;
}

void BMenuField::DoLayout() {
  ...
  if (fLayoutData->label_layout_item != NULL &&
    ...
    divider = fLayoutData->label_width + be_control_look->DefaultLabelSpacing();
  }
...
void BMenuField::InitObject2() {
  ...
  TRACE("frame(%.1f, %.1f, %.1f, %.1f) (%.2f, %.2f)\n", fMenuBar->Frame().left,
        fMenuBar->Frame().top, fMenuBar->Frame().right,
        fMenuBar->Frame().bottom, fMenuBar->Frame().Width(),
        fMenuBar->Frame().Height());

  fMenuBar->AddFilter(new _BMCFilter_(this, B_MOUSE_DOWN));
}

void BMenuField::_DrawLabel(BRect updateRect) {
  CALLED();
...
void BMenuField::_DrawMenuBar(BRect updateRect) {
  CALLED();
...
/*static*/ int32 BMenuField::_thread_entry(void *arg) {
  return static_cast<BMenuField *>(arg)->_MenuTask();
}

int32 BMenuField::_MenuTask() {
  if (!LockLooper())
    return 0;
...
void BMenuField::_InitMenuBar(BMenu *menu, BRect frame, bool fixedSize) {
  CALLED();
  ...
  if ((Flags() & B_SUPPORTS_LAYOUT) != 0) {
    ...
    TRACE("frame(%.1f, %.1f, %.1f, %.1f) (%.2f, %.2f)\n", frame.left, frame.top,
          frame.right, frame.bottom, frame.Width(), frame.Height());
...
void BMenuField::_InitMenuBar(const BMessage *archive) {
  bool fixed;
...
void BMenuField::_AddMenu(BMenu *menu) {
...
void BMenuField::_ValidateLayoutData() {
  CALLED();
...
float BMenuField::_MenuBarOffset() const {
  return std::max(fDivider + kVMargin, kVMargin);
}

float BMenuField::_MenuBarWidth() const {
  return Bounds().Width() - (_MenuBarOffset() + kVMargin);
}

void BMenuField::_DoneTracking(BPoint point) {
  Window()->RemoveCommonFilter(fMouseDownFilter);
}

void BMenuField::_Track(BPoint point, uint32) {}

// #pragma mark - BMenuField::LabelLayoutItem

BMenuField::LabelLayoutItem::LabelLayoutItem(BMenuField *parent)
    : fParent(parent), fFrame() {}

BMenuField::LabelLayoutItem::LabelLayoutItem(BMessage *from)
    : BAbstractLayoutItem(from), fParent(NULL), fFrame() {
  from->FindRect(kFrameField, &fFrame);
}

BRect BMenuField::LabelLayoutItem::FrameInParent() const {
  return fFrame.OffsetByCopy(-fParent->Frame().left, -fParent->Frame().top);
}

bool BMenuField::LabelLayoutItem::IsVisible() {
  return !fParent->IsHidden(fParent);
}

void BMenuField::LabelLayoutItem::SetVisible(bool visible) {
  // not allowed
}

BRect BMenuField::LabelLayoutItem::Frame() { return fFrame; }

void BMenuField::LabelLayoutItem::SetFrame(BRect frame) {
  fFrame = frame;
  fParent->_UpdateFrame();
}

void BMenuField::LabelLayoutItem::SetParent(BMenuField *parent) {
  fParent = parent;
}

BView *BMenuField::LabelLayoutItem::View() { return fParent; }

BSize BMenuField::LabelLayoutItem::BaseMinSize() {
...
BSize BMenuField::LabelLayoutItem::BaseMaxSize() { return BaseMinSize(); }

BSize BMenuField::LabelLayoutItem::BasePreferredSize() { return BaseMinSize(); }

BAlignment BMenuField::LabelLayoutItem::BaseAlignment() {
  return BAlignment(B_ALIGN_USE_FULL_WIDTH, B_ALIGN_USE_FULL_HEIGHT);
}

status_t BMenuField::LabelLayoutItem::Archive(BMessage *into, bool deep) const {
  BArchiver archiver(into);
...
BArchivable *BMenuField::LabelLayoutItem::Instantiate(BMessage *from) {
...
BRect BMenuField::MenuBarLayoutItem::FrameInParent() const {
  return fFrame.OffsetByCopy(-fParent->Frame().left, -fParent->Frame().top);
}

bool BMenuField::MenuBarLayoutItem::IsVisible() {
  return !fParent->IsHidden(fParent);
}

void BMenuField::MenuBarLayoutItem::SetVisible(bool visible) {
  // not allowed
}

BRect BMenuField::MenuBarLayoutItem::Frame() { return fFrame; }

void BMenuField::MenuBarLayoutItem::SetFrame(BRect frame) {
  fFrame = frame;
  fParent->_UpdateFrame();
}

void BMenuField::MenuBarLayoutItem::SetParent(BMenuField *parent) {
  fParent = parent;
}

BView *BMenuField::MenuBarLayoutItem::View() { return fParent; }

BSize BMenuField::MenuBarLayoutItem::BaseMinSize() {
  fParent->_ValidateLayoutData();
...
BSize BMenuField::MenuBarLayoutItem::BaseMaxSize() {
...
BSize BMenuField::MenuBarLayoutItem::BasePreferredSize() {
  return BaseMinSize();
}

BAlignment BMenuField::MenuBarLayoutItem::BaseAlignment() {
  return BAlignment(B_ALIGN_USE_FULL_WIDTH, B_ALIGN_USE_FULL_HEIGHT);
}

status_t BMenuField::MenuBarLayoutItem::Archive(BMessage *into,
                                                bool deep) const {
  BArchiver archiver(into);
  status_t err = BAbstractLayoutItem::Archive(into, deep);
...
BArchivable *BMenuField::MenuBarLayoutItem::Instantiate(BMessage *from) {
...
B_IF_GCC_2(InvalidateLayout__10BMenuFieldb,
           _ZN10BMenuField16InvalidateLayoutEb)(BMenuField *field,
                                                bool descendants) {
  perform_data_layout_invalidated data;
