...
PartitioningDialog::PartitioningDialog(BRect dialogCenter)
    : BWindow(BRect(100, 100, 100, 100), "Partitioning parameters",
      ...
      fEditor(NULL), fEditorView(NULL), fBlocker(-1), fCancelled(NULL),
      fCenter() {
...
// Go
status_t PartitioningDialog::Go(BDiskScannerParameterEditor *editor,
                                bool *_cancelled) {
...
status_t PartitioningDialog::_Init(BDiskScannerParameterEditor *editor) {
  ...
  if (error == B_OK) {
    BView *mainView =
        new (nothrow) BView(BRect(0, 0, 1, 1), "main", B_FOLLOW_ALL, 0);
    BMessage *okMessage = new BMessage(MSG_OK);
    BMessage *cancelMessage = new BMessage(MSG_CANCEL);
    BButton *okButton =
        new (nothrow) BButton(BRect(0, 0, 1, 1), "ok", "OK", okMessage,
                              B_FOLLOW_RIGHT | B_FOLLOW_BOTTOM);
    BButton *cancelButton =
        new (nothrow) BButton(BRect(0, 0, 1, 1), "cancel", "Cancel",
                              cancelMessage, B_FOLLOW_RIGHT | B_FOLLOW_BOTTOM);
    if (okMessage && cancelMessage && okButton && cancelButton) {
      ...
      int32 height = editorRect.IntegerHeight() + 1 +
                     max(okRect.IntegerHeight(), cancelRect.IntegerHeight()) +
                     1 + 3 * vSpacing;
