...
// helper function
static inline void move_items(void **items, int32 offset, int32 count) {
  if (count > 0 && offset != 0)
    memmove(items + offset, items, count * sizeof(void *));
}

BList::BList(int32 count)
    : fObjectList(NULL), fPhysicalSize(0), fItemCount(0), fBlockSize(count),
      fResizeThreshold(0) {
  if (fBlockSize <= 0)
    fBlockSize = 1;
...
BList::BList(const BList &other)
    : fObjectList(NULL), fPhysicalSize(0), fItemCount(0),
      fBlockSize(other.fBlockSize) {
  *this = other;
}

BList::~BList() { free(fObjectList); }
...
bool BList::operator!=(const BList &other) const { return !(*this == other); }
...
bool BList::ReplaceItem(int32 index, void *item) {
  bool result = false;
...
bool BList::SwapItems(int32 indexA, int32 indexB) {
  ...
  if (indexA >= 0 && indexA < fItemCount && indexB >= 0 &&
      indexB < fItemCount) {
...
/*! This moves a list item from posititon a to position b, moving the
        ...
        A C D E F G B H I J
*/
bool BList::MoveItem(int32 from, int32 to) {
...
void *BList::ItemAtFast(int32 index) const { return fObjectList[index]; }

void *BList::Items() const { return fObjectList; }
...
bool BList::HasItem(void *item) const { return (IndexOf(item) >= 0); }

bool BList::HasItem(const void *item) const { return (IndexOf(item) >= 0); }
...
int32 BList::CountItems() const { return fItemCount; }

bool BList::IsEmpty() const { return fItemCount == 0; }
...
bool BList::_ResizeArray(int32 count) {
  ...
  if (newSize != fPhysicalSize) {
    ...
    if (newObjectList) {
      ...
      fResizeThreshold =
          fPhysicalSize >> 2 >= fBlockSize ? fPhysicalSize >> 2 : 0;
    } else
      result = false;
  }
