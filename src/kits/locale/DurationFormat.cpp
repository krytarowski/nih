...
static const UCalendarDateFields skUnitMap[] = {
    UCAL_YEAR,        UCAL_MONTH,  UCAL_WEEK_OF_MONTH, UCAL_DAY_OF_WEEK,
...
BDurationFormat::BDurationFormat(const BLanguage &language,
    ...
    : Inherited(language, conventions), fSeparator(separator),
...
BDurationFormat::BDurationFormat(const BString &separator)
    : Inherited(), fSeparator(separator), fTimeUnitFormat() {
  UErrorCode icuStatus = U_ZERO_ERROR;
  fCalendar = new GregorianCalendar(icuStatus);
  if (fCalendar == NULL) {
    fInitStatus = B_NO_MEMORY;
...
BDurationFormat::BDurationFormat(const BDurationFormat &other)
    : Inherited(other), fSeparator(other.fSeparator),
      fTimeUnitFormat(other.fTimeUnitFormat),
      fCalendar(other.fCalendar != NULL
                    ? new GregorianCalendar(*other.fCalendar)
                    : NULL) {
  if (fCalendar == NULL && other.fCalendar != NULL)
    fInitStatus = B_NO_MEMORY;
}

BDurationFormat::~BDurationFormat() { delete fCalendar; }
...
status_t BDurationFormat::Format(BString &buffer, const bigtime_t startValue,
  ...
  for (int unit = 0; unit <= B_TIME_UNIT_LAST; ++unit) {
    int delta = fCalendar->fieldDifference(stop, skUnitMap[unit], icuStatus);
    if (!U_SUCCESS(icuStatus))
      return B_ERROR;
    ...
    if (delta != 0) {
      if (needSeparator)
        ...
        needSeparator = true;
      status_t status =
          fTimeUnitFormat.Format(buffer, delta, (time_unit_element)unit, style);
      if (status != B_OK)
        return status;
    }
  }
