...
public:
  MyLayoutInfo(int32 elementCount, int32 spacing)
      : fCount(elementCount), fSpacing(spacing) {
  ...
  ~MyLayoutInfo() { delete[] fLocations; }
  ...
  virtual float ElementSize(int32 element) {
    ...
    return fLocations[element + 1] - fLocations[element] - 1 - fSpacing;
  }
  ...
  virtual float ElementRangeSize(int32 position, int32 length) {
    ...
    return fLocations[position + length] - fLocations[position] - 1 - fSpacing;
  }
...
struct ComplexLayouter::Constraint {
  Constraint(int32 start, int32 end, int32 min, int32 max)
      : start(start), end(end), min(min), max(max), next(NULL) {
    if (min > max)
      max = min;
    effectiveMax = max;
  }
...
ComplexLayouter::ComplexLayouter(int32 elementCount, float spacing)
    : fElementCount(elementCount), fSpacing((int32)spacing),
      ...
      fMinMaxValid(false), fOptimizerConstraintsAdded(false) {
  if (fConstraints)
...
// AddConstraints
void ComplexLayouter::AddConstraints(int32 element, int32 length, float _min,
                                     float _max, float _preferred) {
  if (element < 0 || length <= 0 || element + length > fElementCount)
    return;

  TRACE("%p->ComplexLayouter::AddConstraints(%ld, %ld, %ld, %ld, %ld)\n", this,
        element, length, (int32)_min, (int32)_max, (int32)_preferred);
  ...
  if (*slot != NULL && (*slot)->start == element) {
    ...
    Constraint *constraint = new (nothrow) Constraint(element, end, min, max);
    if (!constraint)
      return;
...
// MinSize
float ComplexLayouter::MinSize() {
  _ValidateLayout();
  return fMin;
}

// MaxSize
float ComplexLayouter::MaxSize() {
  _ValidateLayout();
  return fMax;
}

// PreferredSize
float ComplexLayouter::PreferredSize() { return fMin; }

// CreateLayoutInfo
LayoutInfo *ComplexLayouter::CreateLayoutInfo() {
  MyLayoutInfo *layoutInfo =
      new (nothrow) MyLayoutInfo(fElementCount, fSpacing);
...
// CloneLayouter
Layouter *ComplexLayouter::CloneLayouter() {
      ...
      if (!minRedundant || !maxRedundant) {
        ...
        if (constraint->min == constraint->effectiveMax) {
          ...
          success = fOptimizer->AddConstraint(
              constraint->start - 1, constraint->end, constraint->min, true);
        } else {
          // min and max not equal -- add them individually,
          // unless redundant
          if (!minRedundant) {
            success |= fOptimizer->AddConstraint(
                constraint->start - 1, constraint->end, constraint->min, false);
          }
          if (!maxRedundant) {
            success |= fOptimizer->AddConstraint(
...
// _ApplyMaxConstraint
void ComplexLayouter::_ApplyMaxConstraint(Constraint *currentConstraint,
                                          int32 index) {
