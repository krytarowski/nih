...
/*!	This file implements the default catalog-type for the opentracker locale
   ...
   this is provided as part of liblocale.so.
*/
...
/*!	Constructs a DefaultCatalog with given signature and language and reads
   ...
   will give an appropriate error-code otherwise.
*/
DefaultCatalog::DefaultCatalog(const entry_ref &catalogOwner,
  ...
  catalogName << kCatFolder << "/" << fSignature << "/" << fLanguageName
              << kCatExtension;
  ...
  if (status != B_OK) {
    ...
    for (size_t i = 0; i < sizeof(which) / sizeof(which[0]); i++) {
      ...
      if (find_directory(which[i], &path) == B_OK) {
        ...
        catalogName << "/locale/" << kCatFolder << "/" << fSignature << "/"
                    << fLanguageName << kCatExtension;
...
DefaultCatalog::DefaultCatalog(const char *path, const char *signature,
                               const char *language)
    : HashMapCatalog(signature, language, 0), fPath(path) {
  fInitCheck = B_OK;
}
...
status_t DefaultCatalog::SetRawString(const CatKey &key,
...
status_t DefaultCatalog::WriteToResource(const entry_ref &appOrAddOnRef) {
  ...
  if (res == B_OK) {
    res = rsrc.AddResource('CADA', mangledLanguage, mallocIO.Buffer(),
                           mallocIO.BufferLength(), BString(fLanguageName));
  }
...
void DefaultCatalog::UpdateAttributes(BFile &catalogFile) {
  ...
  if (catalogFile.ReadAttr("BEOS:TYPE", B_MIME_STRING_TYPE, 0, &buf, bufSize) <=
    ...
    catalogFile.WriteAttr("BEOS:TYPE", B_MIME_STRING_TYPE, 0, kCatMimeType,
                          strlen(kCatMimeType) + 1);
  }
  if (catalogFile.ReadAttr(BLocaleRoster::kCatLangAttr, B_STRING_TYPE, 0, &buf,
                           bufSize) <= 0 ||
      fLanguageName != buf) {
    catalogFile.WriteAttr(BLocaleRoster::kCatLangAttr, B_STRING_TYPE, 0,
                          fLanguageName.String(), fLanguageName.Length() + 1);
  }
  if (catalogFile.ReadAttr(BLocaleRoster::kCatSigAttr, B_STRING_TYPE, 0, &buf,
                           bufSize) <= 0 ||
      fSignature != buf) {
    catalogFile.WriteAttr(BLocaleRoster::kCatSigAttr, B_STRING_TYPE, 0,
                          fSignature.String(), fSignature.Length() + 1);
  }
  if (catalogFile.ReadAttr(BLocaleRoster::kCatFingerprintAttr, B_UINT32_TYPE, 0,
                           &temp, sizeof(uint32)) <= 0) {
    catalogFile.WriteAttr(BLocaleRoster::kCatFingerprintAttr, B_UINT32_TYPE, 0,
                          &fFingerprint, sizeof(uint32));
  }
}
...
BCatalogData *DefaultCatalog::Instantiate(const entry_ref &catalogOwner,
                                          const char *language,
                                          uint32 fingerprint) {
...
BCatalogData *DefaultCatalog::Create(const char *signature,
                                     const char *language) {
  DefaultCatalog *catalog =
...
extern "C" status_t default_catalog_get_available_languages(
    BMessage *availableLanguages, const char *sigPattern,
    const char *langPattern, int32 fingerprint) {
  ...
  BDirectory appDir(&nref);
  BString catalogName("locale/");
  catalogName << kCatFolder << "/" << sigPattern;
  BPath catalogPath(&appDir, catalogName.String());
  ...
  for (size_t i = 0; i < sizeof(which) / sizeof(which[0]); i++) {
    BPath path;
    if (find_directory(which[i], &path) == B_OK) {
      catalogName = BString("locale/") << kCatFolder << "/" << sigPattern;
