...
/*static*/ status_t BMessageRunner::StartSending(BMessenger target,
                                                 const BMessage *message,
                                                 bigtime_t interval,
                                                 int32 count) {
  int32 token =
      _RegisterRunner(target, message, interval, count, true, be_app_messenger);
...
/*static*/ status_t BMessageRunner::StartSending(BMessenger target,
                                                 ...
                                                 BMessenger replyTo) {
  int32 token =
      _RegisterRunner(target, message, interval, count, true, replyTo);
...
BMessageRunner::BMessageRunner(const BMessageRunner &) : fToken(-1) {}
...
/*!	Initializes the BMessageRunner.
   ...
   microseconds. \param count Specifies how many times the message shall be
   sent. A value less than \c 0 for an unlimited number of repetitions. \param
   replyTo Target replies to the delivered message(s) shall be sent to.
*/
void BMessageRunner::_InitData(BMessenger target, const BMessage *message,
                               bigtime_t interval, int32 count,
                               BMessenger replyTo) {
...
/*!	Registers the BMessageRunner in the registrar.
   ...
   code while trying to register it.
*/
/*static*/ int32 BMessageRunner::_RegisterRunner(BMessenger target,
                                                 const BMessage *message,
                                                 bigtime_t interval,
...
*/
status_t BMessageRunner::_SetParams(bool resetInterval, bigtime_t interval,
