...
*/
BDiskDevice::BDiskDevice() : fDeviceData(NULL) {}

// destructor
/*!	\brief Frees all resources associated with this object.
*/
BDiskDevice::~BDiskDevice() { CancelModifications(); }
...
// SetTo
status_t BDiskDevice::SetTo(partition_id id) { return _SetTo(id, true, 0); }

// Update
/*!	\brief Updates the object to reflect the latest changes to the device.

        Note, that subobjects (BSessions, BPartitions) may be deleted during
   this operation. It is also possible, that the device doesn't exist anymore --
 ...
 *
 * 	\note This call locks the device. You need to call \ref
 * CommitModifications or \ref CancelModifications to unlock it.
 */
...
*/
BDiskDevice &BDiskDevice::operator=(const BDiskDevice &) { return *this; }

// _GetData
status_t BDiskDevice::_GetData(partition_id id, bool deviceOnly,
                               size_t neededSize,
...
// _SetTo
status_t BDiskDevice::_SetTo(partition_id id, bool deviceOnly,
                             size_t neededSize) {
  Unset();
...
status_t BDiskDevice::_SetTo(user_disk_device_data *data) {
  ...
  status_t error =
      BPartition::_SetTo(this, NULL, &fDeviceData->device_partition_data);
  if (error != B_OK) {
...
status_t BDiskDevice::_Update(user_disk_device_data *data, bool *updated) {
  ...
  *updated = false;
  ...
  status_t error =
      _RemoveObsoleteDescendants(&data->device_partition_data, updated);
  if (error != B_OK)
    return error;
