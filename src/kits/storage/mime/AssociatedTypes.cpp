...
*/
status_t AssociatedTypes::GetAssociatedTypes(const char *extension,
                                             BMessage *types) {
...
/*! \brief Guesses a MIME type for the given filename based on its extension
        ...
        \param result Pointer to a pre-allocated \c BString object into which
                      the result is stored. If the function returns a value
   other than \c B_OK, \a result will not be modified. \return - \c B_OK:
   success - \c other error code: failure
*/
status_t AssociatedTypes::GuessMimeType(const char *filename, BString *result) {
  ...
  if (!err) {
    ...
    if (rawExtension && rawExtension[1] != '\0') {
      ...
      /*! \todo I'm just grabbing the first item in the set here. Should we
...
// GuessMimeType
/*! \brief Guesses a MIME type for the given \c entry_ref based on its filename
   extension
...
/*! \brief Sets the list of file extensions for the given type and
   ...
   setting \param types Pointer to a \c BMessage containing an array of
   associated file extensions in its \c Mime::kExtensionsField field.
*/
status_t AssociatedTypes::SetFileExtensions(const char *type,
  ...
  if (!err) {
    ...
    for (int32 i = 0;
         extensions->FindString(kTypesField, i, &extension) == B_OK; i++) {
...
void AssociatedTypes::PrintToStream() const {
  ...
  for (std::map<std::string, std::set<std::string> >::const_iterator i =
           fAssociatedTypes.begin();
       i != fAssociatedTypes.end(); i++) {
    printf("%s: ", i->first.c_str());
    fflush(stdout);
    bool first = true;
    for (std::set<std::string>::const_iterator type = i->second.begin();
         type != i->second.end(); type++) {
      if (first)
        first = false;
      else
        printf(", ");
...
/*! \brief Adds the given mime type to the set of associated types
        ...
        \return
        - B_OK: success, even if the type was already in the associated types
   list - "error code": failure
*/
status_t AssociatedTypes::AddAssociatedType(const char *extension,
                                            const char *type) {
...
/*! \brief Removes the given mime type from the set of associated types
        ...
        \param type The associated mime type
        \return
        - B_OK: success, even if the type was not found in the associated types
   list - "error code": failure
*/
status_t AssociatedTypes::RemoveAssociatedType(const char *extension,
                                               const char *type) {
  status_t err = extension && type ? B_OK : B_BAD_VALUE;
  std::string extStr;
  if (!err) {
...
// ProcessType
/*! \brief Handles a portion of the initial associated types table construction
...
status_t AssociatedTypes::ProcessType(const char *type) {
  ...
  if (!err) {
    ...
    if (fDatabaseLocation->ReadMessageAttribute(type, kFileExtensionsAttr,
      ...
      for (int i = 0; msg.FindString(kExtensionsField, i, &extension) == B_OK;
           i++) {
...
*/
std::string AssociatedTypes::PrepExtension(const char *extension) const {
  if (extension) {
    uint i = 0;
    while (extension[i] == '.')
      i++;
