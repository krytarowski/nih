...
ToolTipView::ToolTipView(BToolTip *tip)
    : BView("tool tip", B_WILL_DRAW | B_FRAME_EVENTS), fToolTip(tip),
      fHidden(false) {
...
ToolTipView::~ToolTipView() { fToolTip->ReleaseReference(); }
...
void ToolTipView::ShowTip() { fHidden = false; }
...
/*!	Tries to find the right frame to show the tool tip in, trying to use the
   ...
   will resize the window if necessary.
*/
  ...
  while (true) {
    ...
    case B_ALIGN_TOP:
      ...
      if (location.y < screenFrame.top) {
        alignment.vertical = firstTry ? B_ALIGN_BOTTOM : B_ALIGN_MIDDLE;
        firstTry = false;
        continue;
      }
      break;
    ...
    default:
      ...
      if (location.y + size.height > screenFrame.bottom) {
        alignment.vertical = firstTry ? B_ALIGN_TOP : B_ALIGN_MIDDLE;
        firstTry = false;
        continue;
      }
      break;
...
/*static*/ BToolTipManager *BToolTipManager::Manager() {
...
void BToolTipManager::HideTip() { fWindow.SendMessage(kMsgHideToolTip); }
...
bigtime_t BToolTipManager::ShowDelay() const { return fShowDelay; }
...
bigtime_t BToolTipManager::HideDelay() const { return fHideDelay; }

BToolTipManager::BToolTipManager()
    : fLock("tool tip manager"), fShowDelay(750000), fHideDelay(50000) {}
