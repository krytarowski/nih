...
struct LoadedImages {
  ...
  LoadedImages() : fLock("BShelf loaded image map") {}
...
private:
  static void _InitSingleton() { sDefaultInstance = new LoadedImages; }
...
static property_info sReplicantPropertyList[] = {
    {"ID", {B_GET_PROPERTY}, {B_DIRECT_SPECIFIER}, NULL, 0, {B_INT32_TYPE}},

    {"Name", {B_GET_PROPERTY}, {B_DIRECT_SPECIFIER}, NULL, 0, {B_STRING_TYPE}},
    ...
    {
        "View", {}, {B_DIRECT_SPECIFIER}, NULL, 0,
    },
...
struct replicant_data {
  ...
  static replicant_data *Find(BList const *list, BView const *view,
                              bool allowZombie);
...
*/
static status_t send_reply(BMessage *message, status_t status,
                           uint32 uniqueID) {
...
static bool find_replicant(BList &list, const char *className,
                           const char *addOn) {
  int32 i = 0;
  replicant_data *item;
  while ((item = (replicant_data *)list.ItemAt(i++)) != NULL) {
    ...
    if (item->message->FindString("class", &replicantClassName) == B_OK &&
        ...
        !strcmp(className, replicantClassName) && addOn != NULL &&
        replicantAddOn != NULL && !strcmp(addOn, replicantAddOn))
      return true;
  }
  return false;
}
...
replicant_data::replicant_data(BMessage *_message, BView *_view,
                               BDragger *_dragger, BDragger::relation _relation,
                               unsigned long _id)
    : message(_message), view(_view), dragger(NULL), relation(_relation),
      id(_id), error(B_OK), zombie_view(NULL) {}

replicant_data::replicant_data()
    : message(NULL), view(NULL), dragger(NULL),
      relation(BDragger::TARGET_UNKNOWN), id(0), error(B_ERROR),
      zombie_view(NULL) {}

replicant_data::~replicant_data() { delete message; }

status_t replicant_data::Archive(BMessage *msg) {
...
// static
replicant_data *replicant_data::Find(BList const *list, BMessage const *msg) {
  int32 i = 0;
  replicant_data *item;
  while ((item = (replicant_data *)list->ItemAt(i++)) != NULL) {
    if (item->message == msg)
...
// static
replicant_data *replicant_data::Find(BList const *list, BView const *view,
                                     bool allowZombie) {
  int32 i = 0;
  replicant_data *item;
  while ((item = (replicant_data *)list->ItemAt(i++)) != NULL) {
...
// static
replicant_data *replicant_data::Find(BList const *list, unsigned long id) {
  int32 i = 0;
  replicant_data *item;
  while ((item = (replicant_data *)list->ItemAt(i++)) != NULL) {
    if (item->id == id)
...
// static
int32 replicant_data::IndexOf(BList const *list, BView const *view,
                              bool allowZombie) {
  int32 i = 0;
  replicant_data *item;
  while ((item = (replicant_data *)list->ItemAt(i)) != NULL) {
...
ShelfContainerViewFilter::ShelfContainerViewFilter(BShelf *shelf, BView *view)
    : BMessageFilter(B_ANY_DELIVERY, B_ANY_SOURCE), fShelf(shelf), fView(view) {
}

filter_result ShelfContainerViewFilter::Filter(BMessage *msg,
                                               BHandler **handler) {
  filter_result filter = B_DISPATCH_MESSAGE;

  if (msg->what == B_ARCHIVED_OBJECT || msg->what == B_ABOUT_REQUESTED)
    return _ObjectDropFilter(msg, handler);

  return filter;
}

filter_result ShelfContainerViewFilter::_ObjectDropFilter(BMessage *msg,
                                                          BHandler **_handler) {
...
filter_result ReplicantViewFilter::Filter(BMessage *message,
                                          BHandler **handler) {
...
BShelf::BShelf(BMessage *data) : BHandler(data) {
  // TODO: Implement ?
}
...
status_t BShelf::Archive(BMessage *data, bool deep) const { return B_ERROR; }

BArchivable *BShelf::Instantiate(BMessage *data) { return NULL; }
  ...
  case B_GET_SUPPORTED_SUITES:
    if (strcmp(prop, "Replicant") == 0) {
      ...
      if (err == B_OK && replicant) {
        ...
        if (err == B_OK && msg->GetCurrentSpecifier(&index, &specifier, &what,
                                                    &prop) != B_OK) {
...
void BShelf::SetDirty(bool state) { fDirty = state; }

bool BShelf::IsDirty() const { return fDirty; }

BHandler *BShelf::ResolveSpecifier(BMessage *msg, int32 index,
  ...
  case 2: {
    ...
    if (err == B_OK && replicant != NULL) {
      ...
      replyMsg.AddString("message",
                         "Cannot find replicant at/with specified index/name.");
      msg->SendReply(&replyMsg);
    }
  }
    msg->PopSpecifier();
  ...
  int32 repIndex;
  status_t err =
      msg->GetCurrentSpecifier(&repIndex, specifier, &form, &property);
  if (err) {
...
status_t BShelf::GetSupportedSuites(BMessage *message) {
  status_t err;
...
bool BShelf::AllowsDragging() const { return fAllowDragging; }

void BShelf::SetAllowsDragging(bool state) { fAllowDragging = state; }

bool BShelf::AllowsZombies() const { return fAllowZombies; }

void BShelf::SetAllowsZombies(bool state) { fAllowZombies = state; }

bool BShelf::DisplaysZombies() const { return fDisplayZombies; }

void BShelf::SetDisplaysZombies(bool state) { fDisplayZombies = state; }

bool BShelf::IsTypeEnforced() const { return fTypeEnforced; }

void BShelf::SetTypeEnforced(bool state) { fTypeEnforced = state; }
...
status_t BShelf::AddReplicant(BMessage *data, BPoint location) {
...
int32 BShelf::CountReplicants() const { return fReplicants.CountItems(); }

BMessage *BShelf::ReplicantAt(int32 index, BView **_view, uint32 *_uniqueID,
...
bool BShelf::CanAcceptReplicantMessage(BMessage *) const { return true; }

bool BShelf::CanAcceptReplicantView(BRect, BView *, BMessage *) const {
  return true;
}

BPoint BShelf::AdjustReplicantBy(BRect, BMessage *) const { return B_ORIGIN; }
...
void BShelf::_InitData(BEntry *entry, BDataIO *stream, BView *view,
                       bool allowDrags) {
  fContainerView = view;
  fStream = NULL;
  fEntry = entry;
  ...
  if (fStream != NULL) {
    ...
    if (archive.Unflatten(fStream) == B_OK) {
      ...
      BMessage replicant;
      for (int32 i = 0; archive.FindMessage("replicant", i, &replicant) == B_OK;
           i++) {
        BPoint point;
...
status_t BShelf::_DeleteReplicant(replicant_data *item) {
  ...
  if (item->message->FindString("add_on", &signature) == B_OK &&
    ...
    if (lock.IsLocked()) {
      LoadedImageMap::iterator it =
          loadedImages->images.find(BString(signature));
...
status_t BShelf::_AddReplicant(BMessage *data, BPoint *location,
                               uint32 uniqueID) {
  ...
  if (fTypeEnforced) {
    ...
    if (data->FindString("shelf_type", &shelfType) == B_OK &&
      ...
      if (Name() && strcmp(shelfType, Name()) != 0) {
        printf("Replicant was rejected by BShelf: The BShelf's type and the "
               "Replicant's type don't match.");
        return send_reply(data, B_ERROR, uniqueID);
      } else {
        printf("Replicant was rejected by BShelf: Replicant indicated a <type> "
               "(%s), but the shelf does not.",
               shelfType);
        return send_reply(data, B_ERROR, uniqueID);
      }
    } else {
      printf(
          "Replicant was rejected by BShelf: Replicant did not have a <type>");
      return send_reply(data, B_ERROR, uniqueID);
    }
  }
  ...
  if (data->FindBool("be:load_each_time")) {
    ...
    if (data->FindString("class", &className) == B_OK &&
      ...
      if (find_replicant(fReplicants, className, addOn)) {
        printf("Replicant was rejected. Unique replicant already exists. "
               "class=%s, signature=%s",
               className, addOn);
        return send_reply(data, B_ERROR, uniqueID);
      }
  ...
  replicant_data *item =
      new replicant_data(data, replicant, dragger, relation, uniqueID);
...
BView *BShelf::_GetReplicant(BMessage *data, BView *view, const BPoint &point,
...
/* static */
void BShelf::_GetReplicantData(BMessage *data, BView *view, BView *&replicant,
                               BDragger *&dragger,
                               BDragger::relation &relation) {
  ...
  if (data->FindMessage("__widget", &widget) == B_OK) {
    ...
    replicant = view;
    dragger =
        dynamic_cast<BDragger *>(_InstantiateObject(&widget, &draggerImage));
...
_BZombieReplicantView_ *BShelf::_CreateZombie(BMessage *data,
                                              BDragger *&dragger) {
...
/* static */
BArchivable *BShelf::_InstantiateObject(BMessage *archive, image_id *image) {
