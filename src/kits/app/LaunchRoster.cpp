...
BLaunchRoster::Private::Private(BLaunchRoster *roster) : fRoster(roster) {}
...
BLaunchRoster::BLaunchRoster() { _InitMessenger(); }
...
port_id BLaunchRoster::GetPort(const char *name) {
  if (be_app == NULL)
    return B_BAD_VALUE;
...
port_id BLaunchRoster::GetPort(const char *signature, const char *name) {
  BMessage data;
...
status_t BLaunchRoster::StopTarget(const char *name, bool force) {
  if (name == NULL)
    return B_BAD_VALUE;
...
status_t BLaunchRoster::SetEnabled(const char *name, bool enable) {
  if (name == NULL)
    return B_BAD_VALUE;
...
void BLaunchRoster::_InitMessenger() {
  ...
  if (daemonPort >= 0 && get_port_info(daemonPort, &info) == B_OK) {
    BMessenger::Private(fMessenger)
        .SetTo(info.team, daemonPort, B_PREFERRED_TOKEN);
  }
}
...
status_t BLaunchRoster::_GetInfo(uint32 what, const char *name,
                                 BMessage &info) {
