...
BLayoutItem::BLayoutItem() : fLayout(NULL), fLayoutData(NULL) {}

BLayoutItem::BLayoutItem(BMessage *from)
    : BArchivable(BUnarchiver::PrepareArchive(from)), fLayout(NULL),
      fLayoutData(NULL) {
...
BLayout *BLayoutItem::Layout() const { return fLayout; }
...
BView *BLayoutItem::View() { return NULL; }
...
void *BLayoutItem::LayoutData() const { return fLayoutData; }

void BLayoutItem::SetLayoutData(void *data) { fLayoutData = data; }
...
void BLayoutItem::AlignInFrame(BRect frame) {
  ...
  if (HasHeightForWidth()) {
    ...
    if (maxSize.width < frame.Width() &&
      ...
      frame.left +=
          (int)((frame.Width() - maxSize.width) * alignment.horizontal);
