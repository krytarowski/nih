...
static const unsigned char kHandBitmap[] = {
    255, 255, 0,   0,   0,   255, 255, 255, 255, 255, 0,   131, 131,
    0,   255, 255, 0,   0,   0,   0,   131, 131, 0,   0,   0,   131,
    0,   0,   131, 131, 0,   0,   0,   131, 131, 131, 131, 131, 0,
    0,   255, 0,   131, 131, 131, 131, 0,   0,   255, 255, 0,   0,
...
struct DraggerManager {
  ...
  DraggerManager()
      : visible(false), visibleInitialized(false), fLock("BDragger static") {}
...
private:
  static void _InitSingleton() { sDefaultInstance = new DraggerManager; }
...
BDragger::BDragger(BRect frame, BView *target, uint32 resizingMode,
                   uint32 flags)
    : BView(frame, "_dragger_", resizingMode, flags), fTarget(target),
      fRelation(TARGET_UNKNOWN), fShelf(NULL), fTransition(false),
      fIsZombie(false), fErrCount(0), fPopUpIsCustom(false), fPopUp(NULL) {
  _InitData();
}

BDragger::BDragger(BView *target, uint32 flags)
    : BView("_dragger_", flags), fTarget(target), fRelation(TARGET_UNKNOWN),
      fShelf(NULL), fTransition(false), fIsZombie(false), fErrCount(0),
      fPopUpIsCustom(false), fPopUp(NULL) {
  _InitData();
}

BDragger::BDragger(BMessage *data)
    : BView(data), fTarget(NULL), fRelation(TARGET_UNKNOWN), fShelf(NULL),
      fTransition(false), fIsZombie(false), fErrCount(0), fPopUpIsCustom(false),
      fPopUp(NULL) {
...
void BDragger::DetachedFromWindow() { _RemoveFromList(); }
...
void BDragger::MouseUp(BPoint point) { BView::MouseUp(point); }
...
void BDragger::MessageReceived(BMessage *msg) {
  ...
  case B_TRASH_TARGET:
    if (fShelf != NULL)
      ...
      BAlert *alert = new BAlert(
          ...
          B_TRANSLATE("OK"), NULL, NULL, B_WIDTH_FROM_WIDEST, B_WARNING_ALERT);
  ...
  case _SHOW_DRAG_HANDLES_:
    ...
    if (fRelation == TARGET_IS_CHILD) {
      ...
      if ((fShelf != NULL && fShelf->AllowsDragging() && AreDraggersDrawn()) ||
          AreDraggersDrawn()) {
        Show();
      } else
        Hide();
...
BHandler *BDragger::ResolveSpecifier(BMessage *message, int32 index,
...
status_t BDragger::Perform(perform_code code, void *_data) {
  ...
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BDragger::MinSize();
    return B_OK;
  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BDragger::MaxSize();
    return B_OK;
...
void BDragger::ResizeToPreferred() { BView::ResizeToPreferred(); }

void BDragger::GetPreferredSize(float *_width, float *_height) {
  BView::GetPreferredSize(_width, _height);
}

void BDragger::MakeFocus(bool state) { BView::MakeFocus(state); }

void BDragger::AllAttached() { BView::AllAttached(); }

void BDragger::AllDetached() { BView::AllDetached(); }
...
bool BDragger::InShelf() const { return fShelf != NULL; }

BView *BDragger::Target() const { return fTarget; }

BBitmap *BDragger::DragBitmap(BPoint *offset, drawing_mode *mode) {
  return NULL;
}

bool BDragger::IsVisibilityChanging() const { return fTransition; }
...
void BDragger::_SetShelf(BShelf *shelf) { fShelf = shelf; }
