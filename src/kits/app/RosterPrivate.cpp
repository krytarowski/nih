...
*/
void BRoster::Private::SetTo(BMessenger mainMessenger,
...
/*!	\brief Sends a message to the registrar.
        ...
        - another error code
*/
status_t BRoster::Private::SendTo(BMessage *message, BMessage *reply,
                                  bool mime) {
  if (message == NULL)
    return B_BAD_VALUE;
  if (fRoster == NULL)
  ...
  const BMessenger &messenger =
      mime ? fRoster->_MimeMessenger() : fRoster->fMessenger;
  if (messenger.IsTargetLocal())
    return B_BAD_VALUE;

  return reply != NULL ? messenger.SendMessage(message, reply)
...
*/
void BRoster::Private::InitBeRoster() { be_roster = new BRoster; }

/*!	\brief Deletes the global be_roster.

        Called after the global destructors are invoked.
*/
void BRoster::Private::DeleteBeRoster() { delete be_roster; }
