...
BPartitioningInfo::BPartitioningInfo()
    : fPartitionID(-1), fSpaces(NULL), fCount(0), fCapacity(0) {}

// destructor
BPartitioningInfo::~BPartitioningInfo() { Unset(); }
...
status_t BPartitioningInfo::ExcludeOccupiedSpace(off_t offset, off_t size) {
  ...
  for (int32 i = 0; i < fCount; i++) {
    if (leftIndex == -1 && offset < fSpaces[i].offset + fSpaces[i].size) {
      leftIndex = i;
    }
...
// PartitionID
partition_id BPartitioningInfo::PartitionID() const { return fPartitionID; }
...
int32 BPartitioningInfo::CountPartitionableSpaces() const { return fCount; }
...
void BPartitioningInfo::PrintToStream() const {
  ...
  for (int32 i = 0; i < fCount; i++) {
    printf("  space at %" B_PRId32 ": offset = %" B_PRId64 ", size = %" B_PRId64
           "\n",
