...
BButton::BButton(BRect frame, const char *name, const char *label,
    ...
    : BControl(frame, name, label, message, resizingMode,
      ...
      fPreferredSize(-1, -1), fFlags(0), fBehavior(B_BUTTON_BEHAVIOR),
      fPopUpMessage(NULL) {
...
BButton::~BButton() { SetPopUpMessage(NULL); }

BButton::BButton(BMessage *data)
    : BControl(data), fPreferredSize(-1, -1), fFlags(0),
      fBehavior(B_BUTTON_BEHAVIOR), fPopUpMessage(NULL) {
...
void BButton::Draw(BRect updateRect) {
  ...
  be_control_look->DrawButtonFrame(this, rect, updateRect, base, background,
                                   flags);

  if (fBehavior == B_POP_UP_BEHAVIOR) {
    be_control_look->DrawButtonWithPopUpBackground(this, rect, updateRect, base,
                                                   flags);
  } else {
...
void BButton::SetLabel(const char *label) { BControl::SetLabel(label); }
...
BButton::BBehavior BButton::Behavior() const { return fBehavior; }

void BButton::SetBehavior(BBehavior behavior) {
...
BMessage *BButton::PopUpMessage() const { return fPopUpMessage; }
...
void BButton::MouseMoved(BPoint where, uint32 code,
...
void BButton::DetachedFromWindow() { BControl::DetachedFromWindow(); }
...
void BButton::GetPreferredSize(float *_width, float *_height) {
...
void BButton::ResizeToPreferred() { BControl::ResizeToPreferred(); }
...
void BButton::FrameMoved(BPoint newPosition) {
  BControl::FrameMoved(newPosition);
}

void BButton::FrameResized(float newWidth, float newHeight) {
  BControl::FrameResized(newWidth, newHeight);
}

void BButton::MakeFocus(bool focus) { BControl::MakeFocus(focus); }

void BButton::AllAttached() { BControl::AllAttached(); }

void BButton::AllDetached() { BControl::AllDetached(); }
...
status_t BButton::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BButton::MinSize();
    return B_OK;

  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BButton::MaxSize();
    return B_OK;
...
BSize BButton::MinSize() {
  return BLayoutUtils::ComposeSize(ExplicitMinSize(), _ValidatePreferredSize());
}

BSize BButton::MaxSize() {
  return BLayoutUtils::ComposeSize(ExplicitMaxSize(), _ValidatePreferredSize());
}

BSize BButton::PreferredSize() {
  return BLayoutUtils::ComposeSize(ExplicitPreferredSize(),
                                   _ValidatePreferredSize());
}

status_t BButton::SetIcon(const BBitmap *icon, uint32 flags) {
  return BControl::SetIcon(icon, flags | B_CREATE_ACTIVE_ICON_BITMAP |
                                     B_CREATE_DISABLED_ICON_BITMAPS);
}

void BButton::LayoutInvalidated(bool descendants) {
...
BSize BButton::_ValidatePreferredSize() {
  if (fPreferredSize.width < 0) {
    ...
    if (icon != NULL) {
      height = std::max(height, icon->Bounds().Height() + minHorizontalMargins);
    }
...
BRect BButton::_PopUpRect() const {
  if (fBehavior != B_POP_UP_BEHAVIOR)
    return BRect();
...
inline bool BButton::_Flag(uint32 flag) const { return (fFlags & flag) != 0; }

inline bool BButton::_SetFlag(uint32 flag, bool set) {
...
extern "C" void B_IF_GCC_2(InvalidateLayout__7BButtonb,
                           _ZN7BButton16InvalidateLayoutEb)(BView *view,
                                                            bool descendants) {
  perform_data_layout_invalidated data;
