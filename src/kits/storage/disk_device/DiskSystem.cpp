...
// constructor
BDiskSystem::BDiskSystem() : fID(B_NO_INIT), fFlags(0) {}

// copy constructor
BDiskSystem::BDiskSystem(const BDiskSystem &other)
    : fID(other.fID), fName(other.fName), fShortName(other.fShortName),
      fPrettyName(other.fPrettyName), fFlags(other.fFlags) {}

// destructor
BDiskSystem::~BDiskSystem() {}

// InitCheck
status_t BDiskSystem::InitCheck() const { return fID > 0 ? B_OK : fID; }

// Name
const char *BDiskSystem::Name() const { return fName.String(); }

// ShortName
const char *BDiskSystem::ShortName() const { return fShortName.String(); }

// PrettyName
const char *BDiskSystem::PrettyName() const { return fPrettyName.String(); }

// SupportsDefragmenting
bool BDiskSystem::SupportsDefragmenting(bool *whileMounted) const {
  if (InitCheck() != B_OK || !(fFlags & B_DISK_SYSTEM_SUPPORTS_DEFRAGMENTING)) {
    if (whileMounted)
      *whileMounted = false;
    return false;
  }
  ...
  if (whileMounted) {
    *whileMounted =
        IsFileSystem() &&
        (fFlags & B_DISK_SYSTEM_SUPPORTS_DEFRAGMENTING_WHILE_MOUNTED) != 0;
  }
...
bool BDiskSystem::SupportsResizing(bool *whileMounted) const {
  if (InitCheck() != B_OK || !(fFlags & B_DISK_SYSTEM_SUPPORTS_RESIZING)) {
    if (whileMounted)
      *whileMounted = false;
    return false;
  }
...
bool BDiskSystem::SupportsMoving(bool *whileMounted) const {
  if (InitCheck() != B_OK || !(fFlags & B_DISK_SYSTEM_SUPPORTS_MOVING)) {
    if (whileMounted)
      *whileMounted = false;
    return false;
  }
...
bool BDiskSystem::SupportsSettingContentName(bool *whileMounted) const {
  ...
  if (whileMounted) {
    *whileMounted =
        (IsFileSystem() &&
         (fFlags & B_DISK_SYSTEM_SUPPORTS_SETTING_CONTENT_NAME_WHILE_MOUNTED));
  }
...
bool BDiskSystem::SupportsWriting() const {
  if (InitCheck() != B_OK || !IsFileSystem())
    return false;
...
status_t BDiskSystem::GetTypeForContentType(const char *contentType,
                                            BString *type) const {
  if (InitCheck() != B_OK)
    return InitCheck();
...
// =
BDiskSystem &BDiskSystem::operator=(const BDiskSystem &other) {
  fID = other.fID;
  fName = other.fName;
