...
BAffineTransform::BAffineTransform(double sx, double shy, double shx, double sy,
                                   double tx, double ty)
    : sx(sx), shy(shy), shx(shx), sy(sy), tx(tx), ty(ty) {}

BAffineTransform::BAffineTransform(const BAffineTransform &other)
    : sx(other.sx), shy(other.shy), shx(other.shx), sy(other.sy), tx(other.tx),
      ty(other.ty) {}
...
bool BAffineTransform::IsFixedSize() const { return true; }

type_code BAffineTransform::TypeCode() const { return B_AFFINE_TRANSFORM_TYPE; }

ssize_t BAffineTransform::FlattenedSize() const { return 6 * sizeof(double); }
...
status_t BAffineTransform::Unflatten(type_code code, const void *_buffer,
                                     ssize_t size) {
...
/*static*/ BAffineTransform BAffineTransform::AffineTranslation(double x,
                                                                double y) {
  return BAffineTransform(1.0, 0.0, 0.0, 1.0, x, y);
}

/*static*/ BAffineTransform BAffineTransform::AffineRotation(double angle) {
  return BAffineTransform(cos(angle), sin(angle), -sin(angle), cos(angle), 0.0,
                          0.0);
}

/*static*/ BAffineTransform BAffineTransform::AffineScaling(double x,
                                                            double y) {
  return BAffineTransform(x, 0.0, 0.0, y, 0.0, 0.0);
}

/*static*/ BAffineTransform BAffineTransform::AffineScaling(double scale) {
  return BAffineTransform(scale, 0.0, 0.0, scale, 0.0, 0.0);
}

/*static*/ BAffineTransform BAffineTransform::AffineShearing(double x,
                                                             double y) {
...
BAffineTransform BAffineTransform::TranslateByCopy(double x, double y) const {
...
BAffineTransform BAffineTransform::ScaleByCopy(double x, double y) const {
  BAffineTransform copy(*this);
  copy.ScaleBy(x, y);
  return copy;
}

BAffineTransform BAffineTransform::ScaleByCopy(const BPoint &center, double x,
                                               double y) const {
  BAffineTransform copy(*this);
...
const BAffineTransform &BAffineTransform::SetScale(double x, double y) {
  double tx;
  double ty;
  double rotation;
  double shearX;
  double shearY;
  if (!GetAffineParameters(&tx, &ty, &rotation, NULL, NULL, &shearX, &shearY)) {
    return *this;
  }
...
BAffineTransform BAffineTransform::ShearByCopy(double x, double y) const {
  BAffineTransform copy(*this);
  copy.ShearBy(x, y);
  return copy;
}

BAffineTransform BAffineTransform::ShearByCopy(const BPoint &center, double x,
                                               double y) const {
  BAffineTransform copy(*this);
...
bool BAffineTransform::IsIdentity(double epsilon) const {
  return IsEqualEpsilon(sx, 1.0, epsilon) &&
         ...
         IsEqualEpsilon(sy, 1.0, epsilon) && IsEqualEpsilon(tx, 0.0, epsilon) &&
         IsEqualEpsilon(ty, 0.0, epsilon);
}

bool BAffineTransform::IsDilation(double epsilon) const {
  return IsEqualEpsilon(shy, 0.0, epsilon) && IsEqualEpsilon(shx, 0.0, epsilon);
}
...
bool BAffineTransform::GetAffineParameters(double *_translationX,
                                           ...
                                           double *_rotation, double *_scaleX,
                                           double *_scaleY, double *_shearX,
                                           double *_shearY) const {
