...
BHandler::BHandler(const char *name) : BArchivable(), fName(NULL) {
  _InitData(name);
}
...
BHandler::BHandler(BMessage *data) : BArchivable(data), fName(NULL) {
...
const char *BHandler::Name() const { return fName; }
...
BHandler *BHandler::NextHandler() const { return fNextHandler; }
...
void BHandler::SetFilterList(BList *filters) {
  ...
  /**
          @note	I would like to use BObjectList internally, but this function is
                          spec'd such that fFilters would get deleted and then
     assigned 'filters', which would obviously mess this up.  Wondering if
                          anyone ever assigns a list of filters and then checks
     against FilterList() to see if they are the same.
   */
...
BList *BHandler::FilterList() { return fFilters; }
...
void BHandler::UnlockLooper() { fLooper->Unlock(); }

BHandler *BHandler::ResolveSpecifier(BMessage *message, int32 index,
...
status_t BHandler::GetSupportedSuites(BMessage *data) {
  /**
          @note	This is the output from the original implementation (calling
                          PrintToStream() on both data and the contained
  BPropertyInfo):

  BMessage: what =  (0x0, or 0)
          entry         suites, type='CSTR', c=1, size=21, data[0]:
  "suite/vnd.Be-handler" entry       messages, type='SCTD', c=1, size= 0,
  ...
  --------------------------------------------------------------------------------
    ...
    InternalName   PGET                          RTSC                 1
                          ...
                          With a good deal of trial and error, I determined that
  the parenthetical clauses are entries in the 'ctypes' field of property_info.
  'ctypes' is an array of 'compound_type', which contains an array of
  'field_pair's.  I haven't the foggiest what either 'compound_type' or
  'field_pair' is for, being as the scripting docs are so bloody horrible.  The
  corresponding property_info array is declared in the globals section.
   */
...
void BHandler::_InitData(const char *name) {
  SetName(name);
...
ObserverList *BHandler::_ObserverList() {
  if (fObserverList == NULL)
...
void ObserverList::_ValidateHandlers(uint32 what) {
...
void ObserverList::_SendNotices(uint32 what, BMessage *notice) {
...
status_t ObserverList::SendNotices(uint32 what, const BMessage *notice) {
  BMessage *copy = NULL;
  if (notice != NULL) {
...
status_t ObserverList::Add(const BHandler *handler, uint32 what) {
  if (handler == NULL)
...
status_t ObserverList::Add(const BMessenger &messenger, uint32 what) {
  vector<BMessenger> &messengers = fMessengerMap[what];
...
status_t ObserverList::Remove(const BHandler *handler, uint32 what) {
  if (handler == NULL)
    return B_BAD_HANDLER;
  ...
  vector<const BHandler *>::iterator iterator =
...
status_t ObserverList::Remove(const BMessenger &messenger, uint32 what) {
  vector<BMessenger> &messengers = fMessengerMap[what];

  vector<BMessenger>::iterator iterator =
      find(messengers.begin(), messengers.end(), messenger);
...
bool FilterDeleter(void *_filter) {
