...
public:
  ...
  void RemoveLocalLayouter(LocalLayouter *localLayouter);
...
protected:
  virtual void DoLayout(float size, LocalLayouter *localLayouter,
                        BLayoutContext *context);
...
public:
  ...
  void InternalGetHeightForWidth(LocalLayouter *localLayouter,
                                 BLayoutContext *context, bool realLayout,
...
private:
  ...
  bool _SetHeightForWidthLayoutContext(BLayoutContext *context);
...
public:
  ...
  void InternalGetHeightForWidth(float width, float *minHeight,
                                 float *maxHeight, float *preferredHeight);

  void AlignWith(LocalLayouter *other, orientation orientation);

  // Archiving stuff
  status_t AddAlignedLayoutsToArchive(BArchiver *archiver);
  status_t AddOwnerToArchive(BArchiver *archiver, CompoundLayouter *requestedBy,
                             bool &_wasAvailable);
  ...
  void PrepareItems(CompoundLayouter *compoundLayouter);
  int32 CountElements(CompoundLayouter *compoundLayouter);
  bool HasMultiElementItems(CompoundLayouter *compoundLayouter);

  void AddConstraints(CompoundLayouter *compoundLayouter, Layouter *layouter);

  float Spacing(CompoundLayouter *compoundLayouter);

  bool HasHeightForWidth();

  bool AddHeightForWidthConstraints(VerticalCompoundLayouter *compoundLayouter,
                                    Layouter *layouter,
                                    BLayoutContext *context);
  void SetHeightForWidthConstraintsAdded(bool added);

  void SetCompoundLayouter(CompoundLayouter *compoundLayouter,
                           orientation orientation);

  void InternalInvalidateLayout(CompoundLayouter *compoundLayouter);
...
private:
  ...
  void _SetHorizontalLayoutContext(BLayoutContext *context, float width);
...
BTwoDimensionalLayout::BTwoDimensionalLayout()
    : fLeftInset(0), fRightInset(0), fTopInset(0), fBottomInset(0),
      fHSpacing(0), fVSpacing(0), fLocalLayouter(new LocalLayouter(this)) {}

BTwoDimensionalLayout::BTwoDimensionalLayout(BMessage *from)
    : BAbstractLayout(from), fLeftInset(0), fRightInset(0), fTopInset(0),
      fBottomInset(0), fHSpacing(0), fVSpacing(0),
...
BTwoDimensionalLayout::~BTwoDimensionalLayout() { delete fLocalLayouter; }
...
void BTwoDimensionalLayout::GetHeightForWidth(float width, float *min,
                                              float *max, float *preferred) {
  if (!HasHeightForWidth())
    return;

  float outerSpacing = fLeftInset + fRightInset - 1;
  fLocalLayouter->InternalGetHeightForWidth(
      BLayoutUtils::SubtractDistances(width, outerSpacing), min, max,
      preferred);
...
status_t BTwoDimensionalLayout::ItemUnarchived(const BMessage *from,
  ...
  for (int i = 0; i < itemCount; i++) {
    ...
    if (item->IsVisible()) {
      ...
      frame.OffsetBy(itemOffset);
      {
        ...
        // min.height, 	max.width, max.height);  if (item->HasHeightForWidth()) {
...
BSize BTwoDimensionalLayout::AddInsets(BSize size) {
  size.width =
      BLayoutUtils::AddDistances(size.width, fLeftInset + fRightInset - 1);
  size.height =
      BLayoutUtils::AddDistances(size.height, fTopInset + fBottomInset - 1);
  return size;
}
...
BSize BTwoDimensionalLayout::SubtractInsets(BSize size) {
  size.width =
      BLayoutUtils::SubtractDistances(size.width, fLeftInset + fRightInset - 1);
...
bool BTwoDimensionalLayout::HasMultiColumnItems() { return false; }

bool BTwoDimensionalLayout::HasMultiRowItems() { return false; }
...
BTwoDimensionalLayout::CompoundLayouter::CompoundLayouter(
    orientation orientation)
    : fLayouter(NULL), fLayoutInfo(NULL), fOrientation(orientation),
      fLocalLayouters(10), fLayoutContext(NULL), fLastLayoutSize(-1) {}
...
orientation BTwoDimensionalLayout::CompoundLayouter::Orientation() {
  return fOrientation;
}

Layouter *BTwoDimensionalLayout::CompoundLayouter::GetLayouter(bool minMax) {
  return fLayouter;
}

LayoutInfo *BTwoDimensionalLayout::CompoundLayouter::GetLayoutInfo() {
  return fLayoutInfo;
}
...
void BTwoDimensionalLayout::CompoundLayouter::AbsorbCompoundLayouter(
  ...
  for (int32 i = count - 1; i >= 0; i--) {
    LocalLayouter *layouter = (LocalLayouter *)other->fLocalLayouters.ItemAt(i);
...
void BTwoDimensionalLayout::CompoundLayouter::Layout(
    float size, LocalLayouter *localLayouter, BLayoutContext *context) {
  ValidateMinMax();
...
void BTwoDimensionalLayout::CompoundLayouter::DoLayout(
    float size, LocalLayouter *localLayouter, BLayoutContext *context) {
...
void BTwoDimensionalLayout::CompoundLayouter::_AddConstraints(
    Layouter *layouter) {
  int32 count = fLocalLayouters.CountItems();
  for (int32 i = 0; i < count; i++) {
...
float BTwoDimensionalLayout::CompoundLayouter::_Spacing() {
...
BTwoDimensionalLayout::VerticalCompoundLayouter::VerticalCompoundLayouter()
    : CompoundLayouter(B_VERTICAL), fHeightForWidthLayouter(NULL),
      fCachedMinHeightForWidth(0), fCachedMaxHeightForWidth(0),
      fCachedPreferredHeightForWidth(0), fHeightForWidthLayoutContext(NULL) {}

Layouter *
BTwoDimensionalLayout::VerticalCompoundLayouter::GetLayouter(bool minMax) {
  return (minMax || !_HasHeightForWidth() ? fLayouter
...
void BTwoDimensionalLayout::VerticalCompoundLayouter::
    InvalidateHeightForWidth() {
...
void BTwoDimensionalLayout::VerticalCompoundLayouter::InternalGetHeightForWidth(
  ...
  if (_SetHeightForWidthLayoutContext(context) ||
    ...
    for (int32 i = 0; i < count; i++) {
      ...
      if (layouter->HasHeightForWidth()) {
        layouter->AddHeightForWidthConstraints(this, fHeightForWidthLayouter,
                                               context);
      }
    }
    ...
    // added yet.
    updateCachedInfo = localLayouter->AddHeightForWidthConstraints(
        this, fHeightForWidthLayouter, context);
  }
  ...
  if (updateCachedInfo) {
    ...
    fCachedPreferredHeightForWidth = fHeightForWidthLayouter->PreferredSize();
  }
...
void BTwoDimensionalLayout::VerticalCompoundLayouter::DoLayout(
    float size, LocalLayouter *localLayouter, BLayoutContext *context) {
  Layouter *layouter;
...
BTwoDimensionalLayout::LocalLayouter::LocalLayouter(
    ...
    : fLayout(layout), fHLayouter(new CompoundLayouter(B_HORIZONTAL)),
      fVLayouter(new VerticalCompoundLayouter), fHeightForWidthItems(),
      fHorizontalLayoutContext(NULL), fHorizontalLayoutWidth(0),
...
BRect BTwoDimensionalLayout::LocalLayouter::ItemFrame(
    Dimensions itemDimensions) {
  ...
  float width =
      hLayoutInfo->ElementRangeSize(itemDimensions.x, itemDimensions.width);
  float height =
      vLayoutInfo->ElementRangeSize(itemDimensions.y, itemDimensions.height);
...
void BTwoDimensionalLayout::LocalLayouter::DoHorizontalLayout(float width) {
  ...
  if (fHorizontalLayoutContext != context || width != fHorizontalLayoutWidth) {
...
void BTwoDimensionalLayout::LocalLayouter::InternalGetHeightForWidth(
    float width, float *minHeight, float *maxHeight, float *preferredHeight) {
...
status_t BTwoDimensionalLayout::LocalLayouter::AddOwnerToArchive(
...
status_t BTwoDimensionalLayout::LocalLayouter::AlignLayoutsFromArchive(
  ...
  for (int32 i = 0; i < count && err == B_OK; i++) {
    err = unarchiver->FindObject(field, i, BUnarchiver::B_DONT_ASSUME_OWNERSHIP,
                                 retriever);
  ...
  if (itemCount > 0) {
    for (int i = 0; i < itemCount; i++) {
      ...
      if (item->IsVisible()) {
        ...
        if (orientation == B_HORIZONTAL) {
          layouter->AddConstraints(itemDimensions.x, itemDimensions.width,
                                   min.width, max.width, preferred.width);

          if (item->HasHeightForWidth())
            fHeightForWidthItems.AddItem(item);

        } else {
          layouter->AddConstraints(itemDimensions.y, itemDimensions.height,
    ...
    for (int element = 0; element < elementCount; element++) {
      fLayout->GetColumnRowConstraints(orientation, element, &constraints);
      layouter->SetWeight(element, constraints.weight);
      layouter->AddConstraints(element, 1, constraints.min, constraints.max,
                               constraints.min);
    }
  }
}

float BTwoDimensionalLayout::LocalLayouter::Spacing(
    CompoundLayouter *compoundLayouter) {
  return (compoundLayouter->Orientation() == B_HORIZONTAL ? fLayout->fHSpacing
                                                          : fLayout->fVSpacing);
}
...
bool BTwoDimensionalLayout::LocalLayouter::AddHeightForWidthConstraints(
  ...
  for (int32 i = 0; i < itemCount; i++) {
    ...
    item->GetHeightForWidth(
        hLayoutInfo->ElementRangeSize(itemDimensions.x, itemDimensions.width),
        &minHeight, &maxHeight, &preferredHeight);
    layouter->AddConstraints(itemDimensions.y, itemDimensions.height, minHeight,
                             maxHeight, preferredHeight);
  }
...
void BTwoDimensionalLayout::LocalLayouter::LayoutContextLeft(
    BLayoutContext *context) {
