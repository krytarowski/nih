...
CatalogAddOnInfo::CatalogAddOnInfo(const BString &name, const BString &path,
                                   uint8 priority)
    : fInstantiateFunc(NULL), fCreateFunc(NULL), fLanguagesFunc(NULL),
      fName(name), fPath(path), fAddOnImage(B_NO_INIT), fPriority(priority),
      fIsEmbedded(path.Length() == 0) {}

CatalogAddOnInfo::~CatalogAddOnInfo() {
  int32 count = fLoadedCatalogs.CountItems();
  for (int32 i = 0; i < count; ++i) {
    BCatalogData *cat = static_cast<BCatalogData *>(fLoadedCatalogs.ItemAt(i));
    delete cat;
  }
...
bool CatalogAddOnInfo::MakeSureItsLoaded() {
  if (!fIsEmbedded && fAddOnImage < B_OK) {
    ...
    if (fAddOnImage >= B_OK) {
      get_image_symbol(fAddOnImage, "instantiate_catalog", B_SYMBOL_TYPE_TEXT,
                       (void **)&fInstantiateFunc);
      get_image_symbol(fAddOnImage, "create_catalog", B_SYMBOL_TYPE_TEXT,
                       (void **)&fCreateFunc);
...
LocaleRosterData::LocaleRosterData(const BLanguage &language,
    ...
    : fLock("LocaleRosterData"), fDefaultLocale(&language, &conventions),
      fIsFilesystemTranslationPreferred(false), fAreResourcesLoaded(false) {
...
int LocaleRosterData::CompareInfos(const void *left, const void *right) {
...
status_t LocaleRosterData::_InitializeCatalogAddOns() {
  ...
  CatalogAddOnInfo *defaultCatalogAddOnInfo = new (std::nothrow)
...
// we have found (what should be) a catalog-add-on:
          ...
          node.SetTo(&entry);
          priority = -1;
          if (node.ReadAttr(kPriorityAttr, B_INT8_TYPE, 0, &priority,
                            sizeof(int8)) <= 0) {
            ...
            if (image >= B_OK) {
              ...
              if (get_image_symbol(image, "gCatalogAddOnPriority",
                                   ...
                                   (void **)&prioPtr) == B_OK) {
                priority = *prioPtr;
                node.WriteAttr(kPriorityAttr, B_INT8_TYPE, 0, &priority,
                               sizeof(int8));
              }
              unload_add_on(image);
            }
          ...
          if (priority >= 0) {
            ...
            CatalogAddOnInfo *addOnInfo = new (std::nothrow)
                CatalogAddOnInfo(dent->d_name, addOnFolderName, priority);
            if (addOnInfo)
...
status_t LocaleRosterData::_SaveLocaleSettings() {
  ...
  if (status == B_OK) {
    path.Append("Locale settings");
    status =
        file.SetTo(path.Path(), B_CREATE_FILE | B_ERASE_FILE | B_WRITE_ONLY);
  }
  if (status == B_OK)
...
status_t LocaleRosterData::_SetPreferredLanguages(const BMessage *languages) {
  ...
  if (languages != NULL &&
    ...
    for (int i = 0; languages->FindString(kLanguageField, i, &langName) == B_OK;
         i++) {
...
LocaleRosterData::_AddPreferredLanguagesToMessage(BMessage *message) const {
  ...
  BString langName;
  for (int i = 0;
       fPreferredLanguages.FindString("language", i, &langName) == B_OK; i++) {
