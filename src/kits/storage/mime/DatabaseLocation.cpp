...
DatabaseLocation::DatabaseLocation() : fDirectories() {}
...
*/
status_t DatabaseLocation::OpenWritableType(const char *type, BNode &_node,
                                            bool create,
  ...
  ssize_t bytesWritten =
      _node.WriteAttr(kTypeAttr, B_STRING_TYPE, 0, type, toWrite);
  if (bytesWritten < 0)
    result = bytesWritten;
...
/*! Reads up to \c length bytes of the given data from the given attribute
   ...
   fails, and the contents of \c data are undefined.

        \param type The MIME type.
        \param attribute The attribute name.
        \param data Pointer to a memory buffer into which the data should be
   copied. \param length The maximum number of bytes to read. \param datatype
   The expected data type.

        \return If successful, the number of bytes read is returned, otherwise,
   an error code is returned.
*/
ssize_t DatabaseLocation::ReadAttribute(const char *type, const char *attribute,
                                        void *data, size_t length,
                                        type_code datatype) const {
...
/*!	Reads a flattened BMessage from the given attribute of the given
        ...
        \param type The MIME type.
        \param attribute The attribute name.
        \param data Reference to a pre-allocated BMessage into which the
   attribute data is unflattened.

        \return A status code.
*/
status_t DatabaseLocation::ReadMessageAttribute(const char *type,
  ...
  ssize_t bytesRead =
      node.ReadAttr(attribute, B_MESSAGE_TYPE, 0, buffer, info.size);
...
/*!	Reads a BString from the given attribute of the given MIME type.
        ...
        If no entry for the given type exists in the database, the function
   fails and the contents of \c str are undefined.

        \param type The MIME type.
        \param attribute The attribute name.
        \param _string Reference to a pre-allocated BString into which the
   attribute data stored.

        \return A status code.
*/
status_t DatabaseLocation::ReadStringAttribute(const char *type,
                                               const char *attribute,
...
/*!	Writes \c len bytes of the given data to the given attribute
        ...
        \return A status code.
*/
status_t DatabaseLocation::WriteAttribute(const char *type,
                                          const char *attribute,
  ...
  if (bytesWritten < 0)
    return bytesWritten;
  return bytesWritten == (ssize_t)length ? (status_t)B_OK
...
/*! Flattens the given \c BMessage and writes it to the given attribute
        ...
        \return A status code.
*/
status_t DatabaseLocation::WriteMessageAttribute(const char *type,
                                                 const char *attribute,
...
*/
status_t DatabaseLocation::DeleteAttribute(const char *type,
...
status_t DatabaseLocation::GetAppHint(const char *type, entry_ref &_ref) {
  ...
  BEntry entry;
  ssize_t status =
      ReadAttribute(type, kAppHintAttr, path, B_PATH_NAME_LENGTH, kAppHintType);
...
/*!	Fetches from the MIME database a BMessage describing the attributes
        ...
        The attribute information is returned in a pre-allocated BMessage
   pointed to by the \c info parameter (note that the any prior contents of the
   message will be destroyed). Please see BMimeType::SetAttrInfo() for a
   description of the expected format of such a message.

        \param _info Reference to a pre-allocated BMessage into which
   information about the MIME type's associated file attributes is stored.

        \return A status code, \c B_OK on success or an error code on failure.
*/
status_t DatabaseLocation::GetAttributesInfo(const char *type,
                                             BMessage &_info) {
...
/*!	Fetches the short description for the given MIME type.
   ...
   short description is copied. If the function fails, the contents of the
               string are undefined.

        \return A status code, \c B_OK on success or an error code on failure.
        \retval B_OK Success.
        \retval B_ENTRY_NOT_FOUND No short description exists for the given
   type.
*/
status_t DatabaseLocation::GetShortDescription(const char *type,
                                               char *description) {
...
*/
status_t DatabaseLocation::GetLongDescription(const char *type,
                                              char *description) {
...
/*!	Fetches a BMessage describing the MIME type's associated filename
   ...
   to by the \c extensions parameter (note that the any prior contents of the
        message will be destroyed). Please see BMimeType::GetFileExtensions()
   for a description of the message format.

        \param extensions Reference to a pre-allocated BMessage into which the
   MIME type's associated file extensions will be stored.

        \return A status code, \c B_OK on success or an error code on failure.
*/
status_t DatabaseLocation::GetFileExtensions(const char *type,
                                             BMessage &_extensions) {
  status_t result =
      ReadMessageAttribute(type, kFileExtensionsAttr, _extensions);
  if (result == B_ENTRY_NOT_FOUND) {
    // return an empty message
...
/*!	Fetches the icon of given size associated with the given MIME type.
        ...
        \return A status code.
*/
status_t DatabaseLocation::GetIcon(const char *type, BBitmap &_icon,
                                   icon_size size) {
...
/*!	Fetches the vector icon associated with the given MIME type.
        ...
        \param type The mime type
        \param _data Reference via which the allocated icon data is returned.
   You need to free the buffer once you're done with it. \param _size Reference
   via which the size of the icon data is returned.

        \return A status code.
*/
status_t DatabaseLocation::GetIcon(const char *type, uint8 *&_data,
                                   size_t &_size) {
...
/*!	Fetches the large or mini icon used by an application of this type
   ...
   mini icon.
        ...
        \param type The MIME type
        \param fileType Pointer to a pre-allocated string containing the MIME
   type whose custom icon you wish to fetch. If NULL, works just like GetIcon().
   ...
   type
*/
status_t DatabaseLocation::GetIconForType(const char *type,
...
/*!	Fetches the vector icon used by an application of this type for files
   ...
   subtype of \c "application/"; that is the intended use however, and calling
        ...
        \param type The MIME type
        \param fileType Reference to a pre-allocated string containing the MIME
   type whose custom icon you wish to fetch. If NULL, works just like GetIcon().
...
status_t DatabaseLocation::GetIconForType(const char *type,
  ...
  if (result == B_OK) {
    ...
    if (result == B_OK) {
      bytesRead =
          node.ReadAttr(iconAttrName, B_VECTOR_ICON_TYPE, 0, buffer, info.size);
    }
...
/*!	Fetches signature of the MIME type's preferred application for the
        ...
        \param type The MIME type of interest
        \param description Pointer to a pre-allocated string into which the
               preferred application's signature is copied. If the function
   fails, the contents of the string are undefined. \param verb \c The action of
   interest
...
status_t DatabaseLocation::GetSupportedTypes(const char *type,
                                             BMessage &_types) {
...
BString DatabaseLocation::_TypeToFilename(const char *type, int32 index) const {
  BString path = fDirectories.StringAt(index);
  return path << '/' << BString(type).ToLower();
}

status_t DatabaseLocation::_OpenType(const char *type, BNode &_node,
                                     int32 &_index) const {
...
status_t DatabaseLocation::_CreateTypeNode(const char *type,
                                           BNode &_node) const {
...
status_t DatabaseLocation::_CopyTypeNode(BNode &source, const char *type,
                                         BNode &_target) const {
  ...
  while (source.GetNextAttrName(attribute) == B_OK) {
    ...
    if (result != B_OK) {
      syslog(LOG_ERR,
             ...
             "type \"%s\": %s",
             attribute, type, strerror(result));
      continue;
    }
