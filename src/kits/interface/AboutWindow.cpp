...
public:
  AboutView(const char *name, const char *signature);
  virtual ~AboutView();
...
StripeView::StripeView(BBitmap *icon)
    : BView("StripeView", B_WILL_DRAW), fIcon(icon) {
...
AboutView::AboutView(const char *appName, const char *signature)
  ...
  fNameView->SetFont(&font,
                     B_FONT_FAMILY_AND_STYLE | B_FONT_SIZE | B_FONT_FLAGS);

  fVersionView =
      new BStringView("version", _GetVersionFromSignature(signature));
  ...
  BScrollView *infoViewScroller = new BScrollView("infoViewScroller", fInfoView,
  ...
  BButton *closeButton =
      new BButton("ok", gSystemCatalog.GetString(ok, "AboutWindow"),
  ...
  BLayoutBuilder::Group<>(this, B_HORIZONTAL, 0)
      ...
      .SetInsets(0, B_USE_DEFAULT_SPACING, B_USE_DEFAULT_SPACING,
...
BBitmap *AboutView::_GetIconFromSignature(const char *signature) {
  if (signature == NULL)
    return NULL;
...
const char *AboutView::Name() { return fNameView->Text(); }
...
const char *AboutView::Version() { return fVersionView->Text(); }
...
BPoint BAboutWindow::AboutPosition(float width, float height) {
...
void BAboutWindow::AddCopyright(int32 firstCopyrightYear,
  ...
  allRightsReserved =
      gSystemCatalog.GetString(allRightsReserved, "AboutWindow");
  text << "\n    " << allRightsReserved;
...
BBitmap *BAboutWindow::Icon() { return fAboutView->Icon(); }

void BAboutWindow::SetIcon(BBitmap *icon) { fAboutView->SetIcon(icon); }

const char *BAboutWindow::Name() { return fAboutView->Name(); }

void BAboutWindow::SetName(const char *name) { fAboutView->SetName(name); }

const char *BAboutWindow::Version() { return fAboutView->Version(); }
