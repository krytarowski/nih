...
void _BStyleRunDescBuffer_::InsertDesc(STEStyleRunDesc *inDesc, int32 index) {
  InsertItemsAt(1, index, inDesc);
}

void _BStyleRunDescBuffer_::RemoveDescs(int32 index, int32 count) {
  RemoveItemsAt(count, index);
}

int32 _BStyleRunDescBuffer_::OffsetToRun(int32 offset) const {
  if (fItemCount <= 1)
    return 0;
  ...
  while (minIndex < maxIndex) {
    ...
    if (offset >= fBuffer[index].offset) {
      if (index >= fItemCount - 1 || offset < fBuffer[index + 1].offset) {
        break;
      } else
        minIndex = index + 1;
    } else
...
void _BStyleRunDescBuffer_::BumpOffset(int32 delta, int32 index) {
...
int32 _BStyleRecordBuffer_::InsertRecord(const BFont *inFont,
  ...
  const STEStyleRecord newRecord = {0, fh.ascent, fh.descent + fh.leading,
                                    style};
...
void _BStyleRecordBuffer_::CommitRecord(int32 index) { fBuffer[index].refs++; }

void _BStyleRecordBuffer_::RemoveRecord(int32 index) { fBuffer[index].refs--; }

bool _BStyleRecordBuffer_::MatchRecord(const BFont *inFont,
                                       const rgb_color *inColor,
                                       int32 *outIndex) {
...
void BTextView::StyleBuffer::InvalidateNullStyle() { fValidNullStyle = false; }

bool BTextView::StyleBuffer::IsValidNullStyle() const {
  return fValidNullStyle;
}

void BTextView::StyleBuffer::SyncNullStyle(int32 offset) {
...
void BTextView::StyleBuffer::SetNullStyle(uint32 inMode, const BFont *inFont,
                                          const rgb_color *inColor,
                                          int32 offset) {
  if (fValidNullStyle || fStyleRunDesc.ItemCount() < 1) {
...
BTextView::StyleBuffer::AllocateStyleRange(const int32 numStyles) const {
  STEStyleRange *range =
      (STEStyleRange *)malloc(sizeof(int32) + sizeof(STEStyleRun) * numStyles);
  if (range != NULL)
...
void BTextView::StyleBuffer::SetStyleRange(int32 fromOffset, int32 toOffset,
...
void BTextView::StyleBuffer::GetStyle(int32 inOffset, BFont *outFont,
...
STEStyleRange *BTextView::StyleBuffer::GetStyleRange(int32 startOffset,
...
void BTextView::StyleBuffer::RemoveStyleRange(int32 fromOffset,
                                              int32 toOffset) {
...
void BTextView::StyleBuffer::RemoveStyles(int32 index, int32 count) {
...
int32 BTextView::StyleBuffer::Iterate(int32 fromOffset, int32 length,
                                      InlineInput *input, const BFont **outFont,
...
int32 BTextView::StyleBuffer::OffsetToRun(int32 offset) const {
  return fStyleRunDesc.OffsetToRun(offset);
}

void BTextView::StyleBuffer::BumpOffset(int32 delta, int32 index) {
  fStyleRunDesc.BumpOffset(delta, index);
}

STEStyleRun BTextView::StyleBuffer::operator[](int32 index) const {
  STEStyleRun run;
...
// ? CompareStyles ?
// ? FilterStyles ?
static void FixupMode(const STEStyle &firstStyle, const STEStyle &otherStyle,
                      uint32 &mode, bool &sameColor) {
  if ((mode & B_FONT_FAMILY_AND_STYLE) != 0) {
  ...
  if ((mode & B_FONT_FALSE_BOLD_WIDTH) != 0) {
    if (firstStyle.font.FalseBoldWidth() != otherStyle.font.FalseBoldWidth()) {
...
void BTextView::StyleBuffer::ContinuousGetStyle(BFont *outFont, uint32 *ioMode,
                                                rgb_color *outColor,
                                                bool *sameColor,
                                                int32 fromOffset,
