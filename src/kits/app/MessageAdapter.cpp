...
inline int32 pad_to_8(int32 value) { return (value + 7) & ~7; }

/*static*/ ssize_t MessageAdapter::FlattenedSize(uint32 format,
                                                 const BMessage *from) {
  switch (format) {
...
/*static*/ status_t MessageAdapter::Flatten(uint32 format, const BMessage *from,
...
/*static*/ status_t MessageAdapter::Flatten(uint32 format, const BMessage *from,
  ...
  case MESSAGE_FORMAT_R5_SWAPPED: {
    ...
    status_t result = _FlattenR5Message(format, from, buffer, &flattenedSize);
    if (result < B_OK) {
      free(buffer);
      return result;
    }
...
/*static*/ status_t MessageAdapter::Unflatten(uint32 format, BMessage *into,
                                              const char *buffer) {
#if !defined(__NetBSD__)
  if (format == KMessage::kMessageHeaderMagic) {
    KMessage message;
    status_t result = message.SetTo(buffer, ((KMessage::Header *)buffer)->size);
    if (result != B_OK)
      return result;
...
#if !defined(__NetBSD__)
/*static*/ status_t MessageAdapter::ConvertToKMessage(const BMessage *from,
                                                      KMessage &to) {
  if (from == NULL)
    return B_BAD_VALUE;
  ...
  for (uint32 i = 0; i < header->field_count; i++, field++) {
    ...
    if (fixedSize) {
      status_t status =
          to.AddArray(name, field->type, fieldData,
      ...
      for (uint32 i = 0; i < field->count; i++) {
        ...
        status_t status =
            to.AddData(name, field->type, fieldData, itemSize, false);
        if (status != B_OK)
          return status;
        fieldData += itemSize;
      }
...
MessageAdapter::_ConvertFromKMessage(const KMessage *fromMessage,
  ...
  if (fromMessage->ReplyPort() >= 0) {
    toPrivate.GetMessageHeader()->flags |=
        MESSAGE_FLAG_REPLY_AS_KMESSAGE | MESSAGE_FLAG_REPLY_REQUIRED;
  }
      ...
      for (int32 i = 0; i < elementCount; i++) {
        ...
        if (field.TypeCode() == B_MESSAGE_TYPE) {
          ...
          if (message.SetTo(data, size) == B_OK) {
            ...
            // just add it
            result = toMessage->AddData(field.Name(), field.TypeCode(), data,
                                        size, field.HasFixedElementSize(), 1);
          }
        } else {
          result = toMessage->AddData(field.Name(), field.TypeCode(), data,
                                      size, field.HasFixedElementSize(), 1);
...
/*static*/ ssize_t MessageAdapter::_R5FlattenedSize(const BMessage *from) {
...
/*static*/ status_t MessageAdapter::_FlattenR5Message(uint32 format,
                                                      const BMessage *from,
                                                      char *buffer,
                                                      ssize_t *size) {
  BMessage::Private messagePrivate((BMessage *)from);
  ...
  *pointer = 0;
  ...
  r5header->checksum = CalculateChecksum((uint8 *)(buffer + 8), headerSize - 8);
...
/*static*/ status_t MessageAdapter::_UnflattenR5Message(uint32 format,
                                                        BMessage *into,
                                                        BDataIO *stream) {
  into->MakeEmpty();
  ...
  while ((flags & R5_FIELD_FLAG_VALID) != 0) {
    ...
    if (format == MESSAGE_FORMAT_R5) {
      for (int32 i = 0; i < itemCount; i++) {
        ...
        result = into->AddData(nameBuffer, type, pointer, itemSize, fixedSize,
                               itemCount);
...
/*static*/ status_t MessageAdapter::_UnflattenDanoMessage(uint32 format,
                                                          BMessage *into,
                                                          BDataIO *stream) {
  into->MakeEmpty();
  ...
  while (offset < size) {
    ...
    case SECTION_SINGLE_ITEM_DATA: {
      ...
      int32 dataOffset = sizeof(dano_single_item) + field->name_length + 1;
      ...
      status_t result =
          into->AddData(field->name, field->type, fieldBuffer + dataOffset,
                        field->item_size, fixedSize);
    ...
    case SECTION_FIXED_SIZE_ARRAY_DATA: {
      dano_fixed_size_array *field = (dano_fixed_size_array *)fieldBuffer;

      int32 dataOffset = sizeof(dano_fixed_size_array) + field->name_length + 1;
      dataOffset = pad_to_8(dataOffset);
      ...
      for (int32 i = 0; i < count; i++) {
        result =
            into->AddData(field->name, field->type, fieldBuffer + dataOffset,
                          field->size_per_item, true, count);
    ...
    case SECTION_VARIABLE_SIZE_ARRAY_DATA: {
      dano_variable_size_array *field = (dano_variable_size_array *)fieldBuffer;

      int32 dataOffset =
          sizeof(dano_variable_size_array) + field->name_length + 1;
      dataOffset = pad_to_8(dataOffset);
      int32 count = *(int32 *)(fieldBuffer + dataOffset);
