...
BNodeInfo::BNodeInfo() : fNode(NULL), fCStatus(B_NO_INIT) {}

BNodeInfo::BNodeInfo(BNode *node) : fNode(NULL), fCStatus(B_NO_INIT) {
  fCStatus = SetTo(node);
}
...
status_t BNodeInfo::InitCheck() const { return fCStatus; }
...
status_t BNodeInfo::GetType(char *type) const {
  ...
  // read the data
  if (result == B_OK) {
    ssize_t read = fNode->ReadAttr(kNITypeAttribute, attrInfo.type, 0, type,
                                   attrInfo.size);
    if (read < 0)
      result = read;
...
status_t BNodeInfo::SetType(const char *type) {
  ...
  if (result == B_OK) {
    if (type != NULL) {
      ...
      ssize_t written = fNode->WriteAttr(kNITypeAttribute, B_MIME_STRING_TYPE,
                                         0, type, toWrite);
      if (written < 0)
        result = written;
  ...
  if (result == B_OK) {
    if (icon != NULL) {
      ...
      if (otherColorSpace) {
        ...
        if (result == B_OK) {
          written =
              fNode->WriteAttr(attribute, attrType, 0, bitmap.Bits(), attrSize);
        }
      } else {
        written =
            fNode->WriteAttr(attribute, attrType, 0, icon->Bits(), attrSize);
      }
      if (result == B_OK) {
        if (written < 0)
          result = written;
...
status_t BNodeInfo::SetIcon(const uint8 *data, size_t size) {
  ...
  if (data && size > 0) {
    ssize_t written =
        fNode->WriteAttr(kNIIconAttribute, B_VECTOR_ICON_TYPE, 0, data, size);
    if (written < 0)
...
status_t BNodeInfo::GetPreferredApp(char *signature, app_verb verb) const {
  ...
  // read the data
  if (result == B_OK) {
    ssize_t read = fNode->ReadAttr(kNIPreferredAppAttribute, attrInfo.type, 0,
  ...
  if (result == B_OK) {
    if (signature) {
      ...
      ssize_t written = fNode->WriteAttr(
          kNIPreferredAppAttribute, B_MIME_STRING_TYPE, 0, signature, toWrite);
      if (written < 0)
        result = written;
      else if (written != (ssize_t)toWrite)
        result = B_ERROR;
...
status_t BNodeInfo::GetAppHint(entry_ref *ref) const {
  ...
  if (result == B_OK) {
    ...
    ssize_t read = fNode->ReadAttr(kNIAppHintAttribute, attrInfo.type, 0, path,
                                   attrInfo.size);
    if (read < 0)
      result = read;
    else if (read != attrInfo.size)
...
status_t BNodeInfo::SetAppHint(const entry_ref *ref) {
  ...
  if (ref != NULL) {
    ...
    if (result == B_OK) {
      ...
      ssize_t written = fNode->WriteAttr(
          kNIAppHintAttribute, B_MIME_STRING_TYPE, 0, path.Path(), toWrite);
      if (written < 0)
        result = written;
      else if (written != (ssize_t)toWrite)
        result = B_ERROR;
  ...
  if (GetType(mimeString) != B_OK) {
    ...
    if (!success && nodeType.GetSupertype(&superType) == B_OK) {
      ...
      if (superType.GetPreferredApp(signature) == B_OK) {
        BMimeType type(signature);
        success = type.GetIconForType(superType.Type(), icon, which) == B_OK;
      }
...
extern "C" status_t GetTrackerIcon__9BNodeInfoP9entry_refP7BBitmap9icon_size(
    BNodeInfo *nodeInfo, entry_ref *ref, BBitmap *bitmap, icon_size iconSize) {
...
*/
BNodeInfo &BNodeInfo::operator=(const BNodeInfo &nodeInfo) { return *this; }
