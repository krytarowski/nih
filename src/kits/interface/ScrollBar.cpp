...
public:
  Private(BScrollBar *scrollBar)
      : fScrollBar(scrollBar), fEnabled(true), fRepeaterThread(-1),
        fExitRepeater(false), fRepeaterDelay(0),
        fThumbFrame(0.0, 0.0, -1.0, -1.0), fDoRepeat(false),
        fClickOffset(0.0, 0.0), fThumbInc(0.0), fStopValue(0.0),
        fUpArrowsEnabled(true), fDownArrowsEnabled(true),
        fBorderHighlighted(false), fButtonDown(NOARROW) {
#ifdef TEST_MODE
...
// scrolls the scrollbar by a little bit after a short delay
int32 BScrollBar::Private::button_repeater_thread(void *data) {
  BScrollBar::Private *privateData = (BScrollBar::Private *)data;
  return privateData->ButtonRepeaterThread();
}

int32 BScrollBar::Private::ButtonRepeaterThread() {
...
BScrollBar::BScrollBar(BRect frame, const char *name, BView *target, float min,
    ...
    : BView(frame, name, B_FOLLOW_NONE,
      ...
      fMin(min), fMax(max), fSmallStep(1.0f), fLargeStep(10.0f), fValue(0),
      fProportion(0.0f), fTarget(NULL), fOrientation(direction) {
...
BScrollBar::BScrollBar(const char *name, BView *target, float min, float max,
...
BScrollBar::BScrollBar(BMessage *data) : BView(data), fTarget(NULL) {
  fPrivateData = new BScrollBar::Private(this);
...
void BScrollBar::AllAttached() { BView::AllAttached(); }

void BScrollBar::AllDetached() { BView::AllDetached(); }

void BScrollBar::AttachedToWindow() { BView::AttachedToWindow(); }

void BScrollBar::DetachedFromWindow() { BView::DetachedFromWindow(); }
...
void BScrollBar::Draw(BRect updateRect) {
  ...
  if (fPrivateData->fBorderHighlighted && fPrivateData->fEnabled) {
    ...
    if (fOrientation == B_HORIZONTAL) {
      AddLine(BPoint(bounds.left, bounds.top), BPoint(bounds.right, bounds.top),
              highlightColor);
    } else {
  ...
  bool enabled = fPrivateData->fEnabled && fMin < fMax && fProportion < 1.0f &&
                 fProportion >= 0.0f;
  ...
  // Draw arrows
  if (fOrientation == B_HORIZONTAL) {
    BRect buttonFrame(bounds.left, bounds.top, bounds.left + bounds.Height(),
                      bounds.bottom);

    _DrawArrowButton(ARROW_LEFT, doubleArrows, buttonFrame, updateRect, enabled,
    ...
    _DrawArrowButton(ARROW_UP, doubleArrows, buttonFrame, updateRect, enabled,
                     fPrivateData->fButtonDown == ARROW1);
    ...
    if (doubleArrows) {
      ...
      buttonFrame.OffsetTo(bounds.left,
                           bounds.bottom - ((bounds.Width() * 2) + 1));
      _DrawArrowButton(ARROW_UP, doubleArrows, buttonFrame, updateRect, enabled,
    ...
    _DrawArrowButton(ARROW_DOWN, doubleArrows, buttonFrame, updateRect, enabled,
  ...
  if (fOrientation == B_HORIZONTAL) {
    ...
    be_control_look->DrawScrollBarBackground(this, leftOfThumb, rightOfThumb,
                                             updateRect, normal, flags,
                                             fOrientation);
  } else {
    BRect topOfThumb(thumbBG.left, thumbBG.top, thumbBG.right, rect.top - 1);

    BRect bottomOfThumb(thumbBG.left, rect.bottom + 1, thumbBG.right,
                        thumbBG.bottom);

    be_control_look->DrawScrollBarBackground(this, topOfThumb, bottomOfThumb,
                                             updateRect, normal, flags,
                                             fOrientation);
  }
  ...
  if (enabled) {
    ...
    be_control_look->DrawButtonBackground(this, rect, updateRect, thumbColor, 0,
    ...
    if (fMin >= fMax || fProportion >= 1.0f || fProportion < 0.0f) {
      ...
      // thumb bevel
      BeginLineArray(4);
      AddLine(BPoint(rect.left, rect.bottom), BPoint(rect.left, rect.top),
              light);
      AddLine(BPoint(rect.left + 1, rect.top), BPoint(rect.right, rect.top),
              light);
      AddLine(BPoint(rect.right, rect.top + 1), BPoint(rect.right, rect.bottom),
              dark2);
...
void BScrollBar::FrameResized(float newWidth, float newHeight) {
  _UpdateThumbFrame();
}
  ...
  if (scrollValue != 0.0) {
    ...
    if (fPrivateData->fRepeaterThread == -1) {
      ...
      fPrivateData->fRepeaterThread =
          spawn_thread(fPrivateData->button_repeater_thread, "scroll repeater",
...
void BScrollBar::MouseMoved(BPoint where, uint32 code,
...
float BScrollBar::Value() const { return fValue; }
...
float BScrollBar::Proportion() const { return fProportion; }
...
BView *BScrollBar::Target() const { return fTarget; }
...
orientation BScrollBar::Orientation() const { return fOrientation; }
...
void BScrollBar::GetPreferredSize(float *_width, float *_height) {
  if (fOrientation == B_VERTICAL) {
    if (_width)
...
void BScrollBar::ResizeToPreferred() { BView::ResizeToPreferred(); }

void BScrollBar::MakeFocus(bool focus) { BView::MakeFocus(focus); }
...
BHandler *BScrollBar::ResolveSpecifier(BMessage *message, int32 index,
                                       BMessage *specifier, int32 what,
                                       const char *property) {
  return BView::ResolveSpecifier(message, index, specifier, what, property);
}

status_t BScrollBar::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BScrollBar::MinSize();

    return B_OK;

  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BScrollBar::MaxSize();
...
bool BScrollBar::_DoubleArrows() const {
...
void BScrollBar::_UpdateThumbFrame() {
  BRect bounds = Bounds();
  ...
  if (fMax > fMin) {
    offset =
        floorf(((fValue - fMin) / (fMax - fMin)) * (maxSize - thumbSize - 1.0));
  }
  ...
  if (fOrientation == B_VERTICAL) {
    fPrivateData->fThumbFrame.bottom =
        fPrivateData->fThumbFrame.top + thumbSize;
    fPrivateData->fThumbFrame.OffsetBy(0.0, offset);
  } else {
    fPrivateData->fThumbFrame.right =
        fPrivateData->fThumbFrame.left + thumbSize;
...
float BScrollBar::_ValueFor(BPoint where) const {
  BRect bounds = Bounds();
  bounds.InsetBy(1.0f, 1.0f);
  ...
  return roundf(fMin + (offset / (maxSize - thumbSize) * (fMax - fMin + 1.0f)));
}

int32 BScrollBar::_ButtonFor(BPoint where) const {
  BRect bounds = Bounds();
  bounds.InsetBy(1.0f, 1.0f);

  float buttonSize = fOrientation == B_VERTICAL ? bounds.Width() + 1.0f
                                                : bounds.Height() + 1.0f;

  BRect rect(bounds.left, bounds.top, bounds.left + buttonSize,
             bounds.top + buttonSize);
...
BRect BScrollBar::_ButtonRectFor(int32 button) const {
  BRect bounds = Bounds();
  bounds.InsetBy(1.0f, 1.0f);
  ...
  BRect rect(bounds.left, bounds.top, bounds.left + buttonSize - 1.0f,
             bounds.top + buttonSize - 1.0f);
...
void BScrollBar::_UpdateTargetValue(BPoint where) {
  if (fOrientation == B_VERTICAL) {
    fPrivateData->fStopValue = _ValueFor(
        BPoint(where.x, where.y - fPrivateData->fThumbFrame.Height() / 2.0));
  } else {
    fPrivateData->fStopValue = _ValueFor(
        BPoint(where.x - fPrivateData->fThumbFrame.Width() / 2.0, where.y));
  }
}

void BScrollBar::_UpdateArrowButtons() {
...
status_t control_scrollbar(scroll_bar_info *info, BScrollBar *bar) {
  ...
  if (bar->fPrivateData->fScrollBarInfo.double_arrows != info->double_arrows) {
    ...
    if (bar->fOrientation == B_VERTICAL) {
      ...
      bar->fPrivateData->fThumbFrame.OffsetBy(multiplier * B_V_SCROLL_BAR_WIDTH,
                                              0);
    }
  }
...
void BScrollBar::_DrawDisabledBackground(BRect area, const rgb_color &light,
  ...
  if (fOrientation == B_VERTICAL) {
    ...
    if (height == 0) {
      ...
      BeginLineArray(4);
      AddLine(BPoint(area.left, area.top), BPoint(area.right, area.top), dark);
      ...
      AddLine(BPoint(area.right, area.bottom), BPoint(area.left, area.bottom),
              dark);
      EndLineArray();
      area.left++;
      area.top += 2;
    ...
    if (width == 0) {
      ...
      BeginLineArray(4);
      AddLine(BPoint(area.left, area.bottom), BPoint(area.left, area.top),
              dark);
      AddLine(BPoint(area.left + 1, area.bottom),
              BPoint(area.left + 1, area.top + 1), light);
      AddLine(BPoint(area.left + 1, area.top), BPoint(area.right - 1, area.top),
              light);
      AddLine(BPoint(area.right, area.top), BPoint(area.right, area.bottom),
              dark);
      EndLineArray();
      area.left += 2;
      area.top++;
...
void BScrollBar::_DrawArrowButton(int32 direction, bool doubleArrows,
  ...
  be_control_look->DrawButtonBackground(this, rect, updateRect, baseColor,
                                        flags, BControlLook::B_ALL_BORDERS,
                                        fOrientation);

  // TODO: Why does BControlLook need this negative inset for the arrow to
  // look right?
  rect.InsetBy(-1.0f, -1.0f);
  be_control_look->DrawArrowShape(this, rect, updateRect, baseColor, direction,
                                  flags, B_DARKEN_MAX_TINT);
}

BSize BScrollBar::_MinSize() const {
  BSize minSize;
  if (fOrientation == B_HORIZONTAL) {
