...
public:
  BSplitLayout(orientation orientation, float spacing = 0.0f);
  ...
  virtual BLayoutItem *AddView(int32 index, BView *child, float weight);
  ...
  void SetCollapsible(int32 first, int32 last, bool collapsible);
  ...
  virtual status_t ItemArchived(BMessage *into, BLayoutItem *item,
                                int32 index) const;
  virtual status_t ItemUnarchived(const BMessage *from, BLayoutItem *item,
                                  int32 index);
...
private:
  ...
  void _InvalidateLayout(bool invalidateView, bool children = false);
  void _InvalidateCachedHeightForWidth();

  SplitterItem *_SplitterItemAt(const BPoint &point, int32 *index = NULL) const;
  SplitterItem *_SplitterItemAt(int32 index) const;

  void _GetSplitterValueRange(int32 index, ValueRange &range);
  int32 _SplitterValue(int32 index) const;

  void _LayoutItem(BLayoutItem *item, BRect frame, bool visible);
  void _LayoutItem(BLayoutItem *item, ItemLayoutInfo *info);
  ...
  void _InternalGetHeightForWidth(float width, bool realLayout,
  ...
  void _AddInsets(float *minHeight, float *maxHeight, float *preferredHeight);
