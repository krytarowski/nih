...
status_t AddOnMonitorHandler::AddAddOnDirectories(const char *leafPath) {
  ...
  for (uint32 i = start; i < sizeof(addOnDirectories) / sizeof(directory_which);
       i++) {
    BDirectory directory;
    node_ref nodeRef;
    BPath path;
    if (find_directory(addOnDirectories[i], &path) == B_OK &&
        path.Append(leafPath) == B_OK && directory.SetTo(path.Path()) == B_OK &&
...
void AddOnMonitorHandler::EntryMoved(const char *name, const char *fromName,
  ...
  bool watchingFromDirectory = _FindDirectory(fromDirectory, device, fromIter);
...
void AddOnMonitorHandler::StatChanged(ino_t node, dev_t device,
                                      int32 statFields) {
...
bool AddOnMonitorHandler::_FindEntry(const node_ref &entry,
...
bool AddOnMonitorHandler::_HasEntry(const node_ref &entry,
...
bool AddOnMonitorHandler::_FindDirectory(
    ino_t directory, dev_t device, DirectoryList::iterator &it,
...
bool AddOnMonitorHandler::_FindDirectory(
    const node_ref &directoryNodeRef, DirectoryList::iterator &it,
    const DirectoryList::const_iterator &end) const {
...
void AddOnMonitorHandler::_AddNewEntry(EntryList &list,
                                       add_on_entry_info &info) {
