...
node_ref::node_ref() : device((dev_t)-1), node((ino_t)-1) {}

node_ref::node_ref(dev_t device, ino_t node) : device(device), node(node) {}

node_ref::node_ref(const node_ref &other) : device((dev_t)-1), node((ino_t)-1) {
  *this = other;
}
...
BNode::BNode() : fFd(-1), fAttrFd(-1), fCStatus(B_NO_INIT) {}

BNode::BNode(const entry_ref *ref) : fFd(-1), fAttrFd(-1), fCStatus(B_NO_INIT) {
  // fCStatus is set by SetTo(), ignore return value
  (void)SetTo(ref);
}

BNode::BNode(const BEntry *entry) : fFd(-1), fAttrFd(-1), fCStatus(B_NO_INIT) {
  // fCStatus is set by SetTo(), ignore return value
  (void)SetTo(entry);
}
...
BNode::BNode(const BDirectory *dir, const char *path)
    : fFd(-1), fAttrFd(-1), fCStatus(B_NO_INIT) {
  // fCStatus is set by SetTo(), ignore return value
  (void)SetTo(dir, path);
}

BNode::BNode(const BNode &node) : fFd(-1), fAttrFd(-1), fCStatus(B_NO_INIT) {
  *this = node;
}

BNode::~BNode() { Unset(); }

status_t BNode::InitCheck() const { return fCStatus; }

status_t BNode::SetTo(const entry_ref *ref) { return _SetTo(ref, false); }
...
status_t BNode::SetTo(const char *path) { return _SetTo(-1, path, false); }
...
ssize_t BNode::WriteAttr(const char *attr, type_code type, off_t offset,
...
ssize_t BNode::ReadAttr(const char *attr, type_code type, off_t offset,
...
status_t BNode::GetAttrInfo(const char *name, struct attr_info *info) const {
  if (fCStatus != B_OK)
    return B_FILE_ERROR;
...
status_t BNode::WriteAttrString(const char *name, const BString *data) {
  status_t error = (!name || !data) ? B_BAD_VALUE : B_OK;
  if (error == B_OK) {
    int32 length = data->Length() + 1;
    ssize_t sizeWritten =
        WriteAttr(name, B_STRING_TYPE, 0, data->String(), length);
...
status_t BNode::ReadAttrString(const char *name, BString *result) const {
...
bool BNode::operator!=(const BNode &node) const { return !(*this == node); }
...
*/
void BNode::set_status(status_t newStatus) { fCStatus = newStatus; }
...
/*!	Initializes the BNode's file descriptor to the node referred to
        ...
        The method will first try to open the node with read and write
   permission. If that fails due to a read-only FS or because the user has no
   write permission for the node, it will re-try opening the node read-only.

        The \a fCStatus member will be set to the return value of this method.

        \param fd Either a directory FD or a value < 0. In the latter case \a
   path must be specified. \param path Either \a NULL in which case \a fd must
   be given, absolute, or relative to the directory specified by \a fd (if
   given) or to the current working directory. \param traverse If the node
   identified by \a fd and \a path is a symlink and \a traverse is \c true, the
   symlink will be resolved recursively.
...
status_t BNode::_GetStat(struct stat *stat) const {
