...
/*!
   ...
   will return 0 for Hour(), Minute(), Second() etc. This can represent
   midnight, but be aware IsValid() will return false.
*/
BTime::BTime() : fMicroseconds(-1) {}

/*!
        Constructs a new BTime object as a copy of \c other.
*/
BTime::BTime(const BTime &other) : fMicroseconds(other.fMicroseconds) {}
...
/*!
   ...
   between 0 and 59 and \c microsecond should be in the range of 0 and 999999.
   If the specified time is invalid, the time is not set and IsValid() returns
   false.
*/
...
*/
BTime::BTime(const BMessage *archive) : fMicroseconds(-1) {
  if (archive == NULL)
    return;
...
*/
/*static*/ bool BTime::IsValid(const BTime &time) { return time.IsValid(); }

/*!
        This is an overloaded member function, provided for convenience.
*/
/*static*/ bool BTime::IsValid(int32 hour, int32 minute, int32 second,
                               int32 microsecond) {
...
*/
BTime BTime::Time() const { return *this; }
...
/*!
        ...
        \c hour must be between 0 and 23, \c minute and \c second must be
   between 0 and 59 and \c microsecond should be in the range of 0 and 999999.
   Returns true if the time is valid; otherwise false. If the specified time is
...
/*!
   ...
   negative it will become earlier. Note: The time will wrap if it passes
   midnight.
*/
BTime &BTime::AddMilliseconds(int32 milliseconds) {
  return _AddMicroseconds(bigtime_t(milliseconds % kMillisecondsPerDay) * 1000);
}
...
*/
int32 BTime::Millisecond() const { return Microsecond() / 1000; }
...
/*!
   ...
   based on the passed diff_type \c type. If \c time is earlier the return value
   will be negativ.
...
/*!
        Returns true if this time is earlier than or equal to \c time, otherwise
   false.
*/
bool BTime::operator<=(const BTime &time) const {
  return fMicroseconds <= time.fMicroseconds;
...
BTime &BTime::_AddMicroseconds(bigtime_t microseconds) {
  ...
  fMicroseconds =
      (_Microseconds() + microseconds + count) % kMicrosecondsPerDay;
  return *this;
}
...
bool BTime::_SetTime(bigtime_t hour, bigtime_t minute, bigtime_t second,
  ...
  fMicroseconds = hour * kMicrosecondsPerHour +
                  ...
                  second * kMicrosecondsPerSecond + microsecond;
...
*/
BDate::BDate() : fDay(-1), fYear(0), fMonth(-1) {}

/*!
        Constructs a new BDate object as a copy of \c other.
*/
BDate::BDate(const BDate &other)
    : fDay(other.fDay), fYear(other.fYear), fMonth(other.fMonth) {}
...
/*!
   ...
   date between 4.10.1582 and 15.10.1582 are considered invalid. If the
   specified date is invalid, the date is not set and IsValid() returns false.
   Also note that every passed year will be interpreted as is.

*/
BDate::BDate(int32 year, int32 month, int32 day) { _SetDate(year, month, day); }
...
BDate::BDate(time_t time, time_type type) {
  ...
  if (timeinfo != NULL) {
    _SetDate(timeinfo->tm_year + 1900, timeinfo->tm_mon + 1, timeinfo->tm_mday);
  }
}
...
*/
BDate::BDate(const BMessage *archive) : fDay(-1), fYear(0), fMonth(-1) {
  if (archive == NULL)
    return;
...
/*!
        ...
        Please note that a date before 1.1.4713 BC, a date with year 0 and a
   date between 4.10.1582 and 15.10.1582 are considered invalid.
*/
bool BDate::IsValid() const { return IsValid(fYear, fMonth, fDay); }
...
/*!
   ...
   given time_type \c type.
*/
BDate BDate::CurrentDate(time_type type) { return BDate(time(NULL), type); }

/*!
        Returns a copy of the current BTime object.
*/
BDate BDate::Date() const { return *this; }
...
/*!
   ...
   date is invalid, the date is not set and the function returns false.
*/
...
/*!
   ...
   date is invalid, the values will be set to -1 for \c month and \c day, the \c
   year will be set to 0.
*/
...
/*!
   ...
   will become earlier. If the current date is invalid, the \c years are not
   added. The day/ month combination will be adjusted if it does not exist in
   the resulting year, so this function will then return the latest valid date.
*/
...
/*!
   ...
   will become earlier. If the current date is invalid, the \c months are not
   added. The day/ month combination will be adjusted if it does not exist in
   the resulting year, so this function will then return the latest valid date.
*/
...
/*!
   ...
   range of 1 to 31, in case the date is invalid it will be -1.
*/
int32 BDate::Day() const { return fDay; }

/*!
        Returns the year fragment of the date. If the date is invalid, the
   function returns 0.
*/
int32 BDate::Year() const { return fYear; }
...
*/
int32 BDate::Month() const { return fMonth; }

/*!
        Returns the difference in days between this date and the given BDate \c
   date. If \c date is earlier the return value will be negativ. If the
   calculation is done with an invalid date, the result is undefined.
*/
int32 BDate::Difference(const BDate &date) const {
  return date.DateToJulianDay() - DateToJulianDay();
}

void BDate::SetDay(int32 day) { fDay = day; }

void BDate::SetMonth(int32 month) { fMonth = month; }

void BDate::SetYear(int32 year) { fYear = year; }

/*!
        Returns the week number of the date, if the date is invalid it will
   return B_ERROR. Please note that this function does only work within the
   Gregorian calendar, thus a date before 15.10.1582 will return B_ERROR.
*/
int32 BDate::WeekNumber() const {
  ...
  */
  ...
  if (!IsValid() || fYear < 1582 || (fYear == 1582 && fMonth < 10) ||
...
/*!
   ...
   years). If the date is invalid, the function will return B_ERROR.
*/
...
/*!
        Returns true if the year of this object is a leap year, otherwise false.
   If the \c year passed is before 4713 BC, the result is undefined.
*/
bool BDate::IsLeapYear() const { return IsLeapYear(fYear); }
...
/*!
   ...
   is valid it will return 365 or 366, otherwise B_ERROR;
*/
...
/*!
        Returns the number of days in the month of the current date. If the date
   is valid it will return 28 up to 31, otherwise B_ERROR;
*/
...
*/
BString BDate::ShortDayName() const { return ShortDayName(DayOfWeek()); }
...
/*!
   ...
   for monday.
*/
/*static*/ BString BDate::ShortDayName(int32 day) {
  if (day < 1 || day > 7)
    return BString();
...
*/
BString BDate::ShortMonthName() const { return ShortMonthName(Month()); }

/*!
        Returns the short month name in case of an valid month, otherwise an
   empty string. The passed \c month must be in the range of 1 to 12.
*/
/*static*/ BString BDate::ShortMonthName(int32 month) {
  if (month < 1 || month > 12)
    return BString();
...
*/
BString BDate::LongDayName() const { return LongDayName(DayOfWeek()); }
...
/*!
   ...
   for monday.
*/
/*static*/ BString BDate::LongDayName(int32 day) {
  if (day < 1 || day > 7)
    return BString();
...
*/
BString BDate::LongMonthName() const { return LongMonthName(Month()); }

/*!
        Returns the long month name in case of an valid month, otherwise an
   empty string. The passed \c month must be in the range of 1 to 12.
*/
/*static*/ BString BDate::LongMonthName(int32 month) {
  if (month < 1 || month > 12)
    return BString();
...
/*!
   ...
   negativ, the function will return an invalid date. Because of the switch from
   Julian calendar to Gregorian calendar the 4.10.1582 is followed by the
   15.10.1582.
*/
/*static*/ BDate BDate::JulianDayToDate(int32 julianDay) {
  BDate date;
...
int32 BDate::_DaysInMonth(int32 year, int32 month) {
  ...
  const int32 daysInMonth[12] = {31, 28, 31, 30, 31, 30,
                                 31, 31, 30, 31, 30, 31};
...
int32 BDate::_DateToJulianDay(int32 _year, int32 month, int32 day) {
  ...
  //calendar, 		Friday October 15, 1582 (the cycle of weekdays was not
  //affected).
  return int32(B_ERROR);
}
...
*/
BDateTime::BDateTime() : fDate(), fTime() {}
...
*/
bool BDateTime::IsValid() const { return fDate.IsValid() && fTime.IsValid(); }

/*!
        Returns the current date and time as reported by the system depending on
   the given time_type \c type.
*/
BDateTime BDateTime::CurrentDateTime(time_type type) {
...
*/
BDate &BDateTime::Date() { return fDate; }

/*!
        Returns the current date of this object.
*/
const BDate &BDateTime::Date() const { return fDate; }

/*!
        Set the current date of this object to \c date.
*/
void BDateTime::SetDate(const BDate &date) { fDate = date; }

/*!
        Returns the current time of this object.
*/
BTime &BDateTime::Time() { return fTime; }

/*!
        Returns the current time of this object.
*/
const BTime &BDateTime::Time() const { return fTime; }

/*!
        Sets the current time of this object to \c time.
*/
void BDateTime::SetTime(const BTime &time) { fTime = time; }
