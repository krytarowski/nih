...
BMessageFormat::BMessageFormat(const BString pattern) : BFormat() {
  _Initialize(UnicodeString::fromUTF8(pattern.String()));
}

BMessageFormat::~BMessageFormat() { delete fFormatter; }

status_t BMessageFormat::InitCheck() { return fInitStatus; }
...
status_t BMessageFormat::Format(BString &output, const int64 arg) const {
  ...
  Formattable arguments[] = {(int64)arg};
...
status_t BMessageFormat::_Initialize(const UnicodeString &pattern) {
  fInitStatus = B_OK;
  UErrorCode error = U_ZERO_ERROR;
  Locale *icuLocale = BLanguage::Private(&fLanguage).ICULocale();
