...
class BManagerBase {
public:
  enum manager_type { ARCHIVE_MANAGER, UNARCHIVE_MANAGER };

  BManagerBase(BMessage *topLevelArchive, manager_type type)
      : fTopLevelArchive(topLevelArchive), fType(type) {
    MarkArchive(topLevelArchive);
  }

  static BManagerBase *ManagerPointer(const BMessage *constArchive) {
    if (!constArchive)
      return NULL;
...
protected:
  ~BManagerBase() { UnmarkArchive(fTopLevelArchive); }
...
public:
  ...
  status_t ArchiveObject(BArchivable *archivable, bool deep, int32 &_token);

  bool IsArchived(BArchivable *archivable);

  status_t ArchiverLeaving(const BArchiver *archiver, status_t err);
...
public:
  ...
  status_t UnarchiverLeaving(const BUnarchiver *archiver, status_t err);
  void Acquire();
