...
typedef enum { SPINNER_INCREMENT, SPINNER_DECREMENT } spinner_direction;

class SpinnerButton : public BView {
public:
  SpinnerButton(BRect frame, const char *name, spinner_direction direction);
...
struct BAbstractSpinner::LayoutData {
  LayoutData(float width, float height)
      : label_layout_item(NULL), text_view_layout_item(NULL), label_width(0),
        label_height(0), text_view_width(0), text_view_height(0),
...
SpinnerButton::SpinnerButton(BRect frame, const char *name,
    ...
    : BView(frame, name, B_FOLLOW_RIGHT | B_FOLLOW_TOP, B_WILL_DRAW),
      fSpinnerDirection(direction), fParent(NULL), fIsEnabled(true),
      fIsMouseDown(false), fIsMouseOver(false), fRepeatDelay(100000) {}
...
void SpinnerButton::Draw(BRect updateRect) {
  ...
  uint32 borders =
      be_control_look->B_TOP_BORDER | be_control_look->B_BOTTOM_BORDER;
  ...
  be_control_look->DrawButtonFrame(this, rect, updateRect,
                                   tint_color(bgColor, frameTint), bgColor, 0,
                                   borders);
  be_control_look->DrawButtonBackground(
      this, rect, updateRect, tint_color(bgColor, bgTint), 0, borders);
...
void SpinnerButton::MouseDown(BPoint where) {
  if (fIsEnabled) {
    ...
    Invalidate();
    fRepeatDelay = 100000;
    MouseDownThread<SpinnerButton>::TrackMouse(
        this, &SpinnerButton::_DoneTracking, &SpinnerButton::_Track);
  }
...
void SpinnerButton::_Track(BPoint where, uint32) {
  ...
  fSpinnerDirection == SPINNER_INCREMENT ? fParent->Increment()
                                         : fParent->Decrement();
...
BRect BAbstractSpinner::LabelLayoutItem::Frame() { return fFrame; }
...
BView *BAbstractSpinner::LabelLayoutItem::View() { return fParent; }

BSize BAbstractSpinner::LabelLayoutItem::BaseMinSize() {
...
BSize BAbstractSpinner::LabelLayoutItem::BaseMaxSize() { return BaseMinSize(); }

BSize BAbstractSpinner::LabelLayoutItem::BasePreferredSize() {
  return BaseMinSize();
}

BAlignment BAbstractSpinner::LabelLayoutItem::BaseAlignment() {
  return BAlignment(B_ALIGN_USE_FULL_WIDTH, B_ALIGN_USE_FULL_HEIGHT);
}

BRect BAbstractSpinner::LabelLayoutItem::FrameInParent() const {
  return fFrame.OffsetByCopy(-fParent->Frame().left, -fParent->Frame().top);
}

status_t BAbstractSpinner::LabelLayoutItem::Archive(BMessage *into,
                                                    bool deep) const {
...
BArchivable *BAbstractSpinner::LabelLayoutItem::Instantiate(BMessage *from) {
...
BAbstractSpinner::TextViewLayoutItem::TextViewLayoutItem(
    BAbstractSpinner *parent)
    : fParent(parent), fFrame() {
...
BRect BAbstractSpinner::TextViewLayoutItem::Frame() { return fFrame; }
...
BView *BAbstractSpinner::TextViewLayoutItem::View() { return fParent; }

BSize BAbstractSpinner::TextViewLayoutItem::BaseMinSize() {
  fParent->_ValidateLayoutData();
...
BSize BAbstractSpinner::TextViewLayoutItem::BaseMaxSize() {
  return BaseMinSize();
}

BSize BAbstractSpinner::TextViewLayoutItem::BasePreferredSize() {
  return BaseMinSize();
}

BAlignment BAbstractSpinner::TextViewLayoutItem::BaseAlignment() {
  return BAlignment(B_ALIGN_USE_FULL_WIDTH, B_ALIGN_USE_FULL_HEIGHT);
}

BRect BAbstractSpinner::TextViewLayoutItem::FrameInParent() const {
  return fFrame.OffsetByCopy(-fParent->Frame().left, -fParent->Frame().top);
}

status_t BAbstractSpinner::TextViewLayoutItem::Archive(BMessage *into,
                                                       bool deep) const {
  BArchiver archiver(into);
  status_t result = BAbstractLayoutItem::Archive(into, deep);
...
BArchivable *BAbstractSpinner::TextViewLayoutItem::Instantiate(BMessage *from) {
...
BAbstractSpinner::BAbstractSpinner(BRect frame, const char *name,
...
BAbstractSpinner::BAbstractSpinner(const char *name, const char *label,
...
BAbstractSpinner::BAbstractSpinner(BMessage *data)
    : BControl(data), fButtonStyle(SPINNER_BUTTON_PLUS_MINUS) {
  _InitObject();
...
BArchivable *BAbstractSpinner::Instantiate(BMessage *data) {
...
BHandler *BAbstractSpinner::ResolveSpecifier(BMessage *message, int32 index,
...
void BAbstractSpinner::MessageReceived(BMessage *message) {
...
void BAbstractSpinner::MakeFocus(bool focus) { fTextView->MakeFocus(focus); }
...
void BAbstractSpinner::ResizeToPreferred() {
  ...
  if (label != NULL) {
    fDivider =
        ceilf(StringWidth(label)) + be_control_look->DefaultLabelSpacing();
  } else
    fDivider = 0.0f;
...
void BAbstractSpinner::SetAlignment(alignment align) { fAlignment = align; }
...
BLayoutItem *BAbstractSpinner::CreateTextViewLayoutItem() {
...
BTextView *BAbstractSpinner::TextView() const {
...
status_t BAbstractSpinner::AllUnarchived(const BMessage *from) {
  ...
  if (unarchiver.IsInstantiated(kTextViewItemField)) {
    TextViewLayoutItem *&textViewItem = fLayoutData->text_view_layout_item;
    result = unarchiver.FindObject(
        kTextViewItemField, BUnarchiver::B_DONT_ASSUME_OWNERSHIP, textViewItem);
...
void BAbstractSpinner::_DrawLabel(BRect updateRect) {
  ...
  float y =
      rect.top +
      roundf((rect.Height() + 1.0f - fontHeight.ascent - fontHeight.descent) /
             2.0f) +
...
void BAbstractSpinner::_InitObject() {
  ...
  if (Label() != NULL) {
    fDivider = StringWidth(Label()) + be_control_look->DefaultLabelSpacing();
  } else
    fDivider = 0.0f;
