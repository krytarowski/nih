...
BNotification::BNotification(notification_type type)
    : BArchivable(), fInitStatus(B_OK), fType(type), fProgress(0.f),
      fFile(NULL), fBitmap(NULL) {}

BNotification::BNotification(BMessage *archive)
    : BArchivable(archive), fInitStatus(B_OK), fProgress(0.0f), fFile(NULL),
      fBitmap(NULL) {
  int32 type;
 ...
 */
status_t BNotification::InitCheck() const { return fInitStatus; }
...
*/
BArchivable *BNotification::Instantiate(BMessage *archive) {
...
/*! \brief Notification's type.
        ...
        \return A value of the notification_type enum that represents
                        notification type.
*/
notification_type BNotification::Type() const { return fType; }
...
*/
void BNotification::SetGroup(const BString &group) { fGroup = group; }
...
*/
void BNotification::SetTitle(const BString &title) { fTitle = title; }
...
*/
void BNotification::SetContent(const BString &content) { fContent = content; }
...
*/
void BNotification::SetMessageID(const BString &id) { fID = id; }
...
void BNotification::SetOnClickApp(const BString &app) { fApp = app; }

const entry_ref *BNotification::OnClickFile() const { return fFile; }
...
int32 BNotification::CountOnClickRefs() const { return fRefs.CountItems(); }
...
int32 BNotification::CountOnClickArgs() const { return fArgv.CountItems(); }
...
*/
const BBitmap *BNotification::Icon() const { return fBitmap; }
