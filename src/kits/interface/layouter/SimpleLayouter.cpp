...
public:
  ...
  ElementLayoutInfo() : size(0), location(0) {}
};
...
public:
  ...
  ElementInfo()
      : index(0), min(0), max(B_SIZE_UNLIMITED), preferred(0), weight(1),
        tempWeight(0) {}

  ElementInfo(int index)
      : index(index), min(0), max(B_SIZE_UNLIMITED), preferred(0), weight(1),
        tempWeight(0) {}
...
public:
  ...
  MyLayoutInfo(int32 elementCount) : fSize(0), fElementCount(elementCount) {
    fElements = new ElementLayoutInfo[elementCount];
  }

  virtual ~MyLayoutInfo() { delete[] fElements; }
...
SimpleLayouter::SimpleLayouter(int32 elementCount, float spacing)
    : fElementCount(elementCount), fSpacing((int32)spacing), fMin(0),
      fMax(B_SIZE_UNLIMITED), fPreferred(0), fMinMaxValid(false),
      fLayoutInfo(NULL) {
...
// destructor
SimpleLayouter::~SimpleLayouter() { delete[] fElements; }

// AddConstraints
void SimpleLayouter::AddConstraints(int32 element, int32 length, float _min,
...
// MinSize
float SimpleLayouter::MinSize() {
  _ValidateMinMax();
  return fMin - 1;
}

// MaxSize
float SimpleLayouter::MaxSize() {
  _ValidateMinMax();
  return fMax - 1;
}

// PreferredSize
float SimpleLayouter::PreferredSize() {
  _ValidateMinMax();
  return fPreferred - 1;
}

// CreateLayoutInfo
LayoutInfo *SimpleLayouter::CreateLayoutInfo() {
...
// CloneLayouter
Layouter *SimpleLayouter::CloneLayouter() {
...
// _CalculateSumWeight
long SimpleLayouter::_CalculateSumWeight(BList &elementInfos) {
...
void SimpleLayouter::_LayoutMax() {
  ...
  //		fLayoutInfo.fElements[infos[i].index].size += sumSize -
  //oldSumSize;
  //	}
  ...
  for (int i = 0; i < count; i++) {
    ...
    fLayoutInfo->fElements[infos[i].index].size += int32(sumSize - oldSumSize);
  }
}
