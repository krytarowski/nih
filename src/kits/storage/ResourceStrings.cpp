...
BResourceStrings::BResourceStrings()
    : _string_lock(), _init_error(), fFileRef(), fResources(NULL),
      fHashTable(NULL), fHashTableSize(0), fStringCount(0) {
  SetStringFile(NULL);
}
...
/*!	\brief Returns the status of the last initialization via contructor or
        ...
        \return \c B_OK, if the object is properly initialized, an error code
                        otherwise.
*/
status_t BResourceStrings::InitCheck() { return _init_error; }
...
/*!	\brief Finds and returns a copy of the string identified by the supplied
        ...
        - \c NULL, if the object is not properly initialized or there is no
   string with ID \a id.
*/
...
/*!	\brief Finds and returns the string identified by the supplied ID.
        ...
        BResourceStrings object and is valid until the object is destroyed or
   set to another file. \param id the ID of the requested string \return - The
   requested string, - \c NULL, if the object is not properly initialized or
   there is no string with ID \a id.
*/
...
// SetStringFile
/*!	\brief Re-initialized the BResourceStrings object to the file referred
   to by the supplied entry_ref. If the supplied entry_ref is \c NULL, the
   object is initialized to the application file. \param ref the entry_ref
   referring to the resource file
*/
status_t BResourceStrings::SetStringFile(const entry_ref *ref) {
  ...
  if (error == B_OK) {
    ...
    const char *name;
    size_t length;
    while (fResources->GetResourceInfo(RESOURCE_TYPE, fStringCount, &id, &name,
                                       &length)) {
      fStringCount++;
    }
    ...
    for (int32 i = 0; error == B_OK && i < fStringCount; i++) {
      if (!fResources->GetResourceInfo(RESOURCE_TYPE, i, &id, &name, &length)) {
        error = B_ERROR;
      }
      if (error == B_OK) {
        const void *data = fResources->LoadResource(RESOURCE_TYPE, id, &length);
        if (data) {
...
// GetStringFile
/*!	\brief Returns an entry_ref referring to the resource file, the object
   is currently initialized to. \param outRef a pointer to an entry_ref variable
   to be initialized to the requested entry_ref \return - \c B_OK: Everything
   went fine. - \c B_BAD_VALUE: \c NULL \a outRef. - other error codes
*/
...
status_t BResourceStrings::_Rehash(int32 newSize) {
  ...
  if (newSize > 0 && newSize != fHashTableSize) {
    ...
    _string_id_hash **newHashTable = new (nothrow) _string_id_hash *[newSize];
    if (newHashTable) {
...
/*!	\brief Returns the hash table entry for a given ID.
        \param id the ID
        \return the hash table entry or \c NULL, if there is no entry with this
   ID
*/
BResourceStrings::_string_id_hash *BResourceStrings::_FindString(int32 id) {
  _string_id_hash *entry = NULL;
  if (fHashTable && fHashTableSize > 0) {
    int32 pos = id % fHashTableSize;
...
BResourceStrings::_string_id_hash::_string_id_hash()
    : next(NULL), id(0), data(NULL), data_alloced(false) {}
...
/*!	\brief Sets the string of the hash table entry.
        ...
        \param makeCopy If \c true, the supplied string is copied and the copy
                   will be freed on destruction. If \c false, the entry points
   to the supplied string. It will not be freed() on destruction.
*/
