...
Supertype::Supertype(const char *super)
    : fCachedMessage(NULL), fName(super ? super : "") {}

// Destructor
//! Destroys the Supertype object
Supertype::~Supertype() { delete fCachedMessage; }
...
const char *Supertype::GetName() { return fName.c_str(); }
...
/*! Each subtype is added as another item in the message's \c Mime::kTypesField
        field. The complete type ("supertype/subtype") is added. The supertype
   itself is not added to the message.
*/
...
/*! \brief Allocates a new BMessage into the BMessage pointer pointed to by \c
   result and fills it with the supertype's subtypes.
