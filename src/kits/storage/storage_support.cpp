...
/*!	\param path the path
        \return \c true, if \a path is not \c NULL and absolute, \c false
   otherwise
*/
bool is_absolute_path(const char *path) { return (path && path[0] == '/'); }

// parse_path
/*!	\brief Parses the supplied path and returns the position of the leaf
   name part of the path and the length of its directory path part.
        ...
        part may be empty though (i.e. \code leafStart == leafEnd \endcode),
   which will happen, if the supplied path consists only of one component.
...
// parse_path
/*!	\brief Parses the supplied path and returns the leaf name part of the
   path and its directory path part.
   ...
   which will happen, if the supplied path consists only of one component.
        ...
        \param fullPath The path to be parsed.
        \param dirPath Pointer to a character array of size \c
   B_PATH_NAME_LENGTH or greater, into which the directory part shall be
   written. May be \c NULL. \param leaf Pointer to a character array of size \c
   B_FILE_NAME_LENGTH or greater, into which the leaf name part shall be
   written. May be \c NULL. \return \c B_OK, if everything went fine,
   B_BAD_VALUE, if the supplied path is invalid.
*/
...
// internal_parse_path
static void internal_parse_path(const char *fullPath, int &leafStart,
                                int &leafEnd, int &pathEnd) {
  if (fullPath == NULL)
    return;
...
/*!	The caller is responsible for deleting the returned directory path name
        and the leaf name.
        \param fullPath the path name to be split
        \param path a pointer to a variable the directory path name pointer
   shall be written into, may be NULL \param leaf a pointer to a variable the
   leaf name pointer shall be written into, may be NULL
*/
status_t split_path(const char *fullPath, char **path, char **leaf) {
  if (path)
    *path = NULL;
...
/*! The length of the first component is returned as well as the index at
        ...
        \param nextComponent the variable the index of the next component is
                   written into. \c 0 is returned, if there is no next
   component. \return \c B_OK, if \a path is not \c NULL, \c B_BAD_VALUE
   otherwise
*/
...
/*! A string containing the first component is returned and the index, at
        ...
        \param component the variable the pointer to the newly allocated string
                   containing the first path component is written into. The
   caller is responsible for delete[]'ing the string. \param nextComponent the
   variable the index of the next component is written into. \c 0 is returned,
   if there is no next component. \return \c B_OK, if \a path is not \c NULL, \c
...
void to_lower(char *str) { to_lower(str, str); }
...
bool device_is_root_device(dev_t device) { return device == 1; }
