...
/*!	\brief Returns the brightness of an RGB 24 color.
        ...
        \return The brightness for the supplied RGB color as a value between 0
                        and 255.
*/
static inline uint8 brightness_for(uint8 red, uint8 green, uint8 blue) {
...
*/
static inline unsigned color_distance(uint8 red1, uint8 green1, uint8 blue1,
...
PaletteConverter::PaletteConverter()
    : fColorMap(NULL), fOwnColorMap(NULL), fCStatus(B_NO_INIT) {}
...
*/
PaletteConverter::~PaletteConverter() { delete fOwnColorMap; }
  ...
  if (error == B_OK) {
    ...
    for (int32 color = 0; color < 32768; color++) {
      ...
      for (int32 i = 0; i < 256; i++) {
        ...
        unsigned distance =
            color_distance(red, green, blue, c.red, c.green, c.blue);
...
/*!	\brief Returns the result of the last initialization via constructor or
        ...
        \return \c B_OK, if the converter is properly initialized, an error code
                        otherwise.
*/
status_t PaletteConverter::InitCheck() const { return fCStatus; }
...
*/
inline uint8 PaletteConverter::IndexForRGB15(uint16 rgb) const {
...
/*!	\brief Returns the palette color index closest to a given RGB 15 color.
        ...
        \return The palette color index for the supplied color.
*/
inline uint8 PaletteConverter::IndexForRGB15(uint8 red, uint8 green,
                                             uint8 blue) const {
...
/*!	\brief Returns the palette color index closest to a given RGB 16 color.
        ...
        \return The palette color index for the supplied color.
*/
inline uint8 PaletteConverter::IndexForRGB16(uint16 rgb) const {
...
/*!	\brief Returns the palette color index closest to a given RGB 16 color.
        ...
        \return The palette color index for the supplied color.
*/
inline uint8 PaletteConverter::IndexForRGB16(uint8 red, uint8 green,
                                             uint8 blue) const {
...
/*!	\brief Returns the palette color index closest to a given RGB 32 color.
        ...
        \return The palette color index for the supplied color.
*/
inline uint8 PaletteConverter::IndexForRGB24(uint32 rgb) const {
  return fColorMap
      ->index_map[((rgb & 0xf8000000) >> 17) | ((rgb & 0xf80000) >> 14) |
                  ((rgb & 0xf800) >> 11)];
}
...
/*!	\brief Returns the palette color index closest to a given RGB 24 color.
        ...
        \return The palette color index for the supplied color.
*/
inline uint8 PaletteConverter::IndexForRGB24(uint8 red, uint8 green,
                                             uint8 blue) const {
  return fColorMap
      ->index_map[((red & 0xf8) << 7) | ((green & 0xf8) << 2) | (blue >> 3)];
}
...
/*!	\brief Returns the palette color index closest to a given RGBA 32 color.
        ...
        \return The palette color index for the supplied color.
*/
inline uint8 PaletteConverter::IndexForRGBA32(uint32 rgba) const {
...
/*!	\brief Returns the palette color index closest to a given Gray 8 color.
        ...
        \return The palette color index for the supplied color.
*/
inline uint8 PaletteConverter::IndexForGray(uint8 gray) const {
...
*/
inline const rgb_color &PaletteConverter::RGBColorForIndex(uint8 index) const {
...
/*!	\brief Returns the RGB 15 color for a given palette color index.
        ...
        \return The color for the supplied palette color index
                        (R[14:10]G[9:5]B[4:0]).
*/
inline uint16 PaletteConverter::RGB15ColorForIndex(uint8 index) const {
...
*/
inline uint16 PaletteConverter::RGB16ColorForIndex(uint8 index) const {
  const rgb_color &color = fColorMap->color_list[index];
...
*/
inline uint32 PaletteConverter::RGBA32ColorForIndex(uint8 index) const {
  const rgb_color &color = fColorMap->color_list[index];
...
/*!	\brief Returns the RGBA 32 color for a given palette color index.
        ...
        \param red Reference to the variable the red component shall be stored
                   into.
        \param green Reference to the variable the green component shall be
   stored into. \param blue Reference to the variable the blue component shall
   be stored into. \param alpha Reference to the variable the alpha component
   shall be stored into.
*/
inline void PaletteConverter::RGBA32ColorForIndex(uint8 index, uint8 &red,
...
/*!	\brief Returns the Gray 8 color for a given palette color index.
        ...
        \return The color for the supplied palette color index.
*/
inline uint8 PaletteConverter::GrayColorForIndex(uint8 index) const {
  const rgb_color &color = fColorMap->color_list[index];
...
/*!	\brief Initialize the global instance of PaletteConverter using the
   system color palette. \return B_OK.
*/
/*static*/ status_t PaletteConverter::InitializeDefault(bool useServer) {
  if (sPaletteConverter.InitCheck() != B_OK) {
    pthread_once(&sPaletteConverterInitOnce,
                 useServer ? &_InitializeDefaultAppServer
...
status_t ConvertBits(const srcByte *srcBits, dstByte *dstBits,
                     int32 srcBitsLength, int32 dstBitsLength, int32 redShift,
                     int32 greenShift, int32 blueShift, int32 alphaShift,
                     int32 alphaBits, uint32 redMask, uint32 greenMask,
                     uint32 blueMask, uint32 alphaMask, int32 srcBytesPerRow,
                     int32 dstBytesPerRow, int32 srcBitsPerPixel,
                     int32 dstBitsPerPixel, color_space srcColorSpace,
                     color_space dstColorSpace, BPoint srcOffset,
                     BPoint dstOffset, int32 width, int32 height, bool srcSwap,
  ...
  dstBits = (dstByte *)((uint8 *)dstBits + ((dstOffsetY * dstBitsPerRow +
                                             dstOffsetX * dstBitsPerPixel) >>
                                            3));

  // Ensure that the width fits
  int32 srcWidth =
      (srcBitsPerRow - srcOffsetX * srcBitsPerPixel) / srcBitsPerPixel;
  if (srcWidth < width)
    width = srcWidth;

  int32 dstWidth =
      (dstBitsPerRow - dstOffsetX * dstBitsPerPixel) / dstBitsPerPixel;
  if (dstWidth < width)
    width = dstWidth;
...
status_t ConvertBits(const srcByte *srcBits, void *dstBits, int32 srcBitsLength,
                     int32 dstBitsLength, int32 redShift, int32 greenShift,
                     int32 blueShift, int32 alphaShift, int32 alphaBits,
                     int32 srcBytesPerRow, int32 dstBytesPerRow,
                     int32 srcBitsPerPixel, color_space srcColorSpace,
                     color_space dstColorSpace, BPoint srcOffset,
                     BPoint dstOffset, int32 width, int32 height, bool srcSwap,
                     readFunc *srcFunc) {
  switch (dstColorSpace) {
  case B_RGBA32:
    ConvertBits(srcBits, (uint32 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 24, greenShift - 16, blueShift - 8, alphaShift - 32,
                alphaBits, 0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000,
  ...
  case B_RGBA32_BIG:
    ConvertBits(srcBits, (uint32 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 16, greenShift - 24, blueShift - 32, alphaShift - 8,
                alphaBits, 0x0000ff00, 0x00ff0000, 0xff000000, 0x00000ff,
                srcBytesPerRow, dstBytesPerRow, srcBitsPerPixel, 32,
                srcColorSpace, dstColorSpace, srcOffset, dstOffset, width,
                height, srcSwap, false, srcFunc, NULL);
    break;
  ...
  /* Note:	we set the unused alpha to 255 here. This is because BeOS
                          uses the unused alpha for B_OP_ALPHA even though it
     should not care about it. */
  case B_RGB32:
    ConvertBits(srcBits, (uint32 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 24, greenShift - 16, blueShift - 8, 0, 0, 0x00ff0000,
                0x0000ff00, 0x000000ff, 0xff000000, srcBytesPerRow,
                dstBytesPerRow, srcBitsPerPixel, 32, srcColorSpace,
  ...
  case B_RGB32_BIG:
    ConvertBits(srcBits, (uint32 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 16, greenShift - 24, blueShift - 32, 0, 0,
                0x0000ff00, 0x00ff0000, 0xff000000, 0x000000ff, srcBytesPerRow,
                dstBytesPerRow, srcBitsPerPixel, 32, srcColorSpace,
                dstColorSpace, srcOffset, dstOffset, width, height, srcSwap,
                false, srcFunc, NULL);
    break;
  ...
  case B_RGB24:
    ConvertBits(srcBits, (uint8 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 24, greenShift - 16, blueShift - 8, 0, 0, 0xff0000,
                0x00ff00, 0x0000ff, 0x000000, srcBytesPerRow, dstBytesPerRow,
                srcBitsPerPixel, 24, srcColorSpace, dstColorSpace, srcOffset,
                dstOffset, width, height, srcSwap, false, srcFunc, WriteRGB24);
    break;

  case B_RGB24_BIG:
    ConvertBits(srcBits, (uint8 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 8, greenShift - 16, blueShift - 24, 0, 0, 0x0000ff,
                0x00ff00, 0xff0000, 0x000000, srcBytesPerRow, dstBytesPerRow,
                srcBitsPerPixel, 24, srcColorSpace, dstColorSpace, srcOffset,
                dstOffset, width, height, srcSwap, false, srcFunc, WriteRGB24);
    break;
  ...
  case B_RGB16:
  case B_RGB16_BIG:
    ConvertBits(srcBits, (uint16 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 16, greenShift - 11, blueShift - 5, 0, 0, 0xf800,
                0x07e0, 0x001f, 0x0000, srcBytesPerRow, dstBytesPerRow,
                srcBitsPerPixel, 16, srcColorSpace, dstColorSpace, srcOffset,
                dstOffset, width, height, srcSwap, dstColorSpace == B_RGB16_BIG,
                srcFunc, NULL);
    break;
  ...
  case B_RGBA15_BIG:
    ConvertBits(srcBits, (uint16 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 15, greenShift - 10, blueShift - 5, alphaShift - 16,
                alphaBits, 0x7c00, 0x03e0, 0x001f, 0x8000, srcBytesPerRow,
  ...
  case B_RGB15_BIG:
    ConvertBits(srcBits, (uint16 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 15, greenShift - 10, blueShift - 5, 0, 0, 0x7c00,
                0x03e0, 0x001f, 0x0000, srcBytesPerRow, dstBytesPerRow,
                srcBitsPerPixel, 16, srcColorSpace, dstColorSpace, srcOffset,
                dstOffset, width, height, srcSwap, dstColorSpace == B_RGB15_BIG,
                srcFunc, NULL);
    break;
  ...
  case B_GRAY8:
    ConvertBits(srcBits, (uint8 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 24, greenShift - 16, blueShift - 8, 0, 0, 0x00ff0000,
                0x0000ff00, 0x000000ff, 0x00000000, srcBytesPerRow,
                dstBytesPerRow, srcBitsPerPixel, 8, srcColorSpace,
                dstColorSpace, srcOffset, dstOffset, width, height, srcSwap,
                false, srcFunc, WriteGray8);
    break;
  ...
  case B_GRAY1:
    ConvertBits(srcBits, (uint8 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 24, greenShift - 16, blueShift - 8, 0, 0, 0x00ff0000,
                0x0000ff00, 0x000000ff, 0x00000000, srcBytesPerRow,
                dstBytesPerRow, srcBitsPerPixel, 1, srcColorSpace,
                dstColorSpace, srcOffset, dstOffset, width, height, srcSwap,
                false, srcFunc, WriteGray1);
    break;

  case B_CMAP8:
    PaletteConverter::InitializeDefault();
    ConvertBits(srcBits, (uint8 *)dstBits, srcBitsLength, dstBitsLength,
                redShift - 32, greenShift - 24, blueShift - 16, alphaShift - 8,
                alphaBits, 0xff000000, 0x00ff0000, 0x0000ff00, 0x000000ff,
                srcBytesPerRow, dstBytesPerRow, srcBitsPerPixel, 8,
                srcColorSpace, dstColorSpace, srcOffset, dstOffset, width,
                height, srcSwap, false, srcFunc, WriteCMAP8);
    break;
...
/*!	\brief Converts a source buffer in one colorspace into a destination
        ...
        \param dstBytesPerRow How many bytes per row the destination buffer has
   got. \param srcColorSpace The colorspace the source buffer is in. \param
   dstColorSpace The colorspace the buffer shall be converted to. \param width
   The width (in pixels) of each row. \param height The height (in pixels) of
   the buffers. \return - \c B_OK: Indicates success. - \c B_BAD_VALUE: \c NULL
   buffer or at least one colorspace is unsupported.
*/
status_t ConvertBits(const void *srcBits, void *dstBits, int32 srcBitsLength,
                     int32 dstBitsLength, int32 srcBytesPerRow,
                     int32 dstBytesPerRow, color_space srcColorSpace,
                     color_space dstColorSpace, int32 width, int32 height) {
  return ConvertBits(srcBits, dstBits, srcBitsLength, dstBitsLength,
                     srcBytesPerRow, dstBytesPerRow, srcColorSpace,
                     dstColorSpace, BPoint(0, 0), BPoint(0, 0), width, height);
}
...
/*!	\brief Converts a source buffer in one colorspace into a destination
        ...
        \param dstBytesPerRow How many bytes per row the destination buffer has
   got. \param srcColorSpace The colorspace the source buffer is in. \param
   dstColorSpace The colorspace the buffer shall be converted to. \param
   srcOffset The offset at which to start reading in the source. \param
   srcOffset The offset at which to start writing in the destination. \param
   width The width (in pixels) to convert. \param height The height (in pixels)
   to convert. \return - \c B_OK: Indicates success. - \c B_BAD_VALUE: \c NULL
   buffer or at least one colorspace is unsupported.
*/
status_t ConvertBits(const void *srcBits, void *dstBits, int32 srcBitsLength,
                     int32 dstBitsLength, int32 srcBytesPerRow,
  ...
  case B_RGBA32:
    return ConvertBits((const uint32 *)srcBits, dstBits, srcBitsLength,
                       ...
                       dstBytesPerRow, 32, srcColorSpace, dstColorSpace,
                       srcOffset, dstOffset, width, height, false, NULL);
    break;
  ...
  case B_RGB24:
    return ConvertBits((const uint8 *)srcBits, dstBits, srcBitsLength,
                       dstBitsLength, 24, 16, 8, 0, 0, srcBytesPerRow,
                       dstBytesPerRow, 24, srcColorSpace, dstColorSpace,
                       srcOffset, dstOffset, width, height, false, ReadRGB24);
    break;
  ...
  case B_RGB16_BIG:
    return ConvertBits((const uint16 *)srcBits, dstBits, srcBitsLength,
                       ...
                       dstBytesPerRow, 16, srcColorSpace, dstColorSpace,
                       srcOffset, dstOffset, width, height,
                       srcColorSpace == B_RGB16_BIG, NULL);
    break;
  ...
  case B_RGBA15_BIG:
    return ConvertBits((const uint16 *)srcBits, dstBits, srcBitsLength,
                       ...
                       dstBytesPerRow, 16, srcColorSpace, dstColorSpace,
                       srcOffset, dstOffset, width, height,
                       srcColorSpace == B_RGBA15_BIG, NULL);
    break;
  ...
  case B_RGB15_BIG:
    return ConvertBits((const uint16 *)srcBits, dstBits, srcBitsLength,
                       ...
                       dstBytesPerRow, 16, srcColorSpace, dstColorSpace,
                       srcOffset, dstOffset, width, height,
                       srcColorSpace == B_RGB15_BIG, NULL);
    break;

  case B_GRAY8:
    return ConvertBits((const uint8 *)srcBits, dstBits, srcBitsLength,
                       dstBitsLength, 8, 8, 8, 0, 0, srcBytesPerRow,
                       dstBytesPerRow, 8, srcColorSpace, dstColorSpace,
                       srcOffset, dstOffset, width, height, false, ReadGray8);
    break;

  case B_GRAY1:
    return ConvertBits((const uint8 *)srcBits, dstBits, srcBitsLength,
                       dstBitsLength, 8, 8, 8, 0, 0, srcBytesPerRow,
                       dstBytesPerRow, 1, srcColorSpace, dstColorSpace,
                       srcOffset, dstOffset, width, height, false, ReadGray1);
    break;
  ...
  case B_CMAP8:
    ...
    return ConvertBits((const uint8 *)srcBits, dstBits, srcBitsLength,
                       dstBitsLength, 24, 16, 8, 32, 8, srcBytesPerRow,
                       dstBytesPerRow, 8, srcColorSpace, dstColorSpace,
                       srcOffset, dstOffset, width, height, false, ReadCMAP8);
    break;
