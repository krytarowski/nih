...
BMessenger::BMessenger() : fPort(-1), fHandlerToken(B_NULL_TOKEN), fTeam(-1) {}

BMessenger::BMessenger(const char *signature, team_id team, status_t *result)
    : fPort(-1), fHandlerToken(B_NULL_TOKEN), fTeam(-1) {
...
BMessenger::BMessenger(const BMessenger &other)
    : fPort(other.fPort), fHandlerToken(other.fHandlerToken),
      fTeam(other.fTeam) {}
...
BHandler *BMessenger::Target(BLooper **_looper) const {
  ...
  if (IsTargetLocal() &&
      (fHandlerToken > B_NULL_TOKEN || fHandlerToken == B_PREFERRED_TOKEN)) {
    gDefaultTokens.GetToken(fHandlerToken, B_HANDLER_TOKEN, (void **)&handler);
    if (_looper)
...
status_t BMessenger::SendMessage(BMessage *message, BHandler *replyTo,
...
status_t BMessenger::SendMessage(BMessage *message, BMessenger replyTo,
                                 bigtime_t timeout) const {
  if (message == NULL)
    return B_BAD_VALUE;
...
status_t BMessenger::SendMessage(BMessage *message, BMessage *reply,
                                 bigtime_t deliveryTimeout,
                                 bigtime_t replyTimeout) const {
  if (message == NULL || reply == NULL)
    return B_BAD_VALUE;

  status_t result = BMessage::Private(message).SendMessage(
      fPort, fTeam, fHandlerToken, reply, deliveryTimeout, replyTimeout);
...
team_id BMessenger::Team() const { return fTeam; }

uint32 BMessenger::HashValue() const { return fPort * 19 + fHandlerToken; }
...
*/
void BMessenger::_SetTo(team_id team, port_id port, int32 token) {
  fTeam = team;
  fPort = port;
  fHandlerToken = token;
}
...
/*!	Initializes the BMessenger object's data given the signature and/or
   ...
   application are running it is undeterminate which one is chosen as the
   target. In case only a team ID is passed, the target application is
   identified uniquely. If both are supplied, the application identified by the
   team ID must have a matching signature, otherwise the initilization fails.
...
*/
void BMessenger::_InitData(const char *signature, team_id team,
                           status_t *_result) {
  status_t result = B_OK;
...
/*!	Initializes the BMessenger to target the local BHandler and/or BLooper.
   ...
   handler belongs to is used -- that means in particular, that the handler must
        already belong to a looper. If both are supplied the handler must
   actually belong to looper.
        ...
        \param result An optional pointer to a pre-allocated status_t into which
               the result of the initialization is written.
*/
void BMessenger::_InitData(const BHandler *handler, const BLooper *looper,
                           status_t *_result) {
  ...
  if (result == B_OK) {
    ...
    if (result == B_OK) {
      ...
      if (locker.IsLocked() && gLooperList.IsLooperValid(looper)) {
        ...
        fHandlerToken =
            handler != NULL ? _get_object_token_(handler) : B_PREFERRED_TOKEN;
        fTeam = looper->Team();
      } else
        result = B_BAD_VALUE;
    }
...
bool operator<(const BMessenger &_a, const BMessenger &_b) {
  ...
  return (a.Port() < b.Port() ||
          (a.Port() == b.Port() &&
           (a.Token() < b.Token() ||
            (a.Token() == b.Token() && !a.IsPreferredTarget() &&
             b.IsPreferredTarget()))));
}

bool operator!=(const BMessenger &a, const BMessenger &b) { return !(a == b); }
