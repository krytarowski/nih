...
public:
  ...
  ItemLayoutInfo()
      : weight(1.0f), layoutFrame(0, 0, -1, -1), min(), max(), isVisible(true),
        isCollapsible(true) {}
};
...
public:
  SplitterItem(BSplitLayout *layout) : fLayout(layout), fFrame() {}
  ...
  virtual BRect Frame() { return fFrame; }

  virtual void SetFrame(BRect frame) { fFrame = frame; }
...
BSplitLayout::BSplitLayout(orientation orientation, float spacing)
    : fOrientation(orientation), fLeftInset(0), fRightInset(0), fTopInset(0),
      fBottomInset(0), fSplitterSize(6),
      fSpacing(BControlLook::ComposeSpacing(spacing)),

      fSplitterItems(), fVisibleItems(), fMin(), fMax(), fPreferred(),

      fHorizontalLayouter(NULL), fVerticalLayouter(NULL),
      fHorizontalLayoutInfo(NULL), fVerticalLayoutInfo(NULL),

      fHeightForWidthItems(), fHeightForWidthVerticalLayouter(NULL),
      fHeightForWidthHorizontalLayoutInfo(NULL),

      fLayoutValid(false),

      fCachedHeightForWidthWidth(-2), fHeightForWidthVerticalLayouterWidth(-2),
      fCachedMinHeightForWidth(-1), fCachedMaxHeightForWidth(-1),
      fCachedPreferredHeightForWidth(-1),

      fDraggingStartPoint(), fDraggingStartValue(0), fDraggingCurrentValue(0),
      fDraggingSplitterIndex(-1) {}

BSplitLayout::BSplitLayout(BMessage *from)
    : BAbstractLayout(BUnarchiver::PrepareArchive(from)),
      fOrientation(B_HORIZONTAL), fLeftInset(0), fRightInset(0), fTopInset(0),
      fBottomInset(0), fSplitterSize(6),
...
void BSplitLayout::SetInsets(float left, float top, float right, float bottom) {
  fLeftInset = left;
  fTopInset = top;
  fRightInset = right;
  fBottomInset = bottom;
...
void BSplitLayout::GetInsets(float *left, float *top, float *right,
                             float *bottom) const {
  if (left)
    *left = fLeftInset;
  if (top)
...
float BSplitLayout::Spacing() const { return fSpacing; }

void BSplitLayout::SetSpacing(float spacing) {
...
orientation BSplitLayout::Orientation() const { return fOrientation; }

void BSplitLayout::SetOrientation(orientation orientation) {
...
float BSplitLayout::SplitterSize() const { return fSplitterSize; }

void BSplitLayout::SetSplitterSize(float size) {
...
BLayoutItem *BSplitLayout::AddView(BView *child) {
  return BAbstractLayout::AddView(child);
}

BLayoutItem *BSplitLayout::AddView(int32 index, BView *child) {
  return BAbstractLayout::AddView(index, child);
}

BLayoutItem *BSplitLayout::AddView(BView *child, float weight) {
  return AddView(-1, child, weight);
}

BLayoutItem *BSplitLayout::AddView(int32 index, BView *child, float weight) {
...
bool BSplitLayout::AddItem(BLayoutItem *item) {
...
bool BSplitLayout::AddItem(BLayoutItem *item, float weight) {
...
float BSplitLayout::ItemWeight(int32 index) const {
...
float BSplitLayout::ItemWeight(BLayoutItem *item) const {
  if (ItemLayoutInfo *info = _ItemLayoutInfo(item))
    return info->weight;
  return 0;
}

void BSplitLayout::SetItemWeight(int32 index, float weight,
...
void BSplitLayout::SetItemWeight(BLayoutItem *item, float weight) {
  if (ItemLayoutInfo *info = _ItemLayoutInfo(item))
    info->weight = weight;
}
...
void BSplitLayout::SetCollapsible(bool collapsible) {
...
BSize BSplitLayout::BaseMinSize() {
  _ValidateMinMax();

  return _AddInsets(fMin);
}

BSize BSplitLayout::BaseMaxSize() {
  _ValidateMinMax();

  return _AddInsets(fMax);
}

BSize BSplitLayout::BasePreferredSize() {
  _ValidateMinMax();

  return _AddInsets(fPreferred);
}

BAlignment BSplitLayout::BaseAlignment() {
  return BAbstractLayout::BaseAlignment();
}

bool BSplitLayout::HasHeightForWidth() {
  _ValidateMinMax();

  return !fHeightForWidthItems.IsEmpty();
}

void BSplitLayout::GetHeightForWidth(float width, float *min, float *max,
                                     float *preferred) {
...
void BSplitLayout::LayoutInvalidated(bool children) {
...
void BSplitLayout::DoLayout() {
  ...
  for (int i = 0; i < itemCount; i++) {
    ...
    if (i > 0) {
      ...
      _LayoutItem(splitterItem,
                  BRect(xOffset, yOffset, xOffset + splitterWidth - 1,
                        yOffset + splitterHeight - 1),
                  true);
    ...
    // place the component
    _LayoutItem(
        item, BRect(xOffset, yOffset, xOffset + width, yOffset + height), true);
...
BRect BSplitLayout::SplitterItemFrame(int32 index) const {
  if (SplitterItem *item = _SplitterItemAt(index))
    return item->Frame();
  return BRect();
}

bool BSplitLayout::IsAboveSplitter(const BPoint &point) const {
  return _SplitterItemAt(point) != NULL;
}

bool BSplitLayout::StartDraggingSplitter(BPoint point) {
...
bool BSplitLayout::DragSplitter(BPoint point) {
...
bool BSplitLayout::StopDraggingSplitter() {
  if (fDraggingSplitterIndex < 0)
    return false;
...
int32 BSplitLayout::DraggedSplitter() const { return fDraggingSplitterIndex; }

status_t BSplitLayout::Archive(BMessage *into, bool deep) const {
...
BArchivable *BSplitLayout::Instantiate(BMessage *from) {
  if (validate_instantiation(from, "BSplitLayout"))
    return new (std::nothrow) BSplitLayout(from);
  return NULL;
}

status_t BSplitLayout::ItemArchived(BMessage *into, BLayoutItem *item,
                                    int32 index) const {
...
status_t BSplitLayout::ItemUnarchived(const BMessage *from, BLayoutItem *item,
                                      int32 index) {
  ItemLayoutInfo *info = _ItemLayoutInfo(item);
...
void BSplitLayout::_InvalidateCachedHeightForWidth() {
...
BSplitLayout::SplitterItem *BSplitLayout::_SplitterItemAt(const BPoint &point,
                                                          int32 *index) const {
...
BSplitLayout::SplitterItem *BSplitLayout::_SplitterItemAt(int32 index) const {
...
void BSplitLayout::_LayoutItem(BLayoutItem *item, BRect frame, bool visible) {
...
void BSplitLayout::_LayoutItem(BLayoutItem *item, ItemLayoutInfo *info) {
...
bool BSplitLayout::_SetSplitterValue(int32 index, int32 value) {
  ...
  // of the item yet.
  bool previousCollapsed =
      (previousSize <= range.previousMin / 2) && previousInfo->isCollapsible;
  bool nextCollapsed =
      (nextSize <= range.nextMin / 2) && nextInfo->isCollapsible;
  ...
  if (previousCollapsed || nextCollapsed) {
    ...
    if (previousCollapsed) {
      if (availableSpace < range.nextMin || availableSpace > range.nextMax) {
  ...
  if (!(previousCollapsed || nextCollapsed)) {
    ...
    if (range.previousMin + range.nextMin + 2 * fSpacing > range.sumValue) {
      ...
      if (previousSize < nextSize && availableSpace >= range.nextMin &&
          availableSpace <= range.nextMax && previousInfo->isCollapsible) {
  ...
  if (fOrientation == B_HORIZONTAL) {
    ...
    previousInfo->layoutFrame.Set(
        left, splitterFrame.top, left + previousSize - 1, splitterFrame.bottom);

    // next item
    left = splitterFrame.left + nextOffset;
    nextInfo->layoutFrame.Set(left, splitterFrame.top, left + nextSize - 1,
    ...
    previousInfo->layoutFrame.Set(splitterFrame.left, top, splitterFrame.right,
                                  top + previousSize - 1);

    // next item
    top = splitterFrame.top + nextOffset;
    nextInfo->layoutFrame.Set(splitterFrame.left, top, splitterFrame.right,
                              top + nextSize - 1);
...
void BSplitLayout::_ValidateMinMax() {
  ...
  if ((realLayout && fHeightForWidthVerticalLayouterWidth != width) ||
    ...
    for (int32 i = 0; i < count; i++) {
      ...
      if (index >= 0) {
        ...
        item->GetHeightForWidth(horizontalLayoutInfo->ElementSize(index),
                                &itemMinHeight, &itemMaxHeight,
                                &itemPreferredHeight);
        verticalLayouter->AddConstraints(index, 1, itemMinHeight, itemMaxHeight,
                                         itemPreferredHeight);
      }
    }
...
float BSplitLayout::_SplitterSpace() const {
...
BSize BSplitLayout::_AddInsets(BSize size) {
  size.width =
...
void BSplitLayout::_AddInsets(float *minHeight, float *maxHeight,
                              float *preferredHeight) {
...
BSize BSplitLayout::_SubtractInsets(BSize size) {
  size.width =
