...
BPath::BPath() : fName(NULL), fCStatus(B_NO_INIT) {}

// Creates a copy of the given BPath object.
BPath::BPath(const BPath &path) : fName(NULL), fCStatus(B_NO_INIT) {
  *this = path;
}

// Creates a BPath object and initializes it to the filesystem entry
// specified by the passed in entry_ref struct.
BPath::BPath(const entry_ref *ref) : fName(NULL), fCStatus(B_NO_INIT) {
  SetTo(ref);
}

// Creates a BPath object and initializes it to the filesystem entry
// specified by the passed in BEntry object.
BPath::BPath(const BEntry *entry) : fName(NULL), fCStatus(B_NO_INIT) {
  SetTo(entry);
}
...
status_t BPath::InitCheck() const { return fCStatus; }
...
status_t BPath::SetTo(const entry_ref *ref) {
  ...
  fCStatus = _kern_entry_ref_to_path(ref->device, ref->directory, ref->name,
                                     path, sizeof(path));
  if (fCStatus != B_OK)
    return fCStatus;
...
const char *BPath::Path() const { return fName; }
...
bool BPath::operator==(const BPath &item) const { return *this == item.Path(); }
...
bool BPath::operator!=(const BPath &item) const { return !(*this == item); }

// Performs a simple (string-wise) comparison of paths for inequality.
bool BPath::operator!=(const char *path) const { return !(*this == path); }
...
static const size_t flattened_entry_ref_size = sizeof(dev_t) + sizeof(ino_t);

// Overrides BFlattenable::IsFixedSize()
bool BPath::IsFixedSize() const { return false; }

// Overrides BFlattenable::TypeCode()
type_code BPath::TypeCode() const { return B_REF_TYPE; }
...
bool BPath::AllowsTypeCode(type_code code) const { return code == B_REF_TYPE; }
...
status_t BPath::Unflatten(type_code code, const void *buffer, ssize_t size) {
  ...
  if (error == B_OK) {
    if (size == (ssize_t)flattened_entry_ref_size) {
      ...
      const flattened_entry_ref &fref = *(const flattened_entry_ref *)buffer;
...
/*!	Sets the supplied path.
        ...
        The path is copied, if \a path is \c NULL the path of the object is set
   to \c NULL as well. The old path is deleted.
...
/*!	Checks a path to see if normalization is required.
        ...
        \param _error A pointer to an error variable that will be set if the
...
bool BPath::_MustNormalize(const char *path, status_t *_error) {
  ...
  if (path[0] != '/')
    ...
    enum ParseState { NoMatch, InitialSlash, OneDot, TwoDots } state = NoMatch;
