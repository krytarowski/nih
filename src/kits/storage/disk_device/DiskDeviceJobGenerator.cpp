...
struct DiskDeviceJobGenerator::PartitionRefInfo {
  PartitionRefInfo() : partition(NULL), reference(NULL) {}
...
DiskDeviceJobGenerator::DiskDeviceJobGenerator(BDiskDevice *device,
    ...
    : fDevice(device), fJobQueue(jobQueue), fMoveInfos(NULL),
      fPartitionRefs(NULL), fContentsToMove(NULL), fContentsToMoveCount(0) {
  // Make sure the arrays are big enough (worst case: all old partitions have
  // been deleted and new ones been created).
  fPartitionCount = fDevice->CountDescendants() + fDevice->_CountDescendants();
...
DiskDeviceJobGenerator::_GenerateChildPlacementJobs(BPartition *partition) {
  ...
  if (childCount > 0 && moveForth + moveBack > 0) {
    qsort(fMoveInfos, childCount, sizeof(MoveInfo), _CompareMoveInfoPosition);
  }
  ...
  while (moveForth + moveBack > 0) {
    ...
    if (moveForth < moveBack) {
      ...
      for (int32 i = 0; i < childCount; i++) {
        ...
        if (info.position > info.target_position) {
          if (i == 0 || info.target_position >= fMoveInfos[i - 1].position +
                                                    fMoveInfos[i - 1].size) {
      ...
      for (int32 i = childCount - 1; i >= 0; i--) {
        ...
        if (info.position > info.target_position) {
          if (i == childCount - 1 ||
              info.target_position + info.size <= fMoveInfos[i - 1].position) {
            // check OK -- the partition wouldn't be moved before
            // the end of the preceding one
            status_t error = _GenerateMoveJob(info.partition);
            if (error != B_OK)
...
status_t DiskDeviceJobGenerator::_GenerateRemainingJobs(BPartition *parent,
  ...
  uint32 changeFlags = partition->fDelegate->MutablePartition()->ChangeFlags();
  ...
  if (!partitionData) {
    ...
    if ((changeFlags & B_PARTITION_CHANGED_PARAMETERS) ||
        compare_string(partition->Parameters(), partitionData->parameters)) {
      if (!parent)
        return B_BAD_VALUE;
  ...
  if (partition->ContentType()) {
    ...
    if (changeFlags & B_PARTITION_CHANGED_INITIALIZATION) {
      ...
      if ((changeFlags & B_PARTITION_CHANGED_CHECK) || repair) {
...
DiskDeviceJobGenerator::_GetMutablePartition(BPartition *partition) {
  ...
  return partition->fDelegate ? partition->fDelegate->MutablePartition() : NULL;
}
...
status_t DiskDeviceJobGenerator::_GenerateInitializeJob(BPartition *partition) {
  ...
  error = job->Init(partition->ContentType(), partition->ContentName(),
...
DiskDeviceJobGenerator::_GenerateSetContentNameJob(BPartition *partition) {
  ...
  error =
      job->Init(partition->ContentName(), B_DISK_DEVICE_JOB_SET_CONTENT_NAME);
  if (error != B_OK) {
    delete job;
    return error;
  }
...
status_t DiskDeviceJobGenerator::_GenerateSetContentParametersJob(
    BPartition *partition) {
  PartitionReference *reference;
  status_t error = _GetPartitionReference(partition, reference);
  if (error != B_OK)
...
// _GenerateRepairJob
status_t DiskDeviceJobGenerator::_GenerateRepairJob(BPartition *partition,
                                                    bool repair) {
  PartitionReference *reference;
  status_t error = _GetPartitionReference(partition, reference);
  if (error != B_OK)
...
DiskDeviceJobGenerator::_GenerateCreateChildJob(BPartition *parent,
  ...
  CreateChildJob *job =
      new (nothrow) CreateChildJob(parentReference, reference);
  if (!job)
    return B_NO_MEMORY;
...
status_t DiskDeviceJobGenerator::_GenerateResizeJob(BPartition *partition) {
  ...
  return _AddJob(new (nothrow) ResizeJob(
      parentReference, reference, partition->Size(), partition->ContentSize()));
}
...
status_t DiskDeviceJobGenerator::_GenerateMoveJob(BPartition *partition) {
  ...
  error = job->Init(partition->Offset(), fContentsToMove, fContentsToMoveCount);
  if (error != B_OK) {
    delete job;
    return error;
  }
...
int DiskDeviceJobGenerator::_CompareMoveInfoPosition(const void *_a,
                                                     const void *_b) {
