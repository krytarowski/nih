...
uint32 LeafNode::Arity() const { return 0; }

status_t LeafNode::SetChildAt(QueryNode *child, int32 index) {
  return B_BAD_VALUE;
}

QueryNode *LeafNode::ChildAt(int32 index) { return NULL; }

// #pragma mark - UnaryNode

UnaryNode::UnaryNode() : fChild(NULL) {}

UnaryNode::~UnaryNode() { delete fChild; }

uint32 UnaryNode::Arity() const { return 1; }
...
QueryNode *UnaryNode::ChildAt(int32 index) {
...
BinaryNode::BinaryNode() : fChild1(NULL), fChild2(NULL) {}
...
uint32 BinaryNode::Arity() const { return 2; }
...
QueryNode *BinaryNode::ChildAt(int32 index) {
  QueryNode *result = NULL;
  if (index == 0)
    result = fChild1;
  else if (index == 1)
...
AttributeNode::AttributeNode(const char *attribute) : fAttribute(attribute) {}
...
DateNode::DateNode(const char *value) : fValue(value) {}
...
SpecialOpNode::SpecialOpNode(query_op op) : fOp(op) {}

status_t SpecialOpNode::GetString(BString &predicate) { return B_BAD_VALUE; }

// #pragma mark - UnaryOpNode

UnaryOpNode::UnaryOpNode(query_op op) : fOp(op) {}
...
BinaryOpNode::BinaryOpNode(query_op op) : fOp(op) {}
...
status_t BinaryOpNode::GetString(BString &predicate) {
  ...
  if (error == B_OK) {
    switch (fOp) {
    case B_EQ:
      predicate << "(" << childString1 << "==" << childString2 << ")";
      break;
    case B_GT:
      predicate << "(" << childString1 << ">" << childString2 << ")";
      break;
