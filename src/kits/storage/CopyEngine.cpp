...
BCopyEngine::BCopyEngine(uint32 flags)
    : fController(NULL), fFlags(flags), fBuffer(NULL), fBufferSize(0) {}

BCopyEngine::~BCopyEngine() { delete[] fBuffer; }

BCopyEngine::BController *BCopyEngine::Controller() const {
  return fController;
}
...
uint32 BCopyEngine::Flags() const { return fFlags; }

BCopyEngine &BCopyEngine::SetFlags(uint32 flags) {
  fFlags = flags;
  return *this;
}

BCopyEngine &BCopyEngine::AddFlags(uint32 flags) {
  fFlags |= flags;
  return *this;
}

BCopyEngine &BCopyEngine::RemoveFlags(uint32 flags) {
  fFlags &= ~flags;
  return *this;
}

status_t BCopyEngine::CopyEntry(const Entry &sourceEntry,
...
status_t BCopyEngine::_CopyEntry(const char *sourcePath, const char *destPath) {
  ...
  if (lstat(sourcePath, &sourceStat) < 0) {
    return _HandleEntryError(sourcePath, errno, "Couldn't access \"%s\": %s\n",
  ...
  if (destExists) {
    if (S_ISDIR(destStat.st_mode)) {
      if (!S_ISDIR(sourceStat.st_mode) ||
        ...
        return _HandleEntryError(
            sourcePath, B_FILE_EXISTS,
  ...
  if (error != B_OK) {
    return _HandleEntryError(sourcePath, error, "Failed to open \"%s\": %s\n",
...
status_t BCopyEngine::_CopyAttributes(const char *sourcePath, BNode &source,
  ...
  while (source.GetNextAttrName(attrName) == B_OK) {
    ...
    // filter
    if (fController != NULL &&
        !fController->AttributeStarted(sourcePath, attrName, attrInfo.type)) {
      if (error != B_OK) {
        _NotifyError(error,
    ...
    if (error != B_OK) {
      error = _HandleAttributeError(
          sourcePath, attrName, attrInfo.type, error,
    ...
    do {
      ...
      // read
      ssize_t bytesRead =
          source.ReadAttr(attrName, attrInfo.type, offset, fBuffer, toRead);
      if (bytesRead < 0) {
        error = _HandleAttributeError(
            sourcePath, attrName, attrInfo.type, bytesRead,
            "Failed to read attribute \"%s\" "
            "of file \"%s\": %s\n",
            attrName, sourcePath, strerror(bytesRead));
        if (error != B_OK)
          return error;
        break;
      }
      ...
      // write
      ssize_t bytesWritten = destination.WriteAttr(attrName, attrInfo.type,
                                                   offset, fBuffer, bytesRead);
      if (bytesWritten < 0) {
        error = _HandleAttributeError(
            sourcePath, attrName, attrInfo.type, bytesWritten,
            "Failed to write attribute "
            "\"%s\" of file \"%s\": %s\n",
            attrName, destPath, strerror(bytesWritten));
        if (error != B_OK)
          return error;
        break;
      }
    ...
    if (fController != NULL) {
      fController->AttributeFinished(sourcePath, attrName, attrInfo.type, B_OK);
    }
  }
...
status_t BCopyEngine::_HandleAttributeError(const char *path,
...
bool BCopyEngine::BController::EntryStarted(const char *path) { return true; }
...
void BCopyEngine::BController::ErrorOccurred(const char *message,
                                             status_t error) {}
