...
bool BGradient::ColorStop::operator!=(const ColorStop &other) const {
  return color.red != other.color.red || color.green != other.color.green ||
         color.blue != other.color.blue || color.alpha != other.color.alpha ||
         offset != other.offset;
}
...
// constructor
BGradient::BGradient() : BArchivable(), fColorStops(4), fType(TYPE_NONE) {}

// constructor
BGradient::BGradient(BMessage *archive)
    : BArchivable(archive), fColorStops(4), fType(TYPE_NONE) {
  if (!archive)
    return;

  // color stops
  ColorStop stop;
  for (int32 i = 0; archive->FindFloat("offset", i, &stop.offset) >= B_OK;
       i++) {
  ...
  if (archive->FindFloat("radial_f_radius",
                         (float *)&fData.radial_focus.radius) < B_OK)
...
// operator=
BGradient &BGradient::operator=(const BGradient &other) {
  SetColorStops(other);
  fType = other.fType;
  return *this;
}
...
bool BGradient::ColorStopsAreEqual(const BGradient &other) const {
  ...
  if (count == other.CountColorStops() && fType == other.fType) {
...
// CountColorStops
int32 BGradient::CountColorStops() const { return fColorStops.CountItems(); }

// ColorStopAt
BGradient::ColorStop *BGradient::ColorStopAt(int32 index) const {
  return (ColorStop *)fColorStops.ItemAt(index);
}

// ColorStopAtFast
BGradient::ColorStop *BGradient::ColorStopAtFast(int32 index) const {
  return (ColorStop *)fColorStops.ItemAtFast(index);
}

// ColorStops
BGradient::ColorStop *BGradient::ColorStops() const {
...
void BGradient::SortColorStopsByOffset() {
  ...
  // such stops.
  const BGradient::ColorStop **first =
      (const BGradient::ColorStop **)fColorStops.Items();
