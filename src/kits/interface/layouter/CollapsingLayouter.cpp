...
public:
  ProxyLayoutInfo(LayoutInfo *target, int32 elementCount)
      : fTarget(target), fElementCount(elementCount) {
...
struct CollapsingLayouter::ElementInfo {
  ...
  ElementInfo() : weight(0), position(-1), valid(false), constraints(5, true) {}
...
CollapsingLayouter::CollapsingLayouter(int32 elementCount, float spacing)
    : fElementCount(elementCount), fElements(new ElementInfo[elementCount]),
      fValidElementCount(0), fHaveMultiElementConstraints(false),
      fSpacing(spacing), fLayouter(NULL) {}
...
float CollapsingLayouter::MinSize() {
  _ValidateLayouter();
  return fLayouter ? fLayouter->MinSize() : 0;
}

float CollapsingLayouter::MaxSize() {
  _ValidateLayouter();
  return fLayouter ? fLayouter->MaxSize() : B_SIZE_UNLIMITED;
}

float CollapsingLayouter::PreferredSize() {
  _ValidateLayouter();
  return fLayouter ? fLayouter->PreferredSize() : 0;
}

LayoutInfo *CollapsingLayouter::CreateLayoutInfo() {
  _ValidateLayouter();
...
Layouter *CollapsingLayouter::CloneLayouter() {
...
Layouter *CollapsingLayouter::_CreateLayouter() {
  if (fLayouter)
    return fLayouter;
...
void CollapsingLayouter::_AddConstraints(int32 position, const Constraint *c) {
  fLayouter->AddConstraints(position, c->length, c->min, c->max, c->preferred);
}
