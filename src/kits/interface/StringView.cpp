...
static property_info sPropertyList[] = {{"Text",
...
BStringView::BStringView(BRect frame, const char *name, const char *text,
    ...
    : BView(frame, name, resizingMode, flags | B_FULL_UPDATE_ON_RESIZE),
      fText(text ? strdup(text) : NULL), fTruncation(B_NO_TRUNCATION),
      fAlign(B_ALIGN_LEFT), fPreferredSize(text ? StringWidth(text) : 0.0, -1) {
}
...
BStringView::BStringView(BMessage *archive)
    : BView(archive), fText(NULL), fTruncation(B_NO_TRUNCATION),
      fPreferredSize(0, -1) {
...
BStringView::~BStringView() { free(fText); }
...
void BStringView::DetachedFromWindow() { BView::DetachedFromWindow(); }

void BStringView::AllAttached() { BView::AllAttached(); }

void BStringView::AllDetached() { BView::AllDetached(); }

// #pragma mark - Layout methods

void BStringView::MakeFocus(bool focus) { BView::MakeFocus(focus); }

void BStringView::GetPreferredSize(float *_width, float *_height) {
...
void BStringView::ResizeToPreferred() {
  ...
  // Resize the width only for B_ALIGN_LEFT (if its large enough already, that
  // is)
...
BAlignment BStringView::LayoutAlignment() {
...
void BStringView::MessageReceived(BMessage *message) {
...
void BStringView::MouseDown(BPoint point) { BView::MouseDown(point); }

void BStringView::MouseUp(BPoint point) { BView::MouseUp(point); }

void BStringView::MouseMoved(BPoint point, uint32 transit,
                             const BMessage *msg) {
...
const char *BStringView::Text() const { return fText; }
...
alignment BStringView::Alignment() const { return fAlign; }
...
uint32 BStringView::Truncation() const { return fTruncation; }

BHandler *BStringView::ResolveSpecifier(BMessage *message, int32 index,
...
void BStringView::LayoutInvalidated(bool descendants) {
...
status_t BStringView::Perform(perform_code code, void *_data) {
  switch (code) {
  case PERFORM_CODE_MIN_SIZE:
    ((perform_data_min_size *)_data)->return_value = BStringView::MinSize();
    return B_OK;

  case PERFORM_CODE_MAX_SIZE:
    ((perform_data_max_size *)_data)->return_value = BStringView::MaxSize();
    return B_OK;
...
BSize BStringView::_ValidatePreferredSize() {
...
B_IF_GCC_2(InvalidateLayout__11BStringViewb,
           _ZN11BStringView16InvalidateLayoutEb)(BView *view,
                                                 bool descendants) {
  perform_data_layout_invalidated data;
