...
Pattern::Pattern(const std::string &string, const std::string &mask)
    : fCStatus(B_NO_INIT), fErrorMessage(NULL) {
  SetTo(string, mask);
}
...
Pattern::~Pattern() { delete fErrorMessage; }

status_t Pattern::InitCheck() const { return fCStatus; }

Err *Pattern::GetErr() const {
  if (fCStatus == B_OK)
    return NULL;
  else
...
/*! \brief Looks for a pattern match in the given data stream, starting from
        ...
        false if not.
*/
bool Pattern::Sniff(Range range, BPositionIO *data,
...
*/
ssize_t Pattern::BytesNeeded() const {
...
bool Pattern::Sniff(off_t start, off_t size, BPositionIO *data,
                    bool caseInsensitive) const {
      ...
      if (caseInsensitive) {
        for (int i = 0; i < len; i++) {
          ...
          if ('A' <= fString[i] && fString[i] <= 'Z')
            ...
            secondChar = fString[i]; // Check the same char twice as punishment
...
void Pattern::SetStatus(status_t status, const char *msg) {
  ...
  if (status == B_OK)
    ...
    if (msg)
      ...
      SetErrorMessage("Sniffer parser error: Pattern::SetStatus() -- NULL msg "
                      "with non-B_OK status.\n"
                      "(This is officially the most helpful error message you "
                      "will ever receive ;-)");
    }
  }
}
