...
BPicture::Private::Private(BPicture *picture) : fPicture(picture) {}
...
BPicture::BPicture() : fToken(-1), fExtent(NULL), fUsurped(NULL) {
  _InitData();
}

BPicture::BPicture(const BPicture &otherPicture)
    : fToken(-1), fExtent(NULL), fUsurped(NULL) {
  _InitData();
...
BPicture::BPicture(BMessage *data) : fToken(-1), fExtent(NULL), fUsurped(NULL) {
  _InitData();
...
status_t BPicture::Unflatten(BDataIO *stream) {
  ...
  if (bytesRead != (ssize_t)sizeof(header) || header.magic1 != 2 ||
      header.magic2 != 0)
    return B_BAD_TYPE;
...
void BPicture::SetToken(int32 token) { fToken = token; }

int32 BPicture::Token() const { return fToken; }
...
// _BPictureExtent_
_BPictureExtent_::_BPictureExtent_(int32 size) : fNewData(NULL), fNewSize(0) {
  SetSize(size);
}
