...
#include "os/interface/SeparatorView.h"
...
BSeparatorView::BSeparatorView(orientation orientation, border_style border)
  ...
  _Init(NULL, NULL, orientation,
        BAlignment(B_ALIGN_HORIZONTAL_CENTER, B_ALIGN_VERTICAL_CENTER), border);
}
...
BSeparatorView::BSeparatorView(const char *label, orientation orientation,
                               border_style border, const BAlignment &alignment)
    : BView("", B_WILL_DRAW | B_FULL_UPDATE_ON_RESIZE) {
  _Init(label, NULL, orientation, alignment, border);
}

BSeparatorView::BSeparatorView(BView *labelView, orientation orientation,
                               border_style border, const BAlignment &alignment)
    : BView("", B_WILL_DRAW | B_FULL_UPDATE_ON_RESIZE) {
  _Init(NULL, labelView, orientation, alignment, border);
}
...
BSeparatorView::BSeparatorView(BMessage *archive)
    : BView(archive), fLabel(), fLabelView(NULL), fOrientation(B_HORIZONTAL),
      fAlignment(
...
BArchivable *BSeparatorView::Instantiate(BMessage *archive) {
...
void BSeparatorView::Draw(BRect updateRect) {
  ...
  if (fLabelView != NULL) {
    ...
    if (fOrientation == B_HORIZONTAL) {
      ...
      case B_ALIGN_CENTER:
        labelBounds.left =
            (labelBounds.left + labelBounds.right - labelWidth) / 2;
        labelBounds.right = labelBounds.left + labelWidth;
        break;
      }
    } else {
      ...
      case B_ALIGN_MIDDLE:
        labelBounds.top =
            (labelBounds.top + labelBounds.bottom - labelWidth) / 2;
        labelBounds.bottom = labelBounds.top + labelWidth;
        break;
      }
    }
  ...
  if (borderSize > 0.0f) {
    if (fOrientation == B_HORIZONTAL) {
      bounds.top = floorf((bounds.top + bounds.bottom + 1 - borderSize) / 2);
      bounds.bottom = bounds.top + borderSize - 1;
      region.Exclude(bounds);
      be_control_look->DrawBorder(this, bounds, updateRect, base, fBorder, 0,
                                  BControlLook::B_TOP_BORDER);
    } else {
      bounds.left = floorf((bounds.left + bounds.right + 1 - borderSize) / 2);
  ...
  if (fLabel.CountChars() > 0) {
    ...
    if (fOrientation == B_HORIZONTAL) {
      DrawString(
          fLabel.String(),
          BPoint(labelBounds.left, labelBounds.top + ceilf(fontHeight.ascent)));
    } else {
      DrawString(fLabel.String(),
                 BPoint(labelBounds.left + ceilf(fontHeight.ascent),
                        labelBounds.bottom));
    }
  }
}
...
BSize BSeparatorView::MinSize() {
  BSize size;
  GetPreferredSize(&size.width, &size.height);
  return BLayoutUtils::ComposeSize(ExplicitMinSize(), size);
}

BSize BSeparatorView::MaxSize() {
  BSize size(MinSize());
  if (fOrientation == B_HORIZONTAL)
...
BSize BSeparatorView::PreferredSize() {
  BSize size;
  GetPreferredSize(&size.width, &size.height);
...
void BSeparatorView::DoLayout() {
  ...
  BRect frame =
      BLayoutUtils::AlignInFrame(bounds, fLabelView->MaxSize(), fAlignment);
...
void BSeparatorView::_Init(const char *label, BView *labelView,
                           orientation orientation, BAlignment alignment,
                           border_style border) {
  fLabel = "";
  fLabelView = NULL;
...
float BSeparatorView::_BorderSize() const {
...
BRect BSeparatorView::_MaxLabelBounds() const {
  BRect bounds = Bounds();
  if (fOrientation == B_HORIZONTAL)
