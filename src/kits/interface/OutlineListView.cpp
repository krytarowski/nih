...
struct ListItemComparator {
  ListItemComparator(compare_func compareFunc) : fCompareFunc(compareFunc) {}
...
static void _GetSubItems(BList &sourceList, BList &destList, BListItem *parent,
                         int32 start) {
...
static void _DoSwap(BList &list, int32 firstIndex, int32 secondIndex,
                    BList *firstItems, BList *secondItems) {
...
BOutlineListView::BOutlineListView(BMessage *archive) : BListView(archive) {
  int32 i = 0;
  BMessage subData;
...
BOutlineListView::~BOutlineListView() { fFullList.MakeEmpty(); }
...
status_t BOutlineListView::Archive(BMessage *archive, bool deep) const {
...
void BOutlineListView::FrameMoved(BPoint newPosition) {
  BListView::FrameMoved(newPosition);
}

void BOutlineListView::FrameResized(float newWidth, float newHeight) {
  BListView::FrameResized(newWidth, newHeight);
}

void BOutlineListView::MouseUp(BPoint where) { BListView::MouseUp(where); }
...
int32 BOutlineListView::FullListIndexOf(BPoint where) const {
...
bool BOutlineListView::FullListIsEmpty() const { return fFullList.IsEmpty(); }
...
void BOutlineListView::Expand(BListItem *item) { ExpandOrCollapse(item, true); }
...
BHandler *BOutlineListView::ResolveSpecifier(BMessage *message, int32 index,
                                             BMessage *specifier, int32 what,
                                             const char *property) {
  return BListView::ResolveSpecifier(message, index, specifier, what, property);
}
...
status_t BOutlineListView::Perform(perform_code code, void *_data) {
  switch (code) {
...
void BOutlineListView::ResizeToPreferred() { BListView::ResizeToPreferred(); }

void BOutlineListView::GetPreferredSize(float *_width, float *_height) {
  BListView::GetPreferredSize(_width, _height);
}

void BOutlineListView::MakeFocus(bool state) { BListView::MakeFocus(state); }

void BOutlineListView::AllAttached() { BListView::AllAttached(); }

void BOutlineListView::AllDetached() { BListView::AllDetached(); }

void BOutlineListView::DetachedFromWindow() { BListView::DetachedFromWindow(); }

void BOutlineListView::FullListSortItems(
    int (*compareFunc)(const BListItem *a, const BListItem *b)) {
  SortItemsUnder(NULL, false, compareFunc);
}

void BOutlineListView::SortItemsUnder(BListItem *superItem, bool oneLevelOnly,
...
int32 BOutlineListView::CountItemsUnder(BListItem *superItem,
...
BListItem *BOutlineListView::EachItemUnder(
    BListItem *superItem, bool oneLevelOnly,
...
BListItem *BOutlineListView::ItemUnderAt(BListItem *superItem,
...
void BOutlineListView::ExpandOrCollapse(BListItem *item, bool expand) {
...
void BOutlineListView::DrawLatch(BRect itemRect, int32 level, bool collapsed,
  ...
  int32 arrowDirection =
      collapsed ? BControlLook::B_RIGHT_ARROW : BControlLook::B_DOWN_ARROW;

  be_control_look->DrawArrowShape(this, latchRect, itemRect, base,
                                  arrowDirection, 0, B_DARKEN_4_TINT);
}

void BOutlineListView::DrawItem(BListItem *item, BRect itemRect,
                                bool complete) {
  if (item->fHasSubitems) {
...
int32 BOutlineListView::_FullListIndex(int32 index) const {
  BListItem *item = ItemAt(index);
...
void BOutlineListView::_PopulateTree(BList *tree, BList &target,
...
void BOutlineListView::_SortTree(BList *tree, bool oneLevelOnly,
                                 int (*compareFunc)(const BListItem *a,
                                                    const BListItem *b)) {
...
void BOutlineListView::_DestructTree(BList *tree) {
  for (int32 index = tree->CountItems(); index-- > 0;) {
    BListItem *item = (BListItem *)tree->ItemAt(index);
...
BList *BOutlineListView::_BuildTree(BListItem *superItem,
                                    int32 &fullListIndex) {
...
void BOutlineListView::_CullInvisibleItems(BList &list) {
  int32 index = 0;
...
bool BOutlineListView::_SwapItems(int32 first, int32 second) {
  ...
  _DoSwap(fList, firstIndex, secondIndex, &firstSubItems, &secondSubItems);
...
/*!	\brief Removes a single item from the list and all of its children.
        ...
        Unlike the BeOS version, this one will actually delete the children,
   too, as there should be no reference left to them. This may cause problems
   for applications that actually take the misbehaviour of the Be classes into
        account.
*/
BListItem *BOutlineListView::_RemoveItem(BListItem *item, int32 fullListIndex) {
...
/*!	Returns the super item before the item specified by \a fullListIndex
        and \a level.
*/
BListItem *BOutlineListView::_SuperitemForIndex(int32 fullListIndex,
                                                int32 level,
                                                int32 *_superIndex) {
  BListItem *item;
  fullListIndex--;
...
int32 BOutlineListView::_FindPreviousVisibleIndex(int32 fullListIndex) {
  fullListIndex--;
