...
BDataPositionIOWrapper::BDataPositionIOWrapper(BDataIO *io)
    : BPositionIO(), fIO(io), fPosition(0) {}
...
ssize_t BDataPositionIOWrapper::ReadAt(off_t position, void *buffer,
                                       size_t size) {
...
off_t BDataPositionIOWrapper::Position() const { return fPosition; }
